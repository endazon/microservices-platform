...
> **realm は `platform`** である（IADR-0197）。
> **［2026-09-11 追記 / #1393］同時に改名した基盤 SPA の public client（`platform-spa`）は realm から撤去した。**
> BFF セッション方式（ADR-0032）で SPA は OIDC を実施せず、ブラウザ向けの認可コードフローを行う
> クライアントは BFF の confidential client（`bff`）ただ 1 つである（IADR-0429）。
