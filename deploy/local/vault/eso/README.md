...
| `externalsecret-object-storage.yaml` | ExternalSecret（`secret/msp/object-storage-credentials` → `object-storage-credentials` accessKey/secretKey・PR-2/IADR-0097。オブジェクトストレージ（SeaweedFS）の S3 資格情報。旧名 `minio-credentials`・IADR-0464） |
...
- ESO 同期は helm install（各 Pod 起動）後に走るため、対象 Secret は一時的に未作成で消費側 Pod が数秒
  ...
  （未設定だと匿名で全許可になるため。IADR-0464 決定 3）。
- PR-3 の OIDC client secret 群（`grafana-oidc`／`vault-oidc`／`headlamp-oidc`。`minio-oidc` は IADR-0464 で撤去）は各機能ゲート
...
- **PR-2（IADR-0097）**: `minio-credentials`（IADR-0464 で `object-storage-credentials` へ改名）／`wikijs-db`／`wikijs-sync`。
- **PR-3（IADR-0098）**: OIDC client secret 群 `minio-oidc`（IADR-0464 で撤去）／`grafana-oidc`／`vault-oidc`／`headlamp-oidc`。
