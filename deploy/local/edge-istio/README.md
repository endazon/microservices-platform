...
mesh 内の 3 Service（`frontend-service` / `bff-service` / `wiki-js`）へ**平文で入っている**（当初は MinIO Console を含む 4 件だったが、IADR-0461 で撤去した）。
...
`admin(50000)` には **mesh 内の 1 件**（`wiki-js`。当初は MinIO Console を含む 2 件だったが IADR-0461 で撤去）が載っている。
