...
| 8 | **Keycloak OIDC ストラテジ**（**opt-in・既定オフ**） | `WIKIJS_OIDC=1` のときだけ。`authentication` の oidc 行と `settings.host` を UPSERT で突き合わせる。**変わったときだけ** `wiki-js` を再起動する。IADR-0342・#1127 |
...
- **Keycloak SSO（OIDC ストラテジ）は段 8 が入れる**（#1127・IADR-0342。以前は射程外で、
