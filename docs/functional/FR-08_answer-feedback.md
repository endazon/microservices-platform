...
---
<!-- trace:
ids: [FR-04, FR-08, UC-01]
adrs: [ADR-0002]
iadrs: [IADR-0010, IADR-0158]
specs: [01_requirements, 01_usecases, 20260703_FR-08_answer-feedback-collection, FR-08_answer-feedback, IADR-0010_feedback-service-and-upsert]
...
AI 回答に対し、利用者が **👍（up）/ 👎（down）** と任意の **コメント** を送信できる。
収集したフィードバックは、品質改善（低評価回答のレビュー・満足率の可視化）に活用する。
`FeedbackService`（専用マイクロサービス。DB per Service の方針による）が保持・集計する。
...
- 一意制約: `(AnswerId, UserId)`。同一利用者の同一回答は 1 行に upsert する（専用サービスで保持し、1 ユーザー 1 回答は upsert で冪等化するという実装判断による）。
...
> **［2026-08-07 追記 / #586］計画側はフィードバック収集の認可を確定した。上表の「認可なし」は計画と食い違う。**
> 計画リポジトリの 02_requirements（コミット `3e58b97`。裁定依頼を反映したもの）で、
> フィードバック収集の要求に次が**確定**として追加された——**投稿には認証を要する（匿名投稿は許さない）／統計は運用者・
> 管理者に限って参照できる／受け入れ基準は「投稿端点が無認証で 401」「統計端点は認証済みでも権限外は 403」
> 「同一利用者が 2 回投稿しても集計は 1 件のまま」**。
> **［2026-08-10 是正 / #521］上表と下記「例外フロー」を実装へ揃えた。**
> 従前の記述は当時の実装の事実としては正しかったが、**BFF と後段の両層へ認可を足した**ので
> 現在は誤りである（投稿は認証必須・統計は運用者/管理者のみ）。
> 関連する同型の記述: [通信仕様書](../api/BFF_bff-surface.md) §エンドポイント一覧・§未決事項 3、
> およびフィードバック保持の実装 ADR。

BFF 集約（検索・質問のチャット画面向け）:
...
- 同一 `(AnswerId, UserId)` の**同時 2 重送信**（ダブルクリック・再試行）: 後勝ちの INSERT が一意制約違反となるが、
  `DbUpdateException` を捕捉して既存行の更新へフォールバックし、冪等を保つ（500 を返さない。upsert で冪等化するという実装判断に従う）。
- 一覧 `GET /feedback` への非管理ロールアクセス: 403（`Comment`/`UserId` を含むため `AdminOnly`）。
- 未認証（JWT 無し）: **401 を返す**（匿名投稿は許さない。計画側の要求で 2026-08-07 に確定）。
  - > **［2026-08-10 是正 / #521］** 従前は「開発・テスト環境では `anonymous` として受理」
    > だった。**この `anonymous` フォールバックは消した**——`(AnswerId, UserId)` のユニーク索引と噛み合い、
    > **無認証の投稿は全員が `anonymous` の 1 行を共有して互いに上書きし合っていた**ためである
    > （指標の汚染に加えて他人の投稿の改変にあたる）。
...
- 決定: 回答フィードバックは専用サービスで保持し、1 ユーザー 1 回答は upsert で冪等化する（実装 ADR）。
