...
- 一意制約: `(AnswerId, UserId)`。同一利用者の同一回答は 1 行に upsert（IADR-0010: 回答フィードバックは専用サービスで保持し、1 ユーザー 1 回答は upsert で冪等化する）。
...
> **［2026-08-10 是正 / #521・[[IADR-0158]]］上表と下記「例外フロー」を実装へ揃えた。**
...
- 同一 `(AnswerId, UserId)` の**同時 2 重送信**（ダブルクリック・再試行）: 後勝ちの INSERT が一意制約違反となるが、
  `DbUpdateException` を捕捉して既存行の更新へフォールバックし、冪等を保つ（500 を返さない。IADR-0010: 回答フィードバックは専用サービスで保持し、1 ユーザー 1 回答は upsert で冪等化する）。
- 一覧 `GET /feedback` への非管理ロールアクセス: 403（`Comment`/`UserId` を含むため `AdminOnly`）。
- 未認証（JWT 無し）: **401 を返す**（匿名投稿は許さない。計画 FR-08・2026-08-07 確定）。
  - > **［2026-08-10 是正 / #521・[[IADR-0158]]］** 従前は「開発・テスト環境では `anonymous` として受理」
...
- 決定: IADR-0010: 回答フィードバックは専用サービスで保持し、1 ユーザー 1 回答は upsert で冪等化する。
