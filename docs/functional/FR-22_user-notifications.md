...
iadrs: [IADR-0009, IADR-0056, IADR-0119, IADR-0121, IADR-0125, IADR-0132, IADR-0142, IADR-0215, IADR-0267, IADR-0270]
...
> メール outbox・送信レート制御・保持）／受け口／①②③ の発火の結線は入っている**。
> **入っていないのは 3 つである** —— **通知サービスの配備**（配備されるまで送出は到達しない）、
> **BFF 端点の実装**、**SMTP リレーの実体**（実環境が要る）。
> 線引きの正本は送出側の実装 ADR の決定 5・5-a・5-b であり、経緯は同名の作業仕様書にある。
> （2026-08-28 更新: 発火の結線が入った。**残るのは配備と BFF 端点と SMTP である。**）
...
> **①②③ の検知・送出・受理の経路は繋がった**（2026-08-28）。**残るのは通知サービスの配備であり、
> 配備されるまで送出は受け口へ到達しない** —— 届かなかったことは送出側の計器
