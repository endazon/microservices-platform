...
1. **1 つでも ON なら索引へ載せる。3 つとも OFF なら載せない。** 全 OFF の資料は文書更新イベント
   ...
   本文を書き換える経路はすべてこの判定を通って再発行する —— 同期の送信に加え、**同期競合の解決で
   「ローカルを採用」を選んだとき**も、露出が 1 つでも ON なら新しい本文が索引へ届く。
   「両方を残す」で作る別名の資料は新規作成の既定（3 つとも OFF）で作られ、元の資料の露出を継がない。
