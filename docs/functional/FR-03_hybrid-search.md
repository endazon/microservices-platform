...
adrs: [ADR-0009, ADR-0016, ADR-0017, ADR-0043, ADR-0057, ADR-0070, ADR-0092]
iadrs: [IADR-0012, IADR-0014, IADR-0149, IADR-0150, IADR-0151, IADR-0256, IADR-0313, IADR-0318, IADR-0339, IADR-0358, IADR-0388, IADR-0422, IADR-0467]
...
issues: [#336, #536, #995, #1116, #1118, #1193, #1253, #1254]
-->
...
### 機密区分で分けたコレクションを束ねる（2026-09-26）

埋め込みはモデルごとにコレクションを分けている（異なるモデルのベクトルは同じ空間で比べられない）。
高機密の文書はセルフホストのモデル（ティア A）で埋め込まれ、既定のモデル（ティア B）とは**別のコレクション**に入る。
検索は**主コレクション**（`Qdrant:CollectionName`）に加えて、**束ねる追加コレクション**（`Qdrant:FusedCollections`。**既定は空**）も読み、
**1 回の検索で 1 つの並び**を返す。**検索面を機密区分で分けない**（「高機密はこちら」と見せることがその区分の文書の存在を明かすため）。

| 項目 | 内容 |
| --- | --- |
| 束ね方 | コレクションごとにベクトル系統と全文系統を引き、**全系統を 1 回の RRF に平らに入れる**（並び: 主ベクトル → 主全文 → 追加ベクトル → 追加全文）。k=60・候補幅は上表と同じ・重みは全系統 1 |
| スコア | 🔴 **比べない。** モデルが違えばスコアの意味が違う。並びは順位だけで決まり、`Score` は RRF の合算値になる |
| 同点 | 先に現れた方が先（上の並び順。主が追加より先） |
| クエリの埋め込み | 追加コレクションは**そのコレクションのモデルで**埋める。検索サービスは埋め込み要求にコレクション名を名乗り、埋め込みゲートウェイが越境判定の**後で**そのコレクションの送信先へ絞る（許されていない区分を開くことはできない）。主コレクションの要求は名乗らない（従来どおり） |
| 全文 | **全文も束ねる。** 全文検索もコレクションを読むので、束ねないと追加コレクションの文書はキーワードでも見つからない |
| 権限 | 🔴 **ABAC フィルタは全コレクションの全系統に常に掛ける。** 利用者のスコープが高機密を許さなくても問い合わせは省かず、権限外の点はフィルタが落とす（省略を統制の担い手にしない） |
| 権限内属性値・削除 | 属性値の候補は**全コレクションの和集合**（件数は持たない）。文書削除は**全コレクションから**消す |
| 既定 | 追加が空なら、戻り値・`Score`・並び・呼び出し回数は束ねる前と**同一**である。Helm はセルフホスト埋め込み（`embedding.enabled`）を有効にしたときだけティア A のコレクションを追加に描画する |

モードごとの縮退: hybrid では追加コレクションだけ埋め込めないとき、そのコレクションのベクトル系統だけを落として全文は引く。
...
- [ ] セルフホスト埋め込みの実配備下で、束ねた検索の関連性（nDCG@10）を既定モデルだけの検索と比べて測る（稼働環境依存）。
