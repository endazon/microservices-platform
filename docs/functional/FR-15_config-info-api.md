...
- **`Unverifiable` は原因で深刻度を分ける。** 収集対象（`Introspection:Services`）に担当サービスの
  登録が無い段は **Warning**、登録済みだが応答しない段は **Info** とする。前者は再起動では解消せず、
  **その段が永久に突合されない**ことを意味するためである。種別 5 分類・深刻度 2 値・対象は段名という
  値域は変わらない。宣言で無効な段には適用しない（検証の必要が無く、恒常的な雑音になるため）。

### 突合の基準の健全性（起動時ガード）

宣言の読み込みは「パスの指定はあるが読めなかった」を黙って許す（段ホストがローカルで既定配線へ
縮退できるようにするための仕様）。構成情報 API 側でそれが起きると**宣言 0 件が突合の基準になり、
実効の購読すべてが `UndeclaredSubscription` として偽陽性で並ぶ**。したがって、宣言のパスが設定されて
いるのに段を 1 件も読み込めなければ**起動を失敗させる**。パス未設定（ローカル・単体試験）は素通りする。
...
| 宣言のパスは設定済みだが段を読み込めない | 起動失敗（マウント漏れ・空ファイル・形式違いの検出） |
| 宣言の段の担当サービスが収集対象に無い | Unverifiable を Warning で報告（恒久的に検証できない） |
...
- [x] 収集対象に登録が無いサービスの段が、一過性の到達不能と区別して警告される
- [x] 宣言のパスを指定しているのに段を読み込めなければ起動しない
