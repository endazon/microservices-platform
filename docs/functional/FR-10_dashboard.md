...
iadrs: [IADR-0011, IADR-0026, IADR-0119, IADR-0265, IADR-0299]
...
### 生産者の配線状況（2026-08-29 実測）

**7 指標のうち生産者があるのは 1 件だけである。**

| 指標 | 生産者 | 備考 |
| --- | --- | --- |
| 孤立文書数 | **あり**（知識グラフ側の定期処理。1 時間周期） | 排他リースを取ったレプリカだけが報告する（全量置換のため） |
| 陳腐化文書数 | 無し | **しきい値が計画側で未確定**。裁定を依頼中 |
...
🔴 **したがって画面には出さない。** 生産者の無い 6 指標は 0 件として返るが、その 0 は
