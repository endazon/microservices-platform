...
### 生産者の配線状況（2026-09-11 実測）

**集計 API の語彙 7 指標のうち、観測値の生産者があるのは 5 件である**（計画の 8 つ目は
Grafana 側のカウンタであり、この表の母集合に入らない。上の 🔴 を参照）。

> ★［2026-09-11 更新］**本表は 2026-09-03 時点の「生産者 2 件」のまま止まっていた。**
> その後 2 件（解決できないリンク数・辺の型ごとの使用件数）が 2026-09-05 に、
> 1 件（未要約クラスタ数）が 2026-09-11 に配線されている。**古い数は本節の中だけでなく
> 画面仕様書・テスト仕様書・画面のコメントにも複写されていた** —— まとめて直した。
...
| 解決できないリンク数 | **あり**（同上。2026-09-05） | リンク先の**名前**を保存し、集計のたびに解決し直す（失敗を保存しない）。内訳の軸は不在と曖昧 |
| 未要約クラスタ数 | **あり**（同上。2026-09-11） | 分母は**日次バッチが検出したクラスタ（コミュニティ）**である。内訳の軸は未要約の理由 3 語。**しきい値は持たない** |
| 辺の型ごとの使用件数 | **あり**（同上。2026-09-05） | 観測値 1 件＝辺 1 本で、内訳の軸に型名を載せる |
| 未定義型のフォールバック警告件数 | **別経路**（可観測性スタックのカウンタ） | ダッシュボードのパネルで観測する |
| 取り込み経路で辞書に無いタグが現れた件数 | **別経路**（同上） | **この 1 指標だけ 0 が正常である** |

🔴 **それでも画面には出さない。** **別経路**の 2 指標は集計 API では 0 件として返り、その 0 は
