...
**［残件］改名・削除に BFF 口はまだ無い。** #634 の `POST /tags` と同じく、口は DocumentService 側にだけ在る
（`/bff/attribute-values` は読み取り専用である）。**SC-09 の画面から改名・削除を操作するには BFF の書き込み口が要る**——
#542 の射程外なので、**#640 へ起票した**。
