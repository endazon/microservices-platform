...
> （FR-19 / FR-21。[[IADR-0119]] の着手保留）、(2) 対象範囲フィルタと出典行の機密区分チップ
> （BFF 契約に載る先が無い）、(3) 共通シェルのパンくず・右レール。
...
| 6-b | **出典行の機密区分チップ**（`社内限`。428-429） | **しない** | **出典に機密区分が載っていない**（`CitationDto` に属性フィールドが無い）。§実装しない要素の理由 (c) |
...
**契約の不足として環流の記録を作成した**（[feedback/20260804_sc01-03-bff-contract-gaps.md](../../feedback/20260804_sc01-03-bff-contract-gaps.md)。
**計画リポジトリへの起票は未了**である）。
...
**(c) 出典行の機密区分チップ（モック #6-b）— 契約に項目が無い。**
hi-fi `sc-01.html:428-429` は出典 1 行に**チップを 2 つ**描く——`社内限`（機密区分）と `組織文書`（種別）である。
実装は後者だけを出す。前者を出せないのは、`CitationDto` が
`(Number, DocumentId, DocumentTitle, ChunkId, SourceUri, Score, Snippet)` であり
**属性フィールドを持たない**ためである（`src/knowledge/backend/Shared/Knowledge.Contracts/Dtos/SearchResultDto.cs`）。
値が無いのに機密区分を表示するには推測した固定値を書くしかなく、**機密区分の推測表示は
取り違えの影響が最も大きい種類の誤り**である。

**これは SC-03 の「機密区分の表示名が計画に無い」（[SC-03 §属性の表示](./SC-03_document-detail.md)）とは
別の論点である**——SC-03 は**値は取れるが表示名が計画に無い**、本件は**値そのものが取れない**。両方を [feedback/20260804_sc01-03-bff-contract-gaps.md](../../feedback/20260804_sc01-03-bff-contract-gaps.md)
...
1. **対象範囲フィルタ**（§実装しない要素 (a)）。BFF 契約の拡張が要る。**環流の記録は作成済み・起票は未了。**
