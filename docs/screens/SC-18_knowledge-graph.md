...
adrs: [ADR-0031, ADR-0033, ADR-0034, ADR-0039, ADR-0049, ADR-0054, ADR-0102]
iadrs: [IADR-0122, IADR-0124, IADR-0134, IADR-0242, IADR-0274, IADR-0386]
specs: [20260823_issue-917_sc18-graph-view, 20260905_issue-1240_sc03-graph-entry-and-placement-e2e, 20260913_1456_sc18-private-note-label]
issues: [#1240, #1456, #452, #911, #917, #962, #980, planning#446, planning#628]
-->

# 画面仕様書: ナレッジグラフビュー

> **［実装状態］`status: completed` は「本仕様書が記述する範囲の実装とテストが揃った」ことを表す**
> （`docs/README.md` 運用ルール 6）。未実装なのは任意要素（レイアウト切替・次数比例のノードサイズ）
> だけである。詳細は §計画（モックアップ・画面設計）との対応 と §未決事項 を見ること。
>
> **［2026-09-05 / #1240］文書詳細画面からの導線が着地した。** 従前ここには
> 「任意要素**と**文書詳細画面からの導線は未実装」と書いていたが、**導線は本画面の範囲外として
> 別 issue へ渡していたもの**であり、その issue が判断を残さずに閉じたまま取り残されていた。
> 本画面から見た変化は「入口が 1 つ増えた」だけで、**本画面のコードは変わっていない**
> （起点は従前どおり `root` クエリで受ける）。
...
- **組織文書＝円＋📄 / 個人資料＝角丸四角＋👤（破線の輪郭）**。ホバーでタイトルと種別ラベルを表示。
  - 🔴 **［2026-09-13 是正］種別ラベルは「組織文書」「個人資料」の 2 値であり、個人資料に括弧書き「（自分のみ）」を付けない**
    （凡例・ホバー・側パネルとも）。**本画面は他の利用者が所有する個人資料も描き得る**（所有者がグラフ表示を ON にし、
    閲覧者が読める場合）が、**ノードの契約は所有者を運ばないため所有者を判別できない**。
    括弧書きを所有者にだけ付す規則に対し、**判別できない面では一律で外すのが適合である**（付けない側に倒す）。
...
| 文書詳細画面からの導線 | する | **［2026-09-05 実装］**文書詳細画面の側に置く（`/graph?root=<id>&hops=2&by=distance`）。**本画面のコードは変わっていない** —— 起点は従前どおり `root` クエリで受ける。ラベルは左ナビ・パンくずと同じ「ナレッジグラフ」で揃える（**モックの「知識グラフ」は採らない**。計画 §用語 が名前の重複を避けている） |
