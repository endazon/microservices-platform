...
| 19 | **共通シェル**: パンくず（413。`ホーム / 運用 / ダッシュボード / 構成ビューア`）・ブランド／ロールバッジ／アバター（412）・左ナビ（414） | — | **対象外（本画面の作業ではない）。** パンくずと画面グループのバッジ（**#446 で実装した**。グループ段「運用」＋ 親画面「ダッシュボード」＋ 自分の 4 段。**権限外では 1 段も描かない**）も他も `app/Layout` が持つ |
