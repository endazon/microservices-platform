...
issues: [#14, #19, #446, #504, #506, #519, #544, #552, planning#190, planning#198]
-->
...
- [画面概要・目的](#画面概要目的)
- [構成バージョン履歴の正データ源（**計画は 2026-08-04 に確定済み**）](#構成バージョン履歴の正データ源計画は-2026-08-04-に確定済み)
- [hi-fi モックアップとの対応（実装する要素／実装しない要素）](#hi-fi-モックアップとの対応実装する要素実装しない要素)
- [レイアウト / 主要素](#レイアウト--主要素)
- [状態表示（INDEX 決定 21「色だけで意味を持たせない」）](#状態表示index-決定-21色だけで意味を持たせない)
- [表示・入力項目](#表示入力項目)
- [アクション・イベント](#アクションイベント)
- [画面遷移](#画面遷移)
- [データソース（BFF 境界）と縮退](#データソースbff-境界と縮退)
- [権限・表示条件](#権限表示条件)
- [実装](#実装)
- [関連仕様](#関連仕様)
- [未決事項](#未決事項)
...
| 19 | **共通シェル**: パンくず（413。`ホーム / 運用 / ダッシュボード / 構成ビューア`）・ブランド／ロールバッジ／アバター（412）・左ナビ（414） | — | **対象外（本画面の作業ではない）。** パンくずと画面グループのバッジ（**#446 で実装した**。グループ段「運用」＋ 親画面「ダッシュボード」＋ 自分の 4 段。**権限外では 1 段も描かない**）も他も `foundation/ui/Layout` が持つ |
