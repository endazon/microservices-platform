...
iadrs: [IADR-0121, IADR-0122, IADR-0124, IADR-0125, IADR-0131, IADR-0134, IADR-0135, IADR-0271, IADR-0272, IADR-0276]
...
> 🔴 **導線の先にある承認欄（文書詳細画面）はまだ無い**（別 issue）。
> 再提示理由の表示も、**理由が実際に付く経路がまだ通っていない**。
> どちらも §計画（モックアップ・画面設計）との対応 と §未決事項 に明記した。
> **「承認できる」とは読まないこと。**
...
flowchart LR
  ...
  SC03 -. 承認欄からの導線（別 issue・未実装） .-> SC21
  SC03 -- 承認・却下を実行（別 issue・未実装） --> SC03
```
...
| 承認・却下は本画面で実行しない | する | 承認・却下の口を公開 API にも置いていない（開けるのは承認欄と同じ変更単位） |
...
1. 🔴 **導線の先（文書詳細の承認欄）が未実装である。** 本画面から遷移しても、その画面に承認 UI は
   まだ無い。承認欄は別 issue の射程であり、本画面は先に置いても害が無い（棚卸しの用途は満たす）
   が、**「承認できる」と読まれないよう説明文で明示している。**
