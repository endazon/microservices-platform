...
---
<!-- trace:
ids: [FR-05, FR-17, FR-18, SC-03, SC-05, SC-09, SC-18, SC-21, UC-10]
adrs: [ADR-0031, ADR-0033, ADR-0034, ADR-0043, ADR-0050, ADR-0051, ADR-0063]
iadrs: [IADR-0121, IADR-0122, IADR-0124, IADR-0125, IADR-0131, IADR-0134, IADR-0135, IADR-0266, IADR-0271, IADR-0272, IADR-0276, IADR-0300, IADR-0364, IADR-0380]
specs: [20260823_issue-918_sc21-ai-suggestions, 20260829_issue-450_ai-suggestion-approval, 20260903_issue-1187_tag-suggestion-reflection-and-dictionary, 20260905_issue-1244_similarity-candidate-source]
issues: [#450, #452, #911, #914, #915, #918, #1014, #1187, #1244, planning#495]
-->
...
> 遷移した先で、**リンク提案・タグ提案とも 1 件ずつ承認・却下できる**。
> **［2026-09-03］タグ提案の承認は対象文書のタグへ反映されるようになった**（従前の
> 「タグ提案は却下だけできる」は失効。資格の扱いは文書詳細画面の仕様書）。
> 🔴 **本画面が書き込みをしないことは変わらない。** 承認・却下は文書詳細でのみ実行する。
> 再提示理由の表示は、**理由が実際に付く経路がまだ通っていない**
> （§計画（モックアップ・画面設計）との対応 と §未決事項）。
> **［2026-09-05］一覧に並ぶ提案を実際に生む供給元が入った。** 従前は生成の類似度候補の供給元が
> 「常に空」で、**本画面は実運用で永久に空だった**（一覧・承認欄・状態遷移はすべて緑のテストを
> 持ちながら、守る対象が 1 件も生まれていなかった）。生成の口はサービスに在るが、**利用者が生成を
> 起動する導線は本画面にも文書詳細にも無い**（§未決事項 4。計画側の裁定待ち）。
...
| タグ提案がタグ辞書の値域に収まる | **する** | **［2026-09-03］生成段と承認段の両方で強制する**（従前の「どの層も強制していない」は失効）。生成段は辞書の名前集合を言語モデルに選ばせる値集合として渡し、返った値を辞書と突き合わせて辞書外を落とす（辞書が引けなければタグ提案を作らない）。承認段は文書サービスが辞書を権威として辞書外を拒み、提案は承認できず却下のみになる。辞書の読み取り主体はサービス自身（内部口）であり、**利用者へ辞書は丸ごと返らない** |
...
1. ~~🔴 **導線の先（文書詳細の承認欄）が未実装である。**~~
   **［2026-08-29 解消］承認欄は実装された。** 各行の導線から遷移した先で 1 件ずつ承認・却下できる。
   ~~**ただしタグ提案は承認できない**~~ **［2026-09-03 解消］計画側の裁定が降り、タグ提案の承認は
   対象文書のタグへ反映されるようになった。**
2. **再提示理由が実際に付く経路が通っていない**（上表参照）。
3. ~~**タグ提案の値域強制が無い**（上表参照）。~~ **［2026-09-03 解消］生成段・承認段の両方で
   強制する**（上表参照）。
4. **提案の生成頻度・滞留時の扱い・提案件数の通知の要否**は計画側が未確定である。
   通知が要ると決まれば、左ナビのバッジ等は本画面ではなく共通シェルの関心になる。
   **［2026-09-05 追記］** 生成そのもの（起点文書 → 類似度候補 → 権限スコープで絞る → 言語モデル → 承認待ち）は
   **利用者の要求時にその利用者のスコープで**走る形でサービスに実装済みで、類似度候補の供給元も入った。
   **無いのは利用者が生成を起動する導線**（文書詳細のボタン／文書更新時の自動起動／定期）であり、
   計画の文書詳細画面は置く要素を 2 つに限っているため実装で先取りしない。計画側へ裁定を依頼する。
