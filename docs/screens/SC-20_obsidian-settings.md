...
adrs: [ADR-0031, ADR-0036, ADR-0037, ADR-0046, ADR-0054, ADR-0096, ADR-0099]
iadrs: [IADR-0121, IADR-0124, IADR-0125, IADR-0127, IADR-0131, IADR-0134, IADR-0135, IADR-0262, IADR-0270, IADR-0338, IADR-0352, IADR-0437, IADR-0444, IADR-0446, IADR-0447]
specs: [20260828_issue-451c_sc19-sc20-screens, 20260828_issue-451a_private-notes-bff, 20260902_issue-1098_obsidian-plugin-pull-stage1, 20260903_issue-1153_obsidian-plugin-push-delete-conflict-stage2, 20260912_1441-1442_private-note-contract-gaps, 20260912_1445-1446_share-targets-and-sync-history, 20260912_1447-1448_current-groups-binding-and-set-valued-matching]
issues: [#451, #1098, #1153, #1441, #1442, #1446, #1447, planning#614, planning#618]
-->
...
>
> ［2026-09-12 追記］**同期履歴（主要素 6）を実装した。** 計画側の裁定で**供給元・表示件数・保持期間・
> 行の内容・失敗の扱い**がすべて定まったためである。供給元は**既存の同期監査ログ**であり、専用の
> 履歴ストアは持たない。**表示は直近 50 件・保持は 3 年**で、どちらも画面が決めた値ではない。
> 🔴 **行に資料のタイトルと Vault のパスは出さない**（同じ裁定。記録そのものが持たない）。
> 🔴 **一時停止／再開は依然として実装していない**（**呼ぶ口が無い**）。
> 見送りの一覧は §計画（画面設計）との対応 にある。
>
> ［2026-09-12 追記］**履歴が 0 件のときの案内を 2 つに読み分けた。** 履歴は**本機能の配備後の同期から**
> 記録されるため、配備前に同期していた利用者には 0 件が「記録されていない」のか「同期していない」のかが
> 区別できない —— **同じ 0 件に 2 つの意味がある。** 接続端末のいずれかに最終同期があれば
> 「配備前の同期は表示されない」と告げ、無ければ従前の案内を出す（題は共通のまま）。
...
| 19 | 同期履歴 | 競合の区画の**下**に置く（失敗理由の文言が「上の『同期の競合』から解決してください」と上を指すため）。列は 実行日時 / 端末 / 方向（送信・受信）/ 内訳 / 結果 / 失敗理由。0 件は「同期の記録はまだありません」（案内は下の 23 で読み分ける） |
| 20 | 履歴の内訳セル | 追加・更新・削除・競合のうち **0 でない項目だけ**を並べる。すべて 0 の行は「—」（空欄にしない） |
| 21 | 履歴の失敗理由 | 契約はコードだけを運び、**利用者が次に何をすればよいか分かる文言は画面が持つ**（7 コード ＋ 未知のコードはコードを添えた文言）。結果は色だけでなく「成功」「失敗」の文言でも示す |
| 22 | 履歴の但し書き | 「直近 50 件を表示します。記録は 3 年間保持されます。資料の名前は履歴に残しません。」を表の直後に置く。🔴 **タイトル・パスの列を持たない**（型にも無い） |
| 23 | 履歴 0 件の読み分け | ［2026-09-12 追記］**題は共通**（「同期の記録はまだありません。」）で、案内だけを分ける。接続端末のいずれかに最終同期がある → 「同期履歴の記録は本機能の配備後の同期から残ります。配備前の同期は表示されません。」／無い → 「Obsidian プラグインから同期すると、ここに結果が並びます。」。🔴 **判断の材料は端末一覧の最終同期**であり、履歴の区画はこの値を自分で引かない（同じ一覧を 2 度引かない） |
...
| 同期履歴 / エラー表示 | する | 供給元は**既存の同期監査ログ**であり専用の履歴ストアを持たない（計画側の裁定）。表示は直近 50 件・保持は 3 年で、**どちらも画面が決めた値ではない**。失敗も並び、理由は「次に何をすればよいか」の文言へ写す。🔴 **タイトル・パスの列を持たない**ことを単体テストとブラウザ E2E が陽性対照（端末の列は在る）つきで固定する。［2026-09-12 追記］**0 件の案内は端末の最終同期で読み分ける**（両側を単体テストとブラウザ E2E が陰性対照つきで固定する） |
| 同期の一時停止 / 再開 | **しない** | 🔴 **呼ぶ口が公開 API に無い**（§未決事項 2）。停止したように見えて同期が続く面を作らない |
...
2. **［2026-09-12 一部解消］同期履歴を実装した**（供給元・件数・保持期間・行の内容・失敗の扱いが
   裁定で定まった）。**同日、0 件の案内を 2 つに読み分けた** —— 履歴が配備後の同期からしか残らないことは
   仕組みの帰結であり解消できないが、**利用者がそれを読み取れる状態にはした**（前掲 主要素 23）。
   🔴 **残るのは一時停止／再開であり、呼ぶ口が無い。**
   **「さらに読み込む」の追加取得も置いていない** —— 裁定は実装設計に委ねたが、要望が出てから足す
   （50 件固定のほうが「表示の件数」と「保持の件数」を混同させにくい）。
   🔴 **未解決のまま残る競合のローカル本文をいつ消すかは、依然として未定である** ——
   履歴の保持（3 年）は**監査ログの数**であり、競合の本文は監査ログではないため揃える根拠が無い。**据え置き。**
3. 🔴 **利用者単位の露出の既定が無い。** 資料単位の上書きだけが在る状態であり、
   計画の「本画面の設定を利用者単位の既定とし、資料単位の上書きが優先する」を満たしていない。
4. 計画が未確定としている論点のうち、**同期の監査ログの記録内容と本人の閲覧は 2026-09-12 の裁定で確定した**。
   🔴 **第三者（管理者・運用者・監査担当）が読めるかは未確定のまま残る** —— 本人の閲覧は
   egress ポリシーの論拠の範囲内だが、第三者の閲覧は**到達する主体を増やす**ため同じ裁定では扱われなかった。
   **画面はこの区画を本人のものとしてのみ描く。**
