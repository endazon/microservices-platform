...
2. **バックリンク欄・ローカルグラフ**。計画は実現方式を未確定としている。描画が SPA 側へ移ったため
   ...
   🔴 **［2026-09-05 / #1240］本画面のブラウザ E2E は、この 2 つについて何も固定していない。**
   計画は「バックリンク欄・ローカルグラフは本画面のみに置き、文書詳細画面には併置しない」と
   確定しており、**文書詳細画面の側の不在は E2E で固定した**。**対になる「本画面に在ること」は
   固定していない** —— 実現方式が未確定だからである。**不在も固定していない**：
   「置かないと決めた」（文書詳細画面の側）と「まだ決まっていない」（本画面の側）は同じ形に見えて
   根拠が違い、後者を固定すると**計画が決めて実装した瞬間に正しい実装がテストを落とす**。
   実現方式が確定したら、そのときの issue が本画面の E2E へ陽性の固定を足す。
