...
issues: [#438, #1102, #1143, #1144, #1245, #1301, #1307, #1410, #1470, #1500, #1525, #1546, planning#650, planning#659]
-->
...
**判定条件（計画の裁定）**: 測定条件を揃えた反復を 3 回以上行い、**1 回目は暖機として捨て**、
残る各反復で実在／非実在の中央値の比が**自己対照**を超えないこと。
自己対照とは**実在側の標本を 2 群に分けた中央値の比**であり、**その環境の測定ノイズの大きさ**である。
🔴 **許容の値を固定値で置かない** —— ノイズの大きさは環境ごとに違い、固定値は速い環境では緩すぎ、
遅い環境では永久に赤になる。**環境自身に測らせる。**
🔴 **自己対照が広ければ `評価不能` であり、合格ではない。**
**「ノイズに埋もれて見えない」を「差が無い」と読まない。**

**［2026-09-25 改訂］判定は 2 段である（計画の追加の裁定）。** 段 1: 実在／非実在の中央値の差が
**自己対照の刻み**（自己対照の中央値が取り得る最小の間隔。標本の分解能と群の大きさから導く）以下なら、
比と自己対照を比べずに合格とする。段 2: 差が刻みを超えれば、上の「比が自己対照を超えないこと」で判定する。
🔴 **自己対照は比より粗い刻みでしか動けず、ちょうど 1.00 に落ちると上限として機能しない**
（床の内側 1 ms の揺れで赤が常態化した）。**表現できない差を上限に使うと、測れないものを測ったことにする。**
...
攻撃者は回数制限の無さを使って推定し得る。検査はそれを分解能未満では検出しない。
