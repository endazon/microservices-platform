...
specs: [20260823_issue-438_keycloak-theme-and-smtp, 20260828_issue-439_sc16-account-settings, 20260831_issue-1102_keycloak-smtp-externalsecret-wiring]
...
>
> 🔴 **さらに、その未成立が「存在秘匿」を破っている。** 稼働環境で測ると、**実在する利用者名の申請は
> 500・実在しない利用者名の申請は 200** を返す。**この差だけで利用者名を列挙できる**
> （§計画（モックアップ・画面設計）との対応 の該当行・§未決事項）。**別途起票済みである。**
...
| 存在秘匿（常に「メールを送信しました」） | 🔴 **しない**（**測って偽と分かった**） | **`smtpServer` が未設定である間、応答が登録有無で分かれる。** 稼働環境での実測: 実在する利用者名を送ると **HTTP 500**「Eメールの送信に失敗しました」、実在しない利用者名を送ると **HTTP 200**「詳細な手順を記載したEメールをすぐに受信してください」。**この差だけで利用者名を列挙できる。** Keycloak の `reset-credentials` フロー自体は既定で存在秘匿だが、**送信の失敗が秘匿の手前に出てくる**。**実値を入れても、SMTP が落ちれば同じ形で再発する**（§未決事項） | 同上 |
...
- 🔴 **存在秘匿の破れをどう塞ぐか（未決・別途起票済み）。** 送信に失敗したときだけ 500 を返す挙動は、
  **実在する利用者名を当てる判定器になる**（§計画との対応 の実測）。**`smtpServer` に実値を入れても
  解けない** —— リレー先が落ちれば同じ差が復活する。**「メールが送れないこと」と「そのアドレスが
  登録されていないこと」を利用者に区別させない**設計が要る（応答を揃えたうえで、失敗は
  [運用ダッシュボード](./SC-10_operations-dashboard.md) 側で観測する）。
- **`smtpServer` の実値投入時期**。実環境の値が供給されてから（手順は整備済み）。
- **開発環境の捕捉用 MTA が未配備（別途起票済み）。** 計画は「開発環境では実送信しない。捕捉用 MTA を
  置く」と確定しているが、実体が無い。[テスト仕様書](../tests/SC-15_password-reset.md) の
  メール経路の項目が「手動（実環境）」で止まっているのはこれが理由である。
