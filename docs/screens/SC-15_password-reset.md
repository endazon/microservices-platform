...
**自己対照の刻み**（自己対照の中央値が取り得る最小の間隔。整数 ms の標本・各群 3 標本では 1 ms）以下なら、
...
**刻み未満の系統差は検出しない** —— 同じ測定器を使う攻撃者にも見えない差であり、受け入れたリスクである。
