...
| **既定のライフサイクル状態** | `Select` | 任意 | **`draft` / `active` / `archived`**（正本は計画 07_abac-attribute-model の `lifecycle` 属性。05_screens §SC-05 が「状態の語彙はこれを正とする」と再確認）＋ **「未指定」** | **［2026-08-16 / #796］** **既定の選択は「未指定」**であり、**選んだときだけ** `defaultAttributes.lifecycle` として送る。**未指定ならキーごと送らない**。この場合サーバが既定値 `active` を入れる（[[IADR-0199]] 決定 4）。**「未指定」を選べることが要件である** —— 計画が「終端の `active` は**指定が無いときだけ**効く」と定めており、`active` を初期選択にすると「明示的に active を指定した」と「指定しなかった」の区別が消える。**`department` と違い終端が予約値ではなく正規の値なので、値では見分けられずキーの有無だけが区別を持つ。** 値域を実装が決めないのは機密区分と同じで、**計画に無い値（`normalized` / `published`）を持ち込まない**（計画が名指しで「計画側の語彙ではない」と書いている） |
