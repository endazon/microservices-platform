...
- 403/404 も中立に扱う。5xx/network は取得失敗（`role="alert"`）。
...
| ok（空縮退） | 200・answer 空/citations 空 | 中立「該当する情報が見つかりませんでした。」 |
| error | 400/403/404/5xx/network | `role="alert"` 実行に失敗（400 はクライアント検証で予防） |
