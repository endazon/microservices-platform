...
| リカバリーコードを登録完了時に 1 回のみ表示 | **しない** | Keycloak の標準機能ではなく、テーマ／必須アクションの作り込みが要る。#438 で扱う | 同上 |
| リカバリーコードを SC-16 から再発行 | **しない** | SC-16（アカウントコンソールのテーマ）が未実装。#438 の射程 | `01_screens.md` §SC-16 |
...
- **リカバリーコードの実現方式**。Keycloak には「リカバリーコード」の標準機能が版により無い（`recovery-authn-codes` は新しめの版の機能）。**採用する版で標準機能が使えるかは実環境の Keycloak 版に依存する**ため、#438 で版を確定してから決める。
