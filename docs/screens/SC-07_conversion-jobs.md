...
> **再変換の確認は 409 `corrections_would_be_lost` を受けてから出す**（[[IADR-0157]]）。
...
> **★［2026-08-10 追記 / #553］planning#198 提案 8（閲覧ロール）の裁定が出た。**
>
...
>
> **つまり [[IADR-0039]] 決定 1（admin または operator）が正しく、[[IADR-0128]] 決定 2 の据え置きは適切だった。**
> **再変換の管理者限定（2026-08-04 確定）は維持**であり、#501 の是正はそのまま有効である。
> **変わったのは閲覧側の向きだけ**である。
>
> 本文および §未決事項 に残る「planning#198 提案 8 の裁定待ち」は
> **当時の状況として正しい記録**なので書き換えない。**現在を示すのは本追記である。**

1. ~~**人手補正の 2 ペイン編集**（§実装しない要素 (a)）。補正投稿 API と本文取得 API が要る。~~
   **契約・画面ともに解消済み**（契約 = 2026-08-10 / #543・[[IADR-0154]]。画面 = 2026-08-10 / #651・
   [[IADR-0157]]）。planning#198 の裁定（2026-08-05）で **Phase 1 = 図のコード化のやり直し**に
   範囲が確定し、契約とその画面を実装した。
