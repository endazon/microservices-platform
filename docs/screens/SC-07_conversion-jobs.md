...
> 「人手補正」2 ペイン編集と「デッドレター」の内訳表示、共通シェルのパンくず・右レール。
> **どちらも契約は載っており（#533 / #543）、残っているのは画面側の作業だけである。**
> 詳細と引き受け先は §hi-fi モックアップとの対応 と §未決事項 を見ること。
>
> **［2026-08-10 / #543］人手補正の契約の不在も解消した。** `GET /bff/conversion/jobs/{id}/figures`・
> `.../figures/{figureId}/image`・`POST .../figures/{figureId}/correction` の 3 本が載った（[[IADR-0154]]）。
> **範囲は Phase 1 = 図のコード化のやり直しに確定している**（旧「変換結果 Markdown 全体の 2 ペイン編集」は
> Phase 2 へ繰り延べ。計画 `01_screens.md:330`）。**本画面の表示は依然として未実装である**——
> 理由が「契約に載る先が無い」から「画面側の作業がまだ」へ変わった。
>
...
| 10 | 行操作「**人手補正**」（420） | **しない** | **契約は #543 で載った**（[[IADR-0154]]）。**画面側が未着手**。§実装しない要素の理由 (a) |
| 11 | 行操作「**結果 →**」（422） | **する** | `succeeded` かつ `documentId` があれば `/docs/$id`（SC-03）への内部リンク。計画の遷移図 `SC07 -- 変換結果 --> SC03` |
| 12 | **人手補正パネル**「図コード（編集可）／元の図画像」＋「補正して再登録」（425-430） | **しない** | 同上 (a)。**Phase 1 は図のコード化のやり直しに範囲確定**（旧「変換結果 Markdown 全体の 2 ペイン」は Phase 2） |
...
1. ~~**人手補正の 2 ペイン編集**（§実装しない要素 (a)）。補正投稿 API と本文取得 API が要る。~~
   **契約は解消済み**（2026-08-10 / #543。[[IADR-0154]]）。planning#198 の裁定（2026-08-05）で
   **Phase 1 = 図のコード化のやり直し**に範囲が確定し、その契約を実装した。
   残るのは**画面へ出す作業**であり、別 issue の起票が要る。
