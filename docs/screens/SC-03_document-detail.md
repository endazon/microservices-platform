...
adrs: [ADR-0102, ADR-0031, ADR-0033, ADR-0034, ADR-0035, ADR-0063, ADR-0070, ADR-0073]
iadrs: [IADR-0451, IADR-0009, IADR-0038, IADR-0119, IADR-0121, IADR-0124, IADR-0126, IADR-0272, IADR-0276, IADR-0290, IADR-0300, IADR-0323, IADR-0355, IADR-0364, IADR-0365, IADR-0386, IADR-0388]
specs: [20260913_1455_sc03-private-note-display, 20260804_issue-502_sc01-03-search-flow, 20260828_issue-1011_version-body-contract, 20260829_issue-450_ai-suggestion-approval, 20260831_issue-1104_suggestion-document-filter, 20260903_issue-1187_tag-suggestion-reflection-and-dictionary, 20260903_issue-1200_sc04-wiki-screen-via-bff, 20260905_issue-1240_sc03-graph-entry-and-placement-e2e, 20260905_issue-1253-1254_bodyless-index-and-hasbody-vocabulary]
issues: [#1455, #1011, #1014, #1104, #1187, #12, #1200, #1240, #1254, #446, #449, #450, #452, #490, #502, #504, #519, #541, #553, #586, #917, #918, planning#197, planning#237, planning#244, planning#473, planning#495]
-->
...
🔴 **［2026-09-13 新設］対象が個人資料のときだけ出る欄である。** 所有者以外がこの画面を開く経路は 2 つある ——
**共有された相手**と、**退職・無効化から 30 日の窓で閲覧する管理者**（平時の管理者は読めない）。
したがって「自分の資料しか開かれない」前提を置かない。

| 要素 | 所有者 | 所有者以外 |
| --- | --- | --- |
| 👤 のラベル | 「個人資料（自分のみ）」 | 「**個人資料**」（括弧書きを付けない） |
| 所有者 | **表示名**（利用者名は出さない） | **同じ**（誰の資料かは問い合わせ先・取り扱いの判断に要る） |
| 公開範囲（非公開／個人指定／グループ指定） | **描く** | 🔴 **欄ごと出さない**（空欄・既定値も描かない。描いた形そのものが共有の有無を伝えるため） |
| 個人資料管理への導線 | 置く | 置く |

- **公開範囲の供給は「所有者だけが読める口」である。** 応答の共有先の写しからは導かない ——
  **写しは指定先の種別を運ばず、個人指定とグループ指定を区別できない。** 3 状態は**後段が導出した値**を
  そのまま描き、個人資料管理の画面と導出点を 2 つにしない。
- **件数は出さない。** 本画面が描くのは**区分**であり、指定先と件数は個人資料管理の持ち場である。
- **表示名を引けないときは「（不明な利用者）」と描く。** 🔴 **利用者名へフォールバックしない** ——
  フォールバックを許すと「識別子を出さない」統制が失敗時にだけ破れる。**無効化済み（退職者）でも表示名は出る。**
- **所有者かどうかの突き合わせは表示の分岐にだけ使う。** 可視性を決めるのは後段であり、画面は 2 本目の判定軸を持たない。
...
| `department` | 部門 | 同上 |
| 上記以外 | **描かない** | 🔴 **［2026-09-13 改訂］本パネルは計画が挙げる 3 カテゴリに閉じる**（下記） |

🔴 **［2026-09-13 改訂］未知のキーは描かない（whitelist）。** 従前は「上記以外はキーをそのまま表示」としており、
**応答に載る属性がそのまま画面へ出ていた** —— 個人資料では所有者（利用者名）・文書スコープ・露出 3 トグルの生の行が、
**読める者すべてに出ていた**。計画は本パネルを「機密区分・部門・タグ」と定めており、**計画が正である**
（実装が先行して乖離していた）。落ちた値は消えるのではなく、**それぞれの持ち場**が描く ——
所有者は下記 §個人資料の表示、個人資料であることは 👤 のラベル、露出 3 トグルは個人資料管理の画面である。
**blacklist（3 キーを名指しで落とす）にしない** —— 次に属性が増えたときに黙って画面へ出るためである。
