...
iadrs: [IADR-0009, IADR-0038, IADR-0119, IADR-0121, IADR-0124, IADR-0126, IADR-0272, IADR-0276, IADR-0290, IADR-0300]
specs: [20260804_issue-502_sc01-03-search-flow, 20260828_issue-1011_version-body-contract, 20260829_issue-450_ai-suggestion-approval]
issues: [#12, #446, #450, #452, #490, #502, #519, #541, #553, #586, #918, #1011, planning#197, planning#237, planning#244, planning#473]
-->
...
> **未実装のまま残っている要素がある**——(1) ナレッジグラフビューへの導線
> （文書間リンクの要求。着手保留の実装 ADR による）、(2) 機密区分の表示名、
...
> ナレッジグラフビューへの導線」と 2 つを 1 項目にまとめていたが、**承認欄だけが着地したので
> その形は誤りになる。** 承認欄の仕様は §AI 提案の承認欄 に、対応表は #8 にある。
> **導線のほうは依然として未実装である**（ナレッジグラフ画面と同じ段で足す）。
>
...
| 8 | **「AI 提案 — この文書に対するリンク・タグ候補」パネル**（423-429） | **一部する** | **［2026-08-29 実装］**§AI 提案の承認欄 参照。**満たしていない条件: タグ提案の「承認」を実行できない** —— 後段はタグ提案を承認しても状態を変えるだけで**文書のタグは 1 つも増えない**（反映経路が未実装）。押せる承認ボタンは偽の作用を約束するため、**表示はし・承認だけを理由つきで実行不可にし・却下は実行できる**とした。計画との差異であり、計画側へ環流する（§未決事項 5） |
...
| 11 | **共通シェル**: パンくず「ホーム / 検索結果 / …」（413）・ブランド／アバター（412） | — | **対象外（本画面の作業ではない）。** パンくず（**#446 で実装した**。本画面の最後の段は文書タイトルであり、画面側が実行時に渡す）・ブランド・アバターは `foundation/ui/Layout` が持つ |
...
### 実装しない要素の理由（#7。**繰り延べであって放棄ではない**）

> **［2026-08-29］本節はもともと #7・#8・#9 の 3 行をまとめて説明していた。**
> **#8（AI 提案の承認欄）と #9（一覧への導線）は実装したので、本節が説明するのは #7 だけである。**
> 以下の本文は**当時の記録**として残す（#8・#9 についてはもう成り立たない）。
...
> **ナレッジグラフビューへの導線は、その画面の実装と同じ段で足す**（まだ足していない）。
> 同じ予告を書いた
...
**タグ提案の「承認」は実行できないものとして描く。却下は実行できる。**

- 後段はタグ提案を承認しても**状態を `approved` にするだけで、文書のタグを 1 つも増やさない**
  （タグの反映経路が実装されていない）。押せる承認ボタンを置くと、**利用者は「承認したのに
  タグが付かない」を「反映が遅れている」と読む**。無い作用を約束しないほうがよい。
- **隠さない。** 隠すと、AI 提案一覧の全行が持つ「文書詳細で確認」の導線がタグ提案について
  行き止まりになる（同画面は「導線を持たない行」を描いてはならないと定めている）。
  加えて**却下は完全に効く**操作であり、隠すと承認欄からも一覧からも処理できなくなる。
- 押せない理由は**画面上のテキストとしても出す**（無効なボタンだけを置くと理由が読めない）。
...
**当該文書での絞り込みは画面側で行っている。** 一覧の口は状態と種類しか受けず、文書での
絞り込みを持たないためである。**権限による絞りはサーバ側で済んでおり**（権限外の文書に関する
提案は件数を含め届かない）、画面側の間引きは表示対象の限定であって秘匿の実施点ではない。
サーバ側へ絞り込みを足す追随は §未決事項 6。
...
1. ~~**AI 提案の承認欄**~~・**ナレッジグラフビューへの導線**。着手保留の実装 ADR の解除後に着手する。
   ...
   **［2026-08-29］承認欄は実装した**（§AI 提案の承認欄）。**残るのはナレッジグラフビューへの導線だけ**であり、
   その画面の実装と同じ段で足す。
...
5. **タグ提案の承認に反映先が無い**（§タグ提案の扱い）。承認しても文書のタグは増えない。
   ...
   裁定が出るまで、承認は理由つきで実行不可のままにする。
6. **提案の絞り込みがサーバ側に無い**（§絞り込みの位置）。本画面は承認待ちの権限内全件を取得して
   画面側で絞っている。件数が増えると無駄が大きく、サーバ側へ文書での絞り込みを足す追随が要る。
