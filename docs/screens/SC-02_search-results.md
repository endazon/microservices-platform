...
> **未実装のまま残っている要素がある**——検索モード切替・並び順・更新日時列（BFF 契約に載る先が無い）と、
...
| 6 | 結果テーブル：**タグ**列（419・421-423） | **する** | `Tag`（分類）。0 件のときは空欄 |
...
3 件はまとめて環流の記録に載せた（[feedback/20260804_sc01-03-bff-contract-gaps.md](../../feedback/20260804_sc01-03-bff-contract-gaps.md)。
**計画リポジトリへの起票は未了**である）。
...
1. **並び順・検索モード・更新日時**（§実装しない要素）。BFF ／ 検索サービスの契約拡張が要る。
   **環流の記録は作成済み・起票は未了。**
