...
1. **並び順**（§実装しない要素 (a)）。**裁定済み**（planning#197 Q5: 関連度〔既定〕／更新日時の新しい順の
   **2 値**。タイトル順・作成者順は採らない）。**残るのは契約と実装**であり、**#532 が引き受ける**。
   #532 は本画面の「更新日時」列と同じ索引項目をソートキーに使う。
   **［2026-08-09 / #536］更新日時（旧 (c)）は解消済み**（契約・索引・画面のすべて）。
   **検索モード（旧 (b)）は #531 が契約だけを解消しており、切替 UI は未実装である**（§実装しない要素 (b)）。
