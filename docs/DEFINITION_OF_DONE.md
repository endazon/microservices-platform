...
`scripts/` の検査器のうち **4 本**は、読む対象が作業ツリーではないため**偽の緑を返しうる**。
...
| C | 作業ツリー | （順序に依存しない） | 29 本 | いつ走らせてもよい |

**したがって順序は `git add -A` → 検査器 → コミット → `check-doc-updated.js` / `check-commit-messages.js`
→ push とする。** 該当の 4 本は、この条件に当てはまるとき**警告を出す**（**失敗はしない**——差分ベースの
検査器は「偽の緑」を返す条件を警告するに留める、という方針である）。
**警告が出たら結果を信用せず、順序を直して走らせ直す。**
...
- [ ] **カバレッジ床を下回っていない**。バックエンドは [`src/coverage-floor.json`](../src/coverage-floor.json)、
      フロントは [`src/vitest.config.ts`](../src/vitest.config.ts) の `thresholds` である。
      **テストを増やしたら床を引き上げる**（ratchet。床の据え置きは事実上の緩和である）
- [ ] **バックエンド標準ライブラリの決定で不採用となったライブラリを増やしていない**。移行したら `scripts/backend-library-baseline.json`
...
クロス監査の運用を定めた実装判断が限定するのは**巡数と再走範囲だけ**であり、
「監査を省略してよい」と読み替えない。
...
- [ ] 2 巡目以降は**是正差分のみ**を渡した（**巡数の上限は無い**。上限は上流裁定に従って撤廃済みである）
- [ ] 監査が**打ち切り条件を明言**している（「これ以上の巡回は不要」または要る範囲の限定）
      —— **上限を外した以上、際限のない巡回を抑えるのはこの項である**
...
> **同型の契約追加を束ねた PR では監査の *対象* が増える**（issue ごと ＋ 束ごと）。
> 本項が限定するのは **1 対象あたりの巡数**であって対象の数ではない。
