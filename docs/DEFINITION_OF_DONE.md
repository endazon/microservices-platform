...
`scripts/` の検査器 32 本のうち **6 本**は、読む対象が作業ツリーではないため**偽の緑を返しうる**。

| クラス | 読むもの | **見えないもの** | 該当 | 打ち手 |
| --- | --- | --- | ---: | --- |
| **A** | `git show HEAD:` / `git diff …HEAD` | **未コミットの変更** | **2 本** | **コミットしてから走らせる** |
| **B** | `git ls-files` | **untracked の新規ファイル** | **4 本** | **`git add` してから走らせる** |
| C | 作業ツリー | （順序に依存しない） | 26 本 | いつ走らせてもよい |

**したがって順序は `git add -A` → 検査器 → コミット → `check-doc-updated.js` / `check-commit-messages.js`
→ push とする。** 該当の 6 本は、この条件に当てはまるとき**警告を出す**（**失敗はしない**。
