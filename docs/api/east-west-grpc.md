...
---
<!-- trace:
ids: [FR-01, FR-02, FR-03, FR-04, FR-05, FR-11, FR-12, FR-13, FR-16, FR-17, FR-18, NFR-02, NFR-09, NFR-16, UC-01, UC-02, UC-04, UC-07, UC-09, UC-10, SC-06, SC-12, SC-17]
adrs: [ADR-0004, ADR-0010, ADR-0011, ADR-0012, ADR-0013, ADR-0016, ADR-0017, ADR-0025, ADR-0029, ADR-0032, ADR-0034, ADR-0036, ADR-0038, ADR-0044, ADR-0062, ADR-0064, ADR-0074, ADR-0075, ADR-0076]
iadrs: [IADR-0009, IADR-0037, IADR-0101, IADR-0104, IADR-0110, IADR-0117, IADR-0122, IADR-0225, IADR-0253, IADR-0256, IADR-0272, IADR-0299, IADR-0316, IADR-0329, IADR-0335, IADR-0354, IADR-0378, IADR-0379, IADR-0384, IADR-0385, IADR-0397, IADR-0400, IADR-0401]
specs: [20260905_issue-1201_east-west-grpc-preconditions, 20260905_issue-1255_east-west-grpc-llm-embedding, 20260905_issue-1255_east-west-grpc-llm-completion, 20260906_issue-1255_east-west-grpc-authz]
issues: [#1201, #1255]
-->
...
- **状態**: gRPC 面を持つのは **4 経路** —— 参照実装（BFF → 認可サービスの権限スコープ解決）、
  埋め込み生成（取り込み・検索 → LLM ゲートウェイ）、テキスト生成（AI 分析・グラフ・変換 →
  LLM ゲートウェイ。一括と**逐次**）、そして**認可サービスの 5 呼び出し元**
  （AI 分析・グラフ・Wiki のスコープ解決＋データソース・MCP の利用者名簿）である。
  **並走中の正は REST** であり、gRPC は構成で opt-in する。残りの経路の移行は別 issue で展開する。
  🔴 **参照実装（BFF → 認可サービス）だけは配備上まだ 1 度も走っていない** —— BFF の
  `ServiceToken` / gRPC 宛先は helm・compose のどちらにも無く、realm の `bff` に
  service account の割当も無い（下記「未決事項」）。他の 8 呼び出し元は配線済みである。
...
- 🔴 **参照実装（BFF → 認可サービス）の配備上の未配線。** `ServiceToken` の資格情報と gRPC の宛先が
  ...
  したがって**この経路は配備上まだ 1 度も走っていない**。他の 8 呼び出し元は本書のとおり配線済みである。
