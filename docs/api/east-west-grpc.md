...
issues: [#1628, #1614, #1575, #1515, #1537, #1520, #1514, #1397, #1201, #1255, #1333, #1318, #1364, #336, #1557, #1532]
-->
...
- **利用者文脈を運べる呼び出し元**［2026-09-27 追記］: 🔴 `user` を信じるのは、呼び出し元が**機械クライアント**で、そのクライアント識別
  （`azp`）が文書サービスの許可集合 `DocumentRead:TrustedUserContextClients` に在るときだけである。**未構成なら `bff` だけ**で、
  構成すると既定を置き換える。許可集合に無い呼び出し元が `user` を付けると `PERMISSION_DENIED`（機械の主体へ読み替えない）。
  `user` を付けない呼び出しは、`platform-service` を持つどのサービスにも従来どおり開いている。
  サービス間トークンのロール（`platform-service`）は別プロジェクトのものを含む多数のサービスアカウントが持つため、ロールだけでは
  「利用者の代わりに読んでいる」ことの根拠にならない。BFF の client 名を変える配備では、先に文書サービスの許可集合へ足すこと。
