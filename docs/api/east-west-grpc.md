...
- **利用者文脈を運べる呼び出し元**［2026-09-27 追記］: 🔴 本文の `user` を信じるのは、呼び出し元が**機械クライアント**で、そのクライアント識別
  ...
  在るときだけである。**未構成なら `aianalysis-service` だけ**で、構成すると既定を置き換える（空白だけなら誰も信じない）。
  許可集合に無い呼び出し元が `user` を付けると、スコープを解決する前に `PERMISSION_DENIED`（機械の視野へ読み替えない）。
  `user` の無い要求は従来どおり誰にも `INVALID_ARGUMENT` である。サービス間トークンのロール（`platform-service`）は別プロジェクトのものを
  含む多数のサービスアカウントが持つため、ロールだけでは「利用者の代わりに検索している」ことの根拠にならない —— 従前はどれもが任意の利用者
  （管理者を含む）を名乗り、その利用者の権限で本文を読めた。AI 分析の client 名を変える配備では、**先に検索サービスの許可集合へ足すこと**
  （逆順だと AI 分析の検索が拒否され、警告を出して引用なしの回答へ静かに縮退する）。
