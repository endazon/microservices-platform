...
specs: [20260823_issue-445_mcp-server-integration, 20260828_issue-1020_internal-mcp-tools, 20260828_issue-452_sc12-mcp-client-management, 20260904_issue-1190_mcp-project-attribute-ban, 20260926_1515_mcp-tool-declarations-grpc]
issues: [#445, #452, #1020, #1190, #1514, #1515, #1516, #1517]
-->
...
| 項目 | 値 |
| --- | --- |
| 契約 | 共有契約の proto（`platform/mcp/v1`）。**申告スキーマの共有契約への昇格はこの proto で行った**。REST の JSON と項目名・数が一致する（試験で固定） |
...
- `endpoint` は申告どおりの**文字列のまま**運ぶ（意味を変えない）。ツールを gRPC で実行するときの扱いは未決である（下記「ツール実行口の応答エンベロープ」）。
...
>
> ［2026-09-26 追記］申告の収集は gRPC へ移したが、**実行の経路は移していない**。実行を gRPC へ移すときに
> 申告の `endpoint`（今は URL）をどう扱うかと、実行口を誰が実装するかは判断待ちである。
