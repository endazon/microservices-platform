...
adrs: [ADR-0004, ADR-0018, ADR-0021, ADR-0024, ADR-0034, ADR-0054, ADR-0062, ADR-0086, ADR-0117]
iadrs: [IADR-0269, IADR-0292, IADR-0297, IADR-0373, IADR-0379, IADR-0462]
specs: [20260823_issue-445_mcp-server-integration, 20260828_issue-1020_internal-mcp-tools, 20260828_issue-452_sc12-mcp-client-management, 20260904_issue-1190_mcp-project-attribute-ban, 20260926_1515_mcp-tool-declarations-grpc, 20260927_issue-1516_mcp-tool-execution-grpc]
...
| メッシュ内部（各サービスのツール実行の gRPC 面） | ツールを申告したサービス | ［2026-09-27 追記］ツールの実行（本サービスは**呼ぶ側**。下記「ツールの実行（gRPC）」）。🔴 受け口はまだどのサービスにも無い |
...
4. **ツールを申告したサービス**へ、申告名で実行を送る（gRPC。下記「ツールの実行（gRPC）」）。🔴 **宛先は申告したサービスとツール名だけで決め、申告の中身の URL は使わない**。
   実行できない（受け口が無い・経路が無い・時間切れ・拒否）ときは結果を返さず拒否する（fail-closed）
...
| 宛先 | 🔴 **公開構成で申告を突き合わせたサービス**の h2c アドレス（申告の収集と同じ `Mcp:GrpcServices:<サービス名>`）。要求の `tool` は**申告名**（公開名ではない）。**申告の中身から宛先を作らない** |
...
> 🔴 **受け口（各サービスの実行口）はまだどのサービスにも無い。** したがって今は、公開構成に載ったツールは一覧に出るが、
> 実行は受け口が無いことによる拒否で終わる。受け口は別の作業で作る。そのとき要求本文で運ぶものを、本サービスが解決した実行スコープから
> **利用者文脈（利用者と操作）とツールの引数**へ改め、受けたサービスが自分で認可を判定する形にする。
> **それまで、本文の実行スコープを信じて認可する受け口を作ってはならない**（そこへ到達できる誰もが任意の主体を名乗れる）。

### 要求と応答（現行）

- 要求: `tool`（申告名）・`arguments_json`（MCP クライアントの引数の JSON。文字列のまま）・`scope`（**暫定**。主体・種別・属性・**個人資料の除外制約**・
  必要スコープ。従前の REST の本文と同じ意味。受け口を作る作業で利用者文脈へ改め、この項目は番号ごと予約へ移す）
