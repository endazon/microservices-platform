...
iadrs: [IADR-0042, IADR-0043, IADR-0127, IADR-0137, IADR-0154, IADR-0356, IADR-0358, IADR-0388]
...
> 変換ジョブ画面（変換状況・失敗一覧・人手補正）と、正規化変換ユースケースの状況照会・再変換に用いる。

## 起点となる計画書（トレーサビリティ）

- **関連機能要求**: 文書正規化（pandoc 本文変換・図のコード化・オブジェクトストレージ保管）
- **関連ユースケース**: 文書を正規化変換する（状況確認・人手補正）
- **関連画面**: 変換ジョブ画面
- **ADR / 実装ADR**:
  - 変換ジョブ読み取りモデル（MVP インメモリ）
  - 変換ジョブ読み取りモデルの永続化（Postgres+EF）＋非同期ストア（本仕様書の対象）
  - デッドレター標識と試行上限（2026-08-06 / #533。`DeadLettered` 列の追加）
  - DB per Service（ConversionService 専用 DB `conversion_svc`）
  - メッセージング（`RawDocumentFetched` 受信・再変換再発行・**再試行→デッドレター**。後継の Wolverine 採用により Superseded・注記は #580）
...
変換状況を照会する手段が無かったため（読み取りモデルの新設）、変換コンシューマ（`RawDocumentFetchedConsumer`）が
受信・成功・失敗の各ライフサイクルを **ConversionJob** に記録する。

その後の永続化の判断により、当初のインメモリ MVP を **Postgres + EF Core** へ永続化した。id は `RawDocumentFetched.FetchId`
...
> ConversionJob は他サービスの DB エンティティと FK を持たない（DB per Service）。
...
## ConversionJobFigures（子テーブル。人手補正 Phase 1 の実装判断・#543）
...
- **デッドレター標識の生存期間**: `DeadLettered` は「この失敗で自動再試行を使い切った」
...
- **並行性（既知の制約）**: `StartAsync` の attempts++ は read-modify-write で楽観的並行制御を持たない。
  単一インスタンス（dev）前提。水平スケール時は行ロックまたは並行トークンを要する（永続化の実装 ADR のフォローアップ）。

## 永続化方針

- PostgreSQL、EF Core（`ConversionJobDbContext`）。DB per Service の方針に従い ConversionService 専用 DB `conversion_svc`。
