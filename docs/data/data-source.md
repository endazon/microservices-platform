...
| `department` | 投入元（ソース）の所属 → データソース既定属性 | **本欄の値**（＝データソース既定属性） | **`unassigned`** |
| `owner` | ソース側の更新者 → 予約値 | **本欄の値**（更新者は運ばれない） | **`system`** |

> **計画の 2 段が実装では 1 段に畳まれている理由。** `department` の「投入元（ソース）の所属」を運ぶ
> 手段は**データソース自身の設定値（＝本欄 `DefaultAttributes`）以外に無い** —— コネクタ契約
> `SourceItem(Path, ModifiedAt, Size)` に部門を運ぶフィールドが無いためである。
> したがって計画の 1 段目と 2 段目は**同じ入力を指しており**、実装では 1 段になる。
> **別の供給源があるのに実装が見落としている、という意味ではない。**
> （計画側の変更履歴 2026-08-15 も同じ簡略化をしている。）
