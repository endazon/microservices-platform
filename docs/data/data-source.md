...
| `department` | 投入元（ソース）の所属 → データソース既定属性 | **本欄の値のみ**（前段は**未実装**）。［2026-09-26］**登録時に未指定なら、登録した管理者の部門グループ（ちょうど 1 つのとき）のコードで本欄を補う** | **`unassigned`** |
| `owner` | ソース側の更新者 → 予約値 | 本欄の値 ＋ **写像表で解決したアイテム単位の更新者**（**Wiki / SaaS / 業務DB の 3 種が更新者を運ぶ**。ファイルサーバーは運べない） | **`system`** |
...
>   **［2026-09-05 更新 / #752］「更新者を載せるコネクタが無い」「Wiki / SaaS は更新者フィールドが無い」は
>   誤りになった。** その取り込み契約は外部組織のものではなく**自前の型**であり、読む項目を増やせば取れた。
>   **Wiki / SaaS は取り込み設定で項目名を指定して受け取り**（既定は `updatedBy`）、
>   **業務DB は列名を指定したときだけ**その列を読む（指定しなければ発行する問い合わせは従前と同一で、
>   **無条件に列を足すと別名を持たない既存の問い合わせが全件失敗する**）。
>   **ファイルサーバーだけは今後も運ばない。** また **「取れなかった」と「取ったら空だった」は
>   分けて数える**（どちらも予約値へ倒れるが、前者は設定していないだけで正常、後者はソース側のデータ不備）。
...
>   **［2026-09-26 / #754］部門コードの値域は利用者裁定で Keycloak realm の部門グループ（`/department/<コード>`）と定まり、
>   部門は登録した利用者の部門グループ所属から導くとされた。** これは本欄（データソースの既定属性）の中で実装した ——
>   **登録時に `department` が未指定（空白・`unassigned` を含む）なら、登録した管理者のトークンが運ぶ所属グループの
>   フルパス（クレーム `group_paths`）から `/department/<コード>` を数え、ちょうど 1 つのときだけそのコードを入れる。**
>   0 個・2 個以上は導かず従来どおり `unassigned` へ倒れる（誤った部門を作らない）。導いた値は本欄に保存されるので
...
>   **［2026-09-26 / #1557］値域の検証が入った。** 本欄で `department` を**明示**した登録・全置換・部分更新は、
...
> | 属性 | 倒れる度合い | 逃れる手段 |
> | --- | --- | --- |
> | `owner` | **ファイルサーバー由来は 100%。他の 3 種は写像表に当たった分だけ逃れる** | API・**データソース管理画面の写像表**から明示指定できる。写像表が効くのは**ソース側が更新者を運ぶとき**であり、［2026-09-05 / #752］**Wiki / SaaS / 業務DB は運ぶようになった**（設定で項目名・列名を指定する） |
> | `department` | **管理者がデータソース管理画面で値を入れず、かつ登録した管理者の部門グループが 1 つに決まらなければ倒れる** | **画面（同登録フォーム）または API から明示指定する**（#767 で画面の経路が開いた）。［2026-09-26 / #754］**登録時は登録した管理者の部門グループからも補われる** |
>
...
> ［2026-09-26 / #754］**ただし登録時は、登録した管理者の部門グループがちょうど 1 つなら、そのコードが入る**
> （所属が 0 個・2 個以上、または身元プロバイダのトークンが所属グループのフルパスを運ばない構成では、従来どおり倒れる）。
