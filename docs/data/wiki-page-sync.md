...
> WikiService が所有する。Issue #118 監査で欠落が判明したため後追いで作成（実装は FR-13 段2 /
> Issue #88 で完了済み）。

## 起点となる計画書（トレーサビリティ）

- **関連機能要求(FR)**: FR-13（正規化文書の Wiki 閲覧。ABAC・横断検索・AI 回答と統合）
- **技術検討(06_technical)・ADR**:
  - ADR-0002 DB per Service（WikiService 専用 DB `wiki_svc`）
...
IADR-0020 により閲覧本文の実体は Wiki.js が保持し、本エンティティは「ゲートウェイでの認可判定・
...
- `DocumentDeleted` → アーカイブまたは削除し、Wiki.js 側へも伝播（IADR-0023。`PathFor(DocumentId)`
  でメタデータ未同期でも正準パスを導出可能）。
- ゲートウェイの一覧・個別取得は `Status = active` かつ ABAC 許可のもののみ可視。
  それ以外は 404（存在秘匿・IADR-0009）。
