...
| 失敗の検知 | 利用者の書き込みの失敗・周期ごとの中断・全員の見送りを計器に数え、アラート（warning）で知らせる。1 人の失敗で周期は止まらない |
