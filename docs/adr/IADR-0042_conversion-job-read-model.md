...
- 関連 ADR: ADR-0003（MassTransit。Superseded by ADR-0027・注記は #580）／ ADR-0012（変換・pandoc）／ [[IADR-0039]]（管理系ロール）／ [[IADR-0029]]（ワーカーの最小 HTTP サーフェス）／ **[[IADR-0127]]（画面側の retry 管理者限定）・[[IADR-0128]]（API 側の retry 管理者限定）** ＝ いずれも本 IADR 決定 3 の retry を部分改定する
