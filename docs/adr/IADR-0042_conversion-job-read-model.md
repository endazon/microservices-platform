...
3. **API（メッシュ内部・ワーカー上）**: `GET /jobs`（`?status=` 絞り込み）・`GET /jobs/{id}`・`POST /jobs/{id}/retry`（原本イベント `RawDocumentFetched` を再発行して再変換）。ワーカー自身は最小 HTTP サーフェスに留め認可は課さない（[[IADR-0029]] と同方針。ingress 非公開）。**認可は BFF で管理者・運用者に限定**（`/bff/conversion/jobs`。[[IADR-0039]]）。フロントは `RequireRole` で存在秘匿。
   ...
   > **［2026-08-05 追記・retry の権限は改定済み（#503 / [IADR-0127](IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1）］**
   > 本決定 3 の「管理者・運用者に限定」は、**`retry` については計画の 2026-08-04 確定により既に否定されている**
   > （05_screens §SC-07「**再変換の実行権限は管理者ロールに限る**」。planning `d980a01` の `01_screens.md:257`）。
   > **`retry` の権限は `platform-admin` 限定が正である。**
   > **API 側の是正は #501** が担当し、#503（PR #508）は画面側のボタンだけを admin 限定にした。
   > すなわち **#501 が閉じるまで、本決定 3 の記述どおりの API（admin/operator）が残る**——
   > これは計画確定事項の未達であって、許容された設計ではない。
   > 一覧・個別取得（`GET /jobs` 系）の認可は本決定のまま admin/operator である。
   > **本 IADR は `Accepted` のまま有効**であり、[[IADR-0127]] は本 IADR を置換しない（retry の権限だけを部分改定する）。
