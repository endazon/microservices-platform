...
> **［2026-08-07 追記・根拠事実の更新 / #599］本決定の前提は失効した。** **［2026-08-07 / #599］FR-19 / FR-20 は SC-19 の「本文を編集（Wiki.js）」導線ただ 1 つを除いて解除された**（計画 `ADR-0037` の着手可否の注記・INDEX 決定 33。実装側は [[IADR-0142]] が [[IADR-0119]] 決定 2 を部分改定）。
> **とくに SC-20（Obsidian 連携設定）の全体は覆らない**と計画が明記したため、「保留中の画面のためだけに部品を先回りで作らない」という本決定の根拠は SC-20 について成り立たない。
...
- フォローアップ:
  - **Dialog の移植**（決定 2）: FR-19 / FR-20 の保留解除後（**［2026-08-07 / #599］SC-20 全体について解除済み**）、#452 が当該画面と同時に行う。
