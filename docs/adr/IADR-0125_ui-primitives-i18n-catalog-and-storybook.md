...
> **［2026-08-07 追記・根拠事実の更新 / #599］本決定の前提は失効した。** **［2026-08-07 / #599］FR-19 / FR-20 の保留は範囲基準へ変わった**（[[IADR-0142]] 決定 1・2。**範囲の正は計画 `ADR-0037` の「着手可否の注記」**であり、ここには転記しない —— 転記すると計画側が動いたとき一斉に腐る）。
> **計画 `05_screens` の SC-19 / SC-20 が要求する確認ダイアログは、いずれも「本文を編集（Wiki.js）」導線ではない**（完全削除・論理削除・公開範囲の変更・Break-glass・全端末の一括失効）。**件数は数え方が争える**〔「確認ダイアログ」と明記された箇所と、ダイアログ一般を含めるかで変わる〕**ので書かない**。
> したがって「保留中の画面のためだけに部品を先回りで作らない」という本決定の根拠は**両画面について**成り立たない。
> **本決定そのものは変えない**（移植するか否かの再判断は引き受け先の #452 に委ねる）。**根拠が変わったことだけを記録する。**
...
- フォローアップ:
  - **Dialog の移植**（決定 2）: FR-19 / FR-20 の保留解除後（**［2026-08-07 / #599］保留は範囲基準へ変わった。[[IADR-0142]]**）、#452 が当該画面と同時に行う。
