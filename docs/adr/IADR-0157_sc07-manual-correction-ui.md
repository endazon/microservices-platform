...
WAI-ARIA の `alertdialog` は「**モーダルであり、開いた時点で焦点が移り、応答するまで
背後を操作できない**」ことを意味する。本確認はページ内に差し込む**非モーダル**の注意であり、
焦点も移さない（共有 UI に Dialog プリミティブが無い。[[IADR-0125]] 決定 1 の公開面に無い）。
