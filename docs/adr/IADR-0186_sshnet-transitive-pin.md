...
## ★ 決定 5: **無採番 `NFR` は IADR-0179 の宣言範囲の外から援用している（PR #723 レビュー 🟡）**

> **［2026-08-14 追記 / #724］★ 本決定が述べた乖離は解消した。**
> **[IADR-0188](./IADR-0188_unnumbered-nfr-applies-to-all-work.md) 決定 1 が IADR-0179 決定 1 の適用範囲を
> 「該当する `NFR-xx` が無い作業」へ一般化した** —— **本作業はいま宣言範囲の内側にある。**
> **本決定は「改定が済むまでの明示」として書かれたものであり、その役目を終えた**（以下は当時の記録である）。

**明記しておく。** [IADR-0179](./IADR-0179_unnumbered-nfr-for-meta-work.md) 決定 1 の**見出しは「メタ作業」に限定されている。**

> **決定 1: 該当する `NFR-xx` が無い*メタ作業*は無採番 `NFR` のままとする**

**本作業はメタ作業ではない** —— **製品の依存の脆弱性修正**である。**つまり宣言範囲の外側から援用している。**

**それでも無採番を採るのは、IADR-0179 が挙げる理由がメタ作業に固有ではないからである。**

| | |
| --- | --- |
| 同 ADR の理由 | **「無理に付けるほうが悪い。実在しない対応づけを作る」** —— **作業がメタか製品かに依らない** |
| 実測 | **`NFR-01`〜`27` に依存のパッチ適用へ当たる番号は無い**（PR #723 のレビューが独立に再実測して同結論） |
| 先例 | **IADR-0179 自身が「製品の作業が裸の `NFR` を併記した例が 2 件」ある**と記録している |

**したがって結論（無採番）は妥当だが、根拠の引き方が宣言範囲を超えている。**
**この乖離を放置すると「メタ作業限定」の文言と実態が離れ続ける**ので、
**IADR-0179 の適用範囲を実態へ合わせる改定を #724 として起票した。**
**本決定はそれが済むまでの、この 1 件についての明示である**（**規約の側を勝手に広げて解釈したまま黙らない**）。
