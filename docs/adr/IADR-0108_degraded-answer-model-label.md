...
2. **値が実モデルと一致しない。** 本経路の purpose は `rag-answer`＝`claude-sonnet-5`（ADR-0022 / [[IADR-0106]]）。
...
4. 呼び出し側で purpose→モデルの対応表を持ち、`rag-answer` なら `claude-sonnet-5` と名乗る — ルーティング
   ロジック（機密区分・ZDR 除外・エンドポイント優先度）の**部分的な複製**であり、[[IADR-0106]] で塞いだ
...
- 呼び出し先不調（`Sent=false` かつ `Model=route 結果`）を空へ潰さないのは、この経路だけは
  **実際に外部呼び出しを試みており**、どのモデルへ向けた試行だったかが監査・障害解析の情報になるため。
  「送信の成否」は従来どおり `Sent` が表す（[[IADR-0104]] の `Sent` と `StopReason` の分離と同じ考え方）。
...
- Supersedes: なし（[[IADR-0101]] / [[IADR-0106]] の決定は不変。両者のレビュー指摘・フォローアップの消化）
- Superseded by: なし
