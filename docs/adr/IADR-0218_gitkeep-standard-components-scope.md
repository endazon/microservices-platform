...
2. **1 PR にまとめる。サービス単位に割らない。** [[IADR-0116]] 規約 4（レビュー可能な変更単位）は
   **差分の行数ではなく判断の数**で測る。44 ファイルはすべて空の `.gitkeep` であり、
   **判断は本 IADR の決定 1〜3 に尽きて 1 つも増えない**。11 PR へ割ると、
   同じ判断を 11 回レビューし、FIFO のマージ待ち行列を 11 本占有するだけである。
...
- 決定 3 で 1 PR にまとめるのは、[[IADR-0116]] 規約 4 が守ろうとしているのが**レビューの認知負荷**であり、
  それは判断の数に比例するからである。空ファイル 44 個の追加に判断は無い。
