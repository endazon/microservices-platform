...
### 1. 認証は k8s auth（本番同等・既存 store 流用）

ESO の `ClusterSecretStore vault-backend`（既存）の認証を **token → kubernetes 認証**へ移行する（本番同等・静的 root
トークンを store に持たない）。Vault の kubernetes auth を有効化し、**Vault 自身の in-cluster SA を reviewer** に使う
...
消費側は store 名参照のみで透過（AST の ExternalSecret も追随＝本番同等化が一元化される）。
