...
> **索引 backfill に関する注記（並行 PR 依存）**: 本 PR は既存債務（0039–0046 未掲載）の解消と併せて索引を欠番なしに
> 揃える。ただし **IADR-0047 の実体ファイルは PR #211（Issue #199）** に、**IADR-0048/0049 は本 PR** に存在する。
> したがって本 PR が #211 より先に develop へマージされると、索引の IADR-0047 行は一時的に実体ファイルを伴わない
> （行内に「（PR #211 で追加）」と明示）。**マージ順序は #211 →（#212）を推奨**する。#211 は README を編集しないため
> 索引更新の競合は生じない。0048/0049 は本 PR が実体を伴うため不整合はない。
