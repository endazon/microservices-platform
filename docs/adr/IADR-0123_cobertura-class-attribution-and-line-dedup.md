...
4. **前提の正しさを実レポートで照合可能にする。** `<coverage>` 要素の `lines-valid` / `lines-covered`
   ...
   >   - 独立検証（AI レビューが .NET SDK のある環境で `Platform.Bff.Tests` を単体実走）でも同傾向で、
   ...
   >   同じ被覆数（1577）を coverlet の分母で割ると branch 率は **17.62% → 16.86%** となり、**床 17 を下回る**。
...
- 良い影響:
  ...
  - 二重記載の扱いが確定し、**計測条件によって実測値が振れる**（266 / 230）状態が解消する。
    ...
    で、旧計数方式の **266 行はちょうど 133 × 2**（二重記載）として説明がつく。
    旧値のもう一方 **230 行**は `Platform.Bff.Tests` **単体実行**という別条件の値であり、新方式の全体集計とは
    直接比較できない（レビューの単体実走〔SDK あり環境〕では 115 行）。
