...
2. **床の値は実測からの整数切り下げとする。初期値は `line 34` / `branch 17`。**
   ...
   > **AST の混入込み・二重記載込み**の値である（[[IADR-0123]] 決定 3 で計数方式が変わった。分母・分子は
   > 約半分になる）。新方式での実測は
   ...
   > なお**分岐の分母は `condition-coverage` の合算**であり coverlet の `branches-valid`（9356）とは
   > 定義が異なる。coverlet の分母で割ると 16.86% となり床 17 を下回るため、**分岐の定義変更は床の
   > 置き直しとセットでしか行えない**（[IADR-0123](IADR-0123_cobertura-class-attribution-and-line-dedup.md)
   > 決定 4 の追記）。
...
4. **`ai-stock-trading`（AST）は集計対象外とする**（`EXCLUDED_UNITS`）。
   ...
   > **266 行はちょうど 133 × 2** であり、二重記載で説明がつく。もう一方の **230 行は
   > `Platform.Bff.Tests` 単体実行という別条件**の値で、新方式の全体集計とは直接比較できない
   > （SDK のある環境での単体実走〔本 PR のレビュー〕では 115 行）。
