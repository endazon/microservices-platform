...
- **参照専用**: 接続情報は `ConnectionUri`（**パスワードを含めない**）＋`Config["password"]`（GET 応答で [[IADR-0053]] の
...
- **失敗/縮退**: DB エラーは例外送出（[[IADR-0051]] 決定3a → watermark 非前進・継続失敗アラート）。`ConnectionUri`／`query`
  未設定は空列挙で縮退。
...
- **プラグイン方針との一貫性**（[[IADR-0051]]）: `IDataSourceConnector` 追加のみ、Map/格納/発行/定期同期は既存を共用。
