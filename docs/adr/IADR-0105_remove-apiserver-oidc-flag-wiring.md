...
- 関連 ADR: [[IADR-0084]]（#328。本 ADR が実装を除去する対象。「⚠️ 2026-07-25 追記」が適用不能の根拠＝単一情報源）／
  [[IADR-0080]]（#271。Headlamp 導入・`oidc:developer` への cluster-admin bind）／[[IADR-0066]]（経路B＝ローカル k8s dev 環境）／
  [[IADR-0087]]（#334。`k8s-local-up.sh` の opt-in ゲート smoke test＝本変更の回帰固定先）／
  [[IADR-0103]]（#354。realm の `headlamp-realm-roles` mapper 恒久化＝現行では inert）
...
[[IADR-0084]]（#328）は `scripts/k8s-local-up.sh` の k3d 経路に apiserver の OIDC 検証フラグを opt-in で配線した。
この opt-in は `HEADLAMP_OIDC_APISERVER` が未設定なら **`HEADLAMP` の値へ追従**する設計である
（`${HEADLAMP_OIDC_APISERVER:-${HEADLAMP:-}}`）。

しかし [[IADR-0084]] の「⚠️ 2026-07-25 追記」のとおり、**k8s 1.30+ ではこの手順が成立しない**。レガシーな
...
3. **`HEADLAMP=1` の意味を戻す**: Headlamp の overlay 適用（`deploy/local/headlamp`）と OIDC client secret
   ...
   `kubectl -n platform-infra create token headlamp-viewer --duration=24h` を案内する（[[IADR-0084]] 追記／#393）。
...
5. **回帰固定**: [[IADR-0087]] の smoke test（`scripts/k8s-local-up.test.js`）で、`HEADLAMP=1` / 旧 env 明示 /
...
- 悪い影響・トレードオフ:
  ...
  - [[IADR-0084]] の決定1・4 は本 ADR により実装が失われる（ADR 本文は履歴として残し、状態を Superseded へ移す）。
- フォローアップ:
...
- Supersedes: [[IADR-0084]]（決定1「`HEADLAMP_OIDC_APISERVER` による opt-in 付与」・決定4「reuse 時の再作成 WARN」の実装。
