...
- 専用ワークフロー [`.github/workflows/doc-links-planning.yml`](../../.github/workflows/doc-links-planning.yml)（`schedule` 夜間 + `workflow_dispatch`）が `actions/checkout` を `submodules: recursive` + `token: ${{ secrets.PLANNING_REPO_TOKEN }}` で実行し、`node scripts/check-doc-links.js --require-planning` を走らせる。
