...
- 関連する実装 ADR: [[IADR-0134]]（分割境界。決定 3 が `manualChunks` の 3 規則を定める）／
  [[IADR-0118]]（床のラチェット）／[[IADR-0145]] 決定 3（**偽陽性は塞ぎ、検出漏れは開示する**）／[[IADR-0141]]（母集合）
...
無関係な PR が止まる。[[IADR-0118]] の床と同じく、**止めるべきものだけを止める。**
...
**[[IADR-0145]] 決定 3 および `.claude/rules/traceability.md` の方針
