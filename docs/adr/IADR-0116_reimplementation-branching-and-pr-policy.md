...
7. **ADR-0035（GraphRAG 検索戦略）は未起案**であり、RAG へのグラフ組み込み部分には着手しない
   ...
   - > **［2026-08-07 追記・事実の更新。本規約 7 の保留は解けた / #586］`ADR-0035` は 2026-08-07 に
     ...
     > **［2026-08-07 追記 / #599］この記述は同日中に古くなった。** **［2026-08-07 / #599］FR-19 / FR-20 は SC-19 の「本文を編集（Wiki.js）」導線ただ 1 つを除いて解除された**（計画 `ADR-0037` の着手可否の注記・INDEX 決定 33。実装側は [[IADR-0142]] が [[IADR-0119]] 決定 2 を部分改定）。
...
>
> - **1 対象あたり、全面巡回は 1 回まで**。2 巡目以降は**是正差分のみ**を見る（**単位は「対象」であって「PR」ではない**。束ねた PR では issue ごと ＋ 束ごとに対象が増える）
> - **2 本の監査は同時に走らせる**（互いの結果を待たせない）
> - **監査に打ち切り条件を明言させる**（「これ以上の巡回は不要」または要る範囲の限定）
> - **対象の性質で分岐する**（値はすべて 1 対象あたり。記録の追随のみ ＝ 全面 1 巡で打ち切り／規約改定・検査器の新設 ＝ 全面 1 巡 ＋ 差分 1 巡。**複数該当なら重い方を採り、分類は件名ではなく差分から決める**）
