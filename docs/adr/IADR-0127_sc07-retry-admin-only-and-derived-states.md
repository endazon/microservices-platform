...
| SC-07 ジョブ状態 | 計画確定の **4 値をそのまま**写す（`queued` 待機中 / `processing` 変換中 / `succeeded` 完了 / `failed` 失敗） | 「デッドレター」の内訳表示（`ConversionJobDto` にデッドレターの標識が無い。**［2026-08-06 追記］#533 / [[IADR-0136]] が `deadLettered` / `maxAttempts` を契約へ加え、この前提は解消した。決定 2 そのもの——「導出できる値だけで表示する」——は不変であり、いまや導出できる**。画面へ出す作業は別途） |
