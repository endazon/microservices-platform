...
**いずれも実データには 0 件である**（`Contracts/Dtos` を走査して確認した）——
すなわち**いま直した 2 件は、現時点の不具合ではなく将来の誤検出と見逃しである**。
**指摘される前に自分で投げて測った。**
...
## 決定 4: `required` の不一致は**ラチェット**にする（既存 20 件は据え置く）

`required` の検査を全数へ当てたところ、**着手時点で 20 件が一致していなかった**（実測）。
**#525 は `AccessScopeResponse` の資源に閉じており**（[[IADR-0139]] 決定 1）、20 件の是正は別の作業である。
リポジトリ既定のラチェット方式（`backend-library-baseline` / `landed-subject-baseline` と同じ）で
**既存を据え置き、新規混入だけを落とす**。

**据え置きは 2 種類に分かれ、まとめて直してはならない**（baseline の `_comment` にも書いた）:

| 種別 | 是正の安全性 |
| --- | --- |
| 応答スキーマ（`DataSourceDto` 10 件・`ConversionJobDto` 3 件） | **安全**。サーバは必ず送っており、生成型が `?` から必須へ変わるだけ |
| 要求スキーマ（`SearchRequest` 等 5 件） | **危険**。`required` を足すと呼び出し側が送信を強制される（#520 §未決事項 6） |
| 要求スキーマの「嘘の必須」（`UpdateDataSourceRequest` 2 件） | **安全**（緩和の向き） |

**「20 件あるから検査を諦める」でも「20 件を今ここで直す」でもない。** 測って、線を引いて、送り先を書いた。
