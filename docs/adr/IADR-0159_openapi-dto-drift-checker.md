...
> **［2026-08-10 追記 / #658・[[IADR-0162]]］ラチェットは撤去した。10 件のうち是正すべき乖離は 3 件だけだった。**
>
> | 内訳 | 件数 | 実体 |
> | --- | --- | --- |
...
>
> **★ 偽陽性 5 件は、本検査器が [[IADR-0132]] の論点 A1＋B1 を要求スキーマへも当てていたためである。**
> B1 は**応答スキーマの規則**であり（[[IADR-0132]] の表題・決定 2 の系が明示）、
> 要求側では C# の既定値が「送らなければこの値になる」を意味する。
> [[IADR-0162]] が到達性（`requestBody` / `responses`）で規則を分けた。
>
...
- ~~**`required` 不一致 10 件の是正**（決定 4）。応答側 3 件と要求側 7 件で扱いが違う。**#658 が引き受ける。**~~
  → **回収済み（2026-08-10 / #658・[[IADR-0162]]）。** 是正 3・偽陽性 5・意図的な差 2 で、ラチェットは撤去した。
