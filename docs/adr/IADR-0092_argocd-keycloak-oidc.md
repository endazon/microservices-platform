...
- 関連 ADR: ADR-0006（GitOps）。ArgoCD ブートストラップ統括 [[IADR-0077]]、Keycloak OIDC 連携の先例
  [[IADR-0090]]（Grafana）、エッジ集約（集約後 URL・ホスト名ベース） [[IADR-0091]]。
...
OIDC に載せる。エッジ集約（#357/[[IADR-0091]]）がマージ済みのため、**最初から集約後 URL（`argocd.localhost:50000`）**
...
[[IADR-0091]] の Traefik `admin:50000` entrypoint＋`argocd.localhost` Ingress（#357 で追加済み）。**`server.rootpath`
...
クレーム（id_token/userinfo）として発行する（他 client 不変・[[IADR-0090]] の Grafana と同型）。`requestedScopes` は
...
`ARGOCD=1` ブロックで install（server-side apply・[[IADR-0077]]/#348）後に、3 つの ConfigMap を
