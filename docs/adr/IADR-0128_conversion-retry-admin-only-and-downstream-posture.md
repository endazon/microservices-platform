...
| 一緒に admin へ絞る | 実装は計画の字面へ寄るが、**#198 の裁定を実装が先取りする**。裁定が「計画側を admin ＋ operator へ改訂する」と出れば戻す作業が要る。さらに同じ [[IADR-0039]] 決定 1 が適用される SC-05・SC-06 だけが取り残され、**同一の差異の扱いが画面ごとにばらつく** |
...
- **[[IADR-0039]] は本 IADR の改定対象外である。** 同 IADR §影響 は
  ...
  なお **PR #508（#503 / IADR-0127。本ブランチには未存在）が [[IADR-0039]] 決定 1 へ「計画との差異は planning#198 で
  裁定待ち」の［2026-08-05 追記］を入れている**。閲覧ロールの差異（本 IADR 決定 2）に関する記載は
  それで足りるため、**本 PR は同じ箇所に追記を重ねない**（重複を避ける）。
