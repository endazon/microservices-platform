...
- 追随できていなかった決定: [[IADR-0062]]（ユニット改名）・[[IADR-0027]]（Composable / Foundation 分割）
- 検査器の置き場所: [[IADR-0130]]（`check-test-spec-coverage.js`）
...
[[IADR-0159]] が実測したとおり**偽陽性は見逃しより重い** —— 無関係な PR の CI を誤って落とすからである。
...
> **［2026-08-11 追記 / #697］★ 超過は解消した**（#623 の減量 段 1〜5 ＋ #697 で 50,000 を下回った。[[IADR-0178]] 決定 4）。
...
| 仕様書 | 誤り | 正 |
| --- | --- | --- |
| `docs/tests/FR-01` / `FR-06` / `FR-07` | `Tests/KnowledgePlatform.IntegrationTests/…` | `Tests/Knowledge.IntegrationTests/…`（[[IADR-0062]]） |
| `docs/functional/FR-13` | `WikiService.Api/Endpoints/…` | `WikiService.Api/Foundation/Endpoints/…`（[[IADR-0027]]） |
...
——**「テストを書いた」と「変更が守られている」は別**（[[IADR-0159]]）を、また実地で踏んだ。
