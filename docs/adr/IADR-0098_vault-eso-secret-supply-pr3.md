...
- 関連 ADR: ADR-0006。ESO 基盤（k8s auth・store 上書き・policy `eso-read`・seed/skip・専用 SA・VAULT 併用ガード）は
  [[IADR-0096]]（PR-1）。同パターンの 2 歩目は [[IADR-0097]]（PR-2）。opt-in オーバーレイは [[IADR-0077]]。
- 対象 OIDC secret の各機能連携: MinIO=[[IADR-0093]]、Grafana=[[IADR-0090]]、Vault=[[IADR-0094]]、Headlamp=[[IADR-0080]]。
...
[[IADR-0096]]（PR-1）で ESO 基盤を敷き、[[IADR-0097]]（PR-2）で minio/wikijs 系を移行した。PR-3 は同一パターンで
