...
4. **SC-02 / SC-03 の取得は `useQuery`** とし、キーは `['bff', <資源>, …]` の形で BFF のパスに対応させる。
   **SC-03 の版履歴は詳細の成功後にだけ有効化する**（`enabled`）——詳細が 404（存在秘匿）のときに
   ...
   > #519 / [[IADR-0135]] 決定 1 が SC-03 を orval 生成フックへ載せ替え、キーは生成キー
   ...
   > `['bff','search', q]` を維持している（[[IADR-0135]] 決定 2）ので、**両者は現在ずれている**。
   >
   ...
   > [[IADR-0135]] 決定 3［2026-08-06 追記］）。**新 ADR は起こさない**——本決定の意図
