...
| 本環境の .NET SDK | **無し**（`dotnet: command not found`。取得も proxy policy で拒否。#486 で実証済み） |
...
| .NET SDK 依存 | **無し** | **有り**（本環境で実行不能） | 無し | 無し（protoc は別途） |
...
- **リフレクション（B）を採らない**のは、契約の抽出に .NET SDK でのビルドが要るためである。
  本作業環境に SDK は無く（上記実測）、検査器をローカルで実行・自己試験できない。CI 専用の検査は
  「壊れたときにしか動かない検査」になり、`scripts.test.js` からの単体テスト（#465 の受け入れ観点）も
  成立しない。**この制約は環境の都合であり、精度の優劣ではない**——正直に記録する。
...
- **共通エンベロープは引き続き繰延する。** 繰延解除条件（段の挿抜による横断的変更の反復・ABAC 属性
  ヒント／トレース ID の本体搬送要件の確定）は未成立であり、エンベロープ項目の確定は上流
...
- 抽出方式の選定は「どれが最も精緻か」ではなく「**今この期間に、実在する契約面を覆えるのはどれか**」で
  ...
  リフレクションは本環境で実行できない。C# ソース解析は精度で劣るが、**唯一いま全契約面を覆える**。
