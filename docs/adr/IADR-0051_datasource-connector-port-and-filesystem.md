...
- 関連 ADR: ADR-0003（MassTransit）／[[IADR-0019]]（データソース既定属性のフェイルセーフ）
...
2. **Discover/Fetch をポートに分離し、Map（属性/タグ・ストレージ・発行）はオーケストレータへ集約（本決定）**:
   コネクタはソース固有 I/O に専念、属性方針（[[IADR-0019]] フェイルセーフ）・ストレージ格納・イベント発行・
...
1. **コネクタポート `IDataSourceConnector`（`Foundation/Ports`）を `DiscoverAsync`（変更検知付き列挙）と
   ...
   ソースメタ→タグ・ABAC 属性の写像と機密区分フェイルセーフ（[[IADR-0019]]）を一元化する。
