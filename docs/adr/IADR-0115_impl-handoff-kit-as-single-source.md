...
- フォローアップ:
  ...
  - 第 7 ラウンド（pin `cff9b6c`）で planning#117 が反映された一方、**キットの新規変更に不具合を発見**した。
    `Bash(git -C planning …:*)` は PR CI では submodule が未取得のため、git が親ディレクトリを遡って
    **実装リポの履歴を静かに返す**（[planning#123](https://github.com/endazon/project-planning/issues/123)）。
    分類 A（キットの内容をそのまま反映）は**キット側の欠陥もそのまま持ち込む**ため、同期のたびに
    「取り込んだ内容が自リポで実際に機能するか」を確かめる必要がある——バイト一致は正しさを保証しない。
  - 7 ラウンドの実績として、**環流した指摘 13 件のうち 11 件がキットへ反映され、固有デルタは
