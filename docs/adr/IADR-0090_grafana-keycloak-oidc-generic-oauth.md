...
- 関連 ADR: ADR-0006（運用基盤）。経路B 可観測性 opt-in オーバーレイは [[IADR-0077]]、経路B k8s dev 統括は
  [[IADR-0066]]、opt-in ゲート横断 smoke test は [[IADR-0087]]。dev ツールの Keycloak OIDC 連携の先例は
  [[IADR-0080]]（Headlamp・confidential client・secret を k8s Secret 供給）。
...
作成）から注入する。[[IADR-0080]] の `headlamp-oidc` と同型。
