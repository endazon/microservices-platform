...
- 関連 ADR: [[IADR-0029]]（構成情報 API の配置・ドリフト粒度）／[[IADR-0033]]（フロント基盤・依存最小）／[[IADR-0036]]（SC-11 可視化方式）
...
- **API 側の履歴ストア新設（2）を採らない**理由:
  ...
  - サービス DB に履歴を複製すると **Git と乖離し得る第二の真実**を生み、永続化・保持・整合の複雑性を
    抱える。[[IADR-0033]]（依存最小）・[[IADR-0036]]（可視化も依存を増やさない）の方針と不整合。
  - 現在バージョンが既に GitOps 注入（[[IADR-0029]]・`ConfigVersionOptions`）であるため、履歴はその
