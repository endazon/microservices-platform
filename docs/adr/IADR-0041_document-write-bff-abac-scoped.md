...
1. **書き込みは platform-admin/operator に限定する**（[[IADR-0039]] の管理系ゲーティングに従う）。
   ...
   > 後段 `DocumentService` 側は `POST` だけ据え置いた（機械クライアントの事情。[[IADR-0044]] の同日追記を見ること）。
読み取り（SC-02/03 用）は従来どおり無制限（ABAC スコープのみ）。BFF は `/bff/documents` の**書き込みサブグループのみ** `RequireRole(admin, operator)` で保護し、既存の読み取りマッピングは変更しない。フロントは `RequireRole` で `/documents`（管理一覧）を出し分ける。
