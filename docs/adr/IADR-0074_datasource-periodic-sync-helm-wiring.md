...
- 関連 ADR: [[IADR-0051]]（定期同期ワーカー本体・増分 watermark の既存決定。既定無効の出自）／[[IADR-0066]]（ローカル k8s dev 環境＝経路B。`values-local.yaml` の出自）
...
監査の Go 判定は経路B（[[IADR-0066]]・`values-local.yaml`）で行う。ここで定期同期が**実際に回り**「15 分以内
