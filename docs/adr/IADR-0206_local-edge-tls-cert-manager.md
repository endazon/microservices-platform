...
- 実装 ADR: [[IADR-0091]]（経路B のエッジは Traefik）、[[IADR-0086]]（issuer と metadata の分離）、
  ...
  > **後継は [[IADR-0220]]**（admin:50000 を TLS 終端にした）。**詳細は決定 4 の追記**を参照。
...
> 「**その解消は #841 が担う**」と書いた。**#841 はその 2 つを解消した**（[[IADR-0220]]）。
> **しかし経路B の平文はまだ尽きていない。**
>
> | 残存平文 | 状態 | 担い手 |
> | --- | --- | --- |
> | admin:50000 の管理ツール 7 件 | **解消済み**（`--entryPoints.admin.http.tls=true` ＋ 7 ルータの `spec.tls`） | #841 / [[IADR-0220]] |
> | 80 の平文併存 | **解消済み**（`web`→`websecure` の恒久リダイレクト） | #841 / [[IADR-0220]] |
...
> issuer を最小案（`keycloak:8080` 維持）に留めたのは [[IADR-0091]] **決定 5** であり、
> **同決定は `Accepted`・live のまま本波でも改定されていない**（本 ADR §[[IADR-0091]] との関係 も
...
> に続けて「**admin(50000) の TLS 化も Tier 3 から外れた**（[[IADR-0220]]・#841）」と書いており、
...
> **`admin:50000` はもう平文でもない** —— [[IADR-0220]] が TLS 終端にした。
> [[IADR-0091]] 決定 4（ホスト名ベース集約）が「そのまま」であることは**引き続き真**である（変わったのは scheme だけ）。
