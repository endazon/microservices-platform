...
- 関連 ADR: [[IADR-0080]]（#271。Headlamp 導入・opt-in・「Pod の SA には広域権限を bind しない」fail-safe）／
  [[IADR-0084]]（#328。apiserver OIDC 適用不能の実測＝token 方式が正式手順である根拠）／
  [[IADR-0105]]（#399。apiserver OIDC 配線の除去。本 ADR はこれを**復活させない**）／
  [[IADR-0087]]（#334。`k8s-local-up.sh` の opt-in ゲート smoke test＝回帰固定先）
- 関連仕様書: `docs/specs/20260728_issue-398_headlamp-viewer-manifest.md`
- Issue: #398（enhancement/infrastructure）。`Refs #328`・#393・#388・#271

## コンテキストと課題

ローカル（経路B）の Headlamp ログインは token 方式が正式手順である（[[IADR-0084]]「⚠️ 2026-07-25 追記」・#393）。
...
ことが fail-safe の要（[[IADR-0080]]）であり、性質が正反対の「権限を bind する SA」を同じファイルに置くと
その不変条件が読み取りづらくなるため。
...
- CRB `headlamp-developer-cluster-admin`（`User: oidc:developer` → `cluster-admin`）は #388 の OIDC 化用資産として
  **現行 inert のまま残す**（[[IADR-0105]] により `oidc:` 接頭辞の identity が生成されないため無効）。
- apiserver への OIDC フラグ／`config.yaml.d` ドロップインは**追加しない**（[[IADR-0105]] の決定を維持）。
...
- **Headlamp Pod の SA（`headlamp`）自体に閲覧権限を bind する**: ログイン不要で誰でも見られる状態になり、
  [[IADR-0080]] の fail-safe（トークンを貼らない限り可視化できない）を壊すため却下。
