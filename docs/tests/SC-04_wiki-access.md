...
| T-05 | 身元あり | `/wiki` を開く | 見出しと左ナビ「Wiki」が実ブラウザで描かれる（陽性対照。ルートの実在はここで固定される） | 画面への到達 | 自動(E2E) |
| T-06 | 同上 | 初期表示の通信 | 身元と共通シェルの通知以外を**一切叩かない**＝別ホストの責務を SPA へ引き込んでいない（陰性対照） | 責務の境界 | 自動(E2E) |
