...
| T-22 | **報告書の種別別モデルと取引判断の改定（IADR-0112 / #420 #421）** | 報告書は方針階層（月報→週報→日報→取引。`AST/04_workflows/03_reporting-cycle`）をなす方針書であり、種別ごとに用途を分けて解決する（`report-monthly→claude-fable-5` / `report-weekly→claude-opus-5` / `report-daily→claude-sonnet-5`）。3 種別が同一モデルへ潰れない。`report-weekly` は `default` と同値だが明示エントリで固定する（無いと `default` 改定で無音に失効する）。あわせて **`confidential` × `report-monthly`** は ZDR 非対応の `claude-fable-5` が除外され `DefaultModel` へ落ちることを固定し、機密区分の設定次第で割当が無音に変わる構造を明示する（IADR-0112 決定2） | 各用途が指定モデルを返す / `confidential` × `report-monthly` は `Model="claude-opus-5"` | FR-11 / `LlmRouterTests.Route_ReportKindPurpose_ResolvesKindSpecificModel`・`Route_Confidential_ReportMonthly_FallsBackToZdrModel`・`CompletionRoutingEndpointTests.PostComplete_ReportKindPurpose_SelectsKindSpecificModel`・`PostComplete_ConfidentialReportMonthly_FallsBackToZdrModel` |
