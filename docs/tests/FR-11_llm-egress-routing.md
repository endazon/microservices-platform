...
| T-02 | 用途別モデル選択 | Model 未指定時、用途に応じてモデルを切替（analysis→fable-5 / rag-answer→sonnet / **diagram-coding→haiku** / default→opus。ADR-0010 / IADR-0022） | `Model` が用途別モデル | FR-11 / `LlmRouterTests`・`CompletionRoutingEndpointTests` |
...
| T-11 | 最難関 fable-5（ADR-0010 / IADR-0022） | 用途 `analysis` は `claude-fable-5` を選択 | `Model="claude-fable-5"` | FR-11 / `LlmRouterTests`・`CompletionRoutingEndpointTests` |
