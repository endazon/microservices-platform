...
| T-65 | 画面・鍵のプロパティ | 「生成」→「やめる」→「生成」→「生成して書き込む」 | 値の欄が無い。「生成」の押下と「やめる」では要求を送らない。**1 つの確認ダイアログ**に、鍵が置き換わる旨（「登録」とは書かない）と OpenD の手動の再起動・再認証を出す。確定で値を空文字として送る | 生成の確認（再起動の確認と統合） | 自動 |
...
| 区分 | ファイル | 対応 |
| --- | --- | --- |
| 画面 | `src/knowledge/frontend/src/features/sc22-secrets/components/SecretItemManagementPage.test.tsx` | T-01〜T-09・T-43・T-64〜T-67・T-72〜T-76 |
