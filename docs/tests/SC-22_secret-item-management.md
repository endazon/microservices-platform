---
title: SC-22 秘密情報・接続設定の管理 テスト仕様書
type: test-spec
status: completed
created: 2026-09-14
updated: 2026-09-26
author: claude
---
<!-- trace:
ids: [FR-05, NFR-18, SC-22]
adrs: [ADR-0032, ADR-0042, ADR-0095, ADR-0104, ADR-0110]
iadrs: [IADR-0009, IADR-0035, IADR-0096, IADR-0135, IADR-0433, IADR-0453, IADR-0454, IADR-0456, IADR-0460]
specs: [20260914_issue-1411_sc22-secret-injection-screen, 20260915_issue-1467_sc22-audit-followups, 20260915_issue-1477_screen-only-poc-setup, 20260925_1502_sc22-supply-source-and-restart-notice, 20260926_1523_sc22-supply-label-and-restart-confirm]
issues: [#1411, #1467, #1477, #1502, #1523]
-->

# テスト仕様書: 秘密情報・接続設定の管理

## テスト対象・範囲

**画面・境界層（`/bff/secrets`）・保管先（Vault）の書き込み権限の字面**である。

**対象外（送り先つき）**:

| 事項 | 理由・送り先 |
| --- | --- |
| 🔴 **実 Vault への疎通** | 自動試験は**保管先の偽物**（k8s 認証・KV v2 の metadata / 部分更新 / 作成を写したもの。削除・破棄された版への部分更新は 404、既に在る項目への作成は権限不足の 403 を返す）に対する固定である。**「緑である」ことは「稼働クラスタで書ける」ことを意味しない。** 稼働クラスタでの確認は手動（T-40）で行う |
| 同期（保管先 → 外部シークレット同期 → Secret）の反映と消費側の作り直し | 境界層が同期先へ**即時同期を依頼すること**（注釈の要求・権限の字面・起動器の導入）までは自動試験で固定する（T-57〜T-63）。外部シークレット同期が実際に Secret を更新し、Reloader が消費側を作り直すことは稼働クラスタで手動（T-40 と同じ場） |
| 監査ログの保持・閲覧 | 記録は構造化ログであり、閲覧はログ基盤側 |

## テスト観点

1. 🔴 **値がどこにも出ないこと**（応答・監査・ログ）。**陽性対照と対で置く** —— 偽物の保管先に値が届いていることを先に確かめる。
2. 🔴 **描いてはいけないものが無いこと**（値の列・一括再投入の口・値の読み出し）。
3. **1 回に 1 プロパティだけを部分更新で書くこと**（同居する構成値を消さない）。
4. **項目の集合の外へ書けないこと**（境界層の 400 と、保管先の権限の字面の両方で）。
5. **ロール限定が 2 層で効くこと**（画面の存在秘匿・境界層）。
6. **失敗を空や成功に縮退させないこと**（保管先が無い・届かない・拒否した）。
7. **起動時に項目の集合を読めなければ起動しないこと**（fail-closed）。

## テストケース一覧

| ID | 前提条件 | 手順 | 期待結果 | 対応受け入れ基準 | 区分 |
| --- | --- | --- | --- | --- | --- |
| T-01 | 4 項目（試験の固定データ） | 画面を開く | 列が 項目名／用途／最終更新日時／最終更新者／供給元／操作 の 6 つだけ。値の列が無い。行は 4 | 一覧の主要素・値の列を置かない | 自動 |
| T-02 | 設定済み・未設定・取得できない が混在 | 一覧を見る | 3 つの状態が別々のバッジで出る。最終更新者は画面で書いた版だけに付き、他は「記録なし」。注記が判定の限界を書く | 未設定の明示と区別 | 自動 |
| T-03 | 一覧が描かれている | 一括の操作を探す | 🔴 **無い**。行ごとの「更新」は 4 つ在る（陽性対照） | 一括再投入を置かない | 自動 |
| T-04 | 更新フォーム | 値と異なる確認を入れて送信を押す | 不一致の警告が出て、**送信ボタンが押せず要求が飛ばない** | 確認入力を 2 度 | 自動 |
| T-05 | 更新フォーム | 一致させて理由を入れて送信 | 1 プロパティ・値・理由だけが送られ、成功後に版が出て入力が空に戻る | 1 項目ずつ更新・理由 | 自動 |
| T-06 | 一覧が 503 | 画面を開く | 失敗の表示が出て、表を描かない（空の一覧にしない） | 保管先に届かないことを見せる | 自動 |
| T-07 | 運用者 | 画面を開く | 開ける | 運用者・システム管理者限定 | 自動 |
| T-08 | 他のロール | 画面を開く | 未検出画面。**一覧を取得しにも行かない** | 権限外に画面を出さない | 自動 |
| T-09 | — | 左ナビの宣言を見る | 「運用」グループ・2 ロールだけ | 左ナビ「運用」 | 自動 |
| T-10 | システム管理者 | 境界層の一覧を引く | 200。6 項目が並ぶ。許可が監査に残る | 一覧 | 自動 |
| T-11 | 運用者・システム管理者 | 一覧と更新を呼ぶ | どちらも 200 | ロール限定 | 自動 |
| T-12 | 他のロール | 一覧と更新を呼ぶ | 403。拒否が監査に残り、保管先へ 1 度も触れない | ロール限定・拒否も記録 | 自動 |
| T-13 | 未認証 | 一覧と更新を呼ぶ | 401。保管先へ触れない | 認証 | 自動 |
| T-14 | 状態が混在 | 境界層の一覧を引く | 3 値が返り、応答に値も値の項目も無い。**保管先の data を読みに行かない**（metadata だけ） | 値を読み出さない | 自動 |
| T-15 | 画面から 1 度書いた後、別経路で版が進む | 一覧を 2 度引く | 1 度目は利用者名、2 度目は null | 最終更新者 | 自動 |
| T-16 | 同居するプロパティを持つ項目 | 1 プロパティを更新 | 部分更新（`application/merge-patch+json`）で 1 プロパティだけ送り、他は残る。全置換の方式を使わない | 部分更新 | 自動 |
| T-17 | 項目が保管先に無い | 更新 | 部分更新が不在を返した後、「無いときだけ作る」方式で作る | 既存を置き換えない作成 | 自動 |
| T-18 | 保持中のトークンが無効になる | 一覧を 2 度・更新を 1 度 | 1 度目のログインを使い回し、無効になったら取り直す | 認証の維持 | 自動 |
| T-19 | 一覧に無い項目（対象外・保留を含む） | 更新 | **400**（404 ではない）。拒否が監査に残り、保管先に触れない | 項目の集合の外へ書けない | 自動 |
| T-20 | 構成値・未知のプロパティ | 更新 | 400。同じ項目の書けるプロパティは通る（陽性対照） | 構成は画面で扱わない | 自動 |
| T-21 | — | 空の値・長すぎる値・長すぎる理由で更新 | いずれも 400。保管先へ書かない | 入力規則 | 自動 |
| T-22 | 更新が成功 | 監査を見る | detail は 項目・プロパティ・版・理由 だけ。**値も値の長さも無い**（陽性対照: 保管先には値が届いている） | 値を記録しない | 自動 |
| T-23 | ログを捕捉 | 成功・拒否・保管先不達の 3 経路で更新 | どのログ行にも値が無い（陽性対照: ログは捕捉できている） | 値を記録しない | 自動 |
| T-24 | 保管先の接続先が未設定 | 一覧と更新 | どちらも 503。失敗が監査に残り、保管先に触れない | 未配備を見せる | 自動 |
| T-25 | 保管先に届かない・ログインを拒まれる | 一覧・更新 | 503 | 不達を見せる | 自動 |
| T-26 | 保管先が書き込みを拒む | 更新 | 502。失敗が監査に残り、最終更新者の記録を作らない | 黙って成功にしない | 自動 |
| T-27 | — | 起動した境界層を見る | 実ファイルの項目集合（6 項目）を読み込んでいる | 起動時の読み込み | 自動 |
| T-28 | 項目集合のファイルが無い | 境界層を起動する | 起動しない | fail-closed | 自動 |
| T-29 | 項目集合の実ファイル | 読み込む | 6 項目・21 プロパティ。構成値は書けるプロパティに入らない。出力ディレクトリに同梱されている | 単一情報源 | 自動 |
| T-30 | 保留・対象外を含むファイル | 読み込む | 一覧に入るのは `items[]` だけ | `items[]` だけを読む | 自動 |
| T-31 | 書ける／書けないの交差・ワイルドカード・`..`・空・壊れた JSON | 読み込む | いずれも例外（起動しない） | fail-closed | 自動 |
| T-32 | 保管先の権限ファイル | path を数える | 項目集合の data ＋ metadata と**完全一致**（多くも少なくもない） | 完全一致パス | 自動 |
| T-33 | 同上 | capability を見る | data は create / patch だけ（全置換の update も無い）、metadata は read だけ。ワイルドカード・list・delete・destroy・sudo が無い | 値を読み返せず、KV を全置換できない権限 | 自動 |
| T-34 | 同上 | 保留・対象外のパスを探す | 無い（陽性対照: 対象の項目は在る） | 集合の外へ書けない | 自動 |
| T-35 | 初期化スクリプト | ロールの束縛先を見る | 専用 SA `bff`・名前空間 `microservices-platform` だけ。`default` を含まない。同期側のロールに相乗りしない | 専用 SA・同期は読み取り専用のまま | 自動 |
| T-36 | ブラウザ・未認証 | 画面へ行く | ログインへ | 認証 | 自動（E2E） |
| T-37 | ブラウザ・運用者 | 画面へ行く | 見出し・左ナビ・未設定の表示が出る | 到達 | 自動（E2E） |
| T-38 | ブラウザ・他のロール | 画面へ行く | 未検出画面。左ナビにも出ず、一覧を呼ばない | 存在秘匿 | 自動（E2E） |
| T-41 | 現在の版が削除・破棄された項目 | 更新 | **409**（削除済みを示す種別）。失敗が監査に残り、**作成の要求を送らず**、保管先の中身と削除状態は変わらず、最終更新者の記録を作らない。一覧は「未設定」のまま | 削除済みの版へ書かない・原因を見せる | 自動 |
| T-42 | 同上（保管先クライアント単体） | metadata の取得と書き込み | 削除・破棄のどちらでも「現在の版が削除されている」を返し、作成の要求を送らない。項目が無いときは従来どおり作る（陽性対照）。在るときは metadata を読まずに部分更新する | 同上 | 自動 |
| T-43 | 更新フォーム | 更新が 409 で失敗する | 「削除されています」「復元」「値は保存されていません」を出す。境界層の日本語の見出しをそのまま出さない | 次の一手を見せる | 自動 |
| T-44 | 入力規則は満たすが本文が上限（64 KiB）を超える。長さの宣言あり／無しの 2 通り | 更新 | **413**。拒否が監査に残り、保管先の data へ触れない（上限が無ければ書き込みまで進む本文である） | 本文の上限 | 自動 |
| T-45 | 値 8192 文字・理由 500 文字をすべて 6 バイトの文字で送る最悪の本文（50 KB 超） | 更新 | 200（陽性対照。上限が入力規則の最大を収める） | 本文の上限 | 自動 |
| T-46 | 壊れた JSON・`null`・JSON でない文字列 | 更新 | **400**。拒否が監査に残り、本文の断片が監査にもログにも出ない（陽性対照: ログは捕捉できている） | 解釈失敗の監査 | 自動 |
| T-47 | JSON でない Content-Type | 更新 | **415**。拒否が監査に残り、保管先に触れない | 解釈失敗の監査 | 自動 |
| T-48 | 他のロール ＋ 壊れた本文 | 更新 | **403**（400 ではない）。拒否が監査に残る —— 本文より先にロールを見る | ロール限定 | 自動 |
| T-49 | 画面から書いた項目を、保管先で消して作り直す（版が 1 に戻り、作成時刻が変わる） | 一覧を作り直しの前後で引く | 前は利用者名（陽性対照）、後は null。**古い記録の版は現在版と同じ 1 のまま**（版だけの突き合わせでは誤った名前が出る） | 最終更新者（誤帰属しない） | 自動 |
| T-50 | 項目集合の実ファイル | 読み込む | 証券会社のログインのパスワードは MD5、OpenD の鍵は生成、Discord の ID 4 件は秘密でない（他は値・秘密）。各項目の同期先 ExternalSecret が契約の Secret 名と一致する | 入力の形・同期先の単一情報源 | 自動 |
| T-51 | 未知の種別・綴り違いのキー・真偽値でない秘密の宣言・パスワードや鍵を「秘密でない」とする宣言・同期先の欠落／書式違反／重複 | 読み込む | いずれも例外（起動しない） | fail-closed | 自動 |
| T-52 | — | 境界層の一覧を引く | 各行がプロパティごとの種別と秘密かどうかを返し、並びが書けるプロパティと一致する | 入力の形 | 自動 |
| T-53 | パスワードのプロパティ（陽性対照: 同じ項目のログイン ID） | 更新 | 保管先には小文字 hex の MD5 が入り、平文は入らない。**平文もハッシュも**応答・監査・ログ・保管先への要求本文に出ない。ログイン ID はそのまま入る | 平文を保存しない | 自動 |
| T-54 | 鍵を生成するプロパティ | 値なしで 2 回更新 | 保管先に RSA 1024 bit の PEM（PKCS1 形式）が入り、2 回目は別の鍵になる。鍵（見出し・本文）が応答・監査・ログに出ない | 鍵をどこにも返さない | 自動 |
| T-55 | 鍵のプロパティへ値を送る／パスワードが空 | 更新 | 400（`invalid-value`）。保管先にも同期先にも触れない | 持ち込んだ鍵を書かない | 自動 |
| T-56 | 秘密でない ID のプロパティ | 更新 | 値はそのまま保管先に入り、応答に出ない | 書き込み専用のまま | 自動 |
| T-57 | 各名前空間の項目（MSP・基盤・株式自動売買の app-secrets・moomoo） | 更新 | 同期先の ExternalSecret へ merge-patch で `force-sync` の注釈だけを送り（Bearer は Pod の SA トークン）、`syncRequested: true`。同期の監査は別の行で granted | 即時同期 | 自動 |
| T-58 | 同期の依頼が拒否・不在・障害・不達 | 更新 | **200**・`syncRequested: false`。書き込みは成立し、書き込みの監査は granted 1 行、同期の監査は failed（`sync-request-failed`） | 依頼の失敗で書き込みを失敗にしない | 自動 |
| T-59 | 同期が構成されていない | 更新 | 200・`syncRequested: false`。API へ送らず、同期の監査は failed（`sync-not-configured`） | 未構成を見せる | 自動 |
| T-60 | 保管先が書き込みを拒む／現在版が削除されている | 更新 | 502 / 409。同期を依頼せず、同期の監査も残さない | 書けていないものを同期させない | 自動 |
| T-61 | 同期依頼の権限の字面（チャートとローカルの Role） | 名前を数える | 名前空間ごとの `resourceNames` が項目集合の同期先と完全一致し、verbs は get / patch だけ、束縛先は SA `bff` だけ。MSP・基盤の同期先の ExternalSecret が実在する | 権限の射程 | 自動 |
| T-62 | 初期化スクリプトの字面 | 種の入れ方を見る | 画面が書く KV への作成は「無い」側の分岐の中で `-cas=0`。部分更新は空の値で何もしない。app-secrets の種はキー集合が契約どおりで、認証の値は realm と同値。moomoo は種を入れない | 再実行で画面の値を消さない | 自動 |
| T-63 | 起動器（`ESO=1`） | 発行コマンドを採る | 名前空間の作成 → 同期依頼の権限の適用 → Reloader（chart と image を pin・見る名前空間を 3 つに限る）の順 | 消費側の作り直しの導入 | 自動 |
| T-64 | 画面・パスワードのプロパティ | 入力して送る | マスクと確認入力があり、MD5 だけが保存される旨を出す。送る本文は平文（変換は境界層）。陽性対照: ログイン ID は注記を出さない | パスワードの入力の形 | 自動 |
| T-65 | 画面・鍵のプロパティ | 「生成」→「やめる」→「生成」→「生成して書き込む」 | 値の欄が無い。「生成」の押下と「やめる」では要求を送らない（初期フォーカスは「やめる」・確定ボタンは破壊的な見た目）。**1 つの確認ダイアログ**に、鍵が置き換わる旨（「登録」とは書かない）と OpenD の手動の再起動・再認証を出す。確定で値を空文字として送る | 生成の確認（再起動の確認と統合） | 自動 |
| T-66 | 画面・秘密でない ID | 入力して送る | 平文の入力で確認入力が無く、秘密ではない旨を出す。陽性対照: 同じ項目の API キーはマスクと確認入力 | 秘密でない値の入力の形 | 自動 |
| T-67 | 画面・保存が成立 | 同期の依頼が成功／失敗 | 「即時同期を依頼しました」／「即時同期を依頼できませんでした」を出す | 同期の成否を見せる | 自動 |
| T-68 | 株式自動売買の app-secrets の同期先だけが無い（他は在る） | 境界層の一覧を引く | その項目だけ `git`、他の 5 項目は `screen`。問い合わせは項目ごとに 1 回の `get`（SA トークンで名乗る。注釈を付けない）で、宛先は項目集合の同期先と一致する。同期の監査を残さない | 供給元（配備時のスイッチの事実を出す） | 自動 |
| T-69 | Kubernetes API が拒否（401 / 403）・障害（5xx）・不達／同期が構成されていない／接続先の URL が壊れている | 境界層の一覧を引く（URL が壊れている場合は書き込みも） | 全項目 `unknown`（🔴 在る／無いのどちらにも倒さない）。構成されていなければ・URL が壊れていれば API へ送らない（一覧は 500 にならず、書き込みは 200 のまま同期の依頼は失敗として記録）。陽性対照: 404 だけが `git` になる | 供給元（推測しない） | 自動 |
| T-70 | 他のロール／保管先に届かない | 境界層の一覧を引く | Kubernetes API へ 1 度も触れない（陽性対照: 通る一覧は問い合わせる） | 権限外に何も漏らさない | 自動 |
| T-71 | — | 起動した境界層を見る | 本番の合成が判定器の本物を登録している（上の T-68〜T-70 は偽物の通信路だけを差し替えた本物の端点を通る） | 合成の証明 | 自動 |
| T-72 | 画面・供給元が 画面／画面以外（値 `git`）／確認できない／値なし の 4 行 | 画面を開く | 3 値が別々のバッジで出る。**同期先が無い項目の表示は「画面以外」で、「Git」とは出さない**。値の無い行は「確認できない」（「画面」「画面以外」を出さない）。判定の方法の注記がある | 供給元の表示 | 自動 |
| T-73 | 画面・供給元が画面以外の項目 | 更新フォームを開いて送る | 送る前に「反映されません」を出し、送信は可能。**確認ダイアログを経ずに送る**。保存後も「反映されません」を出し、同期の成否と再起動の文言を出さない | 書いても効かないことを先に伝える（拒否しない） | 自動 |
| T-74 | 画面・供給元を確認できない項目 | 更新フォームを開いて送る | 反映されるかを確認できない旨を出す（画面以外の警告は出さない）。確認ダイアログは再起動「することがある」「断定できない」として出す。確認までは送らない | 推測しない | 自動 |
| T-75 | 画面・moomoo のログイン情報（OpenD が消費） | 更新フォームを開いて送る | 確認ダイアログに、消費側 OpenD・断たれ得る処理・「この書き込みでは OpenD は再起動しません」・手動の再起動コマンド・再認証を出す（「自動で再起動されます」は出さない）。書き込み後に「OpenD を手動で再起動するまで」を出す | OpenD の手動再起動を送る前に確かめさせる | 自動 |
| T-76 | 画面・株式自動売買の app-secrets（自動で作り直される） | 更新フォームを開いて送る →「やめる」→ 送る →「書き込む」 | 送信の押下では送らず確認ダイアログを開く。消費側・断たれ得る処理・「自動で再起動されます」・配備していない環境の一文を出し、OpenD には触れない。操作は「やめる」「書き込む」の 2 つだけ（別の再起動の操作は無い）。初期フォーカスは「やめる」。「やめる」では送らず、「書き込む」で 1 回送る。書き込み後に「自動再起動を配備していない環境では」を出す | 書き込みの確認＝再起動の確認 | 自動 |
| T-77 | 画面・供給元が画面以外の鍵のプロパティ | 「生成」→「生成して書き込む」 | 確認ダイアログは出す（鍵が置き換わる）。生成の本文は「OpenD にもクライアントにも届きません」で、「読み込むまで」とは書かず、再起動の本文を出さない。確定ボタンは破壊的な見た目・初期フォーカスは「やめる」。書き込み後は「反映されません」 | 画面以外での生成の文言 | 自動 |
| T-40 | 稼働クラスタ（保管先・同期あり） | 画面から 1 プロパティを更新し、同期後の Secret を長さだけで確かめる | 更新したプロパティの長さが一致し、同居するキーが減っていない | 稼働での成立 | 手動（未実施） |

## 自動試験の所在

| 区分 | ファイル | 対応 |
| --- | --- | --- |
| 画面 | `src/knowledge/frontend/src/features/sc22-secrets/components/SecretItemManagementPage.test.tsx` | T-01〜T-09・T-43・T-64〜T-67・T-72〜T-77 |
| 境界層（端点） | `src/platform/backend/Bff/Platform.Bff.Tests/BffSecretItemEndpointTests.cs` | T-10〜T-28・T-41・T-44〜T-49・T-52〜T-60・T-68〜T-71 |
| 境界層（保管先クライアント） | `src/platform/backend/Bff/Platform.Bff.Tests/VaultKvClientTests.cs` | T-42 |
| 境界層（項目集合） | `src/platform/backend/Bff/Platform.Bff.Tests/SecretItemCatalogTests.cs` | T-29〜T-31・T-50・T-51 |
| 保管先の権限の字面 | `src/platform/backend/Bff/Platform.Bff.Tests/SecretItemVaultPolicyTests.cs` | T-32〜T-35 |
| 同期依頼の権限の字面 | `src/platform/backend/Bff/Platform.Bff.Tests/SecretItemExternalSecretRbacTests.cs` | T-61 |
| 初期化スクリプトの字面 | `src/platform/backend/Bff/Platform.Bff.Tests/SecretItemBootstrapSeedTests.cs` | T-62 |
| 起動器 | `scripts/k8s-local-up.test.js` | T-63 |
| Kubernetes API の偽物 | `src/platform/backend/Bff/Platform.Bff.Tests/FakeKubernetesApi.cs` | T-57〜T-60・T-68〜T-70 が使う（既定では同期先がすべて在る。無いものは名前で指定する） |
| ブラウザ | `src/platform/frontend/e2e/sc22-secrets.smoke.spec.ts` | T-36〜T-38 |
| 保管先の偽物 | `src/platform/backend/Bff/Platform.Bff.Tests/FakeVault.cs` | 上記の境界層試験が使う |

## 未決事項

- T-40（稼働クラスタでの疎通）は未実施である。稼働クラスタに触れない作業条件のため、次に保管先を立ち上げた運用の場で確かめる。
  同じ場で、即時同期で数秒以内に Secret が変わること、Reloader が消費側を作り直すこと、生成した鍵を OpenD が読めることも確かめる。
- 同じ場で、供給元の判定を確かめる: 株式自動売買の外部シークレット同期を有効／無効にした 2 通りの配備で、一覧の供給元が
  `screen` / `git` に切り替わること（境界層の権限で `get` が 403 にならないこと。403 なら `unknown` に倒れる）。
