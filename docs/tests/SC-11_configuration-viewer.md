...
| # | 観点 | 検証内容 |
| --- | --- | --- |
| E1 | 認証ガード（未認証の導線） | 未認証で `/admin/config-viewer` を開くと `/login` へ誘導される。🔴 **この 1 本ではルートの実在を測れない**（未知のパスの受け皿が認証ガード配下に居るため区別できない） |
| E2 | 画面への到達（**セッションを与える**） | **運用者**の身元を与えて `/admin/config-viewer` を開くと、見出し・左ナビ「構成ビューア」・0 件の固定文言・版の表示が実ブラウザ・実ビルド成果物の上で描かれる＝陽性対照。**ルートの実在はここで固定される** |
| E3 | 権限による存在秘匿 | 管理ロールを持たない利用者では同じ「見つかりませんでした」が出て、左ナビにも項目が出ず、**構成の端点を呼びにも行かない**＝陰性対照 |
