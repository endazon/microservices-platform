...
| T-10 | ナビ表示（運用者） | 同上 |
| T-11 | 404 の中立表示 | 画面 9 |
| T-12 | 5xx の `role="alert"` | 画面 10 |
| T-13 | 未認証は `/login` | E2E E1 |
| T-14 | 履歴 2 件の表示 | 画面 5 |
| T-15 | 履歴 0 件 | 画面 6 |
| T-16 | 履歴取得失敗の領域縮退 | 画面 8 |
...
| 7 | **ドリフトの縮退** | [[IADR-0129]] 決定 5 | ドリフトだけ落ちても構成は出し続ける。**ヘッダのバッジは出さない**（「0 件」と紛れる） |
| 8 | **履歴の縮退** | 同上 | 履歴だけ落ちても構成は出し続ける |
| 9 | **404 の中立化** | [[IADR-0009]] | 「構成情報は利用できません。」（`role="alert"` を出さない） |
| 10 | **5xx は中立化しない** | 同上 | `role="alert"` で障害として出す |
| 11 | **実効構成が無ければ他も出さない** | [[IADR-0129]] 決定 5 | 構成が取れないとき、ドリフト・履歴の領域を描かず、**ヘッダのドリフトバッジも出さない**（何に対する差分か読めないため）。**404〔秘匿〕と 5xx〔障害〕の両方**で見る（**観点 11 だけはテスト 2 本**） |
| 12 | **参照専用** | 計画 §入力 | 画面上のボタンは**再取得の 1 つだけ**。**先に再取得ボタンと注記が在ることを確かめてから**構成変更の操作が無いことを見る |
| 13 | 再取得 | — | 3 本を取り直す（`invalidateQueries` のみ。手書きの再取得を持たない） |
| 14 | ロケール `en` | ADR-0031 | 見出しが英語で描画される |
