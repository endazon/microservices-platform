...
iadrs: [IADR-0009, IADR-0037, IADR-0126, IADR-0308, IADR-0367]
...
| E-2 | 身元を与えて `/ask` | 見出し・副題・左ナビ項目が実ブラウザで描かれる（陽性対照） |
| E-3 | 同上・初期表示の通信 | 検索も回答ストリームも走らない＝**問う前に回答の口を開けない**（陰性対照） |

> **［2026-09-02 追記 / #1139］従前ここには「認証済みの導線は E2E で実走できない」と書いていた。
> これは誤りである。** 認証状態の一次情報は身元を返す 1 つの応答だけであり、それをネットワーク層で
> 差し替えれば、実ブラウザ・実ビルド成果物の上で認証済みの画面が描かれる（基盤は 1 つも起動しない）。
> 土台は `e2e/support/bffSession.ts` にある。E-2 / E-3 は現にそれを踏んでいる。

**限界**: この層のスタブは**契約の写しであって後段ではない**。実応答との一致は固定しない
（実基盤を起こす検証は別 issue の射程である）。検索・SSE ストリーミング・出典・フィードバックの
挙動は引き続き単体側が担い、3 ルートを 1 本のルータへ載せる**導線テスト**（`searchFlow.test.tsx`）が
画面間の遷移を担う。
