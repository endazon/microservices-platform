...
> **［2026-08-10 消化 / #521］T-15 / T-16 を実装した。** 4 端点への
...
> **いずれも BFF と後段の両層に置いた**（書き込み・管理 API への認可強制＝多層防御）。
>
...
> [通信仕様書](../api/BFF_bff-surface.md)・`docs/api/openapi.yaml`・関連する 2 件の実装 ADR・
> `FeedbackEndpoints.cs`。

- 受け入れ基準（本機能に固有のもの）との対応: 収集=T-01/02、品質改善への活用=T-07/08、冪等=T-03/T-12、
  ...
  **#521 で実装済み**。統計の無認証は T-17、通る側の対は T-18 / T-19）。
