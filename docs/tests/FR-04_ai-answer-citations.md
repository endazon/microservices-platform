...
- 対象: 出典写像ロジック（`CitationMapper`）、`/analysis/ask` 応答、`/bff/analysis/ask` 集約と Authorization 伝播、
  応答が名乗る使用モデル（`AiAnswerDto.Model` / `AskDoneEvent.Model`。縮退応答はゲートウェイ報告値を透過する）、
  **出典の機密区分**（`CitationDto.Confidentiality` と安全側への縮退。#541）、
  **権限外文書を出典・プロンプトへ出さないこと／縮退しても出典を捨てないこと／ゲートウェイへ渡す用途**（#448）。
- 対象外: 横断検索の権限制御の網羅（AuthorizationService 側で検証）、反映時間、負荷/p95。
  用途→モデルの解決そのもの（`LlmRouter` 側。LLM 送信先切替のテスト T-02 / T-19）。
...
> T-17〜T-21 は `CitationMapperTests`（#541 /「出典には機密区分を含める」要求・検索チャット画面の裁定 Q10）。
> **表示名（公開 / 社内限 / 秘 / 取扱制限）はテストしない**——正は計画リポジトリの用語集であり、
> 実装側で表示名を固定すると用語の正が 2 か所へ割れる。テストが固定するのは**値集合と縮退規則**だけである。
>
> T-10〜T-16 は `RagOrchestratorDegradedModelTests`（縮退応答のモデル名の実装判断 / #403）。T-15f は
...
| T-02 | `/analysis/ask/stream` も同じ形で受け取る（検索・チャット画面の本文は SSE 経路である） | `AskStream_PassesAttributeFiltersDownstream` |
...
## 権限外文書を出典に出さない・縮退しても出典は残す（`RagAnswerContractTests`。#448）

#448 の突き合わせで見つかった**死角**を塞ぐ節である。

1. 既存の縮退テストは**検索が 0 件のスタブ**で走るため、「LLM が落ちても**出典は残る**」
   （計画のユースケース例外フロー＝縮退運転）を**一度も確かめていなかった**。出典を捨てる実装へ変えても
   既存テストはすべて緑のままだった（変異試験で実測）。
2. 権限の否定形は検索サービス側にはあったが、**回答の出典の側には無かった**。
3. ゲートウェイへ渡す **`purpose`** を誰も見ていなかった。用途別のモデル割当は**この文字列がキー**であり、
   取り違えても応答は 200 で返るため**静かに別のモデルへ流れる**。

検索スタブは**実際に ABAC フィルタを適用する**（「スコープを渡したか」ではなく
「権限外文書が出典に出ないか」を見るため）。**スコープを渡し忘れても・広げても落ちる。**

| # | 確かめること | 実装 |
| --- | --- | --- |
| T-21 | ★ **権限外文書が出典に現れない**。権限外文書の本文が LLM のプロンプトにも入らない | `権限外文書はRAGの出典に現れない` |
| T-22 | ★ ストリーミング経路も同じ境界を守る | `権限外文書はストリーミングの出典にも現れない` |
| T-23 | ★ **権限の外だけを指す範囲では検索を呼ばない**（漏えいの試行すらしない）。LLM へも送らない | `権限外を指す対象範囲では検索を呼ばず出典も返さない` |
| T-24 | ★ **LLM 不調でも出典は残り**、縮退文言を返す。出典は元文書へのリンクを必ず持つ | `LLM不調でも出典は残り縮退文言を返す` |
| T-25 | ★ **越境拒否でも出典は残り**、縮退文言を返す | `越境拒否でも出典は残り縮退文言を返す` |
| T-26 | ストリーミングは不調時も出典を先に送り、理由を本文にして **done で終端する** | `ストリーミングはLLM不調でも出典を送り理由を本文にして完了する` |
| T-27 | ★ AI 分析は用途 `analysis` で呼ぶ。モデルは呼び出し側で固定しない（`Model` は null） | `AI分析は用途analysisでゲートウェイを呼ぶ` |
| T-28 | 質問回答は用途 `rag-answer` で呼ぶ（分析と同じ用途名にしない） | `質問回答は用途ragAnswerでゲートウェイを呼ぶ` |
| T-29 | 分析・比較・抽出の 3 種別が、検索結果を根拠にしたプロンプトとして後段へ届く | `分析比較抽出の種別がプロンプトへ反映される`（`[Theory]`） |

> 🔴 **`scripts/test-spec-coverage-baseline.json` の更新が要る**（本節が新しいテストクラスを載せたため）。
...
- 検索・チャット画面の出典行に機密区分チップを描く画面テストは、表示を作る issue で足す（本書の対象は契約と写像まで）。
