...
adrs: [ADR-0031]
iadrs: [IADR-0042, IADR-0127, IADR-0128, IADR-0154]
...
| **人手補正の 2 ペイン編集（Phase 1）** | **［2026-08-10 / #651］実装した**——§テストケース 14〜21。契約は #543（[[IADR-0154]]）、画面の決定は [[IADR-0157]] |
...
| 13 | **導出標識**（縮退・補正あり・Mermaid n図） | [[IADR-0157]] 決定 3 / `05_screens:320` | `diagramsRetained` / `hasCorrection` / `diagramsCoded` から導出する。**`status` は 4 値のまま**（縮退したジョブは `succeeded`） |
| 14 | **人手補正（管理者）** | `05_screens:314` / [[IADR-0154]] 決定 6 | 縮退した図のある行から 2 ペインを開き、コードを投稿して成功を伝える。**対象は `coded === false` の図だけ** |
| 15 | **人手補正（運用者に出さない）** | 同上 / [[IADR-0127]] 決定 1 | ボタンが無く「人手補正は管理者のみ実行できます」と理由が出る。**先に行が描かれていることを確かめてから**無いことを見る |
| 16 | **★ 右ペインの画像が解析層を通る** | [[IADR-0157]] 決定 1・5 | `/image` を呼び、**`<img>` まで届く**（オブジェクト URL）。`storage://` を `src` に入れない。**「要求が飛んだ」だけを見てはいけない**——変異試験で、それだけでは `bffFetch` の欠陥を素通りすると実測した |
| 17 | **画像 404 の縮退** | [[IADR-0009]] | 「元の画像を表示できません。」へ倒す。**空白にしない**（読み込み中と区別が付かなくなる） |
| 18 | **補正が失われる確認（409 駆動）** | `05_screens:313` / [[IADR-0157]] 決定 2 | **409 `corrections_would_be_lost` を受けてから**確認を出し、`correctedFigures` を件数として示す。確認前に `discardCorrections` を送らない。確認後は `?discardCorrections=true` で再送する。**確認の同定は role ではなく「破棄ボタンが在ること」で行う**——本確認は非モーダルなので `alertdialog` を名乗らない（[[IADR-0157]] 決定 2） |
| 19 | **`not_retryable` との区別** | [[IADR-0157]] 決定 2 | 同じ 409 でも**拒否であって確認ではない**。取り違えると実行中のジョブに補正破棄を勧めることになる |
| 20 | **直近の操作の結果だけを出す** | [[IADR-0127]] 決定 7 | **retry の失敗 → correct の成功**で古い失敗バナーが消える。**別々のミューテーションでしか再現しない**——同じミューテーションを 2 回叩く形では TanStack Query が自分の `isError` を戻すため、`reset` を外しても緑になる（実測） |
| 21 | 図の取得失敗・補正対象なし | — | 空パネルを見せず、それぞれ理由を出す |
| 22 | ロケール `en` | —| 見出しと状態が英語で描画される |
| 23 | **確認へキーボードで到達できる** | [[IADR-0157]] 決定 2 / AI レビュー 🟡 | 確認が出たら**非破壊側（取り消す）へ焦点が移る**（既定の焦点は `Enter` の当たり先でもあり、破壊側に置くと反射的な連打で補正が消える）。確認は表より**前**に描かれ、それを出すボタンは**表の行の中**にあるため、**前方 Tab では届かない**——焦点を移さないと読み上げは届くのに操作手段が無い |
...
| P5 | **縮退の導出** | `diagramsRetained > 0`。**フィールドが無い場合は「縮退なし」へ倒す**（**［#658 / [[IADR-0162]]］契約は `required` へ移ったが、防御は残す** —— [[IADR-0132]] 決定 3「契約上は必須」と「実行時に必ず来る」は別） |
| P6 | **補正可否** | 縮退した図があること。**権限は混ぜない**（[[IADR-0157]] 決定 3） |
...
- **再変換（`retry`）は admin のみ**（7b / 7c。2026-08-04 確定・[[IADR-0128]] 決定 1）。照会側（3 / 5b）と対で
  ...
  **［2026-08-10 追記 / #553］「planning#198 提案 8 で裁定を仰いでいる」は失効した** ——
