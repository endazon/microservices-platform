...
| 19 | **補正が失われる確認（409 駆動）** | `05_screens:313` / [[IADR-0157]] 決定 2 | **409 `corrections_would_be_lost` を受けてから**確認を出し、`correctedFigures` を件数として示す。確認前に `discardCorrections` を送らない。確認後は `?discardCorrections=true` で再送する |
