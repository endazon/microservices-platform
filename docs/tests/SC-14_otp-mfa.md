...
| T-12 | 同上 | 同上（検査 5） | realm の**全 client** で `directAccessGrantsEnabled` が**真偽値の `false`**（`null`・文字列の `"true"` なども開くと読む。`bearerOnly` の未設定・`null` は数えない）で、非推奨の `directGrantsOnly` が `true`（`"true"` を含む）でない（管理 API で作ると未設定・`null` は開く） | パスワードグラントは browser フローを通らないため、開いていると **OTP を一切問われずにトークンが出る**（MFA のバイパス口） | 自動 |
