...
- 🔴 **全文インデックスの日本語の再現率は部分的で、文書に依存する**（実機で実測）。
  そのため**日本語の語では判定しない** —— 判定に使うのは識別子・型番・略語と、
  語順を替えた合言葉である。日本語の当たり方は実機検証スクリプトが**数字をそのまま出す**に留める。
  どの語まで引ければ受け入れとするかは、実運用の検索ログが出てから決める。
