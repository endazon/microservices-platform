...
- pin 先 `356e8c7` のガイドのメタ情報は `状態: draft` のままである（issue は「PR planning#294 で確定・fixed」とする）。本書は日付と確定 PR で特定する表記（「planning#294 で 2026-08-08 に確定」）を用いた。状態表記の更新は計画リポ側の追随事項である。
