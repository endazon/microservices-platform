...
したがって母集合は **判定それぞれの対象集合**であり、**現状値はすべて基準線（baseline）**として働く。
...
| 6. 索引行の「形」（**#580 から統合**） | 同上の ID セル・リンク先・行末 `\|` | **違反 0 件** |
| — | ファイル総数（**走査基準 `296b4f6` 時点**。現在値ではない） | **144** |
...
1. **`scripts/check-adr-numbering.js` を新設**し、上記の判定を機械化する（**#580 からの統合で最終的に 6 判定**）。
...
変異試験 **M8〜M10** で 3 種すべてを対で固定した（[[IADR-0144]] 決定 6）。
...
1 対 1 に対応せず、先に語彙の規約化が要る）を [[IADR-0144]]「検出しないこと」と
...
## ［2026-08-08 追記 / フェーズ末クロス監査 🟢-5］`.gitignore` への `.claude/worktrees/` 追加

本 PR は `.gitignore` へ `.claude/worktrees/` を 1 行足しているが、**理由がどこにも書かれていなかった**
（本書・[[IADR-0144]]・コミット本文・PR 本文のいずれにも `worktree` の語が無い）。

**理由**: 作業中に `git add -A` が**サブエージェントの worktree を embedded git repository として
追跡下へ取り込んだ**。`git rm --cached` で外したうえで、同じ事故を繰り返さないよう ignore した。
`CLAUDE.md`「雑多な変更は理由を明記する」に照らして記録する。
