...
> **★ Grafana を含めて 4 種**である（issue の逐語は 3 種。判断と根拠は §4.7）。

## 0. 前提（★ 本書の読者が最初に知るべきこと）

### 0.1 IADR 番号は `IADR-0210` を使う（`IADR-0209` ではない）

本 PR が立てる実装 ADR は **`IADR-0210`** である。`IADR-0209` は **#801 の PR が先に取る**。
マージは FIFO で **#801 → 本件**の順に着地させる前提であり、この順序が崩れた場合
（本件が先に着地する／#801 が 0209 を取らない）は **本 PR を `IADR-0209` へ改番する必要がある**。

**この前提が満たされていない状態で `node scripts/check-adr-numbering.js` を走らせると、判定 2（欠番なし）で
必ず fail する** —— 本ブランチ単独では `IADR-0209` が存在せず 0208 → 0210 が飛ぶためである。
これは想定どおりの結果であり、**#801 が develop へ着地した後に本ブランチを rebase すれば解消する**。
検証節（§7）に実際の出力を残す。

### 0.2 この環境では受け入れ基準 1・3 の「実測」ができない

`kubectl` / `helm` / `k3d` / `kustomize` がいずれも**不在**で、稼働クラスタも無い（§7.0 に実測を残す）。
したがって次の 2 つは**この PR では測れない**:

- 受け入れ基準 1 の後半「**Pod 再起動でデータが残る**」
- 受け入れ基準 3 の後半「**保持期間が実効していることを実測で示す**」

**できないことを「できた」と書かない**（[[IADR-0184]]）。本書は §8 で
**「静的に固定できた範囲」と「稼働クラスタでの実測が要る範囲」を受け入れ基準ごとに分けて**書き、
後者は起票者へ委ねる。
...
### 4.6 Loki / Tempo の書き込み権限 — **compose を鏡にする**

compose は `loki` / `tempo` に `user: "0:0"` を付けている（IADR-0079 §3）。根拠は**推測ではなく実測**である
—— 空ボリュームが root:root で生成され、非 root イメージ（uid 10001）が書けず**起動回帰した**。
k8s の PVC（local-path provisioner）でも同じ性質があるため、**等価の Pod `securityContext`
（`runAsUser: 0` / `runAsGroup: 0`）を新 overlay の patch で足す。**

**Prometheus / Grafana には付けない** —— compose も付けておらず、compose では
`prometheus-data:/prometheus` と `grafana-data:/var/lib/grafana` が**現に動いている**ため。
判断基準そのもの（**推測ではなく実測された先例に従う**）を ADR に書く。
...
§4.3 の 4 PVC と 4 patch。Loki / Tempo には `add /spec/template/spec/securityContext` も入れる。
...
4. **マニフェスト静的検査**（既存の同種検査と同じ型・YAML パーサを持ち込まない）:
   ...
   - **compose を鏡にする判断そのものを検査する**: 同名データボリュームのマウント先が
     compose と k8s で一致すること／`user: "0:0"` を持つサービス（loki・tempo）だけが
     k8s 側でも `runAsUser: 0` を持つこと（**両側から読む**）。
...
issue の逐語 5 項目。**各項目に「静的に固定できた／稼働クラスタでの実測が要る」を明記する**（§0.2）。

| # | 受け入れ基準（issue 逐語） | 静的に固定できた範囲 | 稼働クラスタでの実測が要る範囲 |
| ---: | --- | --- | --- |
| 1 | `PERSIST=1` で Qdrant / Prometheus / Loki / Tempo が PVC を持ち、Pod 再起動でデータが残る | **前半は済**（PVC 定義・patch・ゲート分岐・マウント先の config 一致を `k8s-local-up.test.js` が固定。M1 / M3 で検出力を実測） | **後半は未了**。`kubectl delete pod` を跨いでデータが残ることは**測っていない** |
| 2 | 既定（`PERSIST` 未設定）でバイト等価 | **済**（`OPTIN_TOKENS` の既定経路検査が緑。M2-b/c で「混入したら落ちる」を実測）。ただし守っているのは接頭辞トークンである（§7.3） | なし |
| 3 | Prometheus の保持期間が args で明示され、実効していることを実測で示す | **「明示」と「壊れない形」は済**（args の存在・compose とのパリティ・`size(4.0e9 B) < PVC(5.37e9 B)` を数として検査） | **「実効している」は未了**。`/api/v1/status/runtimeinfo` の `storageRetention` を測っていない |
| 4 | `IADR-0082` を改定するか追補する | **済**（`［2026-08-16 追記 / #787］` ブロック ＋ 後継 `IADR-0210` の併記、`updated:` 前進、frontmatter へ後継 ID 追加） | なし |
| 5 | compose 側（`IADR-0079`）とのパリティ差が解消したか、残るなら理由を書く | **済**（Prometheus / Grafana の未マウントと retention 無指定を解消。**マウント先と `user:"0:0"` の集合一致を機械が両側から突き合わせる**） | なし。**残るパリティ差は §8.1** |
...
- **#801 との採番順序**（§0.1）。FIFO が崩れたら `IADR-0209` へ改番する。
  改番時は `.claude/rules/traceability.md`「採番衝突時の改番手順」に従い、
  ファイル名・自称番号・索引・本仕様書・コード内コメント・**PR タイトル**をすべて追随させる
  （実測: 本作業中に一時プローブで改番したとき、`IADR-0210` の参照は **15 ファイル**にあった）。
- **稼働クラスタでの受け入れ（§0.2 / §8 の「未了」欄）は起票者へ委ねる。** 手順:
  ...
  5. Loki / Tempo の Pod が `CrashLoopBackOff` にならないこと（root 実行が効いていることの確認）
