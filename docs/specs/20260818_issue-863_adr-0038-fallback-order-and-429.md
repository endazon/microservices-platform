...
related_specs:
  ...
  - ../operations/llm-model-pin-runbook.md
---
...
- 実装 ADR: 本 PR で [[IADR-0225]] を起こす（内部設計の決定であり、計画 ADR への単純な追随ではない）。
...
| `scripts/test-spec-coverage-baseline.json` | **上の走査では出ない追随点。** テスト仕様書へクラスを載せると [[IADR-0130]] のラチェット床を上げる必要があり、`REQUIRE_REPO_TESTS=1 node scripts/scripts.test.js` が「床の上げ忘れ」で fail する。`node scripts/check-test-spec-coverage.js --update` で 2 件（`LlmFallbackPolicyTests` / `CompletionFallbackEndpointTests`）追加した | 検査器 |
| 本仕様書 | — | — |
...
正本は [[IADR-0225]]。ここには「何をどのファイルに置くか」を書く。
...
429 の再試行は [[IADR-0225]] §結果 のフォローアップとして残す**（現行の 429 の挙動は変わらず
