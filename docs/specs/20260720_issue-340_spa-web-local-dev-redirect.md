...
- 関連 ADR: ADR-0004（認証＝Keycloak）。前提は [[IADR-0076]]（決定3＝ブラウザ OIDC の issuer 統一・手順A）と
  [[IADR-0078]]（frontend の k8s 配信）。
...
> [[IADR-0076]] 手順A は「realm 無改変」で成立すると述べているが、これは **per-session の realm 改変が不要**という意味で、
