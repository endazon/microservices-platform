...
- **リポ相対パス**（`[x](src/ai-stock-trading#1-概要)` や `ディレクトリ src/ai-stock-trading#1`）は
  `src` を owner と読んで**検出してしまう**。`src/ai-stock-trading` は本リポジトリに実在する
  submodule パスである。実在 0 件（`git grep -nE '\(\.{0,2}/?src/ai-stock-trading[^)]*#' -- '*.md'` → 0）。
  **回避**: 相対リンクは `../src/...` と書く（直前が `/` になり後読みで落ちる）か、コードスパンに入れる。
