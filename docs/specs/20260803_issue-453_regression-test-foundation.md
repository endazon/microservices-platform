...
`AiStockTrading.Bff.Endpoints` の行が、`src/platform/` 配下のレポートの**中身**に混入する。

混入量は **230〜266 行**（いずれもすべて被覆済み）。レビューの 2 度の計測（全プロジェクト実行 266 行 /
`Platform.Bff.Tests` 単体実行 230 行）が割れたため、確定値として採らず範囲で記録する。除去後の推定は
230 行なら `line 34.19%`（18664/54596）、266 行なら `line 34.14%`（18628/54560）で、**いずれも床 34 を
上回る**。よって床の値は混入の確定を待たずに有効である。

塞ぐには Cobertura の `<class filename>` で行を帰属させるパーサ改修が要るため分割した。確定値は #468 の
着手時に実レポートで測り直す。
