...
- 実装判断: [[IADR-0063]]（合成方式 A＝ビルド時合成点。承認済み・Accepted）
- Issue: #229（フォローアップ 3・段階実装スライス1）

## 目的・背景

[[IADR-0063]] で承認された合成方式 A（ビルド時合成点）の段階実装スライス1。BFF は全フロントの唯一の入口で
...
- 対象（本スライス）:
  ...
  - [[IADR-0063]] を Accepted 化し、段階計画をドメイン単位移設へ精緻化（下記知見）。
- 対象外（後続スライス）:
...
→ この知見に基づき [[IADR-0063]] の段階計画を「DTO 分離→エンドポイント移設」の二段から「ドメイン単位で
...
- 合成方式・段階計画（ドメイン単位移設へ精緻化）は [[IADR-0063]]（Accepted）に記録。
