...
あり、開発環境・文書統制とは軸が違う。**無理に近い番号を付けない**（[[IADR-0179]] 決定 2）。

→ **場合 2**。したがって**環流しない**。

## 母集合（[[IADR-0141]] 決定 1 の規則 1〜6 ／ `traceability.repo.md` 規則 9・10）
...
| キット配布物（分類 A。`.claude/rules/traceability.md` / `scripts/scripts.test.js` ほか） | 直接編集しない（[[IADR-0115]]） |
...
**[[IADR-0204]] 決定 2 の 3 点で自分で再判定した**（対象は検査器ではないので読み替えを明記する）:
...
### 4. 新規 [[IADR-0224]]（雛形 backend を CI で検証する方式）

[[IADR-0060]] 決定 3「テンプレートは本リポジトリのビルド対象ではない（`src/` 外・どの slnx にも
含めない）」の**括弧内 2 条件は保たれる**（一時複製をビルドする）が、**位置づけそのものは実質的に変わる**。
同型の先行例（雛形 frontend を CI 対象に入れた #801）は [[IADR-0209]] として記録されている。
**決定の記録が無いまま位置づけを変えない。**
...
- **無採番 `NFR` の場合判定の記録が無い**（レーン A / D の仕様書は [[IADR-0179]] を引いて場合 2 と明記）。
