...
- ADR: ADR-0006（運用基盤）。ESO 基盤・k8s auth・段階移行は [[IADR-0096]]（PR-1）／[[IADR-0097]]（PR-2）。opt-in 統括は [[IADR-0077]]。
- 決定: 本作業の設計判断は [[IADR-0098]]（PR-3 対象 secret の ExternalSecret 化・PR-1/PR-2 設計踏襲）。
- 対象 OIDC の各機能連携: MinIO=[[IADR-0093]]／Grafana=[[IADR-0090]]／Vault=[[IADR-0094]]／Headlamp=[[IADR-0080]]。
- Issue: #310（Vault/ESO 本番同等化）。develop 最新（PR-1/PR-2 反映済み）ベース。

## 背景と問題

[[IADR-0096]]（PR-1）で ESO 基盤を敷き、[[IADR-0097]]（PR-2）で minio/wikijs 系を移行した。PR-3 は同じパターンで
