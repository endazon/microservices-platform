...
related_specs:
  - ../adr/IADR-0018_datasource-default-attributes.md
related_adrs:
...
で補完する（`public` の過剰公開でも `restricted` の過剰制限でもない社内基準）。詳細は IADR-0018 参照。

## 変更範囲

- `Domain/DataSource.cs`: `DefaultAttributes` 追加。`Create` に `defaultAttributes` 引数を追加し、
  `confidentiality` 欠落・空を既定値で補完（防御的コピー）。
...
- `Endpoints/DataSourceEndpoints.cs`: `CreateDataSourceRequest.DefaultAttributes` 追加、`sync` で
  `ds.DefaultAttributes` を原本イベントへ付与。
- `Migrations/`: `AddDataSourceDefaultAttributes`（jsonb、既存行は `{}` 既定）＋ Designer ＋ Snapshot 更新。
- テスト: `DataSourceTests`（Create の補完・保持・防御的コピー）、`DataSourceSyncEndpointTests`
  （sync が既定/明示属性を原本へ付与）。

## 受け入れ基準

- [ ] 機密区分未指定で登録したデータソースの `sync` は `confidentiality=internal` を持つ `RawDocumentFetched` を発行する。
- [ ] 明示した機密区分・部門などの属性はそのまま原本イベントへ付与される。
- [ ] `DefaultAttributes` は永続化され、再起動後も同期に反映される（jsonb カラム＋マイグレーション）。
- [ ] 既存テスト（Health / DataSources 一覧）に回帰がない。

## スコープ外

- 実際のファイル取得は依然シミュレート（本作業は属性伝播に限定）。
- AuthorizationService `/attributes/validate` への登録時バリデーション連携は本作業では行わない（フォローアップ）。
