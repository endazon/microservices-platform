...
type: spec
status: done
related_ids: [SC-01, SC-03, SC-05, SC-06, SC-07, SC-08, SC-09, SC-10, SC-11, UC-01, UC-02, UC-03, UC-04, UC-05, UC-06, FR-01, FR-04, FR-06, FR-08, FR-09, FR-10, FR-12, FR-15, ADR-0031, IADR-0009, IADR-0040, IADR-0121, IADR-0122, IADR-0127, IADR-0129, IADR-0131]
author: Claude
created: 2026-08-05
updated: 2026-08-05
plan_refs:
...
| 2 本目 | 各画面の `apiFetch` ＋ 手書き型 → 生成フックへの載せ替え（SC-01 feedback / SC-02 / SC-03 / SC-05 / SC-06 / SC-07 / SC-09 / SC-10 / SC-11） | **やらない**（§残りとして何をどうするか） |
...
| 事項 | 送り先 | 理由 |
| --- | --- | --- |
| **9 ファイルの生成フックへの載せ替え** | **分割 2 本目**（親が issue へ切り出す） | 上記 §分割 |
...
1. `docs/api/openapi.yaml` の `paths` に **`/bff/analysis/ask/stream` を載せる**（`text/event-stream` の
   応答として。`application/json` を持たないため orval は React Query フックを作らず、型だけが出る）。
   ——**「宣言としては在る／生成としては対象外」が 1 か所で読める形にする**。
2. `docs/api/BFF_bff-surface.md`（新設）の一覧表で「生成対象: **いいえ（SSE）**」と明示する。

> **注意**: 1 は「orval が本当にフックを作らないこと」を生成物で確認してから確定する。
> 作ってしまう場合は宣言を載せず、`BFF_bff-surface.md` の明記のみに倒す（[[IADR-0131]] 決定 4）。
...
| # | 事項 | 種別 | 送り先 |
| --- | --- | --- | --- |
| 1 | **画面の載せ替え（分割 2 本目）** | 本 issue の残り | 親が別 issue へ切り出す。対象・手順は §残りとして何をどうするか（9 ファイル・共通の注意 5 点） |
| 2 | **既存の応答スキーマ 23 個に `required` が無い**（数え方 = `components.schemas` 直下で `required` を持たないキー 27 個から、本作業で追加した 4 個を引いた数） | 是正提案 | 変異試験 M2 が示したとおり、`required` の無いスキーマは型検査の網にならない。入れるかは**別 PR**（影響が `?? 既定値` の表現に及び、載せ替え〔2 本目〕とも競合する） |
...
| 7 | **`/bff/feedback`・`/bff/feedback/stats` に端点認可が無い** | 要裁定 | 通信仕様書 §未決事項 3。**本作業では判断しない**（認可の変更は挙動の変更） |
