...
- 制約: [[IADR-0121]] 決定 3（BFF 境界）／[[IADR-0131]] 決定 4（SSE は生成対象外）
...
| `platform/frontend/src/features/**` | ❌ | 合成点（`features/index.ts`）1 ファイルのみで画面が置かれない。詳細と見直し条件は [[IADR-0146]]「検出しないこと」 |
...
- **`platform/frontend/src/features/`**（合成点のみ・[[IADR-0146]]「検出しないこと」）
...
- [x] 例外の許可が**明示的**である ——**禁止対象を `apiFetch` に絞ること自体が例外の明示**
      であり、許可リストという手作業の更新点を作らない（[[IADR-0146]] 決定 2）
