...
雛形はビルド対象外のため単体テストを持てない。代わりに **CI での型検査を回帰テストの代替**とする
（設計 4）。受け入れ基準の「旧契約へ戻すと赤くなる」は、**検査器が実際に効いていることの証跡**として
実行ログを PR に残す（宣言だけの監査は不合格。CLAUDE.md §実装作業の進め方）。

加えて雛形に**サンプルテスト 1 本**（`sample/SamplePage.test.tsx`）を置く。新ユニットが「テストを
どこに置き何を import するか」を雛形から読み取れるようにするためで、実装と同居する規約
（`*.{test,spec}.{ts,tsx}`）を体現させる。
...
2. ~~**雛形の `stores/` をどうするか。**~~ **決着**: 置かない。`src/README.md`「存在しない区分のフォルダは
   作らない」に従い、Zustand の導入（移行第 4 段 / #493）まで `hooks/useSampleFilter.ts` の
   コメントで予約する。
