...
- **`IADR-0209` が欠番になる。** 本 PR は `IADR-0210` を採ったが、`0209` は**未マージの PR #814 が予約**している。
  そのため `check-adr-numbering.js` が `missing-number` で落ちる。
  **`IADR-0144` 決定 3「並行 PR の衝突は着地後にしか見えない」の型**であり、
  **#814 の着地後に develop を取り込めば解消する**（#555 が同じ手順で解消した先例がある）。
  番号は先着尊重（#814 が先にマージされる）。
