...
- 対象外:
  ...
  - 旧データの破棄・切替（#457）
  - LLM 費用の実試算（[14_knowledge-graph-graphrag](../../planning/projects/microservices-platform/06_technical/14_knowledge-graph-graphrag.md) §6 手順 2）。本作業はその**入力となる組み合わせ数**までを出す

## 設計

### 測定する 3 つの粒度

[14_knowledge-graph-graphrag](../../planning/projects/microservices-platform/06_technical/14_knowledge-graph-graphrag.md) §6 手順 3 が
...
2. 無ければ [07_abac-attribute-model](../../planning/projects/microservices-platform/06_technical/07_abac-attribute-model.md) の
...
- [x] 実測結果が数値（組み合わせ数・上位分布）で報告され、計画側 issue / feedback に記録されている
      （[feedback/20260805_abac-attribute-combination-measurement-result.md](../../feedback/20260805_abac-attribute-combination-measurement-result.md)）
...
- 差異: あり（内容・対応: 計画 [07_abac-attribute-model](../../planning/projects/microservices-platform/06_technical/07_abac-attribute-model.md) は
...
- 経路B の実データは取り込み経路（datasource / ingestion）由来であり、**本番相当の分布ではない**。
  この限界を結果に明記する（[14_knowledge-graph-graphrag](../../planning/projects/microservices-platform/06_technical/14_knowledge-graph-graphrag.md) §6 手順 1 は「実装リポジトリまたは本番相当データで」測ると定めている）
...
- ADR-0035 は要約の粒度を「**機密区分単位（4 通り）から始める**」と決定した。実測では機密区分は
  **1 通り**しか実在しないため、決定は**実測に対して安全側**である（粗い側から始める方針とも整合する）。
  費用試算（[14_knowledge-graph-graphrag](../../planning/projects/microservices-platform/06_technical/14_knowledge-graph-graphrag.md) §6 手順 2）の
