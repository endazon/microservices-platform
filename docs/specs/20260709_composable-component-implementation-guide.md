...
られるか」を一気通貫で示す**共通実装ガイド**を `docs/tech/` に新設する。あわせて、計画リポジトリ側にも
プラグイン提供者向け共通仕様（上流版）の有無確認・追記を `/plan-feedback` 経路で環流する。
...
- 差異: あり（計画書の不足の疑い）。計画 `10_composability-design` は設計判断を扱うが、
  プラグイン提供者向けの「共通仕様（提供契約・互換性境界・受け入れ条件）」が上流に確定して
  いるかは本セッションでは確認できなかった（planning サブモジュール未取得）。
  → `/plan-feedback` 記録（`feedback/20260709_composable-implementation-guide-upstream.md`）で環流する。
