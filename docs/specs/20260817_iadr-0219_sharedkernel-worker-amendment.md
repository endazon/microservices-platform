...
- 関連 ADR:
  ...
  - 計画 [`ADR-0030`](../../planning/projects/microservices-platform/07_adr/ADR-0030_backend-application-libraries.md)（選定基準 1〜4）
  - 計画 [`ADR-0041`](../../planning/projects/microservices-platform/07_adr/ADR-0041_result-type-external-library.md)（Result 型の封じ込め）
...
| 実装側の決定 | 裁定 |
| --- | --- |
| [[IADR-0218]] 決定 1: `SharedKernel` は `.gitkeep` の対象**外** | **対象に含める**（計画の構成図が正・**サービス単位**） |
| [[IADR-0218]] 決定 2: `Worker` は `Api` の別形（環流不要） | **`Worker` を標準構成へ追加**（7 → **8 要素**。`Api` と**排他**） |
| [[IADR-0117]]: サービス単位の `SharedKernel` を却下 | **サービス単位を標準構成として認める。ユニット単位（`Platform.Shared.Kernel`）とは併存** |
...
**`status` は両者とも `Accepted` のまま**とする。**先例**: [[IADR-0056]] は決定 3 が
[[IADR-0117]] に部分改定された後も `Accepted` を維持し、本文へ日付つき追記ブロックを置き、
...
`scripts/scripts.repo.test.js` の**単体実行はしない**（#797 / [[IADR-0208]] のガードで exit 1 になる仕様）。
