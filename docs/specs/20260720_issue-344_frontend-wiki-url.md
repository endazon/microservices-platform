...
- 関連 ADR: ADR-0011 / [[IADR-0020]]（Wiki.js の実体・ABAC ゲートウェイ）。実行時 config・k8s 配信の先例は
  [[IADR-0078]]（frontend の `extraEnv` 経由で config.js 変数を注入）。経路B の割り切りは [[IADR-0066]]、
  ブラウザ OIDC issuer 統一（手順A）は [[IADR-0076]]。
...
1. **`deploy/local/values-local.yaml`（経路B）**: frontend ブロックに `extraEnv` を追加し
   ...
   **`wiki-js` の port-forward**（`svc/wiki-js 3300:3000`）に整合させる。経路B は `edge.enabled=false`
   （Istio 未導入）のため、ブラウザ到達は直 port-forward が既定手順（[[IADR-0076]] / README）。
...
純 config 配線（既設計キー [[IADR-0078]] ＋ 手順A port-forward [[IADR-0076]]）であり、新規の設計判断は
