...
```text
走査（同じコマンド・`git add -A` 後）がそのまま返す数 = 64 ファイル
  − 本作業が追加・編集したファイル 51                = 13 ファイル（着手前＝上表の軸 1）
```

**51 の内訳**（すべて本 PR の成果物である）:
...
3. **カバレッジ床は動かさない。**
   実測（`pnpm run test:coverage` / 77 ファイル 963 件すべて成功）:
   全ユニット横断 lines 97.04%（7806/8044）/ branches 91.27%（1735/1901）/ functions 93.36%（534/572）。
...
4. **チャンク床は動かす（+6.61 kB）。**
   622,675 → 629,286 バイト、小チャンク 6 → 4 本。**切り分け済み**——同一環境で本 PR を外して
