...
### `required` 検査を全数へ当てたら既存 20 件が赤くなった（測ってから線を引いた）

`required` まで見ると**着手時点で 20 件が一致していない**。#525 は `AccessScopeResponse` の資源に
閉じており（[[IADR-0139]] 決定 1）、20 件の是正は別の作業なので**ラチェットで据え置く**
（リポジトリ既定の方式）。据え置きは応答側 13 件・要求側 7 件で**是正の安全性が違う**ため、
baseline の `_comment` に分けて書いた（要求側に `required` を足すと呼び出し側が送信を強制される）。
