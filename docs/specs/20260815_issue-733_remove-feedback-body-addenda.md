...
✓ 520 tests passed                   # 516 → 520（新規 4 件）
...
残る **19 ブロック**（うち検査が数えるのは 15。原文由来の 4 は `originText` で除外）があり、**いずれも本作業の承認範囲外**である（§6 参照）。
...
| **3（現行）** | **全 30 ブロック・残 19** | 語順非依存で引き直した |
...
| `## 追記 N（…）`（4） | **そもそも対象外** | **ファイル追加コミット `47006a1` から在る原文**。「後から差し込んだ注記」ではない |
...
残る **15 件**を baseline に持つ **ratchet**（新規混入は fail・既知は許容）とし、
**原文由来の 4 件は `originText` として計数から除く**（既知の残存違反として分類しないため）。
...
| **`feedback/20260803_ai-review-execution-permissions.md` の `## 追記 1〜4`** | **ファイル追加コミット `47006a1` から在る原文**であり、`.claude/rules/traceability.md` が言う「**後から差し込んだ注記**」に当たらない（実測: `git log --diff-filter=A` が同コミットを返す）。**そもそも IADR-0191 決定 2 の禁止対象ではない**が、検査は形で拾うため baseline に載せて許容する |
