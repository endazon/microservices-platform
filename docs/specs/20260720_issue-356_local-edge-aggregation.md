...
- ADR: ADR-0006（運用基盤）。経路B k8s dev 統括は [[IADR-0066]]、prod エッジ（Istio）ルーティングの設計は
  [[IADR-0076]]、opt-in オーバーレイの流儀は [[IADR-0077]]、ゲート横断 smoke test は [[IADR-0087]]。
- 決定: 本作業の設計判断は [[IADR-0091]]（ローカル Traefik エッジ・ホスト名ベース集約・issuer 最小案）。
