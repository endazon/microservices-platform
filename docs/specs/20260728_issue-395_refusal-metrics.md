...
- 設計: [ADR-0006](../../planning/projects/microservices-platform/07_adr/ADR-0006_observability-otel-prom-loki.md)
  （OTel/Prometheus/Loki/Tempo への統一計装）、
  [ADR-0010](../../planning/projects/microservices-platform/07_adr/ADR-0010_llm-gateway.md)（LLM ゲートウェイ）、
  [ADR-0025](../../planning/projects/microservices-platform/07_adr/ADR-0025_llm-model-opus-5.md)（既定 Opus 5・安全性分類器）、
  [[IADR-0104]]（`stopReason` の契約化・`Sent` と `StopReason` は独立した軸）。
- 本作業の実装判断は [[IADR-0110]]。

## 背景と問題

[[IADR-0104]] により終了理由は**ログには**残るようになったが、**残っているのはログだけ**で、
拒否がどの頻度で起きているかを継続的に把握する手段が無い。拒否率は次の判断に直結する運用指標である。

- 既定モデルを Opus 5 にした（ADR-0025 / [[IADR-0101]]）ことで、安全性分類器による拒否が実運用にどれだけ出ているか
...
`Sent=false`（越境拒否）とモデルの `refusal` は**別軸**で表現される（[[IADR-0104]] §決定に従う）。
