...
> **［2026-08-10 変更 / [[IADR-0156]] 決定 1］この節の方針は採らなかった。**
...
**実装中に実データで破綻が確認できたので方針を変えた**（[[IADR-0156]] 決定 1）:
...
### 3. `/bff/admin/config` に認可が無いのは**意図どおり**だった

抽出で「ロール制約なし」と出たので実装を読んだところ、`ConfigBffEndpoints` は
**404 で応答自体を秘匿する**方針で、`RequireAuthorization` を意図的に付けていない
（[[IADR-0009]] の存在秘匿と整合）。**誤検知ではないので `x-roles: []` として記録した。**
