...
3. `claude-fable-5` が claude-managed（ティアB）エンドポイントの `Models` に含まれ、`analysis` で選択される。
...
- `src/Services/LlmGateway/src/LlmGateway.Api/Routing/LlmRoutingOptions.cs`: コメント更新。
