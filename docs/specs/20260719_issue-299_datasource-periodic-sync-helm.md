...
- 実装判断: [[IADR-0074]]（本 PR: Helm 配線方式・間隔根拠・多重実行の扱い）／[[IADR-0051]]（定期同期ワーカー本体・増分 watermark の既存決定）／[[IADR-0066]]（ローカル k8s dev 環境＝経路B）
...
- `datasource` は `scaling.services` に含まれ、本番 HPA は minReplicas 2 → **2 pod が同時に sync ループを回す**。
  ...
  → 単一書き手化（leader election）は [[IADR-0074]] の「却下/先送り」に記録し、フォローアップ issue を別起票。
