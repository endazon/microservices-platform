...
| **M42** | SC-11 の**ドリフト**の失敗を種別で分けず中立文言へ寄せる（[[IADR-0129]] 決定 3 に反する） | `degrades only the drift section when the drift query fails`（1 件） |
...
| --- | --- | --- |
| SC-11 のドリフトバッジ（M41） | ヘッダのバッジが**実効構成の成功ガードの外**にあり、実効構成が落ちてドリフトが成功したときに「明細が無いのに件数だけ残る」（[[IADR-0129]] 決定 5 が名指しで禁じた形） | `hides the drift and history sections …` は `config: 404` としつつ**ドリフトを成功させたまま**で、明細行と履歴の不在だけを見ていた。**ヘッダのバッジについて何も assert していなかった** |
| SC-09 の 409 の詳細（M44） | 409 を検出すると `ApiError.details`（**参照元ポリシー名**）を捨てて固定文言へ置き換えていた（[[IADR-0040]] 決定 2 の目的に反し、旧実装より情報量が減っていた） | fixture の `ApiError` が **`details` 空**で、実サーバ応答（`AuthzEndpoints` が `detail` にポリシー名を並べる）の形を再現していなかった |
...
| バンドルサイズ（633 kB / gzip 190 kB） | 全画面の再実装が終わってからのコード分割（#490 / #496 / #502 / #503 の未決事項を引き継ぐ）。**#503 の 586 kB から 47 kB 増えた**のは本 issue の 3 画面ぶんである。**［2026-08-05 追記］#512 / [[IADR-0134]] で消化**（ルート単位の遅延 ＋ `manualChunks` 3 規則。500 kB 警告は解消し、初期ロードは 632.98 → 577.54 kB / gzip 190.04 → 177.94 kB）。 |
