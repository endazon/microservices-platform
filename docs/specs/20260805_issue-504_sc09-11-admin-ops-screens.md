...
| # | 論点 | planning#197 / #198 との関係 |
| --- | --- | --- |
| 1 | SC-09 の**タグ辞書**（値集合の照会・使用件数・改名の追随） | **planning#198 提案 7 の隣接**だが**別の論点**。#198 提案 7 は「**SC-05 の利用者**が辞書を引けるか」であり、本件は「**SC-09 自身**に辞書を編集する契約が無い」 |
...
| 7 | SC-10 の**閲覧ロール**（計画=運用者・管理者／実装=管理者のみ） | **planning#198 提案 8 と同じ類型だが別の画面・逆向き**。#198 提案 8 は「計画は管理者限定だが実装が広い（SC-05/06/07）」、本件は「**計画は運用者も許すが実装が狭い**（SC-10）」。**#198 へ足すのではなく本記録で新たに問う** |
