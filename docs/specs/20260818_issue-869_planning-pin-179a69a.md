...
［2026-08-18 追記 2 / #869］IADR-0226 で環流した planning#422 が計画側で即日是正された（planning#423・pin `d5fa84b`）。pin を `d5fa84b` へ前進し、`.claude/settings.json` の `//` コメントへキット新版の**【環境条件】節 4 文**（deny と hooks が実効するのはローカル Claude Code に限る／リモートは環境側の防御／planning#422・IADR-0226 の実測／deny は環境非依存の保証ではない）を追随した。あわせて **Context7 MCP の承認を機械化**した（`enabledMcpjsonServers: ["context7"]` ＋ 読み取り専用ツール 2 種の allow。AST#530 と同型で、利用者の明示依頼による）。追随後も `check-kit-sync --require-planning` exit 0・`scripts.test.js` ✓ 666・予算 42,662B 不変。
