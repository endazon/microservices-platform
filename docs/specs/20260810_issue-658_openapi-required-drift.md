...
| 変異 | 期待 |
| --- | --- |
| M1: `ConversionJobDto.diagramsCoded` を `required` から外す | `check-openapi-dto-drift` が落ちる |
| M2: `requiredExceptions` から `UpdateDataSourceRequest.config` を消す | 落ちる（宣言が効いていることの側） |
| M3: **要求側の既定値を C# から外す**（`int TopK = 10` → `int TopK`） | **落ちる**（要求側でも「既定値が無ければ必須」は効いている＝緩めすぎていない側） |
| M4: 到達性判定を無効化（全スキーマを応答側扱い） | 5 件が再び報告される（規則が効いていることの側） |
