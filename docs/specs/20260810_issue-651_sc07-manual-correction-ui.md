...
- 契約は **#543（PR #650・[[IADR-0154]]）で揃っている**。本 issue は**画面側の作業に閉じる**。
...
| 引いたもの | 何のために |
| --- | --- |
| [[IADR-0127]] 決定 7 ＋ `sc06-datasources/DataSourceManagementPage.tsx:56-70` | **`beginOperation()` の実体**。`Object.values(actions)` で列挙し、手書き配列にしない |
| [[IADR-0127]] 決定 1 ＋ 同 `:78-95` | 権限で消す導線に**理由の文言**を残す形 |
| `sc09-admin-abac`（#640 のタグ削除） | **409 の本文を読んで件数を差し込む**形。`ApiError.body` を使う |
| [[IADR-0135]] 決定 4 ＋ `foundation/testing/bffResponse.ts` | 画面テストは `apiRequest` を差し替える。`bffFetch` が読む形をヘルパが決める |
...
| **C** | **`bffFetch` を Content-Type で分岐させる** | **採る**。[[IADR-0121]] 決定 3 が「SPA から出る HTTP はこの関数 1 箇所に収束させる」と定めた**その 1 箇所**であり、直す場所はここ以外にない（[[IADR-0141]] 参照点を 1 つに畳む） |
...
＋ [[IADR-0127]]「状態表示は契約から導出できる値だけで作る」。`deadLettered` と同じ扱いである。
...
（[[IADR-0154]] 決定 4）。画面は**その 409 を受けてから**確認を出し、`correctedFigures` を件数として
...
[[IADR-0154]] 決定 6 で **3 口とも `x-roles: [platform-admin]`**（照会 GET も運用者は 403）。
画面は導線を出さないが、**無言で消さず理由を書く**（[[IADR-0127]] 決定 1 の再変換ボタンと同じ）。
...
| 6 | 直近の操作結果だけが出る | retry 失敗 → 補正成功で**古い失敗バナーが消える**（[[IADR-0127]] 決定 7） |
...
- **デッドレターの内訳表示**（契約は #533）。**同じ画面だが別の資源**であり束ねない（[[IADR-0139]] 判定単位は資源）。
