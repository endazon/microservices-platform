...
- ユースケース（UC）: **UC-03**（文書を管理する）／**UC-04**（データソースを登録・同期する）／
  **UC-06**（文書を正規化変換する）／**UC-02**（AI 分析を依頼する）。
  issue #503 は SC-08 の対応 UC を「UC-05」と書くが、**計画（05_screens 画面一覧・03_usecases UC-02 §関連画面）は
  SC-08 を UC-02 に対応づけている**。UC-05（ABAC 権限を管理する）の関連画面は SC-09 / SC-17 / SC-10 であり
  本 issue の 4 画面に含まれない。**本書は計画を正とし UC-02 を採る**（§計画書との差異 参照）。
...
**画面が API より厳しい状態を一時的に作る。** 逆（API だけ緩い）は計画が名指しで否定しているが、
画面が先に厳しくなること自体は「画面の制御が意味を持たない」状態を作らない。#501 が API を揃えるまでの間、
運用者が API を直接叩けば再変換できる——この差は [[IADR-0127]] 決定 1 に明記し、#501 へ申し送る。
...
| 画面 | 実装する | 実装しない | モックに無いが実装する |
| --- | --- | --- | --- |
| SC-05 | 11 要素 | 4 要素（**契約の不在 2**〔変換列・機密区分の表示名〕・共通シェル 2） | 2 要素（公開/アーカイブ/削除・変更メモ） |
| SC-06 | 8 要素 | 5 要素（**契約の不在 3**〔再試行中・次回同期・設定〕・共通シェル 2） | 2 要素（登録フォームの項目・無効化） |
| SC-07 | 8 要素 | 4 要素（**契約の不在 2**〔デッドレター内訳・人手補正 2 ペイン〕・共通シェル 2） | 1 要素（SC-06 へ戻るリンク） |
| SC-08 | 8 要素 | 3 要素（**契約の不在 1**〔タグ／フォルダのチップ〕・共通シェル 2） | 2 要素（タスク種別・モデル脚注） |

**「契約の不在」は要素名で数えて 8 件**——SC-05 の変換列・機密区分の表示名／SC-06 の再試行中・次回同期・設定／
...
| 事項 | 計画の記載 | 実装 | 根拠 |
| --- | --- | --- | --- |
| **SC-08 の対応 UC** | issue #503 の表は「UC-05」 | **UC-02** として写像する | 計画 05_screens 画面一覧が SC-08 → UC-02、03_usecases UC-02 §関連画面が SC-08 を挙げる。UC-05 の関連画面は SC-09 / SC-17 / SC-10。**計画を正とする** |
| **SC-05〜07 の閲覧ロール** | 05_screens §共通シェル「SC-05/06/07 = 管理者（管理）」 | **admin または operator**（据え置き） | [[IADR-0039]]（Accepted・2026-07-08）が「データソース・変換ジョブ・文書 CRUD はいずれも運用／コンテンツ管理者の職務」として operator を含めた既存決定。**2026-08-04 の確定は「再変換の実行権限」に限られる**ため、本 issue はそこだけを狭める（[[IADR-0127]] 決定 1） |
...
| **SC-08 の機密区分の表示名** | — | 生値 | #502 と同じ（計画が 4 値中 2 値の表示名しか持たない）。本画面では機密区分を表示しない |
...
| 項目 | 引き受け先 |
| --- | --- |
| **SC-07 再変換 API の管理者ロール強制** | **#501**。**#501 が閉じるまで、画面（admin のみ）と API（admin/operator）の権限がずれる**——運用者が API を直接叩けば再変換できる（[[IADR-0127]] 決定 1） |
| 契約の不在 6 件（**要素名基準**。SC-05 の変換列／SC-06 の再試行中・次回同期・設定／SC-07 のデッドレター内訳・人手補正） | `feedback/20260805_sc05-07-admin-contract-gaps.md`。**環流の記録を作成済み・計画リポへの起票は未了（起票は親が行う）** |
...
1. **契約の不在 6 件**（環流記録）。**起票は未了**であり、実装の再開は裁定の後になる。
2. **SC-08 のタグ／フォルダのチップ・機密区分の表示名**（planning#197）。裁定待ち。
3. **画面と API の権限のずれ**（#501）。#501 の完了をもって解消する。
