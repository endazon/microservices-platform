...
- 起点 ID: **NFR**（文書統制のメタ作業）。無採番の根拠は `.claude/rules/traceability.repo.md`
  ...
  当たる番号は 1 件も無い**（[[IADR-0179]] 決定 2 / [[IADR-0188]]）。派生して `FR-14`（可変機能
  ユニットの追加＝雛形の射程）に触れる。
- 関連する実装 ADR: [[IADR-0121]] 決定 2（pnpm workspace のメンバ）／[[IADR-0203]] 決定 3
  （lint-staged の却下理由＝整形範囲の単一情報源）／[[IADR-0056]] 決定 4（旧 npm workspaces）／
  [[IADR-0060]]（雛形を CI の射程へ入れる）
- 関連 issue / PR: #802（本作業）／#777（`dca76ce`。追随の対象となった変更）／#784（雛形のずれが
  顕在化した事故）／#562（整形ゲート）／#768（[[IADR-0203]]）
...
1. `src/pnpm-workspace.yaml` のグロブが 2 → 3 本（`'../templates/*/frontend'` 追加）になったが、
   [[IADR-0121]] 決定 2（`Accepted`）は「`'*/frontend'` と `'packages/*'` を**列挙する**」の形で
   2 本を確定したままである。
2. `src/package.json` に整形・lint の 2 本目の入口（`format:templates` / `lint:templates`）が入ったが、
   [[IADR-0203]] 決定 3 は lint-staged を「**整形範囲の単一情報源を 2 本に割る**」という理由で却下して
...
- `docs/adr/IADR-0205_…:60` の正本表（整形の対象範囲＝`.prettierignore`）: `src/` 内については依然正しい。
  **例外は [[IADR-0203]] 側 1 箇所に置く**（[[IADR-0141]]: 2 箇所に置くと片方が古くなる）。
...
### A. [[IADR-0121]] 決定 2 —— 追記か Superseded か
...
3. [[IADR-0056]] 決定 4 が pnpm へ置換されたときも「置換されたのは名前と lock 形式だけ」として
...
**[[IADR-0203]] 決定 3 への日付つき追記**とする。根拠:

1. 書くべきは「lint-staged は却下・`format:templates` は許可」の**線引き**であり、却下の理由が
   置かれている場所と**同じ 1 箇所**で読めなければ意味がない（[[IADR-0141]]）。
...
着手時実測 **50,082B / 51,200B（余白 1,118B）**。[[IADR-0190]] 決定 4 の余白下限 1,000B により
**足せるのは 118B まで**。

- `:119` は**列挙をやめて正本（`pnpm-workspace.yaml`）を指す**。[[IADR-0205]] が「本文自身が
...
- [x] [[IADR-0121]] 決定 2 が 3 本目を含む（**旧条文は残置**・日付つき追記）
- [x] [[IADR-0203]] 決定 3 に例外の線引き（3 条件）が書かれ、lint-staged の却下理由と読み分けられる
- [x] `CLAUDE.md` の 2 行が追随し、**例外の存在と正本**が読める
- [x] 母集合の是正対象 14 件がすべて追随している（12 ファイル。`CLAUDE.md` と `src/README.md` は各 2 箇所）
- [x] 必読規約の余白が **1,000B 以上**（[[IADR-0190]] 決定 4）——**実績 50,130B / 51,200B・余白 1,070B**
...
- [x] コミット後に `check-doc-updated.js` / `check-commit-messages.js` が緑（[[IADR-0183]] の順序）。
...
また [[IADR-0141]]／運用ガイドの「検査器の追加は同型の事故が 2 回起きたら」に照らしても、
