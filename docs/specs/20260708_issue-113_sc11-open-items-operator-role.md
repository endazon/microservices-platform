---
title: 作業仕様書 — SC-11 未決事項の対応（運用者ロール新設・AdminOrOperator ポリシー）
type: spec
status: in-progress
related_ids:
  - FR-15
...
| # | 未決事項 | 処置 |
| --- | --- | --- |
| 1 | 運用者ロールの新設 | **本作業で対応**（下記「設計」）。IADR-0029 に決定を記録 |
| 2 | 構成情報 API の実装配置 | #112 で決定（変更なし・仕様書に明記済み） |
| 3 | バージョン履歴のデータ源・保持範囲 | #112 で決定（変更なし・仕様書に明記済み） |
| 4 | グラフのレイアウト方針 | #112 完了後の画面実装時に決定（変更なし・仕様書に明記済み） |
...
- 対象:
  1. **ロール定数・ポリシー**: `KnowledgePlatformAuthPolicies` に運用者ロール `platform-operator` と
     ポリシー `AdminOrOperator`（`platform-admin` **または** `platform-operator` を要求）を追加し、
  ...
  3. **テスト**: `AdminOrOperator` ポリシーの許可/拒否（admin ○ / operator ○ / 両方 ○ / その他ロール × / 匿名 ×）
  4. **文書更新**: SC-11 画面仕様書の未決事項 1 を解決済みに更新、`docs/security/security.md` に
     ロール・ポリシーの追記、決定の記録として `docs/adr/IADR-0029` を新設
- 対象外:
  - `AdminOrOperator` を使う実エンドポイント（#112 の構成情報 API・#113 の画面実装で使用する。
    本作業ではポリシー登録まで）
  - 既存 `AdminOnly` エンドポイント（SC-10 ダッシュボード・FR-08 一覧・FR-09 管理系）の権限変更
    （計画上それぞれ管理者向けであり、FR-15 のような「運用者」明示がない。変更は行わない — IADR-0029 参照）
...
```csharp
public const string AdminOrOperator = "AdminOrOperator";   // ポリシー名
public const string OperatorRole = "platform-operator";     // 運用者ロール
```

- `AdminOrOperator` は `RequireRole(AdminRole, OperatorRole)`（いずれか一方で許可）で登録する。
- 既存の `KeycloakRolesClaimsTransformation`（`realm_access.roles` → `ClaimTypes.Role` 展開）は
  ロール名に依存しないため変更不要。
- 命名は既存 `platform-admin` と対称の `platform-operator` とする（根拠は IADR-0029）。

### Keycloak レルム

`roles.realm` に `{ "name": "platform-operator", "description": "FR-15: 構成情報の閲覧（AdminOrOperator ポリシー）…" }` を追加。
クライアントスコープ `roles`（`realm_access.roles` 発行）は既存定義がそのまま適用される。

## 受け入れ基準

- [x] `KnowledgePlatformAuthPolicies` に `AdminOrOperator` / `OperatorRole` が定義され、ポリシーが登録される
- [x] `platform-admin` のみ・`platform-operator` のみ・両方保持のいずれでも `AdminOrOperator` を通過する
- [x] どちらのロールも持たない認証済みユーザー・匿名は `AdminOrOperator` を通過しない（fail-closed）
- [x] Keycloak レルム定義に `platform-operator` が追加されている
- [x] SC-11 画面仕様書の未決事項 1 が解決済みとして更新され、IADR-0029 が作成されている
...
- `AuthorizationService.Api.Tests` に `AdminOrOperator` ポリシーの単体テストを追加する
