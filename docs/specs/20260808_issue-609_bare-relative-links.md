...
- **発見の経緯**: PR #607（[[IADR-0130]] の追補）の作業中。§関連 へ足したリンクの**ファイル名を間違えた**
...
- 分類（[[IADR-0141]] 決定 4）: **「機械検査を新設・改修する」** —— クロス監査は**フェーズ末に 1 回**
...
**これは [[IADR-0141]] の「走査式が返さなかった」は「存在しない」を意味しない、の実例である。**
...
| **frontmatter の ID リスト**（`related_specs` 等） | ✅ | 同じ `isBrokenRef` を通る。`docs/adr/` の `related_specs` は**実際にベア名で書かれている**（例 [[IADR-0141]]） |
...
「何を相対リンクと見なすか」の参照点が 2 つに割れる（[[IADR-0141]]「参照点を 1 つに畳む」）。
