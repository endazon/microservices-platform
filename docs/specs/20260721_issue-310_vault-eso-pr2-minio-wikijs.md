...
- ADR: ADR-0006（運用基盤）。ESO 基盤・k8s auth・段階移行は [[IADR-0096]]（PR-1）。opt-in オーバーレイ統括は [[IADR-0077]]。
- 決定: 本作業の設計判断は [[IADR-0097]]（PR-2 対象 secret の ExternalSecret 化・PR-1 設計踏襲）。
- Issue: #310（Vault/ESO 本番同等化）。**stacked PR**（#368/PR-1 のブランチに積む）。

## 背景と問題

[[IADR-0096]]（PR-1）で ESO 基盤（k8s auth・store 上書き・policy `eso-read`・seed/skip）を敷き、`llm-provider-credentials`
