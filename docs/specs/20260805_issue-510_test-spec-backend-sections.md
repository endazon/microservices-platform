...
### 粒度: テストクラス（`*Tests.cs` のファイル名）× `docs/tests/` 全体
...
- **テストメソッド単位では細かすぎる。** 表の 1 行を消しただけで赤くなり、仕様書の正当な要約
  （「主要ケースのみ挙げる」）を禁じてしまう。
- したがって**クラス（ファイル）単位**を採る。節が落ちればクラス名がどこからも参照されなくなるため
  検出でき、表の行の増減では赤くならない。

### 判定: ratchet（`documented` を床として持つ）

本リポジトリの既存ゲート（写像検査・カバレッジ床・ライブラリ標準・契約互換）と同じ 3 判定にする。

| 事象 | 判定 |
| --- | --- |
| baseline にあるのに `docs/tests/` から消えた（**テストは実在する**） | **fail**（＝今回の欠陥） |
| baseline にあるがテストごと消えた | **fail**（baseline から削除させる。仕様書側の記載も見直させる） |
| `docs/tests/` に載ったのに baseline に無い | **fail**（床を上げっぱなしにさせる。`--update` で更新） |
| 実在するが載っておらず baseline にも無い | **warn**（新規テストの多くは基盤・回帰であり、載せる義務は負わせない） |

**fail-closed**: 走査したテストクラスが 0 件、`docs/tests/` の Markdown が 0 件、baseline が
読めない／壊れている、のいずれも **fail** にする。「見つからないから素通り」は本検査が塞ごうと
している穴と同型である。
...
- **変異試験**: 復帰した節を 1 つ消すと新規検査器が fail すること（実測して報告に貼る）
