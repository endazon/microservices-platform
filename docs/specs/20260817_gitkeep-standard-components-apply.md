...
**本作業は [[IADR-0219]] 決定 3 が「適用は次の波で行う」と明記したものの実行である。**
判断は [[IADR-0218]] 決定 3-2〜3-7 と [[IADR-0219]] 決定 1〜3 に尽きており、**本書で新たな判断はしない。**
...
| 要素 | 実体 | `.gitkeep` | 根拠 |
| --- | ---: | ---: | --- |
| `Api` / `Worker`（**排他**） | 11 / 11 | **0** | [[IADR-0219]] 決定 2。**実行入口は 1 サービスに 1 つ**であり「空の実行入口」は存在しない |
| `Application` | 0 / 11 | 11 | |
| `Domain` | 0 / 11 | 11 | |
| `Infrastructure` | 0 / 11 | 11 | |
| `Contracts` | 0 / 11 | 11 | |
| `SharedKernel` | 0 / 11 | **11** | [[IADR-0219]] 決定 1 により新たに対象 |
...
（[[IADR-0218]] 決定 3-6 の「適用対象 0 件」は [[IADR-0219]] で覆った）。

### 3.5 黙って除外していないもの（規則 6）

| 除外 | 理由 |
| --- | --- |
| **`src/ai-stock-trading/`** | **別プロジェクトの submodule。向こうの issue である**（[[IADR-0218]] 決定 3-7） |
...
1. **55 件の `.gitkeep` を作る。** パスは `src/<unit>/backend/Services/<Name>/src/<Name>.<要素>/.gitkeep`
   （フォルダ名 `<Name>.<要素>` は [[IADR-0218]] 決定 3-3 のまま）
2. **雛形へ 1 件**: `templates/unit-template/backend/Services/SampleService/src/SampleService.SharedKernel/.gitkeep`
3. **`.gitkeep` は空ファイルとする**（[[IADR-0218]] 決定 3-4。計画は「`.gitkeep` だけを置く」と書いている）
4. **`src/README.md` へ注記を足す**（[[IADR-0218]] 決定 3-5）。**2 つの読み替えを書く**:
...
- **`src/README.md` の L50 / L66 は追随不要** —— 既に `<Api|Worker>` の**排他の形**で書かれている
  （`tests/<ServiceName>.<Api|Worker>.Tests/` 等）。[[IADR-0219]] 決定 2 と矛盾しない
