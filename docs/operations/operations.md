...
- **WikiService と Wiki.js**（FR-13 / UC-07 / [IADR-0020](../adr/IADR-0020_wiki-js-deployment-abac-gateway.md)、
  ...
  - **ネットワーク分離**: Wiki.js への ABAC は WikiService ゲートウェイに集約するため、共有/stg/prod では
    ...
    NetworkPolicy）。**dev の compose も既定では Wiki.js を host 公開しない**（[IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md)・#124）。
    Wiki.js 管理 UI へ直接アクセスする dev 作業時のみ、opt-in override を重ねて 3001 を公開する:
    `docker compose -f deploy/docker-compose.yml -f deploy/docker-compose.wiki-direct.yml up -d`。
    公開の混入は `NetworkIsolationTests`（既定 compose 非公開・Helm `wikijs.ingress.enabled: false`）が回帰ガードする。
...
- **管理 UI への直接アクセス（dev のみ・opt-in）**: 下記の初期セットアップ（OIDC 構成・ja ロケール導入・
  API キー発行）はブラウザから Wiki.js 管理 UI（`http://localhost:3001`）へ直接アクセスする必要がある。
  既定 compose は Wiki.js を host 公開しない（[IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md)・#124）ため、
  **override を重ねて起動する**: `docker compose -f deploy/docker-compose.yml -f deploy/docker-compose.wiki-direct.yml up -d`。
  セットアップ完了後は override 無しで再起動し、ABAC ゲートウェイ（WikiService）経由の到達に戻す。
