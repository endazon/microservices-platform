...
- **構成**: env `DepartmentAttributeSync__Mode`（`Off` / `Report` / `Fix`）と `DepartmentAttributeSync__Interval`（既定 `01:00:00`）を
  ...
  🔴 **周期は `hh:mm:ss` 形式だけを受け付け、下限は `00:01:00`**。`60` のような数字だけの値は起動時に落ちる
  （.NET の既定の解釈では 60 日になるため受け付けない）。
...
- **失敗の見え方**: 1 人の書き込みが失敗しても周期は止まらず、他の人は直る。失敗した人は IdP 内部 ID つきでエラーログに出て、
  ...
  `department_sync.cycles.total`）が増える。**0 でない失敗が続くならアラートの対象**である。
...
同スクリプトは 28 項目すべてを再投入し、env で値を渡さなかった項目を**既定値で上書きする** ——
稼働中の認証基盤・データベース・ブローカの資格情報がその既定値に置き換わる。
2026-09-10 の稼働作業でこの危険が判明し、**手順書に無い回避策**がその場で判断された。

**投入してよい項目の集合は `deploy/bootstrap/sc22-secret-items.json` が持つ**（`items[]` だけが対象。
`deferred[]` は認証基盤の宣言と対で書く必要があり、`excluded[]` は稼働中のデータストアを壊すため恒久的に対象外）。

🔴 **コンソール操作は監査ログに乗らない。** Runbook の「記録」節に従い、**使った事実を必ず残すこと。**

**秘密情報を新しい値へ回す（ローテーション）手順**は別の Runbook
[`secret-rotation-runbook.md`](secret-rotation-runbook.md) が定める。上の退避手段は「差し替え」であって回転ではない ——
回転は分類ごとに扱いが違い（画面から回せる `items[]`／ストア側と同時に回す `excluded[]`／いまは回せない `deferred[]`）、
