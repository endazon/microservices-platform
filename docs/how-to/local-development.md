...
| Keycloak | http://localhost:8080 | realm `knowledge-platform` を import 済み |
...
| Wiki.js の OIDC ログインが `Failed to fetch user profile` | Issuer は `http://localhost:8080/realms/knowledge-platform`（ブラウザ経路）で設定する。`keycloak:8080` を指定すると失敗する（`docs/operations/operations.md` 実測記録） |
