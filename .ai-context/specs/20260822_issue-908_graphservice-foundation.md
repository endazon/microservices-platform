...
## カバレッジ床への寄与を実測した経緯（測ってから直した順序ごと残す）

**当初は「テストを足す方向だから床を割る向きではないはず」と考えていた。これは測っていない推測だった。**
実測すると**逆だった**。

`#907`（レポート跨ぎの重複排除に合わせた床の置き直し）が develop へ着地して床が
`line 88 / branch 68` になった時点で、ローカルで A/B を測った（本作業のテストプロジェクトの
レポートを外した 16 件 と、入れた 17 件）。

| | line | branch |
| --- | --- | --- |
| 本作業なし（16 件） | 87.55% | 66.62% |
| 本作業あり（17 件） | 87.58% | **66.19%** |

**床割れ自体は本作業が原因ではない**（16 件の時点で既に割れている。ローカルは Testcontainers を
使う統合テスト 30 件を skip するため）。しかし**差分を取ると branch を 0.34pt 押し下げていた**。
増分が分岐 76 本中 43 本被覆＝**56.6%** で、全体平均（66%）を下回っていたからである。
CI 側の余裕は branch 0.66pt しかないため、半分を消費する挙動だった。

### 原因を追った先で見つかったもの

**仕様書と IADR で契約として明記したのに、テストが 1 件も無い不変条件があった。**

- `GraphDocument.TryApply` の**順序ガード**（IADR 決定 12-4）——「古いイベントを適用しない」。
  緩むと、厳格化イベントの後に遅れて届いた緩和イベントが権限を巻き戻し、
  **権限外文書が見えるようになる**。消費側は #911 だが、**契約は本作業が定義したもの**であり、
  使われる前に契約だけが宙に浮いていた
- `AuthorizedGraphView.Seal` の意味論（許可なし／迂回した未フィルタ／**片端が見えない辺**）
- `EfGraphStore` の空入力と、接続辺が**双方向**に返ること（バックリンクの前提）
- `Edge.OtherEnd`、出所・層の値域
...
| --- | --- | --- |
| 本作業なし（16 件） | 87.55% | 66.62% |
| **本作業あり（17 件）** | **88.00%** | **67.22%** |

増分だけ見ると line 364/373 = **97.6%** / branch 64/76 = **84.2%** となり、**−0.34pt が +0.60pt へ反転**した。

🔴 **これはカバレッジ稼ぎではなく、文書に書いた契約の裏取りである。** 順序が逆——「床を測ったら
押し下げていた → 原因を追ったら未テストの契約が見つかった → 塞いだら反転した」——であったことを
含めて記録する。**測らなければ、契約が宙に浮いたまま出荷していた。**

**`src/coverage-floor.json` は本作業では触っていない。** ローカルの絶対値が床に届かないのは
統合テストの skip が原因であり、確定値は `integration.yml` の実行から読む。
