...
type: spec
status: draft
related_ids: [FR-17, UC-10, ADR-0033, ADR-0034, ADR-0004, ADR-0036, IADR-0240]
author: claude
created: 2026-08-22
updated: 2026-08-22
plan_refs:
...
- 実装 ADR: **IADR-0240**（暫定番号。マージ直前に develop の最大＋1 へ付け替える）
...
詳細（列・索引・制約・トレードオフ）は **データ仕様書 `docs/data/knowledge-graph.md`** と **IADR-0240** に置く。本単位で作るのは `graph_documents` / `edge_types` / `edges` の 3 表である（`ai_suggestions` は #914）。
