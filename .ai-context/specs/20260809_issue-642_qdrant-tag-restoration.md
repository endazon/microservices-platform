...
- 制約: [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md)（Qdrant ペイロード表現。**書き込み・フィルタ・復元を一致させる**）
- 同型の先例: [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) が名指しした「**テストは緑・本番は空**」——
...
| 既存データの移行 | ❌ | [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) と同じ扱い —— 再取込まで旧データは `tags` を持たず、**空欄で出る**（欠落方向であり誤表示ではない） |

## 原因

`MapPayload` は `Attributes` を [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) で復元するようになったが、**`Tags` は空リテラルのまま残った**。
...
**本番（Qdrant）とテスト（InMemory）で通る面が違う** —— [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) が記録した欠陥と同型である。
...
この口を使い始めた瞬間に同じ欠陥が再発する。[IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) の「書き込み・フィルタ・復元を一致させる」も
実装単位に掛かる要求である。**射程外ではなく、射程の性格が「是正」ではなく「予防」だというだけである。**

> **記録として残す理由**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 の「他人の数えを検証せず転記しない」の裏返し）:
...
**既存の設計判断を変えていない。** [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) が「書き込み・フィルタ・復元の表現を一致させる」と決めており、
...
- **既存データ（本修正より前に Retrieval 側の `UpsertAsync` で書かれたチャンク）は `tags` を持たない。**
  [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) の属性と同じく、**再取込（`DocumentUpdated` 再発行）まで空欄で出る**。
