...
- 関連 ADR: ADR-0004（認証＝Keycloak）。方式判断は [IADR-0086](../adr/IADR-0086_oidc-issuer-metadata-split.md)。前提は [IADR-0076](../adr/IADR-0076_edge-bff-routing-and-oidc-hostname.md)（決定3＝ブラウザ OIDC
...
[IADR-0076](../adr/IADR-0076_edge-bff-routing-and-oidc-hostname.md) 決定3 は、ブラウザが受け取る token の `iss` と、サービス側の検証基準（`Auth__Authority`）が
...
6. 設計判断（分離方式・issuer 検証を弱めない根拠・セキュリティ含意）を [IADR-0086](../adr/IADR-0086_oidc-issuer-metadata-split.md) に記録。ADR 索引 README は
   自分の 1 行のみ追記。
