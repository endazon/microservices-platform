...
related_specs:
  ...
  - "../../docs/functional/FR-01_data-source-catalog.md"
  - "../../docs/data/data-source.md"
...
本作業は **[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 が「束ねる」と判定した 2 件**（#534 ＋ #537）を 1 PR で実装する。
束ねる根拠と条件の当て直しは §束の判定 に置く。**issue ごとの節**（§#534 / §#537）を必ず読むこと
（[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 6-3）。
...
- 関連 ADR:
  ...
  - 実装: [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（BFF とロールゲート）／[IADR-0051](../adr/IADR-0051_datasource-connector-port-and-filesystem.md)（コネクタポートと同期オーケストレーション）／
    [IADR-0053](../adr/IADR-0053_wiki-connector-generic-rest-contract.md)（応答での秘密マスク）／[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md)（契約スキーマの後方互換ゲート）／
    [IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2（**契約から導出できる値だけを表示する**。琥珀の予約）／
    [IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md)（`NextSyncAt` は共通間隔）／**[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md)（本作業で新設）**
- 計画書リンク:
  `05_screens/01_screens.md` §SC-06（計画リポ）
  （2026-08-05 の確定「同期健全性・次回同期・更新 API」。利用者裁定〔質問票 第12回 Q14〜Q16〕・planning#198）

## 束の判定（[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 1 の 6 条件を着手直前に当て直した）

[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 の注記は「実効表は 2026-08-07 時点のスナップショットであり、**束を組む直前に
...
| **C. 非破壊側に収まる** | ✅ | `DataSourceDto` への追加はすべて**既定値つきの省略可能パラメータ**（[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2 の非破壊側）。既存の位置パラメータの順序・型は変えない。新エンドポイントは追加のみ |
...
| **F. 契約の追加に閉じる** | ✅（判断を記録する） | 再索引・取り込み側（インデクサ / ワーカー）の変更・外部システムの配備を**伴わない**。**EF の追加マイグレーション 1 本を伴う**が、これは**既存データの移行ではない**（すべて null 許容または既定値 0 の列追加で、バックフィルを行わない）。[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 条件 F が明示的に除外するのは「既存データの**移行**・**再索引**」であり、**非破壊な列追加はエンドポイントの追加と同じく「契約の追加」の側にある**と判断した。根拠と代替案の棄却理由は [IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md) に置く |
...
**代替の担保**: [IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 3 は「本リポジトリはスカッシュのみで**コミット境界は develop に
...
## 母集合（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1・走査基準 `ae66549`）
...
- **対象**: `DataSourceDto` への健全性 3 項目の追加／`DataSource` エンティティへの健全性の永続化と
  ...
  （DataSourceService ＋ BFF）／SC-06 の琥珀表示の実装（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2 の予約の解除）／
...
- **対象外**:
  - **`POST` / `DELETE` の認可を管理者限定へ狭めること**。計画 §SC-06 は「登録・更新・無効化は管理者限定」と
    ...
    本作業で一緒に直すと束の射程を越える（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4「PR ではなく issue を分割する」）。
    **新設する `PUT` / `PATCH` は計画どおり管理者限定で作る**（新しい口を計画に反していない状態で生む）。
    既存 2 口の差異は**別 issue として起票する**（§計画書との差異）。
  - 健全性のプロセス横断集約（複数インスタンス間の合算）。**単一書き手**は
    [IADR-0083](../adr/IADR-0083_datasource-sync-single-writer-advisory-lock.md) のアドバイザリロックで担保されており、本作業は永続化までで足りる（[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md)）。
...
#### 境界の確定 —— **[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 条件 B の前提**

[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 は「**契約（`DataSourceDto` の健全性項目）とアラート発報の境界を着手前に
確定できなければ条件 B を満たさない**」と条件付けている。**実測して確定した。**

**アラート発報は既に実装されている。** `DataSourceSyncService.AlertOnFailure`（`:110-121`）が
連続失敗を記録し、しきい値以上で**構造化ログ（`Alert=true`）**を出す。これは [IADR-0051](../adr/IADR-0051_datasource-connector-port-and-filesystem.md) 決定 3a が
...
- **`RetryLimit`**: 再試行上限（＝継続失敗のしきい値）。**画面が「3/5」の分母を契約から得るために返す**
  （画面へ定数を複写すると [IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2 が禁じた「契約から導出できない表示」に戻る）。
...
**[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md)** に記録する（要旨: プロセスローカルな計数は再起動で消え、読み取りがどのインスタンスへ
...
| 口 | 意味 | 認可 |
| --- | --- | --- |
| `PUT /datasources/{id}` | **全置換**。`Name` / `SourceType` / `ConnectionUri` / `Config` / `DefaultAttributes` を要求どおりに置く。**`Config` / `DefaultAttributes` の省略は 400**（消すなら `{}` を明示。[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md) 決定 6-b） | 管理者限定 |
...
- **`DefaultAttributes` は機密区分のフェイルセーフを必ず通す**（`WithConfidentialityFailsafe`）。
  更新で `confidentiality` を空にできると、fail-closed 検索（[IADR-0012](../adr/IADR-0012_retrieval-search-fail-closed-scope.md)）から文書が落ちる。
...
**変異試験**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 4）: 本作業は**機械検査を新設しない**ため必須ではない。ただし
...
- **差異: あり（1 件・本作業の対象外として別issue化する）**
  ...
  `PUT` / `PATCH` を計画どおり管理者限定で作り、既存 2 口の差異には触れない**（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4）。
...
| `contract-schema-baseline.json` | — | 契約の追加 7 件（**すべて非破壊**）。[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2 の破壊的側には該当しない |
...
| 指摘 | 自分での検証 | 対応 |
| --- | --- | --- |
| 🟡 **`PUT` / `PATCH` の `config` は全置換なので、応答のマスク値（`***`）を書き戻すと秘密が壊れる** | **実測で再現した。** `Update_WritingBackMaskedSecret_KeepsStoredValue` を先に書いたところ `PUT` / `PATCH` の**両方で fail** した（`apiToken` が `"***"` として永続化される） | **是正した。** `SecretConfigMask` を新設し、**マスク値は保存せず既存値を保つ**（[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md) 決定 6）。**繰り延べにしなかった理由**: 読んで書き戻すのは API の最も普通の使い方であり、「将来の編集フォームが踏む」ではなく**いま口を直接叩く運用者が踏む**。#534 が新設した口の欠陥なので本 PR の射程内である |
| 🟢 **差異の是正 issue の番号が本文に無い**（レビュー環境では issue 検索が不可） | 起票は済んでいた（レビュー実行時点では本文へ未反映） | **#628** を PR 本文・本仕様書・画面仕様書 §未決事項 5 へ明記した |
| 🟡（2 巡目）**`PUT` で `config` を省略すると秘密が黙って消える** | **実測で再現した**（`config` / `defaultAttributes` とも 200 で通り、`apiToken` が消える） | **是正した。省略を 400 で拒否する**（[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md) 決定 6-b）。**決定 6 の裏側に穴が残っていた** —— 「送り返したマスク値」は塞いだが「**送らなかった項目**」は塞いでいなかった。**同じ往復事故の逆方向**である |

> **★ 決定 6 を入れた時点で、この逆方向を自分で探すべきだった。** 「読んで書き戻す」経路を塞ぐなら、
> **書き戻す値が「在る」場合と「無い」場合の両方**を見るのが筋である。片方だけ塞いで「直した」と
> 結論した —— `.claude/rules/traceability.md` 型 7「**禁止を書いたら、その逆向きも書けるか考える**」
> （落とす／足す・広げる／狭める）が、**そのまま当てはまる形**だった。
