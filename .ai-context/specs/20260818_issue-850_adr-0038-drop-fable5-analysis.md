...
- 実装 ADR: [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（`analysis` → `claude-fable-5` と `NonZdrModels` の導入）、
  [IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md)（報告書の種別別割当・`NonZdrModels` は変更しないとした決定 2）、
  [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md)（月報のみ ZDR 対応の最上位へ改定）。
...
- **`EgressMatrix.cs`（1 件）**: 「ZDR 非対応モデル（例: `claude-fable-5`）を候補から除外する」という
  **機構の説明の例示**である。機構は `NonZdrModels` に載る任意のモデルを対象とし、`claude-fable-5` は
  ZDR 非対応であるという契約事実（[IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) 決定 2）が変わったわけではない。よって**例示は誤りにならない**。
...
1. **凍結の射程は記録種ごとに定まっており、対象は `docs/specs/`（確定済み）・`feedback/`・`docs/superpowers/` である**
   （[IADR-0166](../adr/IADR-0166_status-vocabulary-and-record-rewrite-boundary.md) 決定 2 の 2026-08-17 追記 / `.claude/rules/traceability.repo.md`）。
...
3. **直前例がある。** 同型の改定（[IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md)・月報の割当変更）を入れた `404b1c3e`
...
- `ReportPurposeModels_AreNotListedAsNonZdr` → **`PurposeModels_AreNotListedAsNonZdr` へ改名し、
  ...
  射程を `report-*` に絞っていたのは [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) 決定 4 の「`analysis` は ZDR 非要件区分限定の
  意図的な例外なので対象に含めない」という前提による。**計画 `ADR-0038` 決定 2 でその例外が消滅した以上、
  絞る理由も消えた** —— 絞ったままだと `report-*` 以外の用途に非 ZDR モデルを割り当てる再発を捕まえられない。
  射程が覆った旨は [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) §決定 の同日追記に記録した（**条文と実装を食い違わせたまま広げない**）。
  **広げたことが効いていることは変異試験で実測する**（§5.2）。
- `LlmRouterTests.cs`: **合成 config は 1 件も削らない**（§3.2）。注記のみ足す。

### 4.3 実装 ADR

[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md) と [IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) へ `［2026-08-18 追記 / #850］` の追記ブロックを置き、`updated:` を前進させる。
...
- **［2026-08-18 追記 / #850・解決済み］[IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) へ追記すべきか**: **すべきであり、本 PR で追記した。**
...
- 決定 3・4・6（フォールバック機構）の実装 issue。**新規 IADR の採番が要る見込みであり、
  ...
  - **［2026-08-18 追記 / #863・解決済み］issue は #863 として起票され、本追記の時点で実装された。**
    採番した実装 ADR は [IADR-0225](../adr/IADR-0225_llm-purpose-fallback-chain-and-429-boundary.md)（用途別フォールバック順序・429 の境界・発火の可観測化）である。
