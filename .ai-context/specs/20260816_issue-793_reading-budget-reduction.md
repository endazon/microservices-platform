...
- 起点 ID: **NFR**（必読規約の減量・キット同期＝文書統制のメタ作業）。
  **無採番の根拠は `.claude/rules/traceability.md`「無採番 `NFR` を許す 2 つの場合」の場合 2**
  ...
  （別紙 [`plan-id-range-history-annex.md`](../../docs/how-to/plan-id-range-history-annex.md) §4 の実測）。
  **場合 2 は環流しない**（[IADR-0179](../adr/IADR-0179_unnumbered-nfr-for-meta-work.md) 決定 2）。着手前に計画側の ID 列を実際に見て判断した。
- 起点 issue: **#793**（#790 が置いた「期限つき保留」の期限）
- 実装ADR: [IADR-0205](../adr/IADR-0205_reading-budget-reduction-for-kit-catchup.md)

## 2. 母集合を自分で引く（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1）
...
- 余白下限 = **1,000B**（[IADR-0190](../adr/IADR-0190_permanent-headroom-by-annexing-examples.md) 決定 4 のラチェット。`scripts.repo.test.js` の
...
**軸を 1 本で終わらせない**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 の規則 5）。
...
| 2 | L46 束ねの条件の詳細（6 条件・判定の単位・上限・射程） | (b) 複写 | 正本は**同リポの [IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1 / [IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 1**。条件の中身は入口が持たない |
...
| 5 | L51 人間の関与 3 点（225B） | (b) 複写 | 正本は運用ガイド。**AI が行動する規範ではない**（読まなくても成果物は壊れない。[IADR-0173](../adr/IADR-0173_annex-extraction-keeps-heading-stubs.md) 決定 2 の反対側） |
...
| 8 | L68–70 「ここへ複写しない」の説明 3 行 | (b) 複写 | 規範（複写しない）は残し、**理由の説明**を [IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) へ寄せる |
| 9 | L143 認証の移行進捗（第 3 段 / #439） | (c) 腐る | 正本は [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 6（**段の進捗は同 IADR が正本**と L136 が既に宣言している） |
| 10 | L145 整形の対象範囲の列挙 | (b) 複写 | **本文自身が「対象範囲の単一情報源は `src/.prettierignore`」と言いながら列挙を複写している** |
| 11 | L139 UI/CSS・L140 i18n の説明部 | (b) 複写 | 正本は [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) / [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) と `src/packages/ui/README.md`。**禁止・強制の規範はすべて残す** |
...
**黙って除外しない**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 の規則 6）。

| 候補 | 除外理由 |
| --- | --- |
| `### TypeScript / React` の禁止・強制（ESLint / 検査器が止める旨） | **規範である。** 別紙・正本へ出すと「読まなかっただけで CI に落ちる」（[IADR-0173](../adr/IADR-0173_annex-extraction-keeps-heading-stubs.md) 決定 2） |
...
| 新しい別紙（`docs/how-to/*-annex.md`）の作成 | **今回は 1 本も作らない。** 削る対象はすべて**正本が既に在る複写**であり、別紙を作ると 3 箇所目になる（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) / [IADR-0178](../adr/IADR-0178_claude-md-defers-to-docs-readme.md) 決定 1）。**畳み先はすべて既存の正本** |
...
- **消さない。** 消すと、将来また保留（分類 B へ戻す）したときに**根拠を検査する装置ごと失われる**。
  [IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 1 が定めた「保留の根拠そのものを機械が検査する」という規律は、
...
- 主張:
  ...
  2. キットの `traceability.md` は本リポと同じ手法（[IADR-0173](../adr/IADR-0173_annex-extraction-keeps-heading-stubs.md) の別紙化・[IADR-0178](../adr/IADR-0178_claude-md-defers-to-docs-readme.md) の
...
| 3 | companion `:24` の凍結の射程が旧いまま（PR #800 / 裁定 planning#369 に未追随） | **`feedback/` についてだけ**射程①への限定と②③（トリアージ結果・裁定の記録／元を消さない自己是正）の「残す」を書いた。**[IADR-0191](../adr/IADR-0191_rewrite-boundary-is-body-vs-frontmatter.md) の 2026-08-16 追記は明文として `feedback/` についてのみ**であり、`docs/specs/` へ及ぶとは書いていない。**したがって `docs/specs/`・`docs/superpowers/` は無条件の「書き換えない」を維持し、両者を区別して書いた**（射程を勝手に広げない）。`docs/specs/` 側は別途の裁定待ちとして射程外 |
...
- `AGENTS.md`（別枠。予算の合算をしない。[IADR-0200](../adr/IADR-0200_reading-budget-population-per-agent.md)）
