...
- 起点 issue: [#379](https://github.com/endazon/microservices-platform/issues/379)
  （`fix(ADR-0025,IADR-0101)`・label `bug` / `priority:should`）。[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) §フォローアップ 3 の消化。
- 計画根拠: ADR-0025（計画リポ）
...
- 設計: ADR-0010（計画リポ）（LLM ゲートウェイ）、
  [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（ゲートウェイ経路）、[IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)（SSE ストリーミング）。
- 本作業の実装判断は [IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md)。
...
同じ「空文字へ縮退」は `stop_reason: "max_tokens"`（thinking が上限を食い切った場合。[IADR-0101](../adr/IADR-0101_default-model-opus-5.md)）
...
- ストリーミング: `res.Delta?.StopReason`（`message_delta`）を保持し、最終チャンクに載せる。
  ...
  （[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) にトレードオフを記録）。
...
- **本文破棄は `refusal` のみ**に限る。`max_tokens` の部分本文は正当な途中結果であり破棄しない
  （破棄すると [IADR-0101](../adr/IADR-0101_default-model-opus-5.md) が想定した「切れた本文」の観測ができなくなる）。
...
- モデル変更・`thinking` パラメータの送信（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) で不採用）。
