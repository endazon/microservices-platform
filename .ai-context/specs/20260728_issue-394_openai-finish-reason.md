...
- 起点 issue: [#394](https://github.com/endazon/microservices-platform/issues/394)（`enhancement`）。
  [IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 2「`SelfHostedProvider` / `CopilotProvider` の終了理由」の消化
  （起点は #379 / PR #391）。
...
- 設計: ADR-0010（計画リポ）（LLM ゲートウェイ・本文凍結）、
  ADR-0025（計画リポ）（`stop_reason` 確認の要求）、
  [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（セルフホスト＝ティアA／Copilot＝ティアC の経路）、[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md)（`stopReason` の契約と refusal の本文破棄）、
  [IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)（SSE ストリーミング）。
- 本作業の実装判断は [IADR-0109](../adr/IADR-0109_openai-finish-reason-normalization.md)。

## 背景と問題（原因の確定）

[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md)（PR #391）は共有契約とポートへ `StopReason` を追加したが、**写像を実装したのは
...
とくに `content_filter`（OpenAI の安全性フィルタ停止）は、[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) が `refusal` について定めた
**本文破棄の fail-safe が効かない**まま断片が下流へ流れる。

### 現状の到達可能性

両エンドポイントは既定 `Enabled=false`（[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)）だが、`values` / `appsettings` の設定 1 つで
...
- **`ClaudeProvider`**（[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) で対応済み。無変更）。
...
- **`StreamAsync` の個別実装**。両プロバイダは `ILlmProvider` の既定実装（`CompleteAsync` を単一チャンクへ
  縮退）を使うため、`StopReason` は最終チャンクへ**自動的に載る**（[IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)）。個別実装は追加しない。
- #395（拒否率の可観測化）・#403（縮退応答のモデル名）。別 issue・別 PR。

## 決定（要約。詳細は [IADR-0109](../adr/IADR-0109_openai-finish-reason-normalization.md)）
...
[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) の決定に従い本文も破棄する。**
...
| T-20f | `content_filter` の本文破棄 | 本文が空になる（[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) の refusal と一貫） |
| T-20g | `length` の本文保持 | 途中結果を破棄しない（[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) と一貫） |
