...
- ユースケース（UC）: なし
- 画面（SC）: なし
- 関連 ADR: [IADR-0067](../adr/IADR-0067_service-image-build-ci-gate.md)（compose を単一情報源にしたイメージビルド CI ゲート）／
  [IADR-0068](../adr/IADR-0068_image-mapping-drift-check.md)（`k8s-local-images.sh` の MAPPING と compose のドリフト検査）／
  [IADR-0070](../adr/IADR-0070_ast-frontend-integration.md)（AST 共有 Dockerfile を context/args 対応で deploy 面へ載せる）／
  [IADR-0071](../adr/IADR-0071_ast-risk-controls-bff-integration.md)／[IADR-0072](../adr/IADR-0072_ast-monitor-bff-integration.md)（同型で risk-management / market-monitor を登録）
