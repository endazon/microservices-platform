...
- 関連 ADR（計画）:
  ...
  **ルーティング = TanStack Router を確定**。§理由 が「ルート・検索パラメータ（`/search?q=` 等）まで
  型安全にできる」を採用根拠に挙げる）／
  ADR-0032（計画リポ）（認証は第 3 段）
- 関連する技術検討・画面設計（計画）:
  13_frontend-stack（計画リポ）（fixed。採用技術一覧が正）／
  01_screens（計画リポ）（**§共通シェル の
...
> 経緯・原文・完了条件は
> feedback/20260804 §追加裁定（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260804_frontend-migration-staging-interpretation.md` へ移設） を正とする。
...
> （01_screens §共通シェル ［2026-08-04 確定］（計画リポ）。
...
- **左ナビ 4 グループ**（01_screens §共通シェル）: 利用者 / 個人 / 管理 / 運用。項目が 0 件の
  ...
  - > **［2026-08-04 追記・是正済み］計画が「総称としての『その他』は使わない」と確定した**
    > （01_screens §共通シェル ［2026-08-04 確定］（計画リポ）。planning#185）。
...
| 旧 13 画面 | 13_frontend-stack「旧画面は**完全に削除する**」（主語は**画面**である） | `home` のみ削除。SC-01〜11 は**ルート定義を書き直し、Page は残した**（＝本 PR 単独では未達） | **削除・再実装は同一段内の #452 に割り当てられている**。feedback/20260804（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260804_frontend-migration-staging-interpretation.md` へ移設） §完了条件（status: accepted）は「`react-router-dom` がワークスペースから消えている（第 2 段）」と「旧 13 画面が削除され再実装されている（第 2 段 / **#452**）」を**別条件として並置**しており、[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 1 も第 2 段を #452 と同一段に置いている。したがって本 PR での Page 残置は**放棄ではなく繰り延べ**であり、#452 が SC-01〜11 の Page を作り直した時点で条件が満たされる。`home` は計画の画面一覧（SC-01〜21）に対応画面が無いため本 PR で削除した |
...
**「最終的に結果が同じになるなら進め方はそれでもいいです」**。
feedback/20260804 §追加裁定（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260804_frontend-migration-staging-interpretation.md` へ移設））。
...
| 項目 | 内容 |
| --- | --- |
| **旧 13 画面の削除・再実装** | SC-01〜11 の Page（実装 2792 行）は本 PR ではルーティング部分のみ書き換え、内部は現行挙動を維持した。**#452 が Page を作り直すまで、13_frontend-stack「旧画面は完全に削除する」は未達である**（feedback/20260804（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260804_frontend-migration-staging-interpretation.md` へ移設） §完了条件 が「旧 13 画面の削除・再実装」を #452 に割り当てている） |
| 画面内容の計画準拠 再設計 | 05_screens（計画リポ） と `05_screens/mockups/`（hi-fi 正）への準拠。レイアウト・項目・文言・警告色（琥珀）等 |
...
- **起点 ID**: ADR-0031（計画 ADR。13_frontend-stack（計画リポ） §採用技術一覧が正）
...
5. **バンドルサイズ。** `index.js` が 537 kB（gzip 158 kB）で Vite の 500 kB 警告に触れる。
   ...
   **［2026-08-05 追記］#512 / [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) で消化**（ルート単位の遅延 ＋ `manualChunks` 3 規則。500 kB 警告は解消し、初期ロードは 632.98 → 577.54 kB / gzip 190.04 → 177.94 kB）。
