...
- 直前の実装判断: [IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md)（報告書の種別別 purpose 割当・#422）。本作業はその**月報エントリのみ**を改定する。
- 計画根拠:
  - AST/ADR-0014（計画リポ）
    （取引判断・報告書生成の割当モデル改定・**Accepted**）§決定1 の割当表。**本作業で月報行の改定が必要**。
  - AST/04_workflows/03_reporting-cycle（計画リポ）
    （報告サイクル。月報→週報→日報→取引の方針階層）。
  - ADR-0010（計画リポ）（LLM ゲートウェイ・本文凍結）§用途別ルーティング。
  - ADR-0025（計画リポ）（グローバル既定 Opus 5・Accepted）。本作業は `default` を変更しない。
  - 06_technical/08_data-egress-policy（計画リポ）（機密区分×ティア越境マトリクス・ZDR の注意点）。
- 計画への環流: AST/ADR-0014 §決定1 の月報割当を改定する新 ADR を計画リポへ起案する（**別 PR**）。
- 要求: FR-11（LLM 送信可否の統制・用途別ルーティング）／FR-06（報告書生成）。
- 本作業の実装判断は [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md)。
...
| モデル | ZDR | 根拠 |
| --- | --- | --- |
| `claude-fable-5` | 非対応 | `NonZdrModels` に列挙（[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)） |
| `claude-opus-5` | 対応 | 未列挙。`DefaultModel`（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md)） |
| `claude-opus-4-8` | 対応 | 未列挙（[IADR-0102](../adr/IADR-0102_trade-decision-model-pinning.md) で確認済） |
| `claude-sonnet-5` | 対応 | 未列挙（[IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md) で明記） |
| `claude-sonnet-4-6` / `claude-haiku-4-5` | 対応 | 未列挙 |

したがって **ZDR 対応モデルのうち最上位は `claude-opus-5`**（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) がグローバル既定＝最上位として採用した版数）。

### 事実 2: 月報だけが機密区分で割当を失う

`report-monthly` に非 ZDR の `claude-fable-5` を割り当てた結果、`confidential` / `restricted` では
`EligibleModels` から除外され `DefaultModel` へ**黙って**落ちる。[IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 決定2 はこれを
...
- **`Models`（利用許可集合）**。`claude-fable-5` は `analysis` が ZDR 非要件区分で使うため残す。削除は
  明示 `Model` 要求をしている呼び出し側に対する破壊的変更（[IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md) と同じ理由）。
- **`analysis` の割当**。ZDR 非要件区分に限って fable-5 を使う意図的な設計（[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)）であり、
  本 issue の対象外。
...
- [x] 採用モデルが claude エンドポイントの `Models`（利用許可集合）に含まれる（#376 / [IADR-0102](../adr/IADR-0102_trade-decision-model-pinning.md) の罠回避）。
