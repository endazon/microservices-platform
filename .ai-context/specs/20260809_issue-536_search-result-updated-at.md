...
related_specs:
  ...
  - "../../docs/tests/FR-03_hybrid-search.md"
...
**索引の作り直しを要求する作業**である。[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 条件 F（契約の追加に閉じる）を満たさないため、
...
**2 経路あることを見落とすと「テストは緑・本番は空」または逆になる。** [IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md) が
...
| 除外 | 理由 |
| --- | --- |
| **#532（並び順の 2 値化）** | **本作業の後続**である。索引項目が入って初めて実装できる。同じ PR にしない（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1・#536 issue 本文が「同じフェーズに束ねるな」と明記） |
...
**既定値つきで追加する**（[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2。既定値の無いメンバー追加は破壊的変更）。
...
**値は Unix epoch ミリ秒の整数で持つ。** 判断の根拠は [IADR-0149](../adr/IADR-0149_search-result-updated-at-indexing.md) に置く（要旨: 文字列 ISO-8601 だと
**後続 #532 の並び替えが辞書順に依存**し、オフセット表記の揺れ（`+09:00` / `Z`）で順序が壊れる。
整数なら表記に依らず一意で、Qdrant のペイロードインデックスにもそのまま載る）。

### 3. 画面（SC-02）

結果テーブルへ「更新日時」列を追加する。**`null` は `—` で描く**（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) が SC-07 で採った形）。
...
- **複写しない理由**: `—` の書き方が画面ごとに割れる。SC-06 の同期健全性で
  「同じ数が 2 箇所に立つと黙って割れる」（[IADR-0148](../adr/IADR-0148_datasource-sync-health-persistence.md) 決定 4）を踏んだのと同じ型である。
- **射程を広げたのではない**: 移したのは**本作業が必要とする 4 行**であり、`@platform/ui` へは入れない
  （**表示文言を持つ**ため。[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 が禁じている）。SC-06 の振る舞いは変えていない
...
| 指摘 | 対応 |
| --- | --- |
| 🟢 [IADR-0149](../adr/IADR-0149_search-result-updated-at-indexing.md) に `related_specs` が無い | **修正した**（作業仕様書・[IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md)・[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md)・SC-02 画面仕様書への逆リンクを追加） |
