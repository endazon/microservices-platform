...
- 起点 issue: [#403](https://github.com/endazon/microservices-platform/issues/403)（`bug` / size S）。
  [IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md) §フォローアップ 3 として記録した内容の起票（#381 / PR #402 の副次発見。
  [IADR-0101](../adr/IADR-0101_default-model-opus-5.md) レビューの 🟢1 も同一箇所を指摘）。
...
- 設計: ADR-0010（計画リポ）（LLM ゲートウェイ・本文凍結）、
  ADR-0022（計画リポ）（定型 RAG 回答＝Sonnet 5）、
  ADR-0025（計画リポ）（既定＝Opus 5）、
  [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（用途別ルーティング）、[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md)（`stopReason` による拒否判別）。
- 本作業の実装判断は [IADR-0111](../adr/IADR-0111_degraded-answer-model-label.md)。
...
2. **その値が実際の用途別モデルと一致しない。** 本経路の purpose は `rag-answer` で、割当は
   `claude-sonnet-5`（ADR-0022 / [IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md)）。`default` 層（Opus 5）の値を名乗るのは、実際に LLM を
   呼んだ場合とも食い違う。
...
- **`AiAnswerDto.Model` / `AskDoneEvent.Model` の型**（`string` のまま。`null` 許容へ変えない）。
  理由は [IADR-0111](../adr/IADR-0111_degraded-answer-model-label.md) §検討した選択肢。フロントの TS 型（`model: string`）も無変更。
...
## 決定（要約。詳細は [IADR-0111](../adr/IADR-0111_degraded-answer-model-label.md)）
...
- [x] LLM を呼び出していない縮退応答が返す「使用モデル」の方針が決定され、実装 ADR（[IADR-0111](../adr/IADR-0111_degraded-answer-model-label.md)）に記録されている
