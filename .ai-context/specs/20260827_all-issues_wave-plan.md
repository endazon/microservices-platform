...
［2026-08-28 追記 / #1021］**波 0〜2 完了・波 3 進行中の経過記録**（再起動からの復旧材料）:

- 波 0・波 1 は完了（波 1 末監査は条件付き合格 → 指摘全反映済み）。
- 波 2 完了: E3a・E3b 切替 / #1016 / #911 / #912（IADR-0281）/ #970 / #989 段 3（BFF・Retrieval・
  AiAnalysis・Graph の消費側 branch 評価 ＋ `AccessScope.Branches` 非破壊追加）。統合時の実測:
  Retrieval 126 / AiAnalysis 89 / Graph 250 テスト緑・両 slnx format 緑・検査器スイート緑。
  フェーズ末クロス監査（adr-guardian ＋ traceability-auditor、範囲 3788a48..ce52912）は起動済み。
- 波 3: 3I（platform フロント第 2 段・IADR-0262）と 3C（#451-b 通知受け口）は統合済み。
  3A（#449）・3B（#451-a）は実装中。3D〜3H は 3A/3B の完了待ち（領域交差のため直列化）。
- 統合手順の実測: サブエージェントの成果は手動 worktree のブランチへコミットさせ、
  親が cherry-pick → 両ユニットのテスト → 検査器 → push の順で統合する（コンテナ再起動でも
  ブランチ上のコミットは生存する。未コミットは消える）。

［2026-08-28 追記 / #1021］**波 3 完了**（head 65dd77c・全 9 トラック統合済み・CI 緑）:

- 3I（platform フロント第 2 段）/ 3A（#449 残実装 5 基準）/ 3B（#451-a BFF 11 端点）/
  3C（#451-b 受け口）/ 3D（SC-19・SC-20 画面）/ 3E（#600 発火結線 = 実態は送出品質 4 点の是正）/
  3F（#447 ⑨⑩ = ai_input 属性と RAG 分離。IADR-0283）/ 3G（#992 = seed と 2 段判定。IADR-0284）/
  3H（#1013 = check-route-manifest 新設）。orval codegen は 3B 後に 1 回（3G は openapi 無変更を実測）。
- 途中の裁定変更: **IADR-0282（単一プロジェクト＋VSA/DDD フォルダ標準。§1-3 の追記参照）** を
  波 3 中に記録し、テンプレート・生きた文書 7 箇所を追随させた。
- レビューボット指摘は全消化（CWE-117 の 4 経路目・文書追随 7 箇所）。既知の AST フレーク
  （RiskSettingsPage.monitorParameters）は 2 回再演し、いずれも再実行 1 回で緑（AST 側 #827 系へ申し送り）。
- フェーズ末クロス監査（adr-guardian ＋ traceability-auditor・範囲 ce52912..65dd77c）を起動済み。
