...
| 4. IADR | `git grep -n -i "both\|別名\|tags: \[\]" -- .ai-context/adr/IADR-0444* .ai-context/adr/IADR-0455* .ai-context/adr/IADR-0352*` | IADR-0444 決定 4（`both` の別名と新規作成の経路）・IADR-0455（露出だけ述べる。正しい）・IADR-0352（プラグイン CLI） | **IADR-0444 に日付つき追記**。予約の IADR-0464 は使わない（新しい判断が無い。計画の決定の実装であり、決定 4 の経路は変えない） |
