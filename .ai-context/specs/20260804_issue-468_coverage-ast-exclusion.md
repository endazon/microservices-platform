...
- 含むもの:
  ...
  5. [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md) の「既知の限界: 合成点テスト経由の混入」
     節を**解消済み**として書き換える。
...
| [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md) | 「既知の限界: 合成点テスト経由の混入」を解消済みへ書き換え、ゲート一覧の対象欄を更新 |
...
- [x] [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md) の「既知の限界」節を解消済みに更新する
...
1. ~~**床の置き直し**（CI 実測後・`src/coverage-floor.json` の 2 定数）。~~ **完了**（run 1144）。
   ...
   [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md)「床の置き方」= 現行の根拠を追記／
   [`docs/DEFINITION_OF_DONE.md`](../../docs/DEFINITION_OF_DONE.md) = **床の値を書いておらず JSON を参照するのみ
