...
**`specMissing` を空にした結果、`scripts/scripts.repo.test.js` の変異試験が壊れた。**
「無主の計画 ID」の変異試験は、その時点でテスト仕様書を持たない ID（＝ UC-01〜UC-07）に
相乗りしていた。全件が仕様書を持つと母集合が空になり、**何を仕込んでも exit 0**（検出力ゼロ）に
なる。合成した計画レンジ（`PLAN_ID_RULES`）で無主を作る形へ直し、陽性対照を同じテストへ
組み込んだ。決定と代替案は `IADR-0322` に置いた。

🔴 **これは「テストを書き足さない」の例外ではない。** 直したのは本作業が壊した装置であり、
UC の受け入れ基準に対応するテストではない。
