...
統合スタックを **5 回**回した（素の状態 3 回・変異 3 本。うち最初の 2 回は自分の不備で落ちた）。

| 回 | 結果 | 中身 |
| --- | --- | --- |
| 1 | 失敗 | **投入器が 401**。`#933` の realm パスワード変更に既定値が追随していなかった（別途修正） |
| 2 | 失敗 | 段 8 の想定が `#659` 以降**陳腐化**（無トークンは 401）／探針のタグが辞書に無く 400 |
| 3 | **成功** | **PASS 18 / FAIL 0**。段 11 `201` ／段 12 `1 件・作成した文書を含む` ／段 13 `0 件` |
| M1 | 検出 | FAIL 4（段 7・11・12・13） |
| M2 | 検出 | 段 7 `/bff/dashboard/summary → 401`（`#948` の症状を再現） |
| M3 | 検出 | 段 11 `403` ＋ 段 12 `0 件`。**段 7 は PASS のまま** |

🔴 **M3 が本作業の存在理由を実証した。** 同じ実行で段 7 の `/bff/documents → 200 []` は PASS のまま、
段 12 だけが落ちた。**同じ応答が、状態コードだけを見る判定では通り、新しい判定では落ちる。**

### 🔴 予測の訂正 —— M1 は 403 ではなく `000`（タイムアウト）だった

本仕様書は「不達 → deny-by-default → 静かに空」を前提にしていたが、**k8s ではハングする**。
ClusterIP に無いポートは RST を返さず落とされるため、deny へ縮退する前に呼び出し側が待たされる
（`#342` の「21 秒タイムアウト → 502」と同じ形）。**「静かに空」は compose の挙動**であり、
`#958` の PR 本文と `#972` の起票文はそれを一般化していた。`IADR-0252` で訂正した。
検査器は `000` を「上流の宛先が不達の疑い」と名指しできるようにした。
