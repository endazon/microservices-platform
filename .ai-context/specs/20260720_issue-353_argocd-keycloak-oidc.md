...
- ADR: ADR-0006（CI/CD・GitOps）。ArgoCD ブートストラップ統括は [IADR-0077](../adr/IADR-0077_local-observability-vault-gitops-overlays.md)、Keycloak OIDC 連携の先例は
  [IADR-0090](../adr/IADR-0090_grafana-keycloak-oidc-generic-oauth.md)（Grafana）、エッジ集約（集約後 URL）は [IADR-0091](../adr/IADR-0091_local-edge-aggregation-traefik.md)、opt-in ゲート smoke test は [IADR-0087](../adr/IADR-0087_k8s-local-up-optin-smoke-test.md)。
- 決定: 本作業の設計判断は [IADR-0092](../adr/IADR-0092_argocd-keycloak-oidc.md)（ArgoCD OIDC・dex 不使用・ホスト名ベース集約 URL・fail-safe local admin）。
