...
- 関連 IADR: [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)（変換ジョブ読み取りモデル）／[IADR-0043](../adr/IADR-0043_conversion-job-persistence.md)（同・永続化）／
  [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 1・決定 5（OpenAPI が BFF 契約の単一情報源／値集合を `enum` にしない）／
  [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 1・決定 2（`required` は C# の非 null 性から起こす）／
  **[IADR-0137](../adr/IADR-0137_conversion-dead-letter-marker.md)（本作業で起票。標識を導出せず記録する）**
...
| 失敗の記録 | `RawDocumentFetchedConsumer` が `catch` で `jobs.FailAsync(...)` を呼び、**例外を再送出**して再試行→デッドレターの挙動を保つ（[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)）。**「この失敗で再試行を使い切ったか」は記録していない** |
...
| D2 | **標識は導出せず記録する** | `Attempts >= MaxAttempts` による導出は**手動再変換で壊れる**——`Attempts` は手動再変換をまたいで累積するため（実測）、2 回目以降の配信中に一時的に真になる（まだ `<queue>_error` へ送られていないのに立つ）。コンシューマは自分が最後の試行かを知れるので、**事実として記録する**。詳細と棄却案は [IADR-0137](../adr/IADR-0137_conversion-dead-letter-marker.md) |
...
| D5 | **DTO への追加は既定値つき** | 既定値の無いメンバー追加は `check-contract-schema.js` が**破壊的**に分類する（[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2）。`bool DeadLettered = false` / `int MaxAttempts = ConversionJobRetryPolicy.MaxAttempts` を**末尾へ追加**する（位置の入れ替えも破壊的） |
| D6 | **OpenAPI では `required` に入れる** | [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 1（C# の非 null 性から起こす）・決定 2（**既定値つきメンバーも入れる**。C# の既定値は引数の既定でありシリアライズの省略ではない）。`bool` / `int` はいずれも非 null |
| D7 | **`enum` にしない** | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 5。ただし本作業が足すのは `boolean` / `integer` であり値集合を持たない（該当なし。**新しい値集合を作らない**ことが D7 の実効） |
...
- [x] **AC-9**: OpenAPI に 2 項目が載り、`required` に入っている（[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 1 ／[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md)）
...
> **ただし frontmatter の `status` は本文とは別である。** 同記録は実装側が `status: open` のままだが、
> 計画側（`draft/feedback/20260805_sc05-07-admin-contract-gaps.md`）は既に `accepted` で、
> 本 PR はその裁定（Q13）を反映している。**この食い違いは本作業では直さない** ——
> #497（PR #523）が「控えの status を原典へ同期する」規約を確立したが全数は追随しておらず、
> 全数突合の結果（**同名で食い違う 12 件 ＋ 改名でペアリングが壊れた 1 件**）は **#563** へ切り出してある。
> 本件はその 12 件のうちの 1 件である。
...
3. **IADR の採番衝突は実際に起きた。改番済みである。** 着手時点の最大は `IADR-0135` で、本作業は
   ...
   **PR タイトルには IADR 番号を含めていない**ため、同節が「最も漏れやすい」とする 4 点目の追随は不要である。
   ...
   > **規約の先着基準は「マージ順」であって「コミット順」ではない。** `.claude/rules/traceability.md`
   > は「番号は先にマージした側が確保する」と定めており、本作業はコミット時刻で代用している。
   > 結果は同じになる見込みだが、**それは #538 を先にマージする段取りを前提としている**。
   > もし #538 が取り下げられて本 PR だけがマージされると、`IADR-0136` が**恒久欠番**として残り
   > 同節の「欠番を作らない」に反する。**その場合は本 IADR を `0136` へ戻すこと**（追随先は上記 24 箇所）。
...
- **RabbitMQ 実機での `<queue>_error` 送出との突合**。標識は読み取りモデル側の記録であり、
  実際のキュー移送そのものは確認していない（[IADR-0137](../adr/IADR-0137_conversion-dead-letter-marker.md) §結果 のトレードオフ）。
