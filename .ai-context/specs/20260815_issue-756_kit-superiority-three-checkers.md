...
- ユースケース（UC）/ 画面（SC）: なし
- 関連 ADR: 計画側なし。実装ADR は [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md)（キット同期の分類）/ [IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md)（分類表）/
  [IADR-0140](../adr/IADR-0140_cross-repo-issue-ref-checker.md)（クロスリポ参照検査）/ [IADR-0143](../adr/IADR-0143_plan-id-qualification-checker-scope.md)（自己除外）/ [IADR-0169](../adr/IADR-0169_cross-repo-ref-scan-beyond-markdown.md)（`.md` 外への走査）/
  [IADR-0183](../adr/IADR-0183_false-green-warning-on-worktree-state.md)（worktree 状態の警告）
...
- [x] `check-commit-messages.js` / `check-cross-repo-refs.js` は**環流先 planning issue 番号**が理由欄に入る
      ...
      `kit-sync-classification.json` の X が**追跡先の無い債務**として残る（[IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md) 決定 2 が
      X に追跡先を必須としている。**環流そのものの義務**は [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 決定 3 で、両者は別の決定である）。**planning#373**（`check-commit-messages.js`）と
