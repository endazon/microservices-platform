...
| 床を引き上げ、根拠を残した | ✅ **88/68 → 90/75**（[2026-08-29 追記] 参照）。据え置き時に宣言した条件（新しい母集合での後段 CI 実測）が満たされたため引き上げた |
...
## ［2026-08-29 追記 / #901］床を引き上げた（88/68 → 90/75）

据え置き時に「**本 PR が develop へ入り、後段 CI が新しい母集合で 1 回実測してから上げる**」と
条件を書いた。その条件が満たされたので実行した。

**基準にした実測**: develop `3939e72` の **Integration** run 33230268352。
`integration.yml` は `--filter` を付けず全量を走らせ、`check-coverage-floor.js` を
`--report-only` 無しで実行する（＝床を実際に判定する経路そのもの）。

```
[check-coverage-floor] レポート 19 件を集計:
  line 91.07%（12683/13926） / branch 76.12%（2942/3865）。床: line 88 / branch 68
```

この run は据え置きの理由だった「分母が動く」本番コード（削除の伝播・資格情報のマスク・
SC-12 の境界層・健全性指標の生産者・SC-17 の身元管理・パンくず）を**すべて含む**。

### 🔴 切り下げ値（91 / 76）を採らなかった

| 床 | 必要 | 耐性 | 被覆単位比 |
| --- | --- | --- | --- |
| line 91 | 12673 行 | **10 行** | **0.079%** |
| line 90 | 12534 行 | 149 行 | 1.17% |
| branch 76 | 2938 本 | **4 本** | **0.136%** |
| branch 75 | 2899 本 | 43 本 | 1.46% |

判断の根拠は **IADR-0118 決定 2 の例外**である（本作業の ［2026-08-29 追記 / #901］ で
条件を数へ落とした）——**切り下げ値の耐性が被覆単位の 0.5% 未満なら 1 つ下を採る。**
line 0.079% / branch 0.136% はいずれも 0.5% 未満なので発動し、90 / 75 を採る。

🔴 **［2026-08-29 追記 / #901］初版はここを pt で論じていた。**
「`coverage-floor.json` が line +0.14pt / branch +0.26pt を『余裕は薄い』と書いており、
切り下げ値はそれよりさらに薄い」と書いていたが、**これは誤った根拠である** ——
比較相手は**分母がまったく違う旧時代の pt** であり、**同じファイルの #900 追記が
まさにその跨時代 pt 比較を戒めている**（分母が変われば pt の意味が変わる。
ratchet では pt ではなく被覆行・被覆分岐の**本数**で見よ）。AI レビューが指摘した。
**結論（90 / 75）は変わらないが、根拠は上の被覆単位比へ置き換えた。**
`src/coverage-floor.json` 側からも同じ pt 比較を削除してある。

### 本追記で本番コードは 1 行も変えていない

床の値と `$comment` のみである。**分母が動く変更と同じ変更単位で床を上げない**という
自分の記述（据え置きの根拠）をここでも守っている。
