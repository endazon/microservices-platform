...
| 床 | 必要 | 余裕 |
| --- | --- | --- |
| line 91 | 12673 行 | **10 行**（+0.07pt） |
| line 90 | 12534 行 | 149 行（+1.07pt） |
| branch 76 | 2938 本 | **4 本**（+0.12pt） |
| branch 75 | 2899 本 | 43 本（+1.12pt） |

`src/coverage-floor.json` は既に **line +0.14pt / branch +0.26pt を「余裕は薄い」**と書いている。
切り下げ値はそれより**さらに薄く**、テスト 1 件の skip や被覆済みコードの削除で
**「成果物は正しいのに赤」**になる。**切り下げは上限であって目標ではない。**
+2 / +7 の ratchet は十分に大きく、1pt 強の余裕を残す 90 / 75 を選んだ。
同じ算術（必要行数と余裕を数で出す）を次に上げる者も踏むよう `$comment` へ書いた。
