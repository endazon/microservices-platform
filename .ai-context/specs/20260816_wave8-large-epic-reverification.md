...
| 3 | #439 BFF セッション方式 | `dotnet`。先行 #438。**フロントだけ先に切ると認証が 2 系統同居し [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 6 の「1 度だけ切替」に反する** | 環境 ＋ 先行 |
| 4 | #440 LLM ゲートウェイ | `dotnet`。加えて **計画 `ADR-0038` が `Proposed`**（着手条件は `Accepted`。[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)） | 環境 ＋ ADR |
...
| **#450** | **着手保留は解除された。** `ADR-0033` / `0034` / `0035` / `0036` / `0037` が**全件 `Accepted`**（[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の 2026-08-07 追記「これにより FR-17 / FR-18 の保留を解除した」）。**ただし `ADR-0039`（SC-18 の描画ライブラリ）は `Proposed`** で別軸が残る |
| **#451** | **着手保留は部分解除。** 2026-08-07 のコメントが「SC-19 の『本文を編集』導線ただ 1 つを除いて着手可能」と明記。`ADR-0046` がその導線を Obsidian 経路へ確定させた。**ただし [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の 2026-08-15 追補（保留継続）と計画 `ADR-0037` の注記（留保は外れた）が同日で逆を向いており、上流に未決着が 1 点ある** |
...
| `#454` 本文のチェックボックスの機械的追随 | [IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md): PR 番号・open/closed・CI 状態は GitHub が正で、書けば二重管理になる。**直したのは誤った判断の記述だけ** |
...
**[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md) が言う「環境依存の判定に賞味期限がある」ことの、最も強い実例**である。
...
4. **1〜3 の結果と時点を書く**（[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md)。前回値を据え置かない）
