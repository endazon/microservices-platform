...
**#542 の後半である**（前半 = #634。契約の定義は [IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md)、本作業の実装方式は [IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md)）。
...
**取り込み経路はタグを生成しない**（[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) 決定 5 / planning#304）。
...
「[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) 決定 2 により狭い」と書いたが、**#637 が着地して前提が変わったので引き直した**——
...
### 触らないもの（**[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) 決定 2 が下流を変えないため。実測で確認した**）
...
| Qdrant ペイロード・`QdrantVectorStore` / `QdrantIngestionVectorStore` / `IVectorStore` | **検索の hot path に辞書引きを増やさない**（[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) 決定 1） |
...
（SC-05・[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) 決定 5）に**正しく弾かれている**。実装ではなくテストの側の追随漏れである。
...
[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 の規則 3「拡張子で絞らない」と同じ話が、**言語の層**でも起きる。
...
#542 の射程外なので **#640 へ起票した**（[IADR-0153](../adr/IADR-0153_tag-identity-storage-and-projection.md) の残件へも記録した）。
