...
## ［2026-08-18 追記 / #859］母集合の軸 2・軸 7 が再現しない（除外の明示と、再現しない値の記録）

波 12 末クロス監査の指摘を受け、§母集合の実測 の軸を `3ad5ad15`（着手前の develop）で
**自分で引き直した**。**本節より上の本文は書き換えていない**（確定済み `docs/specs/` の本文不改変）。

走査条件は本文と同じ（`git grep`・拡張子で絞らず・パス除外は `':!planning' ':!src/ai-stock-trading'` のみ）。

| 軸 | 走査文字列 | 本文の記載 | 引き直し（`3ad5ad15`） | 判定 |
| --- | --- | ---: | --- | --- |
| 1 | `ビルド対象外` | 13 件 | **13 行** / 10 ファイル | **再現する**（件数＝行数） |
| 2 | `ビルド対象` | 40 件 | **41 行** / 26 ファイル | **再現しない** |
| 7 | `unit-template`（ファイル単位） | 41 ファイル | **43 ファイル** | 生成物 2 件の除外で 41 |

### 軸 7: 除外していた 2 件を明示する（規則 6）

43 − 41 = 2 の正体は、**いずれも生成物**である。

| 除外したファイル | 除外の理由 |
| --- | --- |
| `CHANGELOG.md` | **生成物**。`scripts/gen-changelog.js` ＋ `.github/workflows/changelog.yml` が自動更新し、**手で書き足さない**（`CLAUDE.md`「補助成果物の自動生成」）。文言の追随対象ではなく、是正するなら生成元（`scripts/changelog-overrides.json`）で行う |
| `src/pnpm-lock.yaml` | **生成物**。pnpm が書く lockfile であり、人が読む散文ではない |

**除外の判断自体は正しいが、本文に書かれていなかった。**
規則 6 は「引いた結果と、**除外したものとその理由**を作業仕様書に書く」であり、
**「黙って除外した」ことでも事故は起きる** —— 除外理由が残らないと、次に同じ走査をした人が
「引き漏らし」と「意図的な除外」を区別できない。

### 軸 2: 「40 件」はどの条件でも再現しなかった（原因未特定）

**正しい実測値は 41 行 / 26 ファイル**である。試した変種と結果（**いずれも 40 にならない**）:

```console
$ git grep -In "ビルド対象" 3ad5ad15 -- ':!planning' ':!src/ai-stock-trading' | wc -l
41
$ git grep -Il "ビルド対象" 3ad5ad15 -- ':!planning' ':!src/ai-stock-trading' | wc -l
26
$ git grep -n  "ビルド対象" 3ad5ad15 -- ':!planning' ':!src/ai-stock-trading' | wc -l      # -I なし
41
$ git grep -In "ビルド対象" 3ad5ad15 -- ':!planning' ':!src/ai-stock-trading' \
    ':!CHANGELOG.md' ':!src/pnpm-lock.yaml' | wc -l                                        # 生成物除外
41
$ git grep -In "ビルド対象" 3ad5ad15 -- ':!planning' ':!src/ai-stock-trading' ':!templates' | wc -l
39
```

**±1 の食い違いの原因は特定できなかった。** 数を黙って書き直さず、
**正しい実測値（41 行 / 26 ファイル）・走査コマンド・再現しなかった事実**をここに残す
（導出値は走査ではなく計算し直す、が規則 10 の要求であり、**再現しない値は再現しないと書く**）。

なお軸 1 が「13 件」で再現することから、本文の「件数」は**行数**を指していると読める
（軸 7 だけがファイル単位と明記されている）。**単位が軸ごとに違うことも本文に書かれていない**ため、
追試する人は軸ごとに単位を確かめる必要がある。
