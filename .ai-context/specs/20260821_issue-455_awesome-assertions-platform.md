...
> ［2026-08-21 追記 / #455 A-3 段 2］🔴 **「フォークなので API 名は変わらない見込み」は段 2 で破れた。**
> `BeGreaterOrEqualTo`（FluentAssertions v7 の**非推奨エイリアス**）は AwesomeAssertions に**存在せず**、
> `CS1061` でビルドが落ちた。**段 1 が緑だったのは、その 1 箇所が knowledge ユニットにあったから
> にすぎない。** 危険リストは「件数の多い表明」ではなく **「非推奨エイリアス」で引くべきだった**。
> 詳細は段 2 の仕様書（`20260821_issue-455_awesome-assertions-knowledge.md`）§挙動差の確認。
