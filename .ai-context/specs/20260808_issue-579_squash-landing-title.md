...
- 原因の PR: **#568**（`bc7bc8e`）
- 分類（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 4）: **「機械検査を新設する」＋「規約を改定する」** → 重い方を採る。
  クロス監査は**フェーズ末に 1 回**（同 決定 4 の 2026-08-08 追記）
- 制約: **[IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 分類 A** ——`scripts/scripts.test.js` は変更禁止（読むのは可）。
  `.github/workflows/` は GitHub App 権限で編集不可（[IADR-0140](../adr/IADR-0140_cross-repo-issue-ref-checker.md) 決定 2）
...
force push 禁止で直せないため **baseline へ据え置く**（[IADR-0145](../adr/IADR-0145_landed-subject-check-scope.md) 決定 4）。
...
- 一時的に API で引く選択肢はあったが、**引いても機械検査にはできない**（検査器が API に依存できない）ため、
  母集合の一次調査としての価値しか無い。
- **したがってこの軸は引かず、「機械では判定できない」ことを [IADR-0145](../adr/IADR-0145_landed-subject-check-scope.md) 決定 3 として明記する方を選んだ。**
  **黙って落としたのではなく、落とした理由をここに書く**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 規則 6）。
...
モジュールが無い構成は skip＋notice、節がパースできない場合は例外（[IADR-0145](../adr/IADR-0145_landed-subject-check-scope.md) 決定 2）。
...
- **PR タイトルからの ID 脱落**（軸 3・[IADR-0145](../adr/IADR-0145_landed-subject-check-scope.md) 決定 3）。**#579 の起点そのもの。**
...
- [x] スカッシュ着地件名の検査について、**できること／できないこと**（ワークフロー編集不可・
      **ID 脱落は機械判定不能**）が本書と [IADR-0145](../adr/IADR-0145_landed-subject-check-scope.md) に記録されている
