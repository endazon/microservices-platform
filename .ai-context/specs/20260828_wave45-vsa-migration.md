...
- **`check-unit-dependencies.js` の旧判定（規則 3-①②・8 要素プロジェクト参照の層方向）は
  対象 0 件になったが、コード上に残している。** 移送中は新旧が混在するため両方を効かせる設計にしており、
  完了した今は恒常的に「該当なし」を返すだけで誤検出も見逃しも起こさない。
  **次に同ファイルへ触るときに規則 3-③（名前空間走査）へ一本化して削除すること**
  （着地後のレビューの 🟢 指摘）。本波でやらないのは、移送の diff に検査器の削除を混ぜないためである。

## 🔴 移送完了で「新たに誤りになった」記述（規則 10）

**移送そのものより、移送が既存の記述を誤りに変えたことのほうが取りこぼしやすい。**
着地後のレビューが 🔴 で 2 ファイルを挙げ、そこから走査して次を直した:

| ファイル | 何が誤りになったか |
| --- | --- |
| `src/README.md` | 「移送波の一括変換までは現行配置が実態であり、**新規コードも現行配置で書く**」 —— 移送は完了しており、旧配置は 1 つも残っていない。**次のセッションがこれを正として読むと、存在しない配置で新規コードを書き始める** |
| `docs/tech/tech-requirements.md` | 同上の条文 |
| `docs/tech/tech-requirements.md` | **導出値**「`Api` 9 サービス / `Worker` 2 サービス」—— 実測は **`Api` 12 / `Worker` 2**（14 サービス）。**レビューはここを挙げていない。** 数え直して初めて出た（規則 7: 導出値は走査ではなく計算し直す） |

**この 2 ファイルは `CLAUDE.md` が「ユニット構成の単一情報源」と名指ししている。**
単一情報源が古いまま残るのは、分散した記述が古いより悪い —— 読み手が疑わないからである。
