...
related_specs:
  ...
  # ［2026-09-25 追記 / #1499］ここにあった helm の templates/minio は IADR-0464 で撤去した（SeaweedFS へ差し替え）
  - "../../deploy/local/edge/README.md"
  # ［2026-09-25 追記 / #1499］ここにあった deploy/local/minio-oidc の README は IADR-0464 で撤去した（Console の SSO を撤去）
...
- ADR: ADR-0017（サービス間認証・エッジ）/ [IADR-0024](../adr/IADR-0024_object-storage-minio-buckets-and-access.md)（MinIO バケット設計・Console は既定 Ingress 非公開）。
  Keycloak OIDC 連携の先例は [IADR-0090](../adr/IADR-0090_grafana-keycloak-oidc-generic-oauth.md)（Grafana）、エッジ集約は [IADR-0091](../adr/IADR-0091_local-edge-aggregation-traefik.md)。
- 決定: 本作業の設計判断は [IADR-0093](../adr/IADR-0093_minio-keycloak-oidc.md)（MinIO Console OIDC・policy クレーム・集約 URL・fail-safe deny）。
