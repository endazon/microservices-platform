...
[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md)「参照点を 1 つに畳む」。**#583 で同型を 3 回、#626 でも同じ判断をしている。**
...
### ★ 判断 7: **手順書のコマンドは本リポの道具立て（Node.js / .NET）だけで書く**

**初版は列挙コマンドを `python3 -c` で書いていた。誤りである。**

`python3` は [`scripts/setup.sh`](../../scripts/setup.sh) で**コメントアウトされた opt-in**
（`# --- Python（例。使う場合はコメント解除） ---`）であり、**利用保証が無い。**
**手順書は運用者が実行するもの**であり、**手元に無い処理系へ依存させない。**

> **★ 「自分の環境で動いた」は根拠にならない。** 本セッションには `python3` が在る（3.11.15）。
> **在ることと、運用者の環境に在ることは別である。**

#### 母集合（同型を全数で引いた）

`docs/operations/` と `docs/how-to/` の**手順書 8 本**について、
**コードブロック内の実行行が使う実行ファイルを全数で列挙した。**

| 分類 | 実行ファイル |
| --- | --- |
| 宣言済みの道具立て | `node` / `pnpm` / `dotnet` / `git` |
| **その手順が本来必要とする道具** | `kubectl` / `argocd` / `docker` / `psql` / `kcadm.sh` / `curl` |
| **外れ値** | **`python3` —— 本 Runbook の 1 件のみ** |

**同型は 1 件（本 PR 由来）**であるため、検査器は足さず回帰テストで固定する。

#### 置換後のコマンドは実走した

```console
$ node -e "const d=require('./src/.../appsettings.json');\
...
#### 回帰テストは**コードブロック内の実行行だけ**を見る

**素の全文検索にしてはならない。** 本 Runbook は「**なぜ python を使わないか**」を
**説明するために `python3` の語を含む**。全文検索で落とすと、**禁止の理由を書いた側が落ちる。**
（空振り防止に、実行行を 1 行以上拾えたことも確かめる。）
...
| — | （回帰）手順書のコマンドが `node` で書かれている | 判断 7。**手順書 8 本を全数で引き、外れ値は本 Runbook 1 件** |
