...
### 受け入れ基準の射程 —— ［2026-08-22 裁定］**移行の完了は AST を含む。3b の完了は含まない**

planning#450 の裁定が下りた（原文は `origin/main` の `86499f3` / PR #455 で確認）。

> **本表の適用範囲は、基盤（`microservices-platform`）と可変機能ユニット（`ai-stock-trading` 等）の双方である。**
> **採否（とくに ★不採用）の完了条件は、pnpm workspace のメンバ全体から当該技術が消えていることを指す。**

根拠は `src/pnpm-workspace.yaml` のメンバ glob `'*/frontend'` に `src/ai-stock-trading/frontend` が
一致すること。**「ワークスペースから消えている」を字義どおり採る**という読みが採られた。

🔴 **`oidc-client-ts` は 3 ユニットすべてが宣言している**（裁定文の実測。`platform` / `knowledge` /
`ai-stock-trading` がいずれも `^3.1.0`）。**本書が「宣言 2」と書いていたのは
`platform` ＋ `knowledge` の数であり、AST を数えていなかった。**

**したがって 2 段構えにする。**

| | 条件 |
| --- | --- |
| **3b の完了** | `platform` ＋ `knowledge` から**宣言と import の両方が消えている** |
| **移行（★不採用）の完了判定** | 上に加えて **AST から消えている**。AST は submodule で本リポジトリから是正できないため、**AST 側リポジトリの作業として別に追跡する** |

**両リポジトリの作業順序は実装側が定める**と裁定されている。**3b の着地後に AST 側へ起票する**
（先に AST を撤去すると、AST の SPA が合成時にログインできなくなる —— `IADR-0121` 決定 6 と同じ理由）。

**ADR-0032 の go-live 判定は AST の追随を待つ。**
...
## planning#445 の裁定（2026-08-22・確定）—— **ツリー全体への適合が必須。ただし 3b は止まらない**

### ［2026-08-22］裁定が下りた。本節の前提は「待たない」から「確定した」へ変わった

原文は `project-planning` の `origin/main`（`69eab73` / PR #451）で確認した。

- **§ディレクトリ構成 のツリーは変更しない。本構成への適合は実装側の必須事項である**（利用者裁定 2026-08-22）
- **食い違いは 1 ディレクトリの名前ではない**と裁定文が明記した ——
  `foundation/` 直下の 8 つはツリーの **5 項目**にまたがる（`config`/`i18n`/`routing` → `app/`、
  **`api`/`auth` → `lib/`**、`ui`/`notifications` → `components/`、`testing` → `testing/`）。
  **`foundation/` → `app/` の改名だけでは適合にならない**
- 🔴 **`@foundation` エイリアスとディレクトリ名は分離している**ことが裁定文に採られ、
  **必須とするのはディレクトリ構成であってエイリアス名ではない**と明記された

### 🔴 3b への影響 —— **止まらない。分解しても `auth` の中身は書き直さない**

§実装への移行方針 の認証行に追記が入った（原文）:

> `foundation/auth/` を二度書き換えない **［2026-08-22 追記］ディレクトリ名は #445 の裁定に従う**
> —— §ディレクトリ構成 への適合が必須であり、`foundation/auth/` は `app/` / `lib/` 等へ分解される。
> **「二度書き換えない」という意図は変わらない**（実装側の実測では、分解しても `auth` 配下の
> 内部相対 import は変わらず、外部参照はエイリアス経由である）

**つまり「二度書き換え」は回避されている。** 分解は**ファイルの移動**であって `auth` 配下の
**中身の書き直しではない**（内部の相対 import 130 本は移動しても変わらない。実測）。
よって **3b を先に進めてよい。**

**ツリー全体への適合は 3b の射程外**である（実測 143 ファイル / 378 箇所。うち **43 が AST submodule 内**）。
別作業として追跡する。
...
# 🔴 引き継ぎ（2026-08-22 時点・**3b は段 ① まで着地。段 ② 未着手**）

## いまどこか

| 段 | 状態 |
| --- | --- |
| **3a**（BFF 側の受け皿） | ✅ **develop へ着地済み**（`6aeba042` / PR #973） |
| **3b ①**（既定認証スキームの切り替え） | ✅ **develop へ着地済み**（`c34f2f28` / PR #1002）。既定は振り分けスキーム `BffSmart` |
| **3b ②**（`foundation/auth` のセッション方式化） | 🔴 **未着手。** 事前の実測だけ済んでいる（下記） |
| **3b ③**（`oidc-client-ts` 撤去） | 未着手。母集合は実測済み |
| **3b ④**（e2e 13 本の追随） | 未着手 |

## 🔴 いちばん壊されやすい設計 —— `AddPolicyScheme` による振り分け

`BffSessionExtensions.AddBffSession` は、既定スキームを **`BffSmart`（振り分けスキーム）** にし、
`Authorization: Bearer` が在れば JwtBearer、無ければセッション Cookie へ委ねる。

### **「既定ポリシーに 2 スキームを載せる」に単純化してはならない**

**次の担当が「冗長だ」と単純化して壊す典型なので、理由を残す。**

`AddAuthorization(o => o.DefaultPolicy = new AuthorizationPolicyBuilder(A, B)...)` では**足りない**。
理由は、端点が
...
の形で作る**内側のポリシーがスキームを持たない**ことにある。スキームを持たないポリシーは
`context.User` をそのまま見るので、**`context.User` を作った既定スキームにしか従わない**。
既定ポリシーだけを直しても、**ロール要求を持つ端点は片方の経路でしか通らない。**

🔴 **そしてその欠陥は、既定ポリシーの端点でテストすると緑になる。** 気づく機会が無い。
だから**振り分けを認証の側（`AddPolicyScheme`）へ置く** —— そうすると
**既定ポリシーの端点もロール要求の端点も同じように**両方を受理する。

**この形は `DefaultSchemeRoutingTests` の「条件 4」が固定している**
（ロール要求端点に対し Bearer 経路・Cookie 経路の両方が同じ判定を通ること＋資格情報が無ければ 401）。
**単純化するとその 2 件が落ちる。落ちたら単純化をやめること。**

### なぜ Bearer を受理し続けるのか（案 B）

既定を `BffSession` 単体にすると `/bff/*` への **Bearer 呼び出しが 401 になる**（実測）。
`scripts/verify-oidc-edge-flow.sh` は `/bff/*` を Bearer で **4 箇所**叩いており
（220 / 291 / 319 / 345 行）、**統合スタックで実際に動いている唯一の外形確認**である。
移行の副作用でこれを失わない、という判断。

**計画は Bearer 受理を許していない。禁じてもいない。**「言及が無い」は「許可」ではないので、
**実装側の判断**として採り、**狭める条件を `IADR-0251` 決定 9 に書いた**。
**移行期の姿勢であって恒久の姿勢ではない。**

## 変異試験の記録（この 2 件は「やった」ではなく「効いた」）

| 対象 | 変異 | 結果 |
| --- | --- | --- |
| 振り分けの 3 点セット | ハンドラを**常に許可**へ | 🔴 **陰性対照を含む 3 件が落ち、肯定側 6 件は緑のまま。** 陰性対照が無ければ「常に通す実装」が通っていた |
| `RedisTicketStore` | `RemoveAllForSubjectAsync` が索引を無視して**全員**を消す | 🔴 **陰性対照 1 件だけが落ち、他 5 件は緑。** 「1 人の無効化で全員ログアウト」が正しい実装として通っていた |

**テスト件数**: `Platform.Bff.Tests` **278 → 287**（3a 着地時 272 → 本ブランチ 287）。
`scripts.test.js` は 606。

## 🔴 段 ② に入る人へ —— `TestAuthHandler` は**測ってから**触ること

`AddPolicyScheme` を入れたことで、**試験の器が振り分けのどちら側に挿さるかが変わり得る。**
**実測済み**（`TestAuthHandlerLayeringTests`）:

- `BffTestFactory` は `AddAuthentication(TestAuthHandler.SchemeName)` で**既定をさらに上書き**する
  → 🔴 **器は振り分けの「前」に挿さり、振り分けを迂回する**
- ただし振り分け先の 2 スキーム（`BffSession` / `BffSmart`）は**登録されたまま**

**意味**: 既存テストの緑は、**振り分けが正しいことも、セッション経路が動くことも示さない。**
受け持ちは分けてある ——
振り分け＝`DefaultSchemeRoutingTests` ／ セッション経路＝`RedisTicketStoreTests` ／
器の射程＝`TestAuthHandlerLayeringTests`（**既定が `SmartScheme` になったら前提が変わるので
見直せ**、という見張りを assert に入れてある）。

🔴 **器を書き換えるなら、書き換え後の器が「Cookie が無ければ 401」を再現するかを変異で確かめること。**
#948 で `FeedbackStubHandler` が `Authorization` を**記録するだけで検査せず常に成功を返した**ため、
BFF の資格情報転送の欠落が緑を通った実例がある。**認証まわりの器は実体より甘くなりやすい。**

## 段 ③（`oidc-client-ts` 撤去）の母集合（実測済み）

| 対象 | 実測 |
| --- | --- |
| **宣言** | **2**（`platform/frontend/package.json` / `knowledge/frontend/package.json`）。knowledge 側は knip が「真の未使用」と記録しているもの |
| **import** | **10 ファイル**（すべて `platform/frontend/src/foundation/` 配下。`auth/` 8・`testing/` 1・`ui/` 1） |
| **AST** | 宣言 1（`ai-stock-trading/frontend/package.json`）＋ 自前ファイル群 |

**受け入れ基準は 2 段構え**（planning#450 の裁定）:

- **3b の完了** = `platform` ＋ `knowledge` から**宣言と import の両方**が 0
- **移行（★不採用）の完了判定** = 上に加えて **AST から消えている**。AST は submodule で本リポジトリ
  から是正できないため**別リポジトリの作業として追跡**する。**順序は 3b 着地後に AST 側へ起票**
  （先に AST を撤去すると合成時に AST の SPA がログインできなくなる。`IADR-0121` 決定 6 と同型）

## 🔴 条件 3 —— 段 ③ で検証すること

**「BFF が Bearer を受理する」と「SPA がトークンを取得できる」は別である。**
案 B は前者を許すだけで、後者を許していない。

**段 ③ で `oidc-client-ts` を撤去したあと、SPA がトークンを取得する経路が本当に無いことを実測し、
受け入れ基準に入れること。** 撤去前に測っても意味がない（まだ経路が在る）ので、**③ の時点で測る。**

**否定形だけで書かないこと** —— 「トークンが露出しない」だけだと**常に 404 を返す実装**が通る。
「セッション Cookie は在る」「`/bff/auth/me` は 200 で身元を返す」を**同じ装置で対にする**。

## 段 ④ —— e2e 13 本

`platform/frontend/e2e/*.smoke.spec.ts` が **13 本**。🔴 **無改修では済まない**（実測に基づく見込み）
—— ログイン導線が `oidc-client-ts` のリダイレクトから `/bff/auth/login` へ変わるため。

> **見込みと実測のずれの記録**: 起草時は「消費側 18 ファイル・多くは無改修」と見ていたが、
> **実測は 28 ファイル**だった。**e2e 13 本を数え落としていた。**
> 「継ぎ目を保てば多くは無改修」は **`src` 配下 15 件には当てはまり、e2e 13 件には当てはまらない。**

## やり残し一覧

| # | 項目 | 備考 |
| --- | --- | --- |
| 1 | ~~3b ① の PR を出してマージする~~ | ✅ **完了**（`c34f2f28` / PR #1002。CI 42 checks / 失敗 0） |
| 2 | **段 ②**: `foundation/auth` のセッション方式化 | 未着手。上の「器は測ってから触る」を守ること |
| 3 | **段 ③**: `oidc-client-ts` 撤去 ＋ **条件 3 の実測** | 母集合は上表 |
| 4 | **段 ④**: e2e 13 本の追随 | |
| 5 | **AST 側への起票** | 3b 着地後。planning#450 の裁定による |
| 6 | **#972 へ再実行を依頼** | 案 B なので検証スクリプトは壊れないが、**既定スキームは振り分け経由に変わった**。3b 着地後に再実行が要る |
| 7a | 🔴 **#882 の ESLint プラグイン着手の解除は「③ 以降」である** | あちらは `src/pnpm-lock.yaml` の衝突で 3b の着地を待っている。**① は lock に触っていないので解除にならない。** lock が動くのは **③（`oidc-client-ts` を `package.json` から落とす）**の時点である |
| 7 | **#882 へ最終件数を伝える** | `Platform.Bff.Tests` は **① 着地時点で 289**（#882 の記録 232 から +57）。**②③④ でさらに動くので、3b 全体の着地時に確定値を出す** |
| 8 | **ツリー全体への適合**（planning#445） | **3b の射程外**。実測 143 ファイル / 378 箇所、うち **43 が AST submodule 内**。別作業として追跡 |
| 9 | **第 4 段・第 5 段** | 実測: 第 4 段は Zustand / TanStack Table / ECharts / RHF / Zod が**すべて宣言 0 件＝丸ごと未着手**。第 5 段は Knip / Husky / Renovate 実在、**Plop と Commitlint が無い** |

🔴 **`oidc-client-ts` を撤去しても移行は完了しない。** 上の 8・9 が残る。
**「go-live blocker が解けた」と読まれないよう、PR 本文に明記すること。**

## この作業環境で踏んだ落とし穴（次の担当への申し送り）

- **`pnpm` は `VOLTA_FEATURE_PNPM=1` が無いと起動しない。** 無いと husky の `pre-commit` が
  **「整形されていない」という誤った理由**で commit を止める（実際は `format:check` EXIT=0）
- **Bash ツールの cwd はリセットされ得る。** 相対パスのコマンドは共有ツリーに当たり得るので、
  **先頭に `cd <絶対パス>` を置くか `git -C` を使う**
- **不可視の破損に 4 種類ある** —— ①置換が何にも一致しない ②シェルのコマンド置換で内容が消える
  ③生の NUL / BOM ④**エスケープ解釈で制御バイトが混入**（本作業で `\b` が `0x08` になり、
  **JS のパースは通るのに正規表現だけが静かに壊れた**）。**文章ファイルの生成に heredoc を使わず
