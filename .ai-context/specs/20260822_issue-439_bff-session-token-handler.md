...
| **3b ①**（既定認証スキームの切り替え） | **実装・試験とも完了。ブランチ `feat/439-spa-session-switchover` に 8 コミット。PR 未作成** |
...
| # | 項目 | 備考 |
| --- | --- | --- |
| 1 | **3b ① の PR を出してマージする** | ブランチ `feat/439-spa-session-switchover`（8 コミット・push 済み）。**ローカルは全緑**だが CI 未実行 |
...
| 7 | **#882 へ最終件数を伝える** | `Platform.Bff.Tests` は現在 **287**（#882 の記録 232 から +55）。**3b 着地時に確定値を出す** |
