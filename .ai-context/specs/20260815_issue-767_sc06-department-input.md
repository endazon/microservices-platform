...
- 関連 ADR: `ADR-0004`（ABAC）、`ADR-0034`（グラフ探索でもホップごとに ABAC を強制する）、`ADR-0036`（所有者ベース裁量制御）。実装 ADR は [IADR-0019](../adr/IADR-0019_datasource-default-attributes.md)（機密区分のフェイルセーフ既定）・[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md)（取り込み必須属性のフェイルセーフ）・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)（i18n カタログの網羅検査）
- 計画書リンク:
  - `06_technical/09_datasource-connectors.md`（計画リポ） §システム投入経路での `owner` / `department` / `lifecycle`（確定・2026-08-15）
  - `06_technical/07_abac-attribute-model.md`（計画リポ） §文書の基本属性（`department`＝所管部門・**必須**・部門コード）
...
| 3 | 解決できなければ予約値 `unassigned` | 実装済み（バックエンド。PR #753 / [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md)） |
...
1. **後段の空白判定に依存しない。** 空文字を送る形は「後段が `IsNullOrWhiteSpace` で潰してくれる」ことに
   ...
   予約値との区別（＝環流債務の測定値。[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 3）が壊れる。**壊れても静かである。**
...
2. **M5 は「`.po` を空にしただけ」では単体テストが素通りする。** 実行時に読まれるのは**コンパイル済みの
   ...
   **したがって en の未翻訳を止めているのは、実質 `check-i18n-catalogs.js` と `lingui compile --strict`
   ...
   [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 4 が「再生成差分検査だけでは未翻訳を検出できない」と述べた穴の実測でもある。
...
1. ~~**`docs/data/data-source.md` の追随が未消化。**~~ → **同 PR 内で消化した**（3 巡目）。
   ...
   [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 側は同じ事実を**複写せず**、決定 2 の冒頭に短い日付つき注記を置いて対比表へ誘導した。
...
[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) の分割境界を変えても消せない。したがって床を更新した（`scripts/chunk-budget-baseline.json`
