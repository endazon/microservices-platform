...
>
> **したがって検査器を足す**（`scripts/check-openapi-dto-drift.js`。[IADR-0159](../adr/IADR-0159_openapi-dto-drift-checker.md) 決定 3）。
...
> 挙動を変えるので独立した PR とテストが要る（[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 1）。

## 判断

### 判断 1: `granted` を `properties` と **`required` の両方**へ入れる

[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 論点 B の **B1**（既定値つきメンバーも `required` に入れる）に従う。
...
#525 は `AccessScopeResponse` の資源に閉じており（[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 1）、10 件の是正は別の作業なので
...
SC-07 の資源であり、`1 issue = 1 PR`（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1）に反する。**測った費用は #658 へ渡した。**

## 射程外

- **`/bff/*` 5 端点の認可欠落**（#656）。**別資源**（[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md)）。
- **`required` 不一致 10 件の是正**。ラチェットに据え置き、**#658** へ送った（当初 20 件と数えたうちの 10 件は自分の検査器の偽陽性だった）。
- **型の不一致の検査**。C# の `List<X>` と OpenAPI の `array`/`$ref` は 1 対 1 でなく、
  判定器を書くと誤検出の保守コストが上回る（[IADR-0159](../adr/IADR-0159_openapi-dto-drift-checker.md) 決定 3）。**#506 の型誤りは捕まらない。**
