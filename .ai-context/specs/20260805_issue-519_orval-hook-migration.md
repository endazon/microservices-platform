...
- 関連 ADR（計画）:
  ...
  BFF 境界と生成クライアント）
- 関連 IADR: **[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md)（本作業の内部設計判断。本書と対で読む）**・[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md)（OpenAPI を
  BFF 契約の単一情報源とする。**本作業は決定 3 を改定する**。§設計 6）・[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md)（`required` は
  C# の非 null 性から／`?? 既定値` は残す）・[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 3（BFF 境界）・[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md)（SSE と
  検索の URL 状態）・[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md)（決定 3 = 載せ替え前の暫定／決定 5 = invalidate だけ／
  決定 7 = 直近の操作結果）・[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md)（403・404 の中立化と 5xx の区別）・[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）・
  [IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md)（ABAC 管理 API の透過中継と 409 の Problem 詳細）
...
「OpenAPI を変えると型検査が落ちる」という [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 1 の保証は **SC-08 の 1 画面にしか掛かっていない**
...
検索語をキーにした再訪の即時表示（[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) 決定 3・4 が明示した性質）が失われる。
...
5. **既存 2 本の `operationId` を規約へ揃える**（`analysis-ask` / `analysis-analyze` →
   `bff-analysis-ask` / `bff-analysis-analyze`。#506 §未決事項 4）。**[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 3 の改定である**（§設計 6）。
...
8. **変異試験**で「BFF の DTO（＝ OpenAPI）を変えると型検査が落ちる」ことを実測する。
   **#520 の M6 / M7 を再実行する。**
9. 波及する文書を追随させる（通信仕様書・画面仕様書の「通信」節・[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 3 の追記）。

### 対象外（送り先を明記する）

| 事項 | 送り先 | 理由 |
| --- | --- | --- |
| `sc01-search/useAskStream.ts` の **`apiStream`（SSE）** | **やらない（恒久）** | orval は SSE を扱えない（[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 4）。生成物に該当の関数が存在しない |
| `sc08-analysis/useAnalysisTask.ts` の作り | 対象外 | 既に生成フックに載っている。改名の追随（import 1 行）だけ行う |
| **C# → OpenAPI の追随の機械化** | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 2 | 本 issue の射程外。保証の上限は「OpenAPI を変えると落ちる」まで |
...
判断の根拠（選択肢の比較・棄却理由）は [IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) を正とする。本節は**何をどう書くか**を確定する。
...
- 既存の `?? []` は**残す**（[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 3。§設計 5）。
...
- キャッシュキーは現行の `['bff','search', q]` を**維持する**（生成キーが存在しないため。
  [IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) 決定 3 の「検索語がキー」という性質をそのまま保つ）。
...
（モック層の移行を除く）。棄却案は [IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) 決定 3。
...
### 5. `?? 既定値` は消さない（[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 3 の踏襲）
...
### 6. `operationId` の統一（**[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 3 の改定**）

[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 3 は「既存 2 本（`analysis-ask` / `analysis-analyze`）は改名しない——
...
[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 本体には**日付つき［追記］**で改定を記録する（決定を消さない）。
...
- [ ] **画面の挙動が変わっていない**——次が通り続ける（モック層の移行を除きアサーション不変）:
      ...
      `role="alert"` の区別**（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 3。画面内の全クエリに適用）／**SC-09 の 409 の
      Problem 詳細**（[IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md) 決定 2）／**`beginOperation()` による直近の操作結果の表示**
      （[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 7）／**SC-11 の「1 回の再取得で 3 本」**。
...
| # | 事項 | 種別 | 送り先 |
| --- | --- | --- | --- |
| 1 | **テストの fixture が生成型で型付けされていない**（#520 §未決事項 2 は**未解消のまま**）。`jsonResponse(body: unknown)` を経由するため、契約に無いフィールドを持つ fixture も、必須フィールドを欠く fixture も検出されない（実測: M5 の `attempts: 3` は fixture に残っているが型検査に掛からない） | 網の穴 | **[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) フォローアップ 1**。生成 MSW モック（`*.msw.ts` / `*.faker.ts`）へ移すのが筋。**本作業では行っていない**——13 ファイルの fixture を一斉に置き換えると「挙動が変わっていない」ことの確認と混ざる |
| 2 | **C# → OpenAPI の追随は人手のまま**。本作業で届いたのは「**OpenAPI を変えると型検査が落ちる**」までで、「**C# の DTO を変えると落ちる**」ではない | 構造的な穴 | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 2 ／ [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) フォローアップ 1。**載せ替えが済んだいま、残る穴はここだけ**である |
...
| 4 | **SC-02 だけが生成「フック」を使わない**（`/bff/search` が POST であるため。[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) 決定 2） | 自覚した非対称 | 恒久。「生成フックに載っているか」で機械的に検査することはできない——検査するなら「生成物由来の型／関数を import しているか」で見る必要がある |
