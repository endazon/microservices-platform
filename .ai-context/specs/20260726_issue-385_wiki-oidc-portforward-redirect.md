...
- 決定: [IADR-0095](../adr/IADR-0095_wikijs-keycloak-oidc.md)（Wiki.js の Keycloak OIDC 連携）のフォローアップ。edge 集約は [IADR-0091](../adr/IADR-0091_local-edge-aggregation-traefik.md)、
  dev の Wiki.js host 公開（compose `3001:3000`）は [IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md)、ABAC ゲートウェイ前提は [IADR-0020](../adr/IADR-0020_wiki-js-deployment-abac-gateway.md)。
...
不整合の実体は **ポートの取り違え**である。`3001` は **compose（dev）の host 公開ポート**（[IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md)・
...
- `3300` は既に k8s ローカル経路の既定として `deploy/local/README.md` / `values-local.yaml` /
  先行 spec（[20260720_issue-344_frontend-wiki-url](./20260720_issue-344_frontend-wiki-url.md) / [20260725_issue-344_wiki-base-url-edge-alignment](./20260725_issue-344_wiki-base-url-edge-alignment.md)）に
  pin 済み。(b) は これら複数箇所と [IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md) 由来の compose ポート意味論の両方を書き換える広い変更になる。
...
新規 IADR は不要（新たな設計判断は無く [IADR-0095](../adr/IADR-0095_wikijs-keycloak-oidc.md) の適用漏れ修正のため `fix(IADR-0095)` で参照）。
...
7. **誤りの発生源である [IADR-0095](../adr/IADR-0095_wikijs-keycloak-oidc.md) §2 を是正する**。§2 は `3001` を「port-forward 用」と記しており、
   両 README はここから転記された。README だけ直しても ADR を参照した実装者/AI が同じ誤解を再導入するため、
   **履歴不変の原則に従い本文は書き換えず「追記」節で是正**する（[IADR-0032](../adr/IADR-0032_wikijs-dev-exposure-opt-in.md) が [IADR-0026](../adr/IADR-0026_mesh-mtls-supersedes-network-isolation.md) §2 を Amends した前例と同型）。
