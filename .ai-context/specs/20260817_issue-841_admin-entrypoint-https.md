...
#### ★ 規則 8 —— **本書と追記ブロックを書く行為が、母集合を動かした**

上の表は**走査基準 `5ed54b02`（着手時点）の値**である。**その後の是正で、この数は動いた。**

`IADR-0092` / `0093` / `0095` へ入れた追記ブロックは、「本文の記述 → 現在」の読み替え表の中で
**旧 URL を `http://…:50000` の literal な形で引用している**。**引用もヒットする。**

```
$ git grep -I -o -e 'http://[A-Za-z0-9.-]*localhost:50000' -- . ':!planning' ':!src/ai-stock-trading' | wc -l
44
```

**44 → 自己参照 5 件（追記ブロックの引用）を引く → 39** が、着手時点の残存 27 + 12 と一致する。
...
| `IADR-0093` | 2 | **4** | +2 |
| `IADR-0095` | 3 | **5** | +2 |
| `README.md` / `IADR-0084` / `docs/specs/`（10 ファイル） | 4 / 1 / 27 | 4 / 1 / 27 | ±0 |
| **合計** | **39** | **44** | **+5** |

**これは規則 8 が名指しする事故そのものである** —— 規約が挙げる先例（planning#350）は
「**26 行と書いた当の表が検索語を 1 つ含んでおり、公開時点では 27 行だった**」であり、本件は
**5 件・3 ファイル**の同型である。**AI レビューが指摘するまで引き直していなかった。**

**引用を消して数を合わせることはしない。** 追記ブロックの読み替え表は「本文がどう書かれていたか」を
示すためのもので、**旧 URL を書かなければ読み替えが成立しない**。したがって
**数の側に時点と引き算を明記する**（規則 8 が指示する形）。

**追試する人へ**: 上の 44 は**本節を含む HEAD** の値である。本節自身はコードフェンス内にしか
検索語を持たないため母集合を動かさない（`http://…:50000` の literal を地の文へ書いていない）。
...
| `docs/adr/IADR-0091` §決定 3 の 2026-08-16 追記 | 「[IADR-0103](../adr/IADR-0103_local-sso-persistence-and-claim-design.md)（admin:50000 は平文 http）は動いていない」 | **是正**（日付つき追記。**誤帰属＋陳腐化の 2 点**。下記 §2.5） |
...
`related_ids` への `IADR-0220` 併記で置いた。先例は [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md)（[IADR-0219](../adr/IADR-0219_sharedkernel-granularity-and-worker-standard-component.md) が決定 5 を部分改定したとき、
...
先例 [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) の索引行も `Accepted` のままで、部分改定は**タイトル列の文中**に書かれている。
...
[IADR-0220](../adr/IADR-0220_admin-entrypoint-tls-and-http-redirect.md) §決定 5 が「[IADR-0092](../adr/IADR-0092_argocd-keycloak-oidc.md) の**理由の記述だけ**を改める」と宣言していたのに、
...
| --- | --- | --- | --- | --- |
| [IADR-0092](../adr/IADR-0092_argocd-keycloak-oidc.md) | `Accepted` | 50 行「edge は平文 http のため `server.insecure=true`」/ 83 行「dev の edge（平文 http）前提」 | `argocd-cm.url` を https へ | **追記** |
| [IADR-0093](../adr/IADR-0093_minio-keycloak-oidc.md) | `Accepted` | 46 行 `MINIO_BROWSER_REDIRECT_URL=http://…` / 47 行 redirect `http://…/oauth_callback` | `values-local.yaml`・realm を https へ | **追記** |
| [IADR-0094](../adr/IADR-0094_vault-keycloak-oidc.md) | `Accepted` | 48 行「**現状 http** だが将来の TLS 化に備え http/https 両方を登録」/ 78 行 | realm と `bootstrap.sh` から **http 側を削除** | **追記** |
| [IADR-0095](../adr/IADR-0095_wikijs-keycloak-oidc.md) | `Accepted` | 50 行 redirect `http://wiki…/*` / 58 行 **Site URL=`http://wiki…`** / 104 行の表 | realm・`wiki-oidc/README.md` を https へ | **追記** |

**追記の作法は [IADR-0091](../adr/IADR-0091_local-edge-aggregation-traefik.md) / [IADR-0206](../adr/IADR-0206_local-edge-tls-cert-manager.md) と同一である** —— `［2026-08-17 追記 / #841］` の
...
> **しかし「後継への導線を置かない」ことまでは正当化できない** —— [IADR-0091](../adr/IADR-0091_local-edge-aggregation-traefik.md) と [IADR-0206](../adr/IADR-0206_local-edge-tls-cert-manager.md) には
...
| [IADR-0084](../adr/IADR-0084_headlamp-oidc-apiserver-flags.md) | 軸C 1 | **[IADR-0105](../adr/IADR-0105_remove-apiserver-oidc-flag-wiring.md) が Superseded にした非 live** |
...
| [IADR-0103](../adr/IADR-0103_local-sso-persistence-and-claim-design.md) | — | **触らない**（何も間違っていない。§2.5） |

**`docs/adr/README.md` の索引行 4 行（148-151）は触らない。** 理由は [IADR-0206](../adr/IADR-0206_local-edge-tls-cert-manager.md) の索引行と同じである ——
...
| ブラウザ・`curl` が証明書を検証できるか | 信頼ストアは環境の側にある（[IADR-0206](../adr/IADR-0206_local-edge-tls-cert-manager.md) の「検出しないこと」と同じ） |
