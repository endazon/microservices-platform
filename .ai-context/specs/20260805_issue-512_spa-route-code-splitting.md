...
> **内部設計の判断（分割境界・棄却した案・計測の解釈）は [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) を正とする。**

## 起点となる計画書（トレーサビリティ）

- **NFR（非機能要件）**: 計画
  02_requirements §非機能要件（計画リポ） の**性能**。
...
- **ADR（計画）**: ADR-0031（計画リポ）（Accepted。
  React 19 / Vite / **TanStack Router** / TanStack Query / Tailwind v4 ＋ shadcn/ui / Lingui）／
  13_frontend-stack（計画リポ）（fixed）
- **画面（SC）**: 分割の単位が画面（ルート）であるため **SC-01〜SC-11** が対象になる
  （SC-04 は SPA 側の遷移導線のみ）。画面の内容は変えない。
- 関連 IADR: **[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md)（本作業の内部設計判断。本書と対で読む）**・
  [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md)（移行の 5 段）・[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md)（型付きルート木・合成点）・
  [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)（UI プリミティブ・i18n）・[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）・[IADR-0056](../adr/IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット分離）
...
2. **ルート単位の遅延読み込み**（TanStack Router の `lazyRouteComponent`）。対象は
   ...
   型付きルート構成は壊さない**（[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 1）。
3. **初期チャンクに残すものの明示と機械的な固定**（共通シェル・認証・`@platform/ui` のプリミティブ）。
4. **`build.rollupOptions.output.manualChunks` の併用**（実測に基づく。§計測・[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md)）。
5. 分割境界の**回帰ガード**（単体テスト 2 本 ＋ E2E 1 本）。
6. 分割の境界と根拠を **[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md)** に記録する。

### 対象外（送り先を明記する）

| 事項 | 送り先 | 理由 |
| --- | --- | --- |
| **i18n カタログのロケール別遅延読み込み**（ja / en の両方を初期チャンクへ載せている。実測 25.89 kB rendered） | 本書 §申し送り | Lingui の初期化（`initI18n`）と カタログの機械検査（[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 4）に触れる。本 issue の射程は「ルート単位」である |
...
| **AST（`src/ai-stock-trading`）3 画面の遅延化**（実測 62.09 kB rendered） | 本書 §申し送り | 旧契約（`FeatureModule.routes[].element`）は**モジュール初期化時に React 要素を作る**ため、本リポ側だけでは遅延化できない。AST は変更できない（[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md)） |
...
[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 1 の型付きタプル合成と `<Link to>` / `useSearch({ from })` の型安全は無傷である
...
**共通シェルごと空白になる**。理由と棄却案は [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) 決定 2 を正とする。
...
React ランタイムは別建てにする（理由は [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) 決定 3）。
...
受け皿は §申し送り 6 に置いた（[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) §結果 のトレードオフにも明記した）。
...
- [x] **初期チャンクのサイズと分割境界の根拠が [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md)・本書に記録されている。**
      根拠は**計測値に紐づく**こと（「重そうだから」で切らない）。
- [x] **分割前後の実測値を並べて記録した**（初期チャンク・遅延チャンクそれぞれ・gzip 込み）。
- [x] **合成点の型付きルート構成を壊していない**（[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 1。`pnpm run typecheck` が green で、
...
（[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) §結果 のトレードオフ。**採用はしない**——後述）。V5 との差は `ui` 規則に
2 行の除外を足すだけで、`@platform/ui` の公開面（`src/index.ts` の 1 ファイル）・ESLint の深い参照禁止・
[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 は**いずれも無改変**である。動いたのは次の 2 本だけで、他は V5 と完全に同じだった。
...
読み取れること（**採用の根拠であり、[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) 決定 3 の一次資料**）:
...
- **V5b は「Radix が初期側に来る理由」を切り分けた。** 差の実体は `Tabs` が引き込む Radix
  ...
  [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 ではなく `ui` の `manualChunks` 規則である**。V1（`manualChunks` なし）では
...
**ただし移すだけでは「その入口に閉じる」にならない。** `@testing-library/dom` の `configure()` は
**グローバル設定**を書き換えるため、`renderUnitRoute` の中で呼んでも効くのは「その 1 回の描画」ではなく
**そのテストファイル全体**である。つまり同じファイル内に「すぐ失敗すべき」検証を後から足すと、
横断 setup で避けようとしたのと同じ経路がファイル単位で再現する。
そこで **`renderUnitRoute.tsx` の冒頭で既定値を捕まえ、`afterEach` で戻す**ようにした。
これで延長を受けるのは**この入口を実際に呼んだテストだけ**になる
（Vitest はテストファイル単位で環境を分離するため、他のファイルへは元々漏れない）。
...
`afterEach` での復元を足した後も**さらに 3 回連続 green**（59 files / 557 tests・カバレッジ不変）である。
...
1. **i18n カタログのロケール別遅延読み込み**（初期チャンクに ja＋en の両方が載っている。実測 25.89 kB rendered）。
   ...
   [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 3・4（カタログのコミットと再生成差分検査）に触れるため別 issue が要る。
2. **AST 3 画面（62.09 kB rendered）の遅延化** —— **［起票先: 未起票］**。
   旧契約（`FeatureModule.routes[].element`）がモジュール初期化時に React 要素を作るため、
   本リポ側だけでは遅延化できない。[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 2 が既に挙げている
   ...
   **起票先は ai-stock-trading リポジトリ**であり、番号は `AST#NNN` の形で修飾して書く
   ...
   本リポジトリからは AST を変更できず（[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md)）、起票の可否も確認していないため、
...
3. **チャンク構成の機械検査**（変異試験 M6 / M7 が素通りした穴。**［起票先: 未起票 / 本リポジトリ］**）。
   ...
   作法は [IADR-0118](../adr/IADR-0118_backend-coverage-floor.md)（検査スクリプトの追加）・[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md)（baseline による ratchet）・
   [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md)（床の更新手順と変異試験）に倣う。
   ...
   - **判定 2（fail・ratchet）**: **初期ロード JS の合計**（minified・gzip の 2 値）が
     ...
     （[IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) と同じ「更新は明示・差分はレビューで見る」運用）。
...
4. **`@platform/ui` を初期チャンクに置く判断の再考余地**（§計測 の V2 と V5 の差 = **gzip 14.31 kB**）。
   ...
   **この Radix を初期側へ引き込んでいるのは [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1（公開面は `src/index.ts` の 1 ファイル）ではなく、
   本作業が入れた `ui` の `manualChunks` 規則である**——§計測 の V5b が実測で、
   **`ui` 規則から `Tabs.tsx` と barrel だけを除く 2 行**（公開面・ESLint・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) はいずれも無改変）で
...
6. **`build:analyze` が抱える開発依存 11 件を減らす** —— **［起票先: 未起票 / 本リポジトリ］**。
   ...
   **判断の期限は「次に分割を見直すとき」**——[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) の全ての数字がこの計測に依存しており、
...
7. **IADR の採番**: 本 PR は当初 `IADR-0133` を採ったが、**`IADR-0134` へ改番した**
   ...
   `develop`（`9dc6c13`）に一時 **`IADR-0132` が 2 件あった**——#520 / PR #528 の
   `IADR-0132_openapi-required-from-csharp-nullability.md`（先着）と #526 の
   `IADR-0132_abac-dev-seed.md`（後着）が相次いでマージされ、同番のまま残った。
   先着尊重により後着の #526 が `IADR-0133` へ動くため、本 PR がその番号を空けた。
   **その改番は #530（`6a0336e`）で `develop` へ入り、採番衝突は解消している**——
   `IADR-0130`〜`IADR-0134` は欠番なしで揃っている。経緯は `docs/adr/README.md` の
   索引行（〔採番注記〕）にも残した。
