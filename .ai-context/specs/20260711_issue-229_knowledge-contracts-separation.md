...
- 実装判断: [IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md)（契約階層化・イベント移設・URN 固定・DTO/BFF 繰延）
- Issue: #229（フォローアップ 3）
...
- 対象外（[IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md) の後続スライス。#229 に follow-up として残す）:
  - knowledge 固有 DTO の Knowledge.Contracts 移設（BFF の DTO 依存解消とセット）。
  - BFF のユニット別エンドポイント合成方式。

## URN（当初は後方互換固定 → #227/IADR-0062 で撤廃）

> **更新（2026-07-11・#227/[IADR-0062](../adr/IADR-0062_namespace-assembly-unit-rename.md)）**: 本スライス当初は `[MessageUrn]` で旧 URN
...
- [~] 既存 6 イベントの後方互換 → **当初は `[MessageUrn]` 固定で維持したが、#227/[IADR-0062](../adr/IADR-0062_namespace-assembly-unit-rename.md) で
...
- [~] 可変機能ユニット追加時に platform 側の契約・BFF を改修せず拡張できる → **イベント契約について達成**
  ...
  [IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md) の後続スライス（#229 継続）。本 PR は `Refs #229`（Closes ではない）。
...
- 方式・トレードオフ（当初の URN 固定 → #227/IADR-0062 で撤回・DTO/BFF 繰延）は [IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md) に記録。
