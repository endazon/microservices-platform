...
| 同 `docs/screens/SC-01_search-chat.md` §実装しない要素の理由 (b)（当時の経緯説明） | **対象外**（規則 6 の除外理由）。**「以下は当時（実装前）の記録である」配下の凍結された過去記録**であり、モックの原文をそのまま引用している。**生きたガイダンスではない**ため書き換えない —— 実装時に読むのは対応表（モック #11）の側であり、そこへ注記を足した |
