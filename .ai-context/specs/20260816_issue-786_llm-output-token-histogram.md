...
| ③ 出力トークンの Histogram を足すか判断する | **本 PR ＝ 足す**（[IADR-0212](../adr/IADR-0212_llm-output-token-histogram.md)） |
...
## 4. 決定（詳細は [IADR-0212](../adr/IADR-0212_llm-output-token-histogram.md)）
...
明記している。本 PR が埋めるのは**トークン消費量まで**で、**フォールバック発火回数（決定 2）と単価表・金額換算（決定 3）は
未着手**である。
対応表は [IADR-0212](../adr/IADR-0212_llm-output-token-histogram.md) の起点・関連に置いた。
...
**Counter は全 8 経路で計上する**（[IADR-0110](../adr/IADR-0110_llm-completion-stop-reason-metrics.md)・分母を欠けさせない）。
**Histogram は `result=sent` の 2 経路だけ**である。残り 6 経路にはトークン数が概念的に存在しない
（送信していない／応答が返っていない）。**この非対称は意図であり、[IADR-0212](../adr/IADR-0212_llm-output-token-histogram.md) 決定 3 に書く。**
