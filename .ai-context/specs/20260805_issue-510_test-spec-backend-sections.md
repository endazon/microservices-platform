...
- 非機能（**NFR**）: 検査基盤（退行防止ゲート）。規約は
  [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md) と
...
| 4 | 再発防止の機械検査 | `scripts/check-test-spec-coverage.js` ＋ baseline ＋ [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) |
...
> 手順としては正しい（実装エージェント側の権限では触れない）が、本書と [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) が
...
3. **`scripts.test.js` を変更できない**（[IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 分類 A・キットとバイト一致）。新規検査器は
...
- 実測（**本 PR 適用後の作業ツリー**。母集合は `src/{platform,knowledge}/frontend` ＋ `src/packages`
  ...
  **ファイル名で**参照していないものが **28 件**ある。
- **内訳**（［2026-08-05 訂正］当初の記述は 23 件しか説明しておらず **E2E 5 件が漏れていた**。
  再現コマンドと区分表は [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) §決定 5 を正とする）:
  **foundation 15 ＋ `@platform/ui` 6 ＋ 画面単体 2 ＋ E2E 5 ＝ 28**。
- 前 2 区分（21 件）は基盤テストで、画面の受け入れ基準に紐づかない。
- **画面テスト 2 件**（**SC-01 の `citations.test.ts` と SC-04 の `WikiAccessPage.test.tsx`**）は
  **節そのものは存在し**、仕様書がディレクトリ名で参照しているために「ファイル名では見つからない」
  だけである。
- **E2E 5 件**（`platform/frontend/e2e/` の `login` / `sc01-search` / `sc02-results` /
  `sc03-document` / `sc04-wiki` の smoke spec）は**ディレクトリ名ですら参照されておらず、
  `docs/tests/` から一切参照されていない**（SC-05〜SC-11 は自分の smoke spec をファイル名で挙げて
  いるのに SC-01〜SC-04 は挙げていない）。**[IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) §フォローアップ 2 の射程に含める。**
- ここを fail にすると、**仕様書の書き方の統一**という別件の作業を本検査が強制することになる
  （E2E 5 件は「様式の不揃い」ですらないので、なおさら別件の作業になる）。
- バックエンドは表の `ケース` 列にメソッド名、見出しにクラス名を書く様式が定着しており、
  ファイル名キーが実務と一致する。
- フロントエンドへの拡張は、仕様書の参照様式を先に揃えてから別 issue で行う（[IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) §フォローアップ）。
...
> [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) §決定 6 の追記・§フォローアップ 3（**完了**）と対。

## 実装しないこと（と、その理由）

| 事項 | 理由 |
| --- | --- |
| 方向 (a)（仕様書が挙げるテスト名の実在検査） | **今回の欠陥を止めない**（上表）。散文中の識別子をテスト名と判別する必要があり、誤検出の設計が本題（b）より重い。別 issue の候補として [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) へ残す |
...
- **変異試験**（6 種。いずれも実測して報告に貼る。実行後は必ず復元し `git status` で汚れが
  ...
  > [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md) §変異試験が見つけた穴 の追記および §限界 2 の追記を正とする。
