...
## ［2026-08-28 追記 / #941］本書の前提の一部はもう成り立たない —— 続きは第 2 巡へ

**本書「設計判断」の器まわりの記述と、`GraphServiceFactory` に置いた注記
「GraphService は Program.cs でメッセージングを一切構成しない（実測）」は、
2026-08-28 時点で偽である。** `#1016`（graph-delete 段）と `#911`（graph-sync 段）が
GraphService へ Wolverine ホストを入れたため、器がブローカを渡さないと
**6 件は防壁へ到達する前にホスト起動で落ちる**。

**本書の当時の実測が誤っていたのではない**（書いた時点では真だった）。5 日で腐ったのであり、
腐ったことに気付けなかったのは **6 件が一度も実走していない**ためである —— 本書自身が
「6 件とも skip」と記録したその状態が、退行を隠す側に回った。

是正と、そこから引いた一般則は次を正本とする（本書の本文は当時の記録として書き換えない）。

- `.ai-context/specs/20260828_issue-941_edge-type-db-guard-verification.md`
- `IADR-0291`（器は対象サービスの起動時依存に追随させ、欠落は型で止める）
