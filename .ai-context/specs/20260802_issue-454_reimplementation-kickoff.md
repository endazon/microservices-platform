...
- 計画書リンク: `planning/projects/microservices-platform/INDEX.md`（計画リポ）
...
副作用として、**AST との ID 衝突の性質が変わる**。AST 側が自前で採番しているレンジは
**`FR-01..20` / `UC-01..07` / `SC-01..03`**（submodule pin `655e2ed` 時点で実測）。AST の文書には他の `SC`
番号も現れるが、いずれも MSP 画面への**参照**であって AST の採番ではない（`05_screens/01_screens.md` の
`SC-13` / `SC-16`、`07_adr/ADR-0012_mcp-exposure-policy.md` の `SC-11`）。MSP のレンジ拡大により、
重なり方は次のとおり変わる。

| ID レンジ | 改定前 | 改定後 | 修飾を欠いたときの症状 |
| --- | --- | --- | --- |
| `FR-16`〜`FR-20` | AST のみ | **双方に存在**（意味は異なる） | **参照切れ → 誤帰属へ変化**（検査で捕まらない） |
| `FR-01`〜`FR-15` / `UC-01`〜`UC-07` / `SC-01`〜`SC-03` | 双方に存在 | 双方に存在 | 誤帰属（従来どおり） |
| `FR-21` / `UC-08`〜`UC-11` / `SC-04`〜`SC-21` | — | MSP のみ | 衝突しない |

すなわち**今回新たに危険側へ移ったのは `FR-16`〜`FR-20` の 5 件**である。裸で書けば従来は「計画書に
存在しない ID」＝参照切れとして目立ったものが、MSP 側に同番号が生まれたことで**別プロジェクトの要求へ
静かに紐づく**ようになった。`SC-18`〜`SC-21` は MSP のみで、AST 側に対応 ID が無いため衝突しない
（将来 AST が採番を伸ばせば同じ型へ移る）。この差は検査で捕まらないため、規約本文に明記した。
...
- ブランチ名は `<種別>/<起点ID>-<概要>`。起点 ID は子 issue タイトルのスコープ `()` を左から見て
  **最初に現れる具体 ID**（`FR` / `UC` / `SC` / `ADR`）を採り、`NFR` は**併記の有無にかかわらず読み飛ばす**
  （具体 ID が 1 つも無いときに限り `NFR` を使う）。
