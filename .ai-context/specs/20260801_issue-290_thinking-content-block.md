...
- 直前の実装判断: [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md)（月報の ZDR 対応モデルへの改定）。同 IADR は残作業として
...
- 本作業の実装判断は [IADR-0114](../adr/IADR-0114_anthropic-unknown-content-block-sanitizing.md)。
...
[IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) / [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) 以降の `PurposeModels` は次のとおりで、`claude-opus-5` /
`claude-sonnet-5` は **thinking（拡張思考）が既定で有効**、`claude-fable-5` は **常時有効で無効化不可**
（無効化指定は 400）である。[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) の注記どおり `MaxTokens` を思考込みで見積もる対処は入って
...
- **`Anthropic.SDK` のバージョン**（4.0.0 のまま）。上げても未知型で fail-closed する構造は変わらない
  （4.0.0 が `server_tool_use` ですら落ちるのが実証）。詳細は [IADR-0114](../adr/IADR-0114_anthropic-unknown-content-block-sanitizing.md) §検討した選択肢。
- **リクエスト側のパラメータ**（`thinking` / `effort` / `temperature` 等）。`ClaudeProvider` の
  「Opus 5 で 400 になるパラメータを持ち込まない」方針（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md)）は不変。
- **ストリーミング経路**（事実 3 のとおり壊れていない）。
- **割当モデル・ルーティング・機密区分**（[IADR-0112](../adr/IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) / [IADR-0113](../adr/IADR-0113_report-monthly-zdr-model.md) の決定は不変）。
- **[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md) の安全既定**（`refusal` は本文を返さない）。thinking の有無に関わらず維持する。
...
- [x] `refusal` の本文破棄（[IADR-0104](../adr/IADR-0104_llm-stop-reason-refusal.md)）が維持される。
