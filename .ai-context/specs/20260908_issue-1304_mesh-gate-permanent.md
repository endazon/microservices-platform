...
## ★［2026-09-08 追記 / #1328 レビュー］🔴 退路の式が falsy 罠で死んでいた
...
**`istio` の値に関わらず常に `'1'` になる。** `&&` は最初の falsy をそのまま返し、`''` は falsy なので
続く `|| '1'` がそれを上書きする。真理値表を実際に計算した:

| 契機 | `inputs.istio` | 初稿 | 是正後 |
| --- | --- | --- | --- |
| `schedule` | — | `"1"` | `"1"` |
| `push` | — | `"1"` | `"1"` |
| `workflow_dispatch` | `true` | `"1"` | `"1"` |
| `workflow_dispatch` | **`false`** | 🔴 **`"1"`** | ✅ `""` |

**既定 ON 側は偶然正しい**ので、動かしても気づけない。死んでいたのは
**「比較のためメッシュ無しで起こす退路」だけ**である。

**是正**: 真になる分岐が truthy 値を返すように条件を反転する。

```yaml
ISTIO: ${{ (github.event_name != 'workflow_dispatch' || inputs.istio) && '1' || '' }}
```

### 🔴 自己診断が部分一致だったので、この欠陥を通した

初稿の試験は `line.includes("|| '1'")` / `line.includes('!inputs.istio')` と
**式のソース文字列を部分一致で見ていた**。式がどう評価されるかは 1 つも主張していない。
**4 本とも緑で通った。** これは「試験を通すためだけの表面的な検査」であり、
本リポジトリが繰り返し戒めている形そのものである。

**是正**: 式を YAML から取り出し、**3 つの契機で評価して値を主張する**（真理値表）。
GitHub Actions の `&&` / `||` / `!` / `==` / `!=` は JS と同じ短絡規則で定義されているため、
式をそのまま JS として評価してよい。

**変異試験**: 初稿の式へ戻すと
`🔴 istio=false の退路が死んでいる（\`A && "" || "1"\` の falsy 罠。PR #1328 の初稿がこれだった）`
で**新しい試験が赤になる**（実走で確認）。**部分一致の旧試験はこの変異を通していた。**
