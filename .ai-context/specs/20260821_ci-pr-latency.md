...
- キャッシュは**2 回 push して**比較する。
  ...
  **develop が温まっていれば新規 PR ブランチは初回からヒットする。**
  実測（MSP）: PR ブランチの 2 回目以降は完全一致ヒット
  （`Cache restored from key: nuget-Linux-7655d09…`。run `32499045863` / `32502445460`）。
  一方 **develop へマージ直後の初回 run は `Cache not found`** —— PR ブランチが書いたキャッシュは
  develop からは見えないためであり、壊れているのではない（run `32504292683`）。
  **したがって「ミスした」を見たら、まずどのブランチのスコープを見ているかを確かめること。**
...
### ［2026-08-22 追記 / 第 2 弾］逆転監視（`ci-latency-watch.yml`）

**受け入れ基準（追加）**

10. `ci-latency-watch.yml` が**必須チェックに入っていない**（監視自体が待ち時間を増やさない）。
11. `check-ci-latency.js` が**両方向で正しく振る舞う** ——
    逆転しているリポで鳴り、CI 構成の変更をまたいだサンプルでは判定を skip する。
    **片方向だけの確認では「常に鳴る／常に鳴らない」実装と区別できない。**

**検証（実測済み・2026-08-22。両方向）**

- 本リポは **判定を skip**（最小 126 秒 / 中央値 456 秒 = 3.6 倍 → 定常性の門）。
  AST は **exit 1 で鳴った**（中央値 223 秒 > 下限 146 秒）。
- 🔴 **実測して初めて分かった罠を 2 つ潰した。どちらも「監視が付いている」という記録だけを
  残して機能しない壊れ方である。**
  1. **集約ジョブを自身の所要で測っていた。** 本リポの `build-and-test` は決定 2 で
     `needs:` により matrix の脚を束ねる集約ジョブになっており、所要は **15 秒**しかない
     （脚が動いていた 111 秒はそこに現れない。実測・#894 の head）。
     **実際の 1/8 に見え、監視は永久に鳴らなかった。**
     尺度を **check 群の開始からの完了オフセット**（同 126 秒）へ改めた。
     🔴 **決定 2 で「名前を保つ」と決めたことの副作用である** ——
     **名前が同じでも中身が集約ジョブへ変わった以上、過去の run と同じ尺度では測れない。**
  2. **中央値が消えたはずの旧構成を指していた。** 最適化直後のサンプル 12 本のうち
     ...
     **代償として、窓の途中で実際に 2 倍へ伸びた場合も検知が 1 週遅れる**（意図した倒し方）。
