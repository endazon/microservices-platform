...
- 制約: [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md)（多層防御）／[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1（グループ既定を残して `AdminOnly` を積む）／
  [IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1（押せないボタンを置かない）／[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) 決定 2（実効境界はサーバ側）
...
| クライアント | 叩く先 | 保有ロール | 影響 |
| --- | --- | --- | --- |
| `ai-stock-trading-kb-writer`（AST/FR-08） | **`POST /documents`（BFF を経由せず直接）** | **`platform-operator` のみ**（[IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md) が最小権限を理由に admin を却下） | **`AdminOnly` を積むと 403 で止まる** |
...
[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1 が #501 で確立し **#628 が踏襲した形**であり、
...
### 判断 4: 画面は**ロールで出し分ける**（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1）
...
**既存の決定を変えていない。** [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1（積む形）・[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1（押せないボタンを置かない）・
[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md)（多層防御）を**そのまま適用する**だけである。
...
[IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md)（Accepted）は **`platform-admin` の付与を最小権限を理由に明示的に却下**している。
...
| サービス側の `PUT` / `PATCH metadata` / `publish` / `archive` / `DELETE` | **`AdminOnly`**（機械クライアントはこの 5 口を呼ばない。[IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md) 自身が「構造上 `POST` しか発行しない」と記録している） |
...
| [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) 決定 | 「`DocumentService`: 書き込みに admin/operator を要求」 |
| [IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md) 決定 1 | 「書き込みは platform-admin/operator に限定する」 |
| `docs/security/security.md` | 文書書き込み＝admin/operator（`updated:` も 2026-07-19 のまま） |

**除外したもの**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 6 に従い理由を書く）:

| 除外 | 理由 |
| --- | --- |
| `docs/specs/` の過去 PR の作業仕様書 4 件 | **確定済み（書いた時点の記録）**。`.claude/rules/traceability.md` が書き換えを禁じている |
| [IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md) / [IADR-0045](../adr/IADR-0045_bff-document-write-roundtrip-deferral.md) / [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) / [IADR-0047](../adr/IADR-0047_document-confidentiality-server-validation.md) | **`POST` を据え置いたので記述が古くならない**（0075）／**IADR-0044 が課した内容の引用**であって独自の決定ではない（0045）／**別資源**の話である（0042 は変換ジョブ・0047 は機密区分検証） |
...
**残った箇所はすべて正当である**（実測で 1 件ずつ確認した）: 原決定の本文（追記が直後に付く [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) / [IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)）／
**読み取り**についての記述（`IADR-0044` の「読み取りを塞がない」・`SC-05:195` のルートゲート・`SC-05:120` の閲覧者）／
**据え置いた `POST` の記述**（[IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md)・本書・環流記録）／履歴の地の文（`SC-05:237`）／確定済みの `docs/specs/`。
...
**さらに、是正の途中で新たに誤りになる自分の記述も走査対象である**（🔴 で [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) の
...
- [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) の 2026-08-09 追記（#628 が書いた「SC-05 は同型の逸脱が残っている」）を**解消済みへ**
- **［追補 2 で追加］** `docs/api/openapi.yaml`（書き込み 5 口の `summary` / `403`。**orval 生成物も再生成**）／
  [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) 決定／[IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md) 決定 1／`docs/security/security.md`
...
- **★ サービス側 `POST /documents` は裁定待ちの据え置きである**（§追補 1）。
  環流記録は `feedback/20260809_document-write-machine-client.md`（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260809_document-write-machine-client.md` へ移設）、
