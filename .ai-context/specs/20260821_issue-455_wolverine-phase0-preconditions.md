...
## 🔴 他人の数えを検証せず転記した（AI レビューの 🟢 指摘）

検査器ヘッダの「既知の限界」に **「本番 13 呼び出しのうち 6 がこの形（＝見えない）」**と書いたが、
実測は **総数 13 / 見える形 6 / 見えない形 7** である。**「6」は見えない形ではなく見える形の数**だった。

```
git grep -nE 'bus\.Publish[A-Za-z]*\s*\('              … 13（総数。テスト除外）
git grep -nE 'bus\.Publish[A-Za-z]*\s*(<|\(\s*new\s)' …  6（見える形）
差 7 = ConversionJobEndpoints:70 / DocumentEndpoints:92,128,159,188,205 / TagDictionaryEndpoints:136
```

**根本原因は数え間違いではない。** この数値は**射程調査のサブエージェントの報告に書かれていた値を
そのまま書き写した**ものである。`.claude/rules/traceability.repo.md` は

> issue 本文の「反映先」は母集合ではない。**他人の数えを検証せず転記しない。機械検査は無い。**

と定めており、**その規則を破っていた**。しかも本作業は「母集合を自分で引く」「既存記述の実測値を
是正する」ことを主題にしており、**主題そのものに違反した**ことになる。

🔴 **「サブエージェントに数えさせた」は「自分で引いた」ではない。** 規則が言う「他人」には
自分が起動したエージェントも含まれる。エージェントの報告は**母集合の候補**であって母集合ではない。
本仕様書の他の数値（15 / 13 / 36 / 65 / 7）は自分でコマンドを叩いて確かめたが、
**この 1 つだけ確かめずに通した**。

**是正**: ヘッダへ**数え方のコマンドを数値の隣に書き残した**（再現できない数値を残さない、という
既存方針と同じ）。あわせて 🔴 **`DocumentUpdated` の業務上の主経路（`DocumentEndpoints.cs` の
5 箇所）が 1 件も見えていない**ことを明記した —— 表に発行元が出るのは別の呼び出しが
拾えているからであって、網羅しているからではない。
