...
- 関連 ADR: [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（本 PR で作成・BFF 集約とロールゲーティング）、[IADR-0035](../adr/IADR-0035_frontend-role-based-nav-and-existence-hiding.md)（存在秘匿ナビ）、[IADR-0019](../adr/IADR-0019_datasource-default-attributes.md)（機密区分フェイルセーフ）
- Issue: #132（親 #121）

## 目的・背景

SPA 上に SC-06 を実装する。DataSourceService（`/datasources`）は実装済みだが BFF 未プロキシのため、Wave B 方針に従い本 PR で BFF 集約（`/bff/datasources`）も併せて実装する。データソースは文書 ABAC のスコープ対象ではなく運用資産のため、管理者・運用者に限定する（[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)）。
...
- [x] 画面仕様書を作成（[SC-06_datasource-management.md](../../docs/screens/SC-06_datasource-management.md)）— 計画・UC-04 と整合。
...
- 認可は BFF を実効境界とし、UI は表示制御専用（[IADR-0035](../adr/IADR-0035_frontend-role-based-nav-and-existence-hiding.md)）。権限外は 403/401（データソースは存在秘匿対象外。[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) §決定 3）。
