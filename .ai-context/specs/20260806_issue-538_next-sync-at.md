...
- 画面（SC）: **SC-06 データソース管理画面**
  （05_screens/01_screens.md（計画リポ） §SC-06
...
- 関連 IADR: [IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md)（本作業の設計判断）・[IADR-0051](../adr/IADR-0051_datasource-connector-port-and-filesystem.md)（定期同期ワーカー）・[IADR-0074](../adr/IADR-0074_datasource-periodic-sync-helm-wiring.md)（Helm 配線）・
  [IADR-0083](../adr/IADR-0083_datasource-sync-single-writer-advisory-lock.md)（単一書き手化）・[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md)（`required` は C# の非 null 性から）・[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md)（OpenAPI が BFF 契約の正）
- 環流の経緯: feedback/20260805_sc05-07-admin-contract-gaps.md（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260805_sc05-07-admin-contract-gaps.md` へ移設）
...
（[画面仕様書 §実装しない要素 (b)](../../docs/screens/SC-06_datasource-management.md)）。裁定により
...
| 3 | 配備（k8s） | `values.yaml` `services.datasource.dataSourceSync`（`enabled: true` / `intervalSeconds: 300`）→ env `DataSourceSync__Enabled` / `DataSourceSync__IntervalSeconds`（[IADR-0074](../adr/IADR-0074_datasource-periodic-sync-helm-wiring.md)） | **300 秒・有効** |
...
| 同期健全性（連続失敗回数・再試行上限・直近エラー。裁定 Q14） | 別作業。琥珀の充て先は [IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2 のまま空けておく |
| データソース更新 API（裁定 Q16） | 別作業 |
| 次回実行時刻の**永続化**（DB 列） | 導出値であり状態ではない。§設計 決定 4 |

## 設計

### 決定（詳細と却下案は [IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md)）
...
6. OpenAPI では **`nullable: true` かつ `required` に入れない**（[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 論点 A の A1: `required` は
...
前進しない**（[IADR-0051](../adr/IADR-0051_datasource-connector-port-and-filesystem.md) 決定 3a）ため過去の時刻を「次回」として出す。
...
| # | ファイル | 変更 |
| --- | --- | --- |
| 1 | `Knowledge.Contracts/Dtos/DataSourceDto.cs` | 末尾へ `DateTimeOffset? NextSyncAt = null`（**既定値つき＝非破壊**。[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 判定） |
...
| # | 観点 | 起点 | テスト |
| --- | --- | --- | --- |
| S1 | 未起動なら次回は無い | 裁定 Q15 / [IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md) 決定 2 | `NextRunAt_WhenNeverStarted_IsNull` |
...
| W2 | 有効なら起動時刻を起点に記録する | [IADR-0051](../adr/IADR-0051_datasource-connector-port-and-filesystem.md) | `StartSchedule_WhenEnabled_AnchorsAtStartup` |
...
| E2 | 無効時は null | [IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md) 決定 2 | `ListDataSources_WhenPeriodicSyncDisabled_ReturnsNullNextSyncAt` |
| B1 | BFF が透過する | [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) | `GetList_PassesThroughNextSyncAt` |
...
規則は §設計 決定 3・コード注釈・[IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md) §限界 の 3 箇所に残した。
...
3. **マルチレプリカでの位相差**（[IADR-0136](../adr/IADR-0136_next-sync-at-from-worker-cadence.md) §限界）。応答するレプリカ自身の位相を返すため、
   実際に同期を行うレプリカ（advisory lock を取った側・[IADR-0083](../adr/IADR-0083_datasource-sync-single-writer-advisory-lock.md)）とは最大 1 間隔ずれ得る。
