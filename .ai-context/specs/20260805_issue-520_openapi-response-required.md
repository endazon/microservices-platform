...
- 関連 ADR（計画）:
  ...
  BFF 境界と生成クライアント）
- 関連 IADR: **[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md)（本作業は同 ADR フォローアップ 1 の前提／作業仕様書 #506 §未決事項 2 の消化）**・
  **[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md)（本作業の内部設計判断。本書と対で読む）**・[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md)（契約スキーマの正本）・
  [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 3（BFF 境界・手書き HTTP クライアント禁止）
...
「OpenAPI を変えると SPA の型検査が落ちる」（[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 1 が与えると宣言した保証）が効かない。
本作業はこの非対称を解消する。
...
**［着手時の想定］** `orval-bff-only.cjs` は入力から `/bff/` 以外のパスを落とす（[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 3）。
...
という issue の順序判断（[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 1）が正しいことの裏付けでもある
...
3. 既存コードが壊れないことを確認する。**`?? 既定値` の扱いは方針を決めて [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) へ残す**（§設計 3）。
4. **変異試験**で「フィールドを消す／改名すると型検査が落ちる」ことを実測する（§変異試験）。
5. **消化の記録を 2 か所へ入れる**（日付つき［追記］）。**§未決事項という節を持つのは
   [作業仕様書 #506](./20260805_issue-506_openapi-bff-groups.md) の方であり、[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) には無い**
   ——両者を取り違えると「消化した」と書いた先に消化対象が無い状態になる。
   - **[作業仕様書 #506](./20260805_issue-506_openapi-bff-groups.md) §未決事項 2**（＝消化対象の本体）。
   - **[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) §結果 > フォローアップ 1**（＝本作業はその**前提**を消化する。
...
| **C# → OpenAPI の追随の機械化** | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 2 | 本 issue の射程外。実測は #506 §実測 5 |
| テストの fixture（`ANSWER` 等）を生成型で型付けする | **§未決事項 2** | 現状 fixture は `unknown` を経由するため型検査に掛からない。掛けると別の設計判断が要る |

## 設計

内部設計の判断（`required` の起こし方・`?? 既定値` の扱い）は [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) を正とする。
本節は**何をどう書くか**を確定する。
...
（[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 2 の系）。応答側の `default` は「欠けていたらこの値と読め」の意味なので、
...
「型の上では常に左辺」になる。**それでも消さない。** 判断の根拠は [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 3。
...
- [ ] **応答スキーマの `required` が C# の非 null 性と一致する**（§設計 1 の 25 行のうち
      ...
      **本 PR の最初のコミット本文は「25 個すべて `ファイル:行`」と書いており、この点で本書と食い違う**
      ——履歴は書き換えないため（force push 禁止）、正しいのは本書と [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) 決定 1 の側である。
...
- [ ] **消化の記録が[作業仕様書 #506](./20260805_issue-506_openapi-bff-groups.md) §未決事項 2 に日付つき
      ［追記］として入っている**（＝消化対象の本体。**[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) に §未決事項という節は無い**）。
- [ ] [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) §結果 > フォローアップ 1 に日付つき［追記］があり、**本作業が消化したのは
...
[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 4 が SSE 除外について採ったのと同じ考え方で、**担保は成果物と変異試験に置く。**
...
| 2 | **テストの fixture が生成型で型付けされていない**（`AnalysisDashboardPage.test.tsx:28` の `ANSWER` は `jsonResponse(body: unknown)` を経由するため、`CitationDto` の `number` / `snippet` を欠いていても型検査に掛からない） | 網の穴 | **#519**。載せ替えで MSW モック（`*.msw.ts`）を使うなら自然に解消する。使わないなら fixture へ `satisfies AiAnswerDto` を付ける小さな判断が要る<br>**［2026-08-05 追記］#519 では解消しなかった**——載せ替えのモック層は `apiRequest` へ移した（[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) 決定 4）が fixture は `unknown` を経由したままである。**穴は空いたまま**で、[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) フォローアップ 1 が引き継ぐ |
| 3 | **画面の載せ替え（9 ファイル）** | 本 issue の残り | **#519**。**本 PR で生成型はすでに必須化されている**ので、載せ替えた瞬間に「消したフィールドを読んでいる」箇所が型エラーになる（M6 / M7 が素通りしているのは、まさにこの載せ替えがまだ無いためである）。**［2026-08-05 追記］#519 本文の誤記（本 issue を `#516` と書いていた）は訂正済みで、「#520 は先に消化済み・生成型は既に必須化されている」旨も本文へ追記されている**——引き継ぎに際して #519 を読み直す必要は無い。**［2026-08-05 追記 2］#519 で載せ替えが完了し、M6 / M7 は落ちるようになった**（[作業仕様書 #519](./20260805_issue-519_orval-hook-migration.md) §変異試験） |
| 4 | **C# → OpenAPI の追随は人手のまま**。しかも本作業で `required` を増やしたぶん、**C# 側で `?` を足したのに OpenAPI の `required` を外し忘れると「嘘の必須」が残る**面が増えた | 構造的な穴 | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 2 ／ [IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md) フォローアップ 1。**本 issue で穴が広がったことは自覚した差異である** |
| 5 | **`IADR-0132` の採番衝突は解消済み**——**［2026-08-05 追記］**並行作業（`wt512`）が `IADR-0133` を確保したため、本 PR の `IADR-0132` は**改番不要**である。当初の懸念（`.claude/rules/traceability.md` §採番衝突時の改番手順＝**先着尊重**により後発が改番する）は発生しなかった。**［2026-08-05 追記2 / #512］**その後 #526 が `IADR-0132` を同番のまま `develop` へマージしたため（`9dc6c13`）**`IADR-0132` は 2 件ある**。先着尊重で改番するのは後着の #526 側であり、本 PR（先着）の `IADR-0132` は引き続き**改番不要**である。#526 の改番先 `IADR-0133` を空けるため、`wt512` は [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) へ改番し、#526 側の改番は **#530（`6a0336e`）** で入った | 運用（解消） | — （対応不要。改番が必要になった場合の追随先は本仕様書 / `docs/adr/README.md` / [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) の追記 / `docs/api/BFF_bff-surface.md` / **PR タイトル**） |
...
| 7 | **`.github/workflows/frontend-tests.yml` の `paths` に `docs/api/openapi.yaml` が無い**（`frontend.yml` には在る）。**`openapi.yaml` だけを触る PR では、単体テスト＋カバレッジのジョブが起動しない**——本 PR は生成物も変えたので `src/*/frontend/**` で起動したが、契約だけを直す PR（例: `required` の追随、本 PR の Y-1 相当）では走らない。**「契約の変更がカバレッジ床を割らない」ことを確かめる経路が片方だけ欠けている** | **CI の穴** | **親（権限外）**。`.github/workflows/` は GitHub App 権限では編集できないため本 PR では触っていない。`frontend-tests.yml` の 2 か所の `paths`（`push` / `pull_request`）へ `docs/api/openapi.yaml` を足すのが対応 |
| 8 | **`src/orval-bff-only.cjs` に「`components.schemas` は素通りする」という知見が無い。** 同ファイル冒頭コメントは「生成の段階で落としてしまえば、そもそも関数が存在しない」とだけ書き、**落とすのが `paths` だけである**ことに触れていない。この前処理を次に読む人は、#520 の実装者と同じ誤った想定（`/bff/` 外のスキーマは生成されない）に入る | 知見の置き場所 | **本 PR では `docs/api/BFF_bff-surface.md` §横断の規約 5 へ書いた**（受け入れ基準「プロダクションコード差分 0 行」を守るため、コード側にコメントを足していない）。**コードの傍に置くべき知見が仕様書側にしか無い**状態は残る——次に同ファイルを触る issue（#519 等）でコメントへ移すのが妥当 |
