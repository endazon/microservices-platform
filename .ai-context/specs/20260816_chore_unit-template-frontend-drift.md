...
- 関連 ADR: 計画 `ADR-0031`（フロントエンドスタック）。実装 ADR は
  [IADR-0056](../adr/IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット構成）/ [IADR-0060](../adr/IADR-0060_submodule-unit-operations.md)（submodule 運用・雛形）/ [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md)（Shared 2→3）/
  [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md)（SPA 移行の段分割・pnpm・`@platform/ui`）/ [IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md)（TanStack Router のユニット合成）/
  [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)（UI プリミティブ・i18n・ナビグループ）
...
| 改定 | 雛形の状態 |
| --- | --- |
| #490 / [IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md): 旧契約 `FeatureModule` → 型付きルート factory | **旧契約のまま** |
| #591 / [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 2: npm workspaces → pnpm（依存の宣言が必須になった） | **依存宣言ゼロ** |
| [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 4: フロントの参照許可先 1 → 2（`@platform/ui` 追加） | **1 のまま** |
| [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 6・9: ナビ文言の `MessageDescriptor` 化・ナビグループ必須化 | **未反映** |
...
- **バックエンド雛形**（`templates/unit-template/backend/`）。3 つの検査器を通り、記述（xUnit v2 の据え置き
  理由・Wolverine・[IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) の 2→3・[IADR-0064](../adr/IADR-0064_standalone-build-props-fallback.md) の MSB4092 回避）はいずれも現行と整合する。
...
| `src/platform/frontend/src/features/index.ts:10,32` | 対象外（AST は旧契約のまま。[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md) / [IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 2） |
...
[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) 決定 1 と `src/README.md` §項 4 に従い、ユニットが公開する契約を 2 本にする。

- `createSampleUnitRoutes = (shell: ShellRoute) => [...] as const` — **戻り値へ型注釈を書かない**
  （`readonly AnyRoute[]` を付けるとルート ID とパスの union が失われる。[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md) §実測）。
  この禁止を雛形のコメントで明示する。
- `sampleUnitNavItems: readonly PlanNavItem[]` — `group` を型で強制する。総称フォールバックが
  廃止済み（05_screens §共通シェル ［2026-08-04 確定］）のため、宣言漏れは画面がナビから静かに
  消えることを意味する。
- ナビ文言は `msg` マクロの `MessageDescriptor`（[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 6）。
- JSX を含むため `index.tsx` にする。
- **旧契約は雛形から完全に消す。** `FeatureModule` は AST（[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md)）のための互換ブリッジであり、
...
- 雛形 README: 依存規則を **1 → 2**（`@foundation` ＋ `@platform/ui`。[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 4 が
...
- 同 §4 へ **i18n 抽出対象（`src/lingui.config.ts` の `catalogs[0].include`）への追加手順**を足す
  （AI レビューの指摘で判明）。ここは**ハードコードの列挙**で pnpm workspace のような自動認識をせず、
  **足し忘れるとそのユニットの文言が抽出されず、未翻訳検査（[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 4）の外側になる**
  ——「翻訳漏れ 0 件」に見えて実際は測っていない状態になる。本 PR が作った欠落ではないが、
  この節を包括的に書き直した以上ここで埋める。**グロブへ置き換えないこと**を併記した
  （抽出対象は本リポジトリ所有のユニットに限る決まりで、`*/frontend/src` は AST を巻き込む）。
...
- **platform の `foundation/` は解釈の余地がある。** ADR-0019 / [IADR-0056](../adr/IADR-0056_repo-unit-structure-platform-knowledge.md) のユニット分割
  （アプリホストと features の分離）との整合という実装側の解釈が `src/lingui.config.ts:34` と
  [IADR-0033](../adr/IADR-0033_frontend-spa-foundation.md) 決定 3（「Bulletproof React の Feature First と両立する」）に残る。ただし**これは実装側の
  主張であって計画の文言ではない**。
...
これは [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 6 で第 3 段（#439）まで据え置きと**既に記録済み**であり、想定内である。
...
1. **【要・裁定】実装側（knowledge 13 画面 ＋ platform）の是正をどう扱うか。** 計画は fixed で
   ...
   規模が本作業と 2 桁違うため別 issue とし、[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4（1 PR が大きくなる場合は issue を分割）
...
4. **本作業の issue 起票。** [IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1（1 issue = 1 PR）に従い起票する。既存の open issue に
