...
6. **後継 IADR（IADR-0154）** と [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) への日付つき追記（#545 の回答どおり）

**射程外**（理由つき）:

| 除外するもの | 理由 |
| --- | --- |
| **SC-07 画面の 2 ペイン編集 UI** | 本 issue の表題は「**API を新設する**」であり、画面は別作業である。**別 issue を起票する**（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1: 1 issue = 1 PR） |
...
## 母集合（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1・走査基準は #648 §7 で改めた「語ではなくファイルから引く」）
...
認可は照会が admin ＋ operator、`retry` が `AdminOnly` の AND 合成（[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）。
...
| [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) 表題 | 「状況照会・**人手補正 API**」 | **#545 のとおり後継 IADR で実体を定める** |
...
独立した真偽値にした [IADR-0137](../adr/IADR-0137_conversion-dead-letter-marker.md) 決定 1 と**同じ扱い**にする。
[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md)「**状態表示は契約から導出できる値だけで作る**」に従い、**導出元を DTO へ載せる。**
...
[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) 決定 3 が列挙する口は `GET /jobs`・`GET /jobs/{id}`・`POST /jobs/{id}/retry` の 3 本であり、
**同 ADR は当時の決定を正しく記述している。** 表題の「人手補正 API」は設計時点の想定である。

表題だけ直すと「**人手補正 API はどこで決まったのか**」が宙に浮く。Phase 1 は
[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) が想定していた口とは**範囲の違う別の決定**である（図のコード化に限る・補正版を正とする・
マージを採らない）。よって **[IADR-0154](../adr/IADR-0154_manual-figure-correction-phase1.md) を後継として起こし、[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) へは日付つき追記**を残す。
...
群の既定を残して個々の端点へ `AdminOnly` を積む **AND 合成**（[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）で揃える。
サービス側にも同じ制約を置く多層防御（[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md)）——ただしワーカーは認可を課さない方針
（[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md) / [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 3）なので、**ネットワーク分離の回帰ガード
...
`ConversionJobDto` へは **末尾に既定値つきで**追加する（[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2）:
...
| 層 | 口 | 認可 |
| --- | --- | --- |
| ワーカー | `GET /jobs/{id}/figures` | 課さない（[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)） |
...
| 8 | 契約の後方互換 | 既存 `ConversionJobDto` の位置を変えていない（[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2） |
...
[IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md) の透過中継へ改め、**両文書も直した**（規則 8）。
...
`docs/data/conversion-job.md:117` にも同じ記述があった。**3 箇所とも実態へ揃えた**（[IADR-0154](../adr/IADR-0154_manual-figure-correction-phase1.md) 決定 5）。
...
**指摘のとおりだが、変更しない。** 単一図取得の口をワーカーへ足すと [IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)
...
- **#545 は本 issue の着手で解消**する（後継 IADR = [IADR-0154](../adr/IADR-0154_manual-figure-correction-phase1.md)）。
