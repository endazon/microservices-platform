...
- 関連 ADR: 計画 `ADR-0031`。実装 ADR は [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 1（第 5 段 = 運用系）/ [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md)（キット同期の
  分類）/ 本作業で新設する [IADR-0203](../adr/IADR-0203_renovate-husky-hook-scope.md)。関連 issue: #768（本件）/ #493（親。Knip / Plop を引き受け open で
...
`renovate.json` の `description` と [IADR-0203](../adr/IADR-0203_renovate-husky-hook-scope.md) が持つ。
...
- 差異: **あり**。計画 `13_frontend-stack` の第 5 段は Husky を **lint-staged / Commitlint と組**で挙げるが、
  ...
  この判断は [IADR-0203](../adr/IADR-0203_renovate-husky-hook-scope.md) に記録した。**親 #493 は Knip / Plop と併せてこの 2 件も引き受けたまま open で残る**
