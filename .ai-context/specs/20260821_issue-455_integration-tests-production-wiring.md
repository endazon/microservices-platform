...
### 🔴 AI レビューが見つけた fail-open —— 失敗の分かりやすさが退行していた

本 PR の変更で、**RabbitMQ コンテナの起動に失敗したときの失敗モードが変わっていた**。

`RabbitMqFixture` は起動失敗を `catch { IsAvailable = false; }` で握り潰すため、
`ConnectionString` は `null` のまま残る。

| | 従前 | 本 PR（是正前） |
| --- | --- | --- |
| 上書き | `cfg.Host(_rabbit.ConnectionString)` に `null` を直接渡す | 上書きを**省く** |
| 結果 | **即座に例外**で落ちる | `Program.cs` の既定 `amqp://guest:guest@rabbitmq:5672` へ**静かにフォールバック** |
| 見え方 | 原因が分かる | **原因不明の DNS / 接続タイムアウト** |

「緑になる」種類の fail-open ではないが、**壊れていることは分かっても原因が分からない**形へ
退行していた。**本番配線を使うようにしたこと自体が生んだ相互作用**である。

**是正**: フィクスチャを渡された以上、繋ぎ先はそのコンテナでなければならない。
`ConnectionString` が `null` なら **`InvalidOperationException` で止める**（何を確認すべきかを
メッセージに書く）。

**変異試験で実測した**（`_container = null` を挿入して起動失敗を模した）:

```
変異あり: Failed: 14, Passed: 29, Total: 43
          → "RabbitMqFixture の接続文字列が null である（コンテナの起動に失敗した可能性が高い）。"
変異なし: Failed:  0, Passed: 43, Total: 43
```

**復旧を確認した**（変異残骸 0・`git diff` 空）。

### 🔴 trace ブロックの追随漏れ —— これで 2 回目である

`docs/tech/tech-requirements.md` の trace ブロック `specs:` へ、本作業の仕様書
（および Phase 0 の前作業仕様書）を足していなかった。**AI レビューの指摘で判明。**

🔴 **同型の事故はこれで 2 回目である**（1 回目は PR #883 の `iadrs:` に `IADR-0231` を
足し忘れた件）。そのとき私は「**同型が 2 回起きたら検査器を足す根拠になる**」と書いた。
**条件が満たされた。**

ただし**この PR では検査器を足さない**。理由は、機械的な判定規則が自明でないためである ——
「この文書はどの仕様書を引くべきか」を機械が知る一般的な手段が無い。安易に足すと
偽陽性だらけの検査になり、**赤の常態化**（本リポジトリが繰り返し警告している失敗モード）を招く。

**判定規則の設計を含めて別 issue で扱う。** 候補として、**同一コミット内で
`.ai-context/specs/` に新規ファイルが追加され、かつ `docs/**.md` が変更されたのに、
その文書の `specs:` に新規仕様書が含まれていない**、という差分ベースの規則がある。
