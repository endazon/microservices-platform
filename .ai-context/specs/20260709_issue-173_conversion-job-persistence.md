...
- 画面(SC): SC-07（変換ジョブ画面）
- 関連 ADR: [IADR-0043](../adr/IADR-0043_conversion-job-persistence.md)（本 PR で作成・永続化＋非同期ストア）、[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)（読み取りモデル・MVP インメモリ）、
  [[ADR-0002]]（DB per Service）、[[ADR-0003]]（MassTransit）、[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)（ワーカー最小 HTTP）
- Issue: #173（[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) フォローアップ）

## 目的・背景

[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) で SC-07 実現のため導入した変換ジョブ読み取りモデル `IConversionJobStore` は MVP として
**インメモリ実装**（singleton）であり、再起動でジョブ履歴が消失し、複数インスタンス間で共有されず、
監査・長期保全ができない。本 PR で DataSourceService 準拠の **Postgres + EF Core** に永続化する
（[IADR-0043](../adr/IADR-0043_conversion-job-persistence.md)）。抽象 `IConversionJobStore` は分離済みのため、実装差し替え＋非同期化に閉じ、
...
- 対象外:
  ...
  - 変換出力（Markdown）の手編集 UI（人手補正は再変換に限定・[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)）。
  - デッドレター突合・履歴保持方針・水平スケール時の並行制御（[IADR-0043](../adr/IADR-0043_conversion-job-persistence.md) follow-up）。
...
- 非同期ストア＋scoped 生存期間・EF は [IADR-0043](../adr/IADR-0043_conversion-job-persistence.md) に記録。同期 EF アンチパターンを避けるため
