...
並ぶため、**2 つまとめて 1 つの画面実装として扱うのが自然**である（[IADR-0150](../adr/IADR-0150_search-sort-after-retrieval.md) フォローアップ）。
...
| 検索（配線） | `RetrievalService.../Foundation/Services/HybridSearchService.cs` | **取得後に並べ替える**（[IADR-0150](../adr/IADR-0150_search-sort-after-retrieval.md) 決定 1・3・4） |
...
| `updated_at` のペイロードインデックス | **[IADR-0150](../adr/IADR-0150_search-sort-after-retrieval.md) 決定 5 で不要と判断した。**[IADR-0149](../adr/IADR-0149_search-result-updated-at-indexing.md) のフォローアップは「ストア側で並べる」前提だったが、決定 1 で取得後に並べるため使われない |
...
**`enum` にしない**（[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 5）。**未知値・未指定は既定へ縮退する** ＝ 旧クライアントは従来どおり。
...
- **`updated`**: 系統に依らず `candidateK`（＝ `max(topK * 4, topK)`）件を取得し、
  ...
  **単系統（`keyword` / `semantic`）でも候補を広げる**（[IADR-0150](../adr/IADR-0150_search-sort-after-retrieval.md) 決定 3）。
...
- **[IADR-0149](../adr/IADR-0149_search-result-updated-at-indexing.md) のフォローアップに追記**（ペイロードインデックスは不要と判断した旨。**前提が変わった**）
