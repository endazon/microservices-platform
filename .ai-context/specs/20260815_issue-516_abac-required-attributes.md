...
- 計画書リンク: `09_datasource-connectors.md` §システム投入経路での `owner` / `department`（計画リポ）（**確定・2026-08-15**。裁定依頼 planning#344）
...
- **F（図の補正後の再投入）**: **通常は経路 1 の補完が原本イベント経由で伝播する**ため、本作業の修正が効く。
  ...
  （[IADR-0154](../adr/IADR-0154_manual-figure-correction-phase1.md) 決定 3）と警告している。**扱う属性が 1 → 3 に増えたぶん影響面は広がったが、
...
> [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 4 が定めたとおり**反転させた**（`Create_FillsLifecycleWithActive`）。
>
> | 属性 | 解決順 | 終端 |
> | --- | --- | --- |
...
- [x] **否定形テストを [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 4 のとおり反転させた**
