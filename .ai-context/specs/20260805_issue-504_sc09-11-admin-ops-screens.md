...
related_specs:
  ...
  - ../../docs/tests/SC-09_admin-abac-settings.md
  - ../../docs/tests/SC-10_operations-dashboard.md
  - ../../docs/tests/SC-11_configuration-viewer.md
...
- 関連 ADR（計画）:
  ...
  ADR-0018（計画リポ）（SC-11 の前提）
- 関連 IADR: **[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md)（本作業の内部設計判断。本書と対で読む）**・[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）・
  [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)（FR-17〜21 の着手保留）・[IADR-0046](../adr/IADR-0046_config-version-history-source.md)（構成バージョン履歴の正データ源）・
  [IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)・[IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md)・[IADR-0035](../adr/IADR-0035_frontend-role-based-nav-and-existence-hiding.md)・[IADR-0036](../adr/IADR-0036_sc11-config-viewer-visualization.md)・[IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md)・
  [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md)・[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md)・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)・[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md)
...
issue #504 は「**辺の型そのもの**は FR-17 に属し [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) で保留中だが、**その値集合の管理画面は
...
[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1・2 は「FR-17 の実装には着手しない。着手条件は前提 ADR が **`Accepted`** になること
...
06_technical/05_observability-ops.md（計画リポ）
（`:94-96`）が次を定める。
...
これは実装側の [IADR-0046](../adr/IADR-0046_config-version-history-source.md)（Accepted・2026-07-09）と**同じ内容**である。**実装の変更は要らない**——
...
wireframe/sc-11.html（計画リポ） が揃っている。
...
3 画面をまとめる理由は issue #504 が述べるとおり **「権限による出し分けと存在秘匿（[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)）が
...
| 事項 | 送り先 | 理由 |
| --- | --- | --- |
| **SC-09 の「辺の型」区画**（辞書・3 層構成・対称/非対称・逆向きの表示語・使用件数・削除拒否/改名） | **[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留解除後**（前提 = ADR-0033 の `Accepted` 化） | **FR-17 に属する**（上記 §射程確認）。契約も無い |
...
| **SC-10 の「ナレッジ健全性」節**（孤立文書・未解決リンク・未要約クラスタ・陳腐化文書）・**辺の型ごとの使用件数**・**フォールバック警告** | **[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留解除後** | **FR-17 / FR-18 に属する**（計画は「起案・2026-08-01。Phase 3」と明記） |
| **共通シェルのパンくず・権限バッジ** | 共通シェルの作業（#452 系） | `foundation/ui/Layout` の責務。#490 仕様書が #452 へ渡している |
| **右レール AI チャットパネル** | 移行**第 4 段** | [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 1・5 |
...
| SC-18〜21 | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留解除後 | 前提 ADR が `Proposed` |
| `oidc-client-ts` の撤去 | 第 3 段（#439） | [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 6 |

## 設計

内部設計の判断（選択肢の比較・棄却理由）は [IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) を正とする。本節は実装の形を記す。
画面ごとの詳細は画面仕様書（[SC-09](../../docs/screens/SC-09_admin-abac-settings.md) /
[SC-10](../../docs/screens/SC-10_operations-dashboard.md) / [SC-11](../../docs/screens/SC-11_configuration-viewer.md)）を正とする。
...
| --- | --- | --- | --- | --- |
| SC-09 | **`platform-admin` のみ** | §共通シェル「SC-09・SC-12・SC-17 = **システム管理者**」／§SC-09「システム管理者ロール限定」 | 同じ | [IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md)（BFF も AdminOnly。operator も 403） |
| SC-10 | **`platform-admin` のみ**（**据え置き**） | §SC-10「**運用者・管理者**ロール限定」 | **管理者のみ**（差異） | データ源 `/bff/dashboard/summary` と後段 `DashboardService` がともに **AdminOnly**。画面だけ広げると運用者に 403 の画面を見せることになる（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 4）。**§環流 の提案 5 で裁定を求める** |
| SC-11 | **`platform-admin` または `platform-operator`** | §SC-11「管理者・運用者ロール限定。権限外にはメニュー・画面自体を表示しない」 | 同じ | [IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md)（`ConfigViewer` ポリシー）／API は 404 で秘匿（[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)） |
...
4. サーバ側の 403 / 404 は**同一の中立文言**へ寄せる（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 3）。
...
| 種別 | 対象 | 根拠 |
| --- | --- | --- |
| **A. FR の着手保留** | SC-09 の**辺の型区画**（タブ・表・削除拒否の注記・3 層構成の注記）／SC-10 の**ナレッジ健全性節**（4 KPI・辺の型ごとの使用件数・フォールバック警告・除外の注記） | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1・2（FR-17 / FR-18。前提 ADR-0033 は `Proposed`） |
...
（#503 で SC-05 のタグ辞書整合が実際に漏れた）。本 issue で該当するのは次の **3 件**であり、
...
| 3 | SC-11 | ドリフト明細の行は「**する**」 | hi-fi `436`（wireframe `133`）の明細行は **3 セルとも契約の値域外**（種別「パラメータ差異」・深刻度「中」・対象 `embed.batch_size`）。実装は `BindingMismatch` / `Warning` / `embed` へ**読み替えて**再現しており、**`target` が常に段名である**ため「パラメータ粒度のドリフトが `embed` 段を光らせる」場面は再現しない。**契約の不在ではなく計画の内部矛盾**（`10_composability-design.md:180` が 5 分類を確定済み） |
...
計画 13_frontend-stack §shadcn/ui 派生の範囲（計画リポ）
...
| 単体テスト | `pnpm run test` | **57 files / 539 tests** 全 green（本作業前は 53 files / 473 tests。**うち 3 件はクロス監査〔#511〕の是正で追加**） |
| カバレッジ | `pnpm run test:coverage` | 後述（床を 88/88/84/83 → **90/90/88/85** へ引き上げ） |
| ビルド | `pnpm run build` | green（`dist/assets/index-*.js` 632.98 kB / gzip 190.04 kB） |
...
| i18n カタログ | `node scripts/check-i18n-catalogs.js` | green（2 ロケール・未翻訳 0 件。ja / en とも **323 件**——`msgid` の総数からカタログ先頭のヘッダ 1 件を除いた数。本作業で 145 件増） |
...
| 3 | SC-11 | ドリフト明細（#14）は「**する**」 | hi-fi `436` の明細行は **3 セルとも契約の値域外**（「パラメータ差異」／「中」／`embed.batch_size`）。実装は `BindingMismatch` / `Warning` / `embed` へ読み替えており、**`target` が常に段名**であるため「パラメータ粒度のドリフトが段を光らせる」場面は再現しない |

**行の判定が二値（する／しない）だと、「する」行の中に隠れた部分未実装が集計に載らない**
（#503 が SC-05 のタグ辞書整合で踏んだ穴と同型）。**3 件とも環流記録の提案 3・6・8 として渡した。**
**3 件目（SC-11）は前 2 件と性質が違う**——前 2 件は**契約の不在**だが、SC-11 は
**契約が計画の確定どおりで、モックだけが確定以前の粒度で残っている**（**計画の内部矛盾**）。
したがって求める是正は**モックの側**である。
...
| --- | --- | --- | --- |
| 全ユニット横断（本 PR） | **95.93%**（5447/5678） | **89.77%**（1124/1252） | **91.83%**（416/453） |
| MSP 所有分（本 PR） | **95.24%**（4180/4389） | **90.69%**（857/945） | **93.08%**（323/347） |
...
**件数の基準を先に置く**（本書 §受け入れ基準 1 が「同じ事象を数える基準が 2 つあるので、件数を書くときは
必ず基準を添える」と定めたのに従う）。下表は **M1〜M40 ＋ M8b ＋ M41〜M44 の 45 行**だが、
**変異の件数は 44 件**である——**M8b は M8 の是正確認**であって独立した変異ではないためである。

**44 件のうち 43 件は最初から落ちた。1 件（M8）が素通りしたので、テストを直して落ちることを再確認した。**
**M41〜M44 の 4 件はクロス監査（PR #511 の指摘 A・C・B）の是正で追加した分**であり、
是正前は当該の振る舞いが**テストの網に載っていなかった**（後述）。
...
| **M41** | SC-11 のドリフトバッジを**実効構成の成功ガードの外**へ戻す（＝是正前の実装） | `hides the drift and history sections when the effective config is unavailable` ／ `hides the drift badge when the effective config fails with a server error`（**2 件**） |
| **M42** | SC-11 の**ドリフト**の失敗を種別で分けず中立文言へ寄せる（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 3 に反する） | `degrades only the drift section when the drift query fails`（1 件） |
| **M43** | SC-11 の**履歴**の失敗を同上 | `degrades only the history section when the history query fails`（1 件） |
| **M44** | SC-09 の 409 から `toMessages`（参照元ポリシー名）を落とし固定文言だけにする | `explains a 409 when deleting a referenced attribute and keeps the server detail`（1 件） |

#### 変異試験の網に載っていなかった 2 か所（M41 / M44。クロス監査で発覚）

**変異試験は「壊すと落ちるか」を見る手法であり、最初から壊れている箇所は、
壊す前の assert が無ければ検出できない。** 本 PR の初版で 2 か所がこれに当たった。

| 箇所 | 初版の状態 | テストが素通りした理由 |
| --- | --- | --- |
| SC-11 のドリフトバッジ（M41） | ヘッダのバッジが**実効構成の成功ガードの外**にあり、実効構成が落ちてドリフトが成功したときに「明細が無いのに件数だけ残る」（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 5 が名指しで禁じた形） | `hides the drift and history sections …` は `config: 404` としつつ**ドリフトを成功させたまま**で、明細行と履歴の不在だけを見ていた。**ヘッダのバッジについて何も assert していなかった** |
| SC-09 の 409 の詳細（M44） | 409 を検出すると `ApiError.details`（**参照元ポリシー名**）を捨てて固定文言へ置き換えていた（[IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md) 決定 2 の目的に反し、旧実装より情報量が減っていた） | fixture の `ApiError` が **`details` 空**で、実サーバ応答（`AuthzEndpoints` が `detail` にポリシー名を並べる）の形を再現していなかった |

**教訓: fixture が実サーバ応答の形を再現していないと、変異試験の網はそこに掛からない。**
「壊して落ちるか」の前に「**正しい応答の形を与えているか**」を確かめる必要がある。
...
（保留の追跡は [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) とプロダクトコードのコメント・画面仕様書が担う）。
...
（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 6）。
...
| **SC-09 の辺の型辞書** | 05_screens §SC-09 §主要素・§辺の型（値集合）の管理（3 層構成・対称/非対称・逆向きの表示語・使用件数・削除拒否/改名） | **実装しない** | **FR-17 に属する**（計画自身が節見出しに明記。画面一覧も FR-17 を挙げる）。前提 **ADR-0033 は `Proposed`** であり [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 2 の着手条件（`Accepted`）を満たさない。**裁定は不要**——前提 ADR の確定を待つ |
...
| **SC-10 のナレッジ健全性節** | 05_screens §SC-10 §ナレッジ健全性（4 KPI・辺の型ごとの使用件数・フォールバック警告・固定文言の注記） | **実装しない** | **FR-17 / FR-18 に属する**（計画自身が「起案・2026-08-01。Phase 3」と明記。指標は ADR-0033 に由来）。[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)。**裁定は不要** |
| **SC-10 の閲覧ロール** | 05_screens §SC-10「**運用者・管理者**ロール限定」 | **`platform-admin` のみ**（据え置き） | `/bff/dashboard/summary` と後段 `DashboardService` がともに `AdminOnly`。画面だけ広げると「開くと必ず 403 になる画面」になる（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 4）。**planning#198 提案 8 と同じ類型だが向きが逆**。§環流 提案 7 |
...
| **SC-11 のドリフト明細の例示** | `05_screens/mockups/hi-fi/sc-11.html:436`（wireframe `133`）「パラメータ差異／中／`embed.batch_size`」 | **`BindingMismatch` / `Warning` / `embed` へ読み替え**（**部分未実装**） | 契約（`DriftDetector`）は種別 5 値・深刻度 2 値・`Target` は常に段名であり、計画 `10_composability-design.md:180` が**その 5 分類を確定済み**である。**モックだけが確定以前の粒度で残っている**（計画の内部矛盾）。§環流 提案 8 |
...
**契約の不在 6 件**（要素名基準）＋ **閲覧ロール 1 件** ＋ **モックと確定済み値域の食い違い 1 件**
（= 提案 1〜8）を新しい環流記録
...
| 8 | SC-11 の **hi-fi / wireframe のドリフト例**（種別・深刻度・対象の 3 セルが計画確定の値域外） | 新規。**契約の不在ではなく計画の内部矛盾**（`10_composability-design.md:180` が 5 分類を確定済みで、モックだけが確定以前の粒度）。**求めるのは画面ではなくモックの是正**。planning#198 §「計画の内部で食い違っている可能性」と同型 |
...
- SPA 移行の完了条件のうち **SC-09〜11 の 3 画面**（#452 の分割 3 本目・最後）。
  **これで旧 13 画面のうち #452 が引き受けた分は片付いた**（残るのは SC-12＝#445 待ちと SC-18〜21＝[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 待ち）。
...
- **クロス監査（PR #511）の指摘 9 件の是正**（SC-11 のドリフトバッジのガード・決定 3 の射程と文面・
  SC-09 の 409 詳細・決定 7 の位置づけ・裸 `#198` の修飾・SC-11 の部分未実装 3 件目・
  テストケース数の母集合・変異試験の件数の基準・`related_ids` の非対称）。
...
| **SC-11 の hi-fi / wireframe のドリフト例が計画確定の 5 分類・2 深刻度・段名と揃っていない**（計画の内部矛盾。求めるのは**モックの是正**） | 同記録の提案 8。**起票は親** |
...
| SC-09 の辺の型辞書・SC-10 のナレッジ健全性節 | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留解除後（ADR-0033・0034・0035 の `Accepted` 化） |
| SC-12 | #445 待ち |
| SC-18〜21 | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留解除後 |
...
| バンドルサイズ（633 kB / gzip 190 kB） | 全画面の再実装が終わってからのコード分割（#490 / #496 / #502 / #503 の未決事項を引き継ぐ）。**#503 の 586 kB から 47 kB 増えた**のは本 issue の 3 画面ぶんである。**［2026-08-05 追記］#512 / [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) で消化**（ルート単位の遅延 ＋ `manualChunks` 3 規則。500 kB 警告は解消し、初期ロードは 632.98 → 577.54 kB / gzip 190.04 → 177.94 kB）。 |
...
1. **SC-09 は計画の 4 区画のうち 2 区画しか無い**（辺の型＝保留／タグ辞書＝契約の不在）。
   ...
   **SC-11 のドリフト明細も、モックの例示（3 セルとも計画確定の値域外）を読み替えて再現している**
   （**行数基準では捕まらない部分未実装の 3 件目**。提案 8）。
...
6. **クロス監査（PR #511）で 2 か所の実バグ／後退が見つかり是正した。** (a) SC-11 のドリフトバッジが
   実効構成の成功ガードの外にあり「明細が無いのに件数だけ残る」形になっていた（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 5 違反）。
   (b) SC-09 が 409 の `ApiError.details`（参照元ポリシー名）を捨てており、旧実装より情報量が減っていた。
   **どちらもテストが素通りしていた**——理由と教訓は §変異試験 の「網に載っていなかった 2 か所」にある。
   あわせて [IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 3 の射程（従クエリにも効く）を明記し、SC-11 のドリフト・履歴の
   エラー表示を主クエリと同じ作法へ揃えた。

## 未決事項

1. **契約の不在 6 件 ＋ 閲覧ロール 1 件 ＋ SC-11 のモック粒度 1 件**（環流記録の提案 1〜8）。
   **起票は親**。裁定までは当該要素を実装しない。
2. **FR-17 / FR-18 の保留解除**（ADR-0033・0034・0035 の `Accepted` 化）。解除後に
   SC-09 の辺の型区画と SC-10 のナレッジ健全性節を実装する（[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 6 の手順に従う）。
