...
- 関連 ADR:
  ...
  - 計画 `ADR-0030`（計画リポ）（ライブラリ標準・選定基準 2「構成要素を増やさない」）
  - 計画 `ADR-0019`（計画リポ）（ユニット第一構成・決定 4）
...
（[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1・4。適用は多数の空フォルダを一度に足す作業であり、決定の是非と混ぜるとレビューできない）。
...
- `SharedKernel` 列の「対象外」は決定 1 に依る。**per-service の実体は 11 サービスすべてで無い**が、
  集約先 `Platform.Shared.Kernel`（未作成）へ置くことが [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) で確定済みである。
- **各サービスは 1 プロジェクトに畳まれている。** 層は `<Name>.{Api,Worker}/Foundation/` の
  フォルダとして存在する（[IADR-0027](../adr/IADR-0027_composability-folder-structure.md)・`src/README.md`）。実測: `Foundation/Domain` は **8 / 11**、
...
| | 決定 |
| --- | --- |
| 1 | `SharedKernel` の**論理粒度はサービス単位**（計画の図が正）。ただし**物理配置は [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) が集約済み**であり、per-service の `.gitkeep` は**置かない** |
...
> **本リポジトリには [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md)（Accepted）があり、案 B「サービス単位に `<Name>.SharedKernel` を置く
...
（[IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) が正）を分けて書き、`.gitkeep` の適用対象からは外す**形にした。詳細は IADR-0218 決定 1。
...
1. **【要・環流】計画 §規範性と [IADR-0117](../adr/IADR-0117_platform-shared-kernel-placement.md) の関係を計画側で明示してもらう。**
