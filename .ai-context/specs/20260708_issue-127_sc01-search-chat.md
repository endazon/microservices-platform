...
- 関連 ADR: ADR-0010 ／ [IADR-0007](../adr/IADR-0007_llm-egress-routing-config-driven.md)（egress）／ **[IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)（SSE ストリーミング・新規）** ／ [IADR-0033](../adr/IADR-0033_frontend-spa-foundation.md)
- Issue: #127（親 #121）

## 目的・背景

本システムの主入口。横断検索と根拠付き AI 回答（**真の SSE ストリーミング**表示・出典併記）＋👍/👎、出典クリックで
SC-03/SC-04 へ遷移。ユーザー判断により、backend は SSE ストリーミングに対応し（[IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)）、frontend は SSE で
真のストリーミングを実装する。`/bff/search` のスタブも実装して横断検索を実データ化する。

## 対象範囲

- 対象（backend, [IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)）:
...
- 差異: あり（記録）。SC-01 の「ストリーミング」を真の SSE で実装するため backend を拡張（[IADR-0037](../adr/IADR-0037_llm-sse-streaming.md)）。egress（FR-11）は不変。
