...
- **BFF・DashboardService の両層**で閲覧の認可を admin ＋ operator へ広げる（[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) 多層防御）
...
| SC-09（管理者設定） | 計画が **`platform-admin` のみ**と定める（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md)）。本件と無関係 |

## 母集合（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1・走査基準 `6447062`）
...
| **SC-09** | `[Admin]` | **管理者のみ**（[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md)） |
| **SC-10（本件）** | `[Admin]` | **管理者・運用者** ← **食い違い** |

**SC-10 だけが、計画が運用者を含むのに実装が admin のみである。**
[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 は管理系画面を admin **または** operator と定めており、**本件はその形へ戻す**ことになる。
...
[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1 が SC-07 で踏んだ穴（画面だけ先に変えて API が追随していない）と同型であり、**同じ轍を踏まない**。
...
| 理由 | **[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) により節ごと着手保留**（FR-17 / FR-18）。引き受けは #504 / #452 |
...
| 8 | [IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 4 | 日付つき追記（決定は置換しない） |
...
| 7 | [IADR-0011](../adr/IADR-0011_dashboard-service-usage-aggregation.md) 決定「認可」 | なし | **ADR 本文** → 日付つき追記 |
| 8 | [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1 の根拠 | なし | **他 ADR からの引用**（「`/bff/dashboard` が使う既存の `AdminOnly`」）→ 日付つき追記 |

**#8 は自分でも危うく「別資源」へ分類しかけた。** [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) は変換ジョブの再変換についての決定で
...
| [IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md):74,82 | 「SC-10 ダッシュボード（FR-10）は…**`AdminOnly` のまま維持する**」 | 含む（**ファイルは開いたが 1 行しか見なかった**） |
...
[IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md) も「ファイルは走査に出ていたのに 1 行だけ見て閉じた」——**開いたら全文読む。**
...
[IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md) は本 PR の 3 巡目（`cb38b0c`）で決定本文・影響欄へ追記したにもかかわらず、
...
**検査器を起票した（#649）**。本 PR には入れない（1 issue = 1 PR。[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1）:
...
| **C** | **`updated:` < PR の最初のコミットの日付** | **1** | **[IADR-0030](../adr/IADR-0030_operator-role-and-config-viewer-policy.md) だけを正しく挙げる** |
...
- **#543（人手補正 API）は本 PR に含めない。** 別資源であり、[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1（1 issue = 1 PR）に従う。
