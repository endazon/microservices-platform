...
- 関連 ADR（計画）:
  ...
  BFF 境界と生成クライアント）
- 関連 IADR: **[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md)（本作業の内部設計判断。本書と対で読む）**・[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 3（BFF 境界・
  手書き HTTP クライアント禁止）・[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md)（契約スキーマの正本）・[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）・
  [IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md)（ABAC 管理 API の透過中継）・[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md)・[IADR-0129](../adr/IADR-0129_sc09-11-admin-ops-screen-composition.md)
- 本リポジトリの起点: **#506**（親 #454。出所は #502 / PR #505 §付-1、#503 / PR #508 §未決事項、
  #504 / PR #511 §未決事項）

## 目的・背景

`CLAUDE.md` と [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 3 は「BFF への呼び出しは **orval 生成フック**（入力は
...
issue #506 と [IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4 に従い、**2 本に分割する**。
...
内部設計の判断（選択肢の比較・棄却理由）は [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) を正とする。本節は書く内容を確定する。
...
> 契約は完全なまま、罠のフックは存在しない。判断の根拠は [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 4。
...
認可メタデータは AND 合成されるため実効要件は admin のみ。[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）。
...
（[IADR-0040](../adr/IADR-0040_admin-abac-bff-passthrough-and-admin-only.md) 決定 2）。
...
| 3 | `sc03-document/useDocumentQueries.ts` | 3 本 | `useBffDocumentDetail` / `useBffDocumentContent` / `useBffDocumentVersions` | **版履歴は詳細の成功後にだけ有効化**（[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) 決定 4）。`enabled` を生成フックの `query` オプションで渡す |
...
3. **キャッシュキーが変わる**。生成フックのキーは `` [`/bff/…`] `` であり、現状の
   ...
   （[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 5 の「invalidate だけを行う」作法は維持する）。
...
| --- | --- | --- | --- |
| M3 | **C# の DTO を変えても SPA の型検査は落ちない** | `openapi.yaml` は**手書き**であり、C# → OpenAPI の追随が人手だから（実測 5） | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) フォローアップ 2。**本 issue の射程外** |
...
| # | 事項 | 種別 | 送り先 |
| --- | --- | --- | --- |
| 1 | **画面の載せ替え（分割 2 本目）** | 本 issue の残り | **#519**（起票済み）。対象・手順は §残りとして何をどうするか（9 ファイル・共通の注意 5 点）。**#520 を先に通す方が手戻りが少ない**（同じ生成型を触るため競合し、`required` の有無で生成される型の省略可否が変わる）<br>**［2026-08-05 追記］#519 で消化した**（[作業仕様書 #519](./20260805_issue-519_orval-hook-migration.md) / [IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md)）。**本書の申し送りと実測が食い違った点が 3 つある**——(a) `useBffSearch` は **mutation** であり照会には使えない（SC-02 は生成された操作関数を `useQuery` に据えた）、(b) **落とし穴は 5 点ではなく 6 点**（テストのモック層が `apiFetch` に当たっており 13 ファイルが赤くなる）、(c) 生成フックの `TError` が `void` になり `error instanceof ApiError` が型エラーになる |
...
| 3 | **C# → OpenAPI の追随が人手** | 構造的な穴 | [IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 1 の但し書き・フォローアップ 2。透過中継の応答を覆える方式が要る |
| 4 | 既存 2 本の `operationId` 不統一（`analysis-ask` / `analysis-analyze`） | 小さな是正 | 2 本目で `useAnalysisAnalyze` に触るついでが最も安い<br>**［2026-08-05 追記］#519 で消化した**（`bff-analysis-ask` / `bff-analysis-analyze`。[IADR-0131](../adr/IADR-0131_openapi-as-bff-contract-source.md) 決定 3 の改定＝[IADR-0135](../adr/IADR-0135_generated-client-adoption-and-cache-keys.md) 決定 5） |
