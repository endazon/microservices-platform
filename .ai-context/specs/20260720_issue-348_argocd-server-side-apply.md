...
- ADR: ADR-0006 / ADR-0007（CI/CD・GitOps）。ローカル ArgoCD opt-in ブートストラップの統括は
  [IADR-0077](../adr/IADR-0077_local-observability-vault-gitops-overlays.md)（local observability/vault/gitops overlays）。ゲート横断 smoke test は [IADR-0087](../adr/IADR-0087_k8s-local-up-optin-smoke-test.md)（#334）。
...
ため **IADR は起票しない**（統括は既設 [IADR-0077](../adr/IADR-0077_local-observability-vault-gitops-overlays.md)。純スクリプト修正）。
