...
- 関連 ADR: ADR-0004（認証＝Keycloak）／可観測性（Loki/Tempo）。方式判断は [IADR-0079](../adr/IADR-0079_infra-persistence-compose.md)。既存 [IADR-0020](../adr/IADR-0020_wiki-js-deployment-abac-gateway.md)（Wiki.js↔Keycloak OIDC・共有 Postgres 利用の先例）／[IADR-0066](../adr/IADR-0066_local-k8s-dev-environment.md)（経路B は対象外の根拠）。
...
（`deploy/local/`＝経路B）は [IADR-0066](../adr/IADR-0066_local-k8s-dev-environment.md) が統べる環境で、その infra が `emptyDir`（Pod 再起動で再 init）である旨は
...
詳細な設計判断と根拠は [IADR-0079](../adr/IADR-0079_infra-persistence-compose.md) に記録する。要点:
...
2. **Loki / Tempo → 名前付きボリューム**。`loki-data` を `/tmp/loki`、`tempo-data` を `/tmp/tempo`（＝各 config の
   ...
   生成される既知の挙動に対し、非 root イメージでも書き込めるよう `user: "0:0"` を付与する（[IADR-0079](../adr/IADR-0079_infra-persistence-compose.md) §3）。
...
| 採用方式と storage パス整合の判断が IADR に記録 | [IADR-0079](../adr/IADR-0079_infra-persistence-compose.md) |
