...
**契約の定義は [IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) が (a)(b)(c) をまとめて行う。** 本仕様書はそのうち **(a) 値集合の照会・追加**と
**(b) 使用件数**の実装を扱う。**(c) 改名の追随と保持方式の移行は #635** である。

## 射程と、issue を分割したこと

**#542 は 1 PR に収まらない。** [IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4（PR ではなく issue を分割する）に従い、
#542 を親として **#634（本作業）** と **#635** の 2 つへ分けた。

分割の根拠は[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) が定めた**「概ね 50 ファイル / +2500 行を超えるなら分ける」**である。
下記「母集合」の実測がこれを超える。
...
**契約そのものは [IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) が 3 つまとめて定める。**
...
**誤りの側から引いた**（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 規則 1）——「文書タグ」で検索せず `Tags` で全部引いてから、
...
| 相乗りする口 | `/bff/attribute-values`（#540 で新設。[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 4 が拡張点を用意している） |
...
| **辺の型（値集合）の辞書** | 知識グラフ（FR-17）は [IADR-0142](../adr/IADR-0142_fr19-20-scoped-release-by-overturn-range.md) が着手条件を別に定めている。[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) フォローアップに記録済み |
| `src/ai-stock-trading` | 別プロジェクトの submodule。変更しない |

## 実装方針

1. **辞書エンティティ**: DocumentService に `Tag`（識別子・表示名・作成日時）を新設する（[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 1）。
   マイグレーション 1 本。
2. **契約**: `Knowledge.Contracts` に辞書の DTO を足す。
   **`AttributeValuesResponse` へ管理者スコープ専用のフィールドを足す**（既定 `null`。
   一般利用者の応答形は #540 から変えない。[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 4 / [IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2）。
3. **使用件数**: 現行版の `Document.Tags` を数える。**版履歴は数えない。アーカイブ済みは数える**
   （[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 2）。**移行前なので表示名の一致で数える暫定である**ことをコメントとテストに残す。
4. **BFF**: `/bff/attribute-values` の 1 系統を保つ。読み取りは `ConfigViewer`、追加は `AdminOnly`
   （[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 5）。**新しい読み取り口を作らない。**
5. **一般利用者の経路は変えない**——候補は Qdrant の facet のまま（[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 4）。
...
**`Id` は #634 の時点でも要る。** 主キーであり、[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 6 の
...
**版履歴は読んでいない**（[IADR-0152](../adr/IADR-0152_tag-dictionary-contract.md) 決定 2）。
...
**実効境界は DocumentService 側**であり（[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) 多層防御 / [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) 決定 2）、
...
| `check-contract-schema` | **baseline を更新**（`TagDto` / `CreateTagRequest` / `TagDictionaryResponse` の型追加 ＋ `AttributeValuesResponse.Dictionary` の**既定値ありメンバー追加**。**破壊的変更 0 件**。[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) 決定 2） |
