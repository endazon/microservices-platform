...
- ユースケース（UC）/ 画面（SC）: なし
- 関連 ADR:
  ADR-0030（計画リポ）（Accepted・本作業の一次情報）／
  ADR-0027（計画リポ）（Wolverine）／
  ADR-0029（計画リポ）（gRPC/REST）／
  ADR-0020（.NET 10）／[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md)（進行規約）
- 計画書リンク: 12_backend-application-stack（計画リポ）（`fixed`・棚卸し表が正）
...
- 対象:
  1. **技術要件書の書き直し**（[`docs/tech/tech-requirements.md`](../../docs/tech/tech-requirements.md)）— 新標準を反映する
  ...
  6. 年 1 回の保守状況点検（AwesomeAssertions・Wolverine）を[運用仕様書](../../docs/operations/)に記載する
- 対象外:
...
> 🔴 ［2026-08-22 追記 / #441］**この段落の「各サービスの再実装 issue は移行と同時に baseline から
> 自プロジェクトを削除する」は誤りであった。**baseline の残件はすべて `MassTransit` であり、
> 行が減る最小単位は**イベント辺**（1 イベントの発行元＋全購読先を一括）で、辺はサービスをまたぐ。
> **§対象外 の「サービス間通信（ブローカー・トランスポート）は #441 の担当」のほうが正である。**
> 裁定は [IADR-0234](../adr/IADR-0234_wolverine-migration-boundary-455-441.md) 決定 1・2・3。
> **本 issue（#455）は同 IADR の着地をもってクローズしてよい** —— 受け入れ基準は「検査器が成功する」で
> あり、baseline がゼロになることを条件にしていない。残りは #441 が持つ。
...
- [ ] [`docs/tech/tech-requirements.md`](../../docs/tech/tech-requirements.md) が新標準（Vertical Slice / Minimal API /
