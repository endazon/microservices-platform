...
- 上位のゲートウェイ設計は ADR-0010（計画リポ）（Accepted・本文凍結）。
  用途別モデル表は `06_technical/04_ai-rag-stack.md`。
- 要求: FR-11（LLM 送信可否の統制・用途別ルーティング）。
- 既存の実装決定: [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（既定 Opus・fable-5／Copilot 経路）。本作業はその**モデル版数のみ**を更新する。
- 本作業の実装判断は [IADR-0101](../adr/IADR-0101_default-model-opus-5.md)（既定 `max_tokens` の引き上げ根拠を含む）。
...
- **思考の既定有効化（最重要）**: `max_tokens` 据え置きは回答途中切れに直結する。引き上げ根拠は [IADR-0101](../adr/IADR-0101_default-model-opus-5.md)。
- **既定値の引き上げでは救済されない呼び出し元がある**: `max_tokens` を明示指定している呼び出しは既定値の
  ...
  ai-stock-trading 側の対応を先行または同時にマージする必要がある（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) フォローアップ 5）。
...
- **`stop_reason: "refusal"`**: Opus 5 はサイバー系の安全性分類器を持ち HTTP 200 + `refusal` を返し得る。
  ...
  ハンドリング追加は本作業のスコープ外とし、[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) にフォローアップとして記録する。
...
- マージ済みの point-in-time 記録（`docs/specs/20260706_*`・`feedback/*`・[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md) 本文）の追随改変。
  履歴不変の原則に従い、最新の roster は本仕様書と [IADR-0101](../adr/IADR-0101_default-model-opus-5.md) を正とする。
