...
> 「起点 ID の種別」／[IADR-0179](../adr/IADR-0179_unnumbered-nfr-for-meta-work.md)）。

## 起点となる計画書（トレーサビリティ）

- 機能要求（FR）: 該当なし（工程の規律であり、稼働する製品の要件ではない）
- ユースケース（UC）/ 画面（SC）: 該当なし
- 関連 ADR: [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md)（キットを足場の単一情報源とし固有デルタを 4 種に限定）/
  [IADR-0140](../adr/IADR-0140_cross-repo-issue-ref-checker.md)（クロスリポジトリ issue 参照の検査器）/ [IADR-0169](../adr/IADR-0169_cross-repo-ref-scan-beyond-markdown.md)（走査を Markdown 以外へ拡張）/
  [IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md)（キット分類表と突合検査）
...
## 母集合の引き方（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1 の規則 1〜6 ／ `traceability.repo.md` の規則 9・10）
...
1. **[IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 決定 1（Accepted）**: 分類 A は「キットの内容で上書きし、`diff` でバイト一致を
