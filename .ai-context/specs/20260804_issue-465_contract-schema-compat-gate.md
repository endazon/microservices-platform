...
- ユースケース（UC）/ 画面（SC）: なし
- **本作業が機械化する規約の典拠**:
  `06_technical/10_composability-design.md`（計画リポ）
  §3（100 行目）の原文——
...
- 含むもの:
  ...
  6. [`scripts/README.md`](../../scripts/README.md)（表・使い方・CI ジョブ表・**破壊的変更の手順**節）と
     [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md)（ゲート一覧・テスト種別表・切り出し項目の消し込み）の追随。
- 含まないもの:
...
- **.NET SDK は本 PR の実装セッションに存在しない**（`dotnet: command not found`。取得も agent proxy の
  policy denial で拒否）。実測記録は
  [`20260804_issue-486_bff-csproj-comment-iadr0117.md`](./20260804_issue-486_bff-csproj-comment-iadr0117.md)
  「ビルド検証の実行可否」節（裸の issue 番号ではなくファイルを直接指す——番号だけ書くと、#486 の
  issue 本文は別の話題であるため典拠を辿った人が迷子になる）。
  **SDK の在否はセッションの base image 構成に依存する時点の事実であり、恒久的な事実ではない**
  （[`scripts/setup.sh`](../../scripts/setup.sh) は SDK を入れない）。実際、同じ PR のレビューセッションでは
  `dotnet 10.0.302` が存在し `Platform.Shared.Contracts` のビルドに成功したという実測がある。
  したがって本書・IADR-0122 の「SDK 不在」はすべて**この実装セッション時点の観測**として読むこと。
  抽出方式の決定自体は SDK の在否に依存しない独立根拠（外部依存ゼロ・自己完結・ローカル/CI 同一挙動・
  `--self-test` 可能性）でも維持される。再評価条件は
  [IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) フォローアップ 5 を参照。
...
| .NET SDK 依存 | **無し**（SDK の在否に左右されない） | **有り**（SDK を持たないセッションでは実行不能） | 無し | 無し（protoc 別途） |
...
**B（リフレクション）を落とした主因は、本 PR の実装セッションに .NET SDK が無かったこと**であり、
精度の優劣ではない（前掲の測定条件のとおり、これは**時点の観測**であってリポジトリの恒久的な性質では
ない）。これを曖昧にしないため IADR にも本書にも明記する。SDK が要る検査器は、
(1) ローカルで実行できず「壊れたときにしか動かない検査」になる、
(2) `scripts.test.js`（#465 の受け入れ観点）から単体テストできない、
(3) 自己試験の負例を一時ツリーで実走査できない——の 3 点が同時に成立しない。

ただし**選定そのものは SDK の在否に依存しない**。検査器を外部依存ゼロ（Node 標準モジュールのみ）で
自己完結させると、ローカルでも CI でも、SDK のあるセッションでも無いセッションでも**同一に動く**。
`--self-test` が一時ツリーを実走査でき `scripts.test.js` から単体テストできるのも、ビルド成果物に
依存しないためである。SDK が恒常的に利用可能になった場合の再評価条件は
[IADR-0122](../adr/IADR-0122_contract-schema-source-and-compat-gate.md) フォローアップ 5 に定めた
（(a) SDK の恒常的な利用可能性・(b) 構文解析の限界が実際に顕在化・(c) 自己試験と単体テストの維持見通し、
の 3 つがすべて成立したときに限り再評価する。(a) だけでは切り替えない）。

**D（proto）は将来の正本である。** ADR-0029（計画リポ）
...
| [`docs/tests/TEST_STRATEGY.md`](../../docs/tests/TEST_STRATEGY.md) | ゲート一覧へ 1 行・テスト種別表の「契約」行を実装済みへ・切り出し項目の消し込み・ratchet 原則へ追記 |
...
- **docs 先行コミットから後続コミットの成果物へ張る前方リンクは、バッククォート表記が安全である**（squash マージ前提なら統合ブランチ上では解決するが、中間コミット単体では破損する。未マージ成果物への前方参照の扱いは [`docs/DEFINITION_OF_DONE.md`](../../docs/DEFINITION_OF_DONE.md) と
  [テスト戦略](../../docs/tests/TEST_STRATEGY.md) の同項と同じ作法）。
