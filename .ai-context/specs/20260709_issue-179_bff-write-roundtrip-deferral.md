...
- 関連 ADR: [IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)（BFF ABAC 書き込みゲート）／[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md)（後段の認可）／[IADR-0045](../adr/IADR-0045_bff-document-write-roundtrip-deferral.md)（本件の決定）
...
- [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md)（#174）が後段 `DocumentService` に課したのは**ロール認可のみ**で、**文書単位の ABAC
  スコープ照合ではない**ことを実コード（`DocumentEndpoints.cs`）で確認した。
- したがってプリフライト GET は**冗長ではなく、ABAC 書き込みゲートの唯一の実施点**（[IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)）。
...
1. [IADR-0045](../adr/IADR-0045_bff-document-write-roundtrip-deferral.md) を新設し、「2 往復を維持・最適化は実測まで保留」「素朴なプリフライト GET 削除は
