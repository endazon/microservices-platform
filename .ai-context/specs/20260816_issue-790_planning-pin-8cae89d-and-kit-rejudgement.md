...
- 起点 ID: **NFR**（計画の追随・キット同期・検査器＝メタ作業）。
  ...
  **文書・規約・キット同期の統制を扱う番号は 1 件も無い**（別紙
  [`plan-id-range-history-annex.md`](../../docs/how-to/plan-id-range-history-annex.md) §4 で実測済み）。
  **場合 2 は環流しない**（計画側に不足があるわけではない。[IADR-0179](../adr/IADR-0179_unnumbered-nfr-for-meta-work.md) 決定 2）。
  着手前に計画側の ID 列を実際に見て判断した（§2.2 の走査がその実測でもある）。
- 起点 issue: **#790**（pin 更新 A ＋ キット追随の再判定 F の束ね。束ねの判定は §7）
- pin: `4d6a7d6` → **`8cae89d`**（5 コミット）
- 関連 ADR: 実装ADR **[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md)**（本作業で新設）／[IADR-0201](../adr/IADR-0201_class-c-rejudgement-and-fail-closed-kit-checks.md)（部分改定）／
  [IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md)（分類表と X の追跡義務）／[IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 決定 2・3／[IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md)（0 件走査の門）／
  [IADR-0183](../adr/IADR-0183_false-green-warning-on-worktree-state.md)（worktree 警告）／[IADR-0200](../adr/IADR-0200_reading-budget-population-per-agent.md)（必読予算の母集合）。計画 ADR: 該当なし
- 分類（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 4 ＝ 監査強度の分岐）: **機械検査を改修する**（既存検査器 2 本の差し替え・
...
- 記録先: [`docs/how-to/plan-id-range-history-annex.md`](../../docs/how-to/plan-id-range-history-annex.md)。
...
### 2.5 必読規約の母集合（[IADR-0200](../adr/IADR-0200_reading-budget-population-per-agent.md) 決定 1・`check-reading-budget.js` が正）
...
| `check-planning-pin-freshness.js`（X） | #749 / [IADR-0202](../adr/IADR-0202_pin-freshness-comparison-source.md) が別途判定済み。ファイル領域が交差するため直列化する |
| `docs/how-to/session-handoff.md` | **並行 PR #789 が編集中**（交差）。「`scripts.test.js` は変更禁止（分類 A）」の記述が古くなるが、本 PR では触らない（[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 影響欄に明記し #793 へ送った） |
...
| 過去 IADR（0115 / 0130 / 0145 / 0187）の「`scripts.test.js` は分類 A」記述 | **当時の記録**である。後付けの書き換えは記録の改竄にあたる（`traceability.repo.md` §Superseded の作法）。live な最新版 [IADR-0201](../adr/IADR-0201_class-c-rejudgement-and-fail-closed-kit-checks.md) にだけ日付つき追記を入れた |
| `feedback/20260801_impl-handoff-kit-gaps.md` | 確定済みの環流記録（本文は書き換えない。[IADR-0191](../adr/IADR-0191_rewrite-boundary-is-body-vs-frontmatter.md)） |
...
**実測で入らないことが判ったので、[IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md) 決定 2 が定める「期限つきの暫定（X）」を採った。**
...
**保留の期限は機械が持つ**（[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 1）: `scripts.repo.test.js` に
...
—— 実効している側が一度も試験されなくなるため。[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 4）。追跡: **planning#380**（起票済み）。
...
**キット版は 0 件走査の門（#664 / [IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md)）を持たない。** これは**違反入力に対する出力の突合では
...
| **worktree 状態の警告（[IADR-0183](../adr/IADR-0183_false-green-warning-on-worktree-state.md)）** | **無し**（planning#374 の裁定文が明記） | **あり** |
| `EXCLUDED_DIRS` の export | 無し | あり |
| 未使用の `trackedMarkdown` の export | あり（主経路は使っていない） | 無し |

**判定**: [IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 3 のとおり **キット版を土台に採り、失う 2 点を目印つきで再付与**した。
...
| `check-doc-links` / `check-adr-numbering` / `check-plan-id-qualification` / `check-doc-type-vocabulary` / `check-doc-status-vocabulary` / `check-planning-pin-freshness` | 結果は PR 本文（[IADR-0183](../adr/IADR-0183_false-green-warning-on-worktree-state.md) の順序で `git add -A` の後に実行） |
...
- issue 本文は「planning#370 の裁定で実効上限が 2 → 4 件になった直後の最初の適用例」と述べるが、
  **本リポの `CLAUDE.md` と [IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) は依然「名目 3 件・実効 2 件」である**（改定 IADR は #791 の担当）。
...
- **issue #790 本文の「3. 分類 A の drift 2 件を解消する ── キット原文で上書きするのが分類 A の定義である」に
  ...
  [IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md) 決定 2 が X に「**期限つきの暫定**」を認めているため、追跡先とラチェットを付けて保留した。
  **issue 本文の想定（+3,002B が予算に入る）が実測と食い違っていた**ことが原因であり、判断は
  [IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) に残した。**人間の確認を求める。**
