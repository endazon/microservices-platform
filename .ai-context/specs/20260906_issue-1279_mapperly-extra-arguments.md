...
★［2026-09-06 追記 その 2 / #1279］🔴 **上の是正自身が規則 2 を破っていた —— 複合引用 `決定 N・M` を取りこぼした。**

1 回目の是正（`7c21a537`）は単発の `決定 N` 引用だけを直し、**`決定 1・2` の形で 2 つ並べている 3 箇所**を
見落とした。AI レビューが検出した。

| 箇所 | 直した |
| --- | --- |
| `DocumentEndpoints.cs:132` / `PrivateNoteEndpoints.cs:106` / `EndpointMappingWrapperTests.cs:8` | `IADR-0406 決定 1・2` → **`決定 1`** |

**なぜ誤りか**: 改番前の `IADR-0405`（現 `IADR-0406`）では、決定 1 ＝ 線、決定 2 ＝「導出は今それを
持っている端に残す」という**汎則**であり、この 3 箇所（タグ辞書引き・`?? string.Empty` の縮退・
ラッパの試験）はどれも汎則の適用例として妥当だった。ところが起草の途中で**旧決定 2 を決定 1 へ畳んだ**ため、
現在の決定 2 は「認可の**判定結果の名**は写す・**判定**は写さない」という `AiSuggestionMapper` 固有の話に
変わっている。**この 3 箇所は認可と無関係なので、決定 2 を巻き込んでいる分だけ誤りである。**

🔴 **規則 2（あり得る形をすべて列挙してから引く）の破れである。** `決定 N）` `決定 N:` の形だけで引き、
**`決定 N・M` という区切りの形を列挙しなかった。** 同一の是正の中で、同じ母集合を 2 回引き直すことになった。

**引き直した走査**（誤りの側から。複合形を明示的に含めた）:

```console
$ git grep -nE "IADR-040[56][^0-9].{0,40}決定 [0-9]\s*・" -- src/ .ai-context/ docs/
（是正後）
.ai-context/specs/20260906_issue-1233_restricted-project-controls.md:214  ← IADR-0405 決定 1・実測 4（#1233 側。正しい）
```

**残る複合引用は 1 件で、それは `IADR-0405`（#1233）の正しい引用である。**
