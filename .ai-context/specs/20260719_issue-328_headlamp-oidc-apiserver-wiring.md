...
- 関連 ADR: ADR-0004（認証＝Keycloak）。方式判断は [IADR-0084](../adr/IADR-0084_headlamp-oidc-apiserver-flags.md)。既存 [IADR-0080](../adr/IADR-0080_headlamp-k8s-management-ui.md)（Headlamp 導入・
  OIDC token passthrough・RBAC＝`oidc:developer` に cluster-admin bind）／[IADR-0066](../adr/IADR-0066_local-k8s-dev-environment.md)（経路B＝k3d dev 環境）／
  [IADR-0076](../adr/IADR-0076_edge-bff-routing-and-oidc-hostname.md)（issuer ホスト名＝手順A・in-cluster 正準名 `http://keycloak:8080`）。
- Issue: #328（本 issue・運用/dev・priority:should）。#271（PR #327・IADR-0080）のフォローアップ。

## 目的・背景（As-Is）

[IADR-0080](../adr/IADR-0080_headlamp-k8s-management-ui.md)（#271）で Headlamp を dev へ opt-in 導入した。認証は **OIDC token passthrough**（Headlamp が利用者の
...
| apiserver フラグ | 値 | 根拠 |
| --- | --- | --- |
| `oidc-issuer-url` | `http://keycloak:8080/realms/microservices-platform` | in-cluster 正準名（[IADR-0076](../adr/IADR-0076_edge-bff-routing-and-oidc-hostname.md) 手順A・[IADR-0066](../adr/IADR-0066_local-k8s-dev-environment.md) と整合） |
...
ロール/グループ別の権限分離は [IADR-0080](../adr/IADR-0080_headlamp-k8s-management-ui.md) 決定2どおり `poc-*` の役割で、`developer` は dev スーパーユーザー。
...
3. issuer は in-cluster 正準名 `http://keycloak:8080/realms/microservices-platform`（[IADR-0076](../adr/IADR-0076_edge-bff-routing-and-oidc-hostname.md) 手順A整合）。
...
7. 設計判断（issuer 到達性・claim マッピング・opt-in ゲート）を [IADR-0084](../adr/IADR-0084_headlamp-oidc-apiserver-flags.md) に記録。ADR 索引 README は自分の 1 行のみ追記。
