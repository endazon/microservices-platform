...
> （`.claude/rules/traceability.md`「無採番 `NFR` を許す 2 つの場合」の**場合 2**。[IADR-0179](../adr/IADR-0179_unnumbered-nfr-for-meta-work.md) 決定 2）。
> **環流しない。**
...
- 機能要求（FR）: なし（メタ作業）
- ユースケース（UC）/ 画面（SC）: なし
- 関連 ADR: [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 決定 2（分類 A/B/C）／[IADR-0192](../adr/IADR-0192_kit-sync-classification-and-check.md) 決定 2（X の定義）／
  **[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 2（分類 X 再判定の判定基準。本作業の判定基準そのもの）**／
  [IADR-0183](../adr/IADR-0183_false-green-warning-on-worktree-state.md)（`lib/worktree-state.js` の結線）／[IADR-0130](../adr/IADR-0130_test-spec-coverage-ratchet.md)（0 件走査の門）／
  [IADR-0207](../adr/IADR-0207_pr-title-trailing-number-must-be-own.md)（PR タイトル末尾の `(#NNN)`）
- 計画書リンク: `planning/tools/impl-handoff-kit/HOWTO.md`（計画リポ）
...
## ★ 判定基準 —— [IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 2 の 3 点突合
...
だけであり、いずれも [IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) 決定 2 の種に当たる。**恒久的に正しいデルタを X に置かない**
（分類表 `$comment`）。
...
| `docs/adr/IADR-0204` / `IADR-0201` / `IADR-0207` | **決定記録**であり、当時の実測と当時の判断を述べている（「環流待ち planning#379」は #790 時点の事実）。本文への後付け注記は**決定を変えるときだけ**。本作業は既存決定（[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 2）の**適用**であって改定ではない |
...
| `feedback/` の既存記録 | 本件で状態が動く記録は無い。#799 の環流記録は [IADR-0207](../adr/IADR-0207_pr-title-trailing-number-must-be-own.md) 決定 7 の判断で **`feedback/` へ置いていない**（未送付 0 件のラチェットのため。キットが実装済みになった今、置く必要も消えた） |
...
### ★ 変異試験 —— 2 方向の実測（[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 2 の点 2）
...
残る X 7 件（本件の射程外）: `.claude/rules/traceability.md`（[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 1 のラチェットつき暫定）／
...
3. **`docs/adr/IADR-0204` / `IADR-0201` / `IADR-0207` に後付け注記を入れなかった。**
   ...
   （[IADR-0204](../adr/IADR-0204_kit-catchup-deferral-with-expiry-ratchet.md) 決定 2）の**適用**であって改定ではないと判断した。
