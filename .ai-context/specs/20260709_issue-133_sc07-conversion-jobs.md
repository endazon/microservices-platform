...
- 機能要求（FR）: FR-12（文書正規化）
- 関連 ADR: [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)（本 PR で作成・変換ジョブ読み取りモデル）、[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（管理系ロール）、[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)（ワーカー最小 HTTP）、[[ADR-0003]]（MassTransit）
- Issue: #133（親 #121）
...
- 対象:
  - ConversionService: `IConversionJobStore`＋インメモリ実装（[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)。MVP・永続化は follow-up）、コンシューマでの成功／失敗記録（失敗は再送出しリトライ挙動を保持）、`/jobs`（一覧 `?status=`・個別・retry 再変換）エンドポイント。
...
- 対象外:
  - 変換ジョブの永続化（Postgres+EF）・複数インスタンス共有（[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) follow-up。別 issue）。
  - 変換出力（Markdown）の手編集 UI（人手補正は再変換に限定）。

## 受け入れ基準（Issue #133）との対応

- [x] 画面仕様書を作成（[SC-07_conversion-jobs.md](../../docs/screens/SC-07_conversion-jobs.md)）— 計画・UC-06 と整合。
...
- 変換読み取りモデルはインメモリ MVP（[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md) §決定 2）。失敗記録後に例外再送出でリトライ挙動不変（[[ADR-0003]]）。
