...
| # | 基準 | 写像先 |
| --- | --- | --- |
| AC-1 | 部門グループが 0 個になった利用者の属性 `department` が、同期（`Fix`）の後に消える | T-56 |
| AC-2 | 全利用者の列挙が途中で失敗した（ページの失敗・打ち切り）周期では、誰の属性も消えない。失敗は計器とログで知らせる（否定の試験） | T-57 |
| AC-3 | 1 つ属する利用者はこれまでどおり直る。2 個以上に属する利用者は触らない。サービスアカウントは消さない | T-58 |
| AC-4 | 属性辞書の `department`（利用者・文書の両スコープ）の許可値が realm の部門グループのコードと一致する | T-59 |
| AC-5 | realm を読めないときは「不明」と示し、既存の値を消さない | T-60 |
| AC-6 | 属性辞書の `department` の値を手で足す・消すことはできない（部門の追加・削除は realm の部門グループで行う） | T-61 |
...
| `docs/tests/SC-17_user-account-management.md` T-45 | 0 個の扱いを改め、T-56〜T-58 を足す |
| `docs/tests/SC-09_admin-abac-settings.md` | T-59〜T-61 相当を足す |
...
- 受け入れ基準 AC-1〜AC-6 は T-56〜T-61（テスト仕様書: 利用者アカウント管理／管理者設定）に写像し、いずれも緑。
- 変異（いずれも赤になることを確かめ、`git show HEAD:<path> > <path>` で戻した）:
  - 同期の「列挙が未完了なら 0 個の人を足さない」判定を外す → T-57（truncated）が赤。
  - 消す直前の所属の読み直しを外す → T-56 の歯止めの試験が赤。
  - サービスアカウントの除外を外す → T-58 ほか 7 件が赤。
  - realm を読めないときにも辞書へ当てはめる → T-60 の 3 件が赤。
