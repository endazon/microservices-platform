...
- [x] SC-19 の一覧・容量表示・新規作成・論理削除・復元・完全削除（単票／一括）が `/bff/*` から呼べる
- [x] SC-20 の端末一覧・発行・再発行・個別失効・一括失効・露出 3 トグルが `/bff/*` から呼べる
- [x] **本人は自分の資料へ到達できる**（陽性対照）
- [x] **他人の資料・他人の端末へは到達できない。応答は 403 ではなく 404**（存在秘匿）
- [x] **無認証は全端点で 401**（`/bff/*` の不変条件）
- [x] **write ポリシーが無い主体の書き込みは 403**。同じ主体の**読み取りは通る**（陽性対照）
- [x] 書き込み経路が `/authz/scope` へ送る action が **`write`** である（観測点で固定）
- [x] 後段の 507（容量上限）・409（パス重複・未削除）の**本文が詰め替えられずに届く**
- [x] トークンの平文が発行・再発行の応答**以外**に現れない
- [x] `docs/api/openapi.yaml` の `x-roles` と実効ロールが一致する（`check-bff-authz-docs.js`）
...
**2026-08-28 に実施。いずれも元へ戻し、最終実行は 35/35 緑・BFF 全体 398 件緑を確認した。**

| # | 変異 | 実測 |
| --- | --- | --- |
| 1 | `Forwarding` の Authorization 転送を落とす（＝本人絞りを外す） | **19 件 fail**（後段が主体を決められず 401。一覧・書き込み・端末のほぼ全経路が落ちる） |
| 2 | 書き込みの `BffScopeAction.Write` を `Read` へ劣化させる | **6 件 fail**（write 拒否の主体が 5 経路すべてで書き込めてしまい、action の観測点も割れる） |

戻し忘れの確認: 変異後のファイルを元本から復元し、`git diff` の対象が意図した差分のみであること・
両ユニットのビルドと全テストが緑であることを最終確認した。

## 実施の記録（2026-08-28）

- **公開したのは 11 口**（§公開する端点の表のとおり）。除外 6 口の理由は §母集合 軸 1 のまま変わっていない。
- 🔴 **契約 DTO の置き方を途中で改めた。** 当初は `Knowledge.Contracts` へ写しを置く前提だったが、
  後段（`DocumentService.Api.Foundation.Endpoints`）に**同名の record が既に在り**、
  両方を `using` する既存テストがビルドできなくなった（`CS0104` 3 件で実測）。
  別名や alias で逃げず、**タグ辞書の先例（`TagDto` を `Knowledge.Contracts` に置き
  DocumentService が使う）に合わせて定義を 1 つに畳んだ** —— 同じ形が 2 つあると、
  契約検査（`check-openapi-dto-drift` / `check-contract-schema`）が片方しか見ず静かに割れる。
  併せて論理削除・一括失効の**匿名型の応答を契約型へ替えた**（匿名型は openapi とも突き合わない）。
  影響: 後段の 2 ファイルから record 宣言を削除、後段テスト 3 ファイルへ `using` を追加。
  **JSON の形は変わっていない**（後段テスト 187 件が緑のまま）。
- **`docs/tests/SC-20_obsidian-settings-screen.md` を新設した。** 本 PR のテストが `SC-20` を
  参照したことで `check-test-traceability` が「実装先行・仕様書なし」を検出したためである
  （SC-20 の欠落は着手前から warn に出ていた）。
- **床の更新で他者の残件を巻き込んだ。** `check-test-spec-coverage --update` は全仕様書を
  一括で読むため、**着手時点（3c82642）で既に「床の上げ忘れ」だった FR-17 の 3 件**
  （`EdgeTypeResolverTests` / `LinkEdgeSyncTests` / `ObsidianLinkParserTests`）が同じ更新に入る。
  分離できないので、**隠さずここに書く**。
- **着手時点から赤いままのもの（本 PR とは無関係）**: `check-doc-links`
  （`IADR-0281` が `docs/note.md` を指す破損リンク 1 件）。本 PR では触っていない。
