...
- 起点 issue: [#381](https://github.com/endazon/microservices-platform/issues/381)
  （`feat(ADR-0022)`・label `enhancement` / `priority:should`）。[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) §フォローアップ 4 の消化。
- 計画根拠: ADR-0022（計画リポ）
...
- 設計: ADR-0010（計画リポ）（LLM ゲートウェイ・本文凍結）、
  [IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md)（ゲートウェイ経路・ZDR 除外）、[IADR-0101](../adr/IADR-0101_default-model-opus-5.md)（既定 Opus 5・max_tokens 4096）、
  [IADR-0102](../adr/IADR-0102_trade-decision-model-pinning.md)（`Models` 許可一覧に載せないとピン留めが無効化される罠）。
- 本作業の実装判断は [IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md)。
...
### 踏んではならない罠（[IADR-0102](../adr/IADR-0102_trade-decision-model-pinning.md) / #376 の再発防止）
...
これは [IADR-0102](../adr/IADR-0102_trade-decision-model-pinning.md) が取引判断のピン留めで実際に踏んだ罠であり、本作業でも同じ構造を持つ。
...
（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) の決定）ため、Sonnet 5 で 400 を返す破壊的パラメータは存在しない。差分は上表の
...
- マージ済みの point-in-time 記録（`docs/specs/20260706_*`・`feedback/*`・[IADR-0022](../adr/IADR-0022_default-opus-and-fable5-copilot-routes.md) 本文）の追随改変。
- `realm.json`（本作業と無関係）。
...
- [x] 実装 ADR（[IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md)）に記録し、機能仕様書・テスト仕様書・コード内コメントを更新した

## 検証: `max_tokens = 4096` は Sonnet 5 で足りるか

[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) が 1024 → 4096 へ引き上げた根拠は「従来どおりの本文長（〜1024 相当）＋ adaptive thinking の
...
`[1] + [2]` を合わせても、[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) が見込んだ「本文＋思考」の配分は 4096 の内側に収まる。よって
...
異なるラベルが表示に載る、という指摘（[IADR-0101](../adr/IADR-0101_default-model-opus-5.md) レビューの 🟢1）は妥当である。
...
[IADR-0106](../adr/IADR-0106_rag-answer-sonnet-5.md) §フォローアップに残し、別 issue で扱う。
