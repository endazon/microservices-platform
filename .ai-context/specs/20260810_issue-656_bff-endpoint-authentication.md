...
ハンドラ内 `AuthorizeAsync(user, ConfigViewer)` ＋ **404 存在秘匿**を採る形である（[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)。
...
[IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) の多層防御に反する状態である。
...
### 判断 2: 404 の存在秘匿を壊さない（[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) 決定 3）

`GET /bff/documents/{id}` は現在、権限外・不在ともに **404** を返す（[IADR-0009](../adr/IADR-0009_wiki-browsing-404-hides-existence.md)）。
...
### 判断 3: 検査器が「無認証」と「認証のみ」を見分ける（[IADR-0156](../adr/IADR-0156_bff-authz-contract-checker.md) の拡張）
