...
利用者条件に持つポリシーは、配備されても 1 度もマッチしない**。倒れる向きは deny（fail-closed）なので
...
★［2026-09-07 追記 / #1323］**解消した。** realm の多値マッパー・抽出点の集約（6 → 1）・
評価器の交差判定を 1 本の PR で着地させた（[[IADR-0411]]）。
**本仕様書の当時の記述は当時の事実として残す。**
