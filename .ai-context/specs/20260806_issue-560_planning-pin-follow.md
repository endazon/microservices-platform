...
[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の着手条件は前提 ADR が `Accepted` であることなので、
...
- [`IADR-0119`](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) への日付つき追記 —— **着手保留の権威文書は
  規約ファイルではなくこちらである。** 同 IADR は §フォローアップ 1 で「前提 ADR の状態を確認するたびに
  `［YYYY-MM-DD 追記］` を残す」運用を自ら定めており、直近の追随は PR #550 で行われていた。本 pin は
  まさにその運用が想定する種類の状態変化（`ADR-0035` に実測が反映されたが `Proposed` のまま）を扱うため、
  規約ファイルだけを直すと、**#450 / #451 の着手時に IADR-0119 だけを見た読み手が 2026-08-04 時点の情報で
  止まる**。前提 5 件が全件 `Proposed` であることを実測して追記した。
