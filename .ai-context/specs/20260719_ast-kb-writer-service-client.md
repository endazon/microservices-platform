...
> 設計判断は [IADR-0075](../adr/IADR-0075_ast-kb-writer-service-client.md)。AST 側は AST リポの `IADR-0093` / PR `feat/FR-08-kb-writer-cross-realm-s2s`。

## 背景（着手前確認）

- `POST /documents` は [IADR-0044](../adr/IADR-0044_backend-service-authorization-defense-in-depth.md) で `platform-admin`/`platform-operator` 必須、検証レルムは `microservices-platform`。
