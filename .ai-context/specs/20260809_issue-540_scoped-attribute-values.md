...
- **前提となる計画 ADR: ADR-0043（計画リポ）（`Accepted`・裁定 Q2）**
...
**#540 を単独で実装する。** [IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 の表は #540 と #542 を「**束ねる**」と事前判定していたが、
...
**束ねないが、決定 4 の「1 系統」は守る** —— #540 は**口をスコープで拡張できる形**に作る（[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 4）。
...
| スコープ → Qdrant フィルタ | `RetrievalService.../QdrantVectorStore.BuildAttributeFilter` | **同じ組み立てを使う**（検索段と定義を一致させる。[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 1） |
...
| 置き場所 | 形 | 候補の出所として使えるか |
| --- | --- | --- |
| Qdrant ペイロード `attributes` | **ネスト構造体** `attributes -> {k: v}`（[IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md)） | **使う**（検索段と同じ集合になる） |
...
| **#539（SC-01 / SC-08 の画面側フィルタ）** | 本作業が作る口を**使う側**である。[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 が「別資源・順序依存は束ねる理由にならない」と判定済み |
| `AttributeDefinition` の CRUD（`/authz/attributes`） | 既存の管理者限定 API。本作業は**読み取りの新しい口**を足すだけで、既存口は触らない |
| キャッシュ層 | **[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 3 で入れないと判断した**（主体ごとに変わり共有できず、キー忘れで他人の候補が漏れる面を作る） |
...
1. **契約**: 照会の要求（属性キー）と応答（**値の配列だけ**）を `Knowledge.Contracts` へ足す。
   **件数のフィールドを作らない**——[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 2・4（#542 が管理者スコープ専用に足せる形に保つ）。
...
3. **BFF**: `BffScopeResolver` でスコープをサーバ側解決し、後段へ渡す。
   **スコープが解決できないときは空配列**（404 にも 403 にもしない。[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) 決定 5）。
4. **画面は触らない**（#539 の射程）。
...
- **[IADR-0139](../adr/IADR-0139_domain-bundled-contract-prs.md) 決定 5 の表へ追記**（束ねの判定を覆したこと。**本作業で実施済み**）
...
| 指摘 | 対応 |
| --- | --- |
| 🟡 **実 Qdrant での facet の動作と payload index の要否が未検証**。`QdrantIngestionVectorStore.EnsureCollectionsAsync` は `tags` / `attributes.<key>` に索引を張っていない | **[IADR-0151](../adr/IADR-0151_scoped-attribute-value-facets.md) のフォローアップへ残課題として明記した。索引の付与は本 PR で行わない。** 検索段の ABAC フィルタも同じキーを未索引で引いており、**索引の要否は照会 API 固有ではなくフィルタ経路全体の性能判断**である。ここで片側だけ張ると「照会は速いが検索は遅い」という非対称を作る。実 Qdrant を立てるテスト構成が本リポに無いことも実測で確認した |
| 🟢 `ToPayloadKey` が `tags` だけ `OrdinalIgnoreCase` で、属性キーは大小を区別する理由が読めない | **`AttributeValueDto.cs` にコメントを足した。** `tags` は本コードが所有するリテラルなので畳んでよいが、**属性キーは投入時に書いたキーがそのままペイロードのキーになる**（[IADR-0014](../adr/IADR-0014_qdrant-attribute-payload-key.md)）ため、畳むと書き込み側と一致せず**黙って空集合が返る** |
...
| 指摘 | 対応 |
| --- | --- |
| 🟢 **「後段失敗はいずれも空配列へ縮退する」は実装より広い言い方**。非 2xx は `Results.StatusCode` で透過し、空配列へ畳むのは `HttpRequestException` / `TaskCanceledException`（接続断・タイムアウト）だけである | **指摘のとおりで、私の記述が不正確だった。** `BFF_bff-surface.md` の縮退表へ注記を足し、コード側のコメントも「後段へ到達できないときだけ」と直した。**`/bff/search` も同じ形**なので、注記は片方だけでなく**表全体に掛かる位置**へ置いた |

**「縮退」という語が 2 つの事象を飲み込んでいたのが原因である。** deny-by-default が守るのは
「**権限外の存在を示さない**」ことであって、**後段の障害を利用者から隠すことではない**。
後段が 500 を返せば 500 が出る——これは正しい振る舞いであり、隠すと運用側が不調に気づけない。

### 3 巡目のレビュー（🔴 0・🟡 0・🟢 0）

指摘は無かったが、**補足で「非 2xx 透過パスを狙い撃ちしたテストが `/bff/search` にも `/bff/attribute-values` にも無い」と指摘された**（重大度は付いていない。既存パターンだからである）。

**これは受け入れて足した。** 2 巡目で私は「非 2xx はそのまま透過する」と**文書に書いた**——
**書いた主張はテストで固定する**のが本作業の一貫した扱いである（件数が無いことも同じ理由で固定した）。
文書だけが主張し、実装がいつ変わっても誰も気づかない状態を残さない。

- `BffTestFactory.AttributeValuesStatusCode`（既定 `OK`）を足し、後段の応答コードを差し替えられるようにした。
  **`AssumptionsStatusCode` / `RiskControlsStatusCode` / `MonitorStatusCode` と同じ作法**である（既存の家風に合わせた）。
- `PostAttributeValues_WhenDownstreamFails_PropagatesStatus_NotEmptyList`（`[Theory]` 500 / 400）を足した。
  テスト仕様書は **T-16**。
- **`/bff/search` 側には足していない。** 本 issue の射程外であり、[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1（1 issue = 1 PR）に従う。
  **同じ欠落が `/bff/search` にもある**ことはここに記録する。

**platform の自動テストは 370 → 372 件**（`[Theory]` 2 ケース）。
