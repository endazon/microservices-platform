...
判断と実測は [`IADR-0322`](../adr/IADR-0322_wolverine-application-assembly-pinning.md)。
...
**混雑は原因ではない。** 直列化そのものは残す（IADR-0322 §検討して採らなかった案）。
...
6 ラウンドの下地になった。→ 対称に `PLATFORM_TEST_POSTGRES` を足した（IADR-0322 決定 6）。
...
**起きていない事象へ予防的に手を入れない**（#1038 が禁じた作法と同型）。申し送りは IADR-0322 §射程外。
