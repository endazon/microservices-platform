...
**本節は #816 を書いた環境についての記述であり、事実として残す**（[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md) 決定 2「覆った事実を消さない」）。
...
**できないことを「できた」と書かない**（[IADR-0184](../adr/IADR-0184_feedback-dispatch-checker-verbatim.md)）。§8 は引き続き
...
**本リポで最も繰り返し起きている事故の型**である（[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) 決定 1）。
...
（[IADR-0210](../adr/IADR-0210_local-k8s-observability-persistence.md) 決定 6）。**撤回の根拠は稼働中の k3s での実測である。**
...
（[IADR-0210](../adr/IADR-0210_local-k8s-observability-persistence.md) 決定 4 が鏡の射程を限る）。
...
**#816 の設計にはこの決定も記述も無かった。** 統合で足した（[IADR-0210](../adr/IADR-0210_local-k8s-observability-persistence.md) 決定 7）。
...
**この記録は #816 の環境についてのものであり、消さない**（[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md) 決定 2）。
...
> [IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md) の「同型の事故が 2 回起きたら検査器・規約を足す」の母数に当たる 1 件である
...
| --- | --- |
| `#787: 可観測性 overlay は root 実行へ落とさない（local-path は 0777 で作る）`（**入れ替え**） | §4.6 / [IADR-0210](../adr/IADR-0210_local-k8s-observability-persistence.md) 決定 6 |
| `#787: PVC を掴む Deployment は Recreate（RWO と RollingUpdate は両立しない）` | §4.8 / [IADR-0210](../adr/IADR-0210_local-k8s-observability-persistence.md) 決定 7 |
...
**[IADR-0116](../adr/IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1（1 issue = 1 PR）へ戻すための統合**である
...
容量が違うのか」が読み取れない。**覆った決定と、その根拠になった実測を残す**（[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md) 決定 2）。
