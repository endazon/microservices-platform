...
- 関連 ADR: `ADR-0004`（ABAC）、`ADR-0034`（ホップごとの ABAC 強制）、`ADR-0036`（所有者ベース裁量制御）。
  実装 ADR は [IADR-0019](../adr/IADR-0019_datasource-default-attributes.md)（機密区分のフェイルセーフ既定）・[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md)（取り込み必須属性のフェイルセーフ。
  決定 4 が `lifecycle` の終端 `active` を持つ）・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)（i18n カタログの網羅検査）
- 計画書リンク:
  - `06_technical/09_datasource-connectors.md`（計画リポ）
    §システム投入経路での `owner` / `department` / `lifecycle`（確定・2026-08-15。`lifecycle` は同日追補）
  - `06_technical/07_abac-attribute-model.md`（計画リポ）
    §文書の基本属性（`lifecycle`＝状態・**必須**・値域 `draft` / `active` / `archived`）
  - `05_screens/01_screens.md`（計画リポ） §SC-05 / §SC-06
...
| 3 | 終端 `active` | 実装済み（バックエンド `DataSource.DefaultLifecycle`。[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 4） |
...
3. **バックエンドの受け口（`.../DataSourceService.Api/Foundation/Domain/DataSource.cs`）** ——
   ...
   `Create` / `Update` / `Patch` / `GetEffectiveAttributes` の 4 経路が同じ関数を通る（[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 1）。
...
| 20 | `どの画面も送っていない`（**誤りになる文そのもの**） | **2** | **2** | [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) L184（live）／`docs/specs/20260815_issue-516_…`（**確定済み**） |
| 21 | `値のばらつき` | **1** | **1** | [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) L209（live） |
...
| `docs/data/data-source.md` | **変更（1 行の日付つき注記）** | L119「ソース単位で下書き扱いにしたい場合は既定属性で `draft` を指定する」が**経路を書いていない**。API しか無いと読めるため、SC-06 の欄を指す 1 文を足す。**理由書きは複写しない**（正は [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md)。#767 が同じ扱いを採った） |
...
| `features/sc03-document/attributes.test.ts`（軸 1） | **除外** | SC-05 / SC-03 の**文書**属性ラベルの話（`known = ['confidentiality','department']`）。**データソースの既定属性とは別の経路**であり、本 PR の射程外（[IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) 決定 5 が「保存前検証は強めない」と定める） |
...
- 未指定時に何が起きるかを補助文で伝える（**「予約値」ではなく「既定値」と書く** —— [IADR-0199](../adr/IADR-0199_ingestion-required-attribute-failsafe.md) が
...
ことの下限コスト」**であり、[IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) の分割境界を変えても消せない。
