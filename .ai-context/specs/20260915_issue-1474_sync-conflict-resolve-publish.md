...
- [ ] 露出 3 トグル OFF の個人資料で `local` 解決 → 発行されない（陽性対照: 本文はローカル版・版は +2〔サーバ版の push で +1・解決で +1〕）
- [ ] ゴミ箱の個人資料への `local` / `both` 解決は 409 `deleted`（本文を書かず・発行しない）。`server` は通る

> ［2026-09-15 追記 / #1474］**フェーズ末監査（PR #1476・別文脈のエージェント・条件付き合格）の指摘を反映した。**
> D1（必須）: ゴミ箱の個人資料でも `local` 解決が本文を上書きし、露出 ON なら索引へ再発行していた（push・移動は 409 `deleted` で拒否）→ 同じ 409 を返すガードを置いた（`server` は資料を変えないので通す）。
> D2（推奨）: 発行が監査記録の前にあり、発行が例外になると解決の監査行が残らなかった → 発行を監査記録の後へ移した。
> D4（推奨）: 上の受け入れ基準の「版は +1」は誤りで、試験どおり +2 である → 訂正した。
> D3（計画の問い）: 「両方を残す」の別名コピーが露出を引き継ぐか → planning#636 へ環流した。
