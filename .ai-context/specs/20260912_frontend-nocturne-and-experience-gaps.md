...
［2026-09-12 追記 / planning#613 / planning#614］**環流は起票済みで、計画側の裁定も出た（利用者裁定 2026-09-12）。**
- **B 表の `SC-19` / `SC-20`（planning#614）**: 6 要素はいずれも**計画本文が既に定義していた**（SC-19 主要素 1・2・5、SC-20
  主要素 3・5・6）。**不足は計画ではなく BFF 契約**である。計画側は SC-19 / SC-20 に「供給元と契約の現状」の表を足し、
  契約が来るまで**描かない（列そのものを出さない）**ことを裁定 planning#286 の 3 点セットと同じ形で明記した。
  **本実装の「描かずに空けている」は正しい**。残作業は契約の拡張で、#1441（`PrivateNoteDto`: 公開範囲・同期状態・タグ。
  🔴 公開範囲の指定先の単位は ADR-0036 §未確定事項 5 の裁定待ち）と #1442（同期契約: 同期対象範囲・競合解決・同期履歴。
  🔴 「直近 N 件」の N と保持期間は計画に無く裁定待ち）へ起票した。
- **B 表の `SC-10`（planning#614）**: 🔴 **報告は成立しなかった。** 「用途別」「モデル別」「利用実績」の語は MSP のモック全体で
  0 件で、モックは Q25・Q26 の裁定（LLM コストは Grafana へ委ねる）に従っている。計画側は SC-10 本文へ「LLM 利用実績の表は
  本画面に置かない」を明示した。**本書 B 表の SC-10 行の「LLM 利用実績の表」は本書作成時の誤読であり、実装は変えない。**
- **A 表の `SC-03` / `SC-21` と AST 側の 2 件（planning#613）**: AST の SC-03 維持率の位置・SC-01 見出しは**本文が正**で、
  モック側が訂正された（モックが自己矛盾していた）。計画の規則（`docs/mockup-placement-rules.md`）は「本文が正・
  モックが正は利用者から新たに受領したときの例外」へ反転した。記録は planning `projects/ai-stock-trading/10_feedback/
  20260912_mock-vs-body-conflict.md` / `projects/microservices-platform/10_feedback/20260912_mock-elements-without-contract.md`。
