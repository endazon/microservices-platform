...
| M-5 | **コメントアウトされた旧参照**を残す（`# uses: …@v1`） | ✅ 拾わない（陰性） |
| M-6 | ★ 陽性対照 —— そのコメントを外す | ✅ 拾う（無条件に無視していない） |
| M-7 | **`- uses:`（steps のハイフン記法）**の浮動参照 | ✅ 拾う |
| M-8 | 同記法で SHA 固定 | ✅ 通す（記法だけで落とさない） |
| M-9 | 記法が混在した版の不一致 | ✅ 拾う |
...
🔴 **M-5 は 2 巡目のレビューが見つけた私の欠陥である。** 初稿は本文全体へ `matchAll` を掛けており、
`# uses: anthropics/claude-code-action@v1`（**旧版をコメントで残す**という移行途中のありふれた編集）を
有効な参照として拾って落ちた。**本検査器が防いでいる事故を、検査器自身が起こす形**である。
⇒ 行単位で走査し、インデントを除いた行頭が `#` の行を除外した。
**末尾コメント（`@SHA # v1（…）`）は従来どおり参照として拾う**（M-4 と対で固定した）。

🔴 **M-7 は 3 巡目のレビューが見つけた私の欠陥である。** 行頭アンカーを `^uses:` にしていたため、
`- uses:`（steps のハイフン記法。**本リポジトリの他 8 ワークフローが使っている書き方**）を
1 件も拾えなかった。**参照 0 件は fail-open の「何も言わない」分岐へ落ちる**ので、
射程外になったことが誰にも見えない —— 最も悪い壊れ方である。
⇒ アンカーを `^(?:-\s*)?uses:` へ緩め、記法ごとに陽性・陰性を対で置いた。

**3 巡とも「検査器が静かに無力化する形」を突かれている。** 私が置いた門は、
自分自身が no-op になる経路を毎回持っていた。
