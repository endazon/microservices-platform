...
- [ ] 5. 変異試験: `scripts/lib/worktree-state.js` を退避／無効化すると **#842 のテストが fail** する
      （no-op スタブへ差し替えると **#842 だけが fail し #836 の 2 本は緑**。退避（ファイル削除）では
      `check-doc-updated.js` の即時 require が先に落ちて companion 全体が止まる）
      🔴 **［2026-08-21 追記 / #842］この基準は未達である。** 再現したところ no-op スタブでも手前のテストが
      先に落ち、「#842 だけが fail」は**一度も観測できていない**。**チェックを `[x]` から外した** ——
      チェックボックスは「いま満たしているか」を表す状態欄であり、本文プロズの後付け書き換えには
      当たらない（#717 / [[IADR-0191]] 決定 2 と同じ扱い）。**取り消し線は引かない** ——
      基準そのものは今も妥当で、示せていないのは証明方法の側である。詳細は §訂正 4。
...
**受け入れ基準 5 は「未達」として扱い、上のチェックボックスを `[x]` から外した。**

### それでも `Closes #842` を維持する理由

**#783 と同じ扱い（`Refs` へ格下げ）にはしない。両者は性質が違う。**

| | #783 | #842 |
| --- | --- | --- |
| 受け入れ基準の出どころ | **issue 本文からの直接引用**（4 件） | **本仕様書の自己定義**（issue 本文に番号付き基準は無い） |
| issue が「やること」として求めた核 | ①検証ジョブ ②統合スタックを CI で起こす経路 | 正しい `lib` を置いた一時 dir で検査器を走らせ、**worktree 警告が実際に出ることを固定する** |
| 核の充足 | **① だけ。かつ ① 自体も基準 1 が未達** | **満たしている** —— `dd6c867` の
`#842: 正しい lib を置くと結線が実際に働く（警告行が出る。黙って no-op へ落ちない）` が緑 |
| 未達なもの | **issue が literal に求めた基準そのもの** | **自己定義の「追加の証明方法」** |

**つまり #783 は issue の要求が未達、#842 は自分で足した証明方法が未達である。** 前者は
`Closes` にできない。後者は issue をクローズしてよいが、**「変異試験で示した」という記述は取り下げる。**

🔴 **この非対称は説明を要する** ——「自分の前回の数えも他人の数えと同じ検証を要る」と言いながら、
**#783 は格下げし #842 は無言で `Closes` のままにしていた**。AI レビューの指摘（🟡）が正しい。
**説明を書かずに扱いを変えていたことが問題**であり、扱いそのものは上の表のとおり妥当である。
