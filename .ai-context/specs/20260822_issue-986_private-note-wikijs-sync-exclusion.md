...
### 🔴 この 2 つは「動く」点で見分けがつかない

**個人資料が実データに 1 件も存在しない現在、どちらの実装も「個人資料を同期しない」。** 否定形テストだけでは通ってしまう。

**分けられるのは「`doc_scope` を持たない文書が同期される」という陽性対照テストだけである。**

**したがって受け入れ条件に、その陽性対照が `!= "organization"` の変異で落ちることの実測を含める**（§6）。
...
| --- | --- |
| 除外条件を外す | **1** |
| `== "private-note"` を `!= "organization"` に変える | 🔴 **3**（2 と 1 は通ってしまう） |

**両方を実測してから完了とする。** 変異が当たったことを先に assert する（`git diff` で当該箇所のみ変化＋ビルド EXIT=0）。
