...
- 関連 ADR: [IADR-0046](../adr/IADR-0046_config-version-history-source.md)（履歴の正データ源＝GitOps 層・非永続 surfacing）／[IADR-0029](../adr/IADR-0029_config-info-api-placement-and-drift-granularity.md)（構成情報 API 配置）／
  本作業の配線判断は [IADR-0069](../adr/IADR-0069_config-history-gitops-injection-wiring.md)
- Issue: #192（親 #123 の唯一の任意残項目を切り出し）／API 契約・SC-11 表示は #139（PR #189・[IADR-0046](../adr/IADR-0046_config-version-history-source.md)）で実装済み／
...
- プラットフォームのサービスへの履歴ストア新設（[IADR-0046](../adr/IADR-0046_config-version-history-source.md) で明確に不採用。第二の真実を作らない）。
- 保持範囲の決定（GitOps 側＝Git 履歴／ArgoCD 保持リビジョン数に委ねる。[IADR-0046](../adr/IADR-0046_config-version-history-source.md)）。
- **実 ArgoCD/Git ログからの自動履歴生成（ライブ CD 供給）**。実値 end-to-end の達成はライブ CD 依存のため
  後続／環境待ちとし、本 PR は配線として独立に発行する（`Refs #192`。[IADR-0069](../adr/IADR-0069_config-history-gitops-injection-wiring.md) 「影響」参照）。

## 設計判断（[IADR-0069](../adr/IADR-0069_config-history-gitops-injection-wiring.md)）

- 履歴は**注入経路を現在バージョンと共有**する（新機構を足さない・[IADR-0046](../adr/IADR-0046_config-version-history-source.md) 準拠）。ASP.NET の構成配列規約に
...
- `hadDrift` は注入時に判明していれば `"true"/"false"` を出し、未設定なら env 自体を出さない（`bool?` の null＝不明を
  保つ。遡及計算しない・[IADR-0046](../adr/IADR-0046_config-version-history-source.md)）。
...
- [x] [IADR-0046](../adr/IADR-0046_config-version-history-source.md)（正データ源＝GitOps 層・非永続 surfacing）に違反しない（履歴ストア新設をしない）。
- [x] tech/infra いずれか該当の仕様と、必要な IADR を更新（本仕様書＋[IADR-0069](../adr/IADR-0069_config-history-gitops-injection-wiring.md)＋operations/deployment/argocd/FR-15）。
