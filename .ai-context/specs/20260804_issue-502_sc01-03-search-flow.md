...
related_specs:
  ...
  - ../../docs/screens/SC-03_document-detail.md
  - ../../docs/tests/SC-01_search-chat.md
  - ../../docs/tests/SC-02_search-results.md
  - ../../docs/tests/SC-03_document-detail.md
...
- 関連する技術検討（計画）:
  ...
  08_data-egress-policy（計画リポ）（外部 CDN・Web フォント・analytics を使わない）／
  INDEX（計画リポ） 決定 21（色だけで意味を持たせない）・決定 26（`private-note` の画面ラベルは「個人資料」）
- モックアップ（**実装の正**）:
  hi-fi/sc-01.html（計画リポ） /
  sc-02.html（計画リポ） /
  sc-03.html（計画リポ）
- 関連 IADR: **[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)（FR-17〜21 の着手保留）**・[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md)・[IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md)・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)・**[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md)（本作業の内部設計判断。本書と対で読む）**
- 本リポジトリの起点: **#502**（親 #452 / #446 / #454。前提 #490＝PR #495・#496＝PR #499 はマージ済み）

## 目的・背景

13_frontend-stack §実装への移行方針（計画リポ）
...
1. **SC-01 / SC-02 / SC-03 の再実装**（hi-fi モックアップと 05_screens を正とする）。
   - サーバー状態は **TanStack Query**（[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md)）。
...
| 事項 | 送り先 | 理由 |
| --- | --- | --- |
| **SC-03 の AI 提案承認欄**（FR-18） | **保留解除後の後続 issue** | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1「保留の対象は当該 FR を実現するプロダクトコードと、**その受け入れを担う画面**」。着手条件（決定 2）は前提 ADR の **`Accepted`** 化であり、`ADR-0033` / `ADR-0034` / `ADR-0035` は planning `d980a01` 時点で **`Proposed`** |
...
| **SC-01 の出典行の機密区分チップ** | 同上 | `CitationDto` に属性フィールドが無く、値そのものを出せない。§2 |
...
内部設計の判断（選択肢の比較・棄却理由）は [IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) を正とする。本節は実装の形を記す。
画面ごとの詳細は画面仕様書（[SC-01](../../docs/screens/SC-01_search-chat.md) /
[SC-02](../../docs/screens/SC-02_search-results.md) / [SC-03](../../docs/screens/SC-03_document-detail.md)）を正とする。
...
| 種別 | 対象 | 根拠 |
| --- | --- | --- |
| **A. FR の着手保留** | SC-03 の AI 提案承認欄・SC-18 導線／SC-01 の個人資料トグル・`👤` 出典 | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1・2。前提 ADR が `Proposed` |
| **B. 契約の不在** | SC-01 の対象範囲フィルタ・**出典行の機密区分チップ**／SC-02 の検索モード・並び順・更新日時列／SC-03 の機密区分表示名 | BFF ＋ 検索サービスの契約に載る先が無い。実測は `feedback/20260804_sc01-03-bff-contract-gaps.md` |
...
**B の数え方（以降の「6 件」の基準）**: 本書は B を**要素名で数えて 6 件**とする——対象範囲フィルタ・
出典行の機密区分チップ・検索モード・並び順・更新日時・機密区分の表示名。環流記録の表は**行で数えて 7 行**である
（対象範囲フィルタが「絞り込み条件を要求へ載せる口」と「**権限内**候補を得る口」の 2 行に分かれるため）。
...
**アイコンも文言も持たない**（[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 の「プリミティブは文言を持たない」に従う）。
...
- [ ] **AI 提案承認欄・SC-18 導線を実装していないことと理由（[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)）が仕様書に明記されている。**
      → [SC-03 画面仕様書](../../docs/screens/SC-03_document-detail.md) §hi-fi モックアップとの対応 #7〜#9 と §実装しない要素の理由。
...
（[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) 決定 3）。数字の出所を取り違えないよう明記する。
...
| # | 指摘 | 是正 |
| --- | --- | --- |
| 1 | [IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) が Accepted な [IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 5 と**同じ計画未決事項に二重回答**し、キャッシュ方針が食い違うのに §関連 が無言だった | IADR-0126 へ**適用範囲**（SC-01 本文のみ／右レールは決定 5 が正）と**並存する決定**を明記。計画未決事項へ答えたのは決定 5 である旨へ改めた。IADR-0121 決定 5 の直下へ日付付き［追記］を入れ、§関連 の部分改定リストへ 3 件目として追加。**方針差は §フォローアップ 1 に「未解決」として明記**し、統一は第 4 段で決定 5 の改定として判断する |
...
| 7 | [IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 の「8 件」が `Tag` 追加で 9 件になったのに無記録 | 決定 1 の直下へ日付付き［追記］（3 情報源の突き合わせによる追加根拠つき）と §関連 への部分改定を追加。`src/packages/ui/README.md` の一覧と「8 部品は判定表と一致」の記述も更新 |
...
**この不具合の性質**: [IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) 決定 3 は「検索語の単一情報源は URL」と定めたのに、
...
| 事項 | 計画の記載 | 実装 | 根拠 |
| --- | --- | --- | --- |
| **SC-03 の AI 提案承認欄・SC-18 導線** | 05_screens §SC-03「SC-03 に置くのは次の 2 つのみである: ①SC-18 への導線、②AI 提案の承認欄」 | **実装しない** | **FR-17 / FR-18** に属し、[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1 が「その受け入れを担う画面」まで保留対象としている。決定 2 の着手条件は前提 ADR の `Accepted` 化であり、`ADR-0033` / `0034` / `0035` は planning `d980a01` 時点で全件 `Proposed`。**繰り延べであって放棄ではない**（保留解除後の後続 issue） |
...
| SC-03 の AI 提案承認欄・SC-18 導線、SC-01 の個人資料まわり | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の**保留解除後**の issue（解除は決定 6 の手順で行う） |
| 契約の不在 6 件（**要素名基準**。§2 の「B の数え方」） | **planning#197 の裁定待ち**（`feedback/20260804_sc01-03-bff-contract-gaps.md`）。裁定後にバックエンド側の issue が要る |
...
| バンドルサイズ（571 kB / gzip 170 kB） | 全画面の再実装が終わってからのコード分割（#490 / #496 の未決事項を引き継ぐ）。**［2026-08-05 追記］#512 / [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) で消化**（ルート単位の遅延 ＋ `manualChunks` 3 規則。500 kB 警告は解消し、初期ロードは 632.98 → 577.54 kB / gzip 190.04 → 177.94 kB）。 |
...
1. **着手保留中の機能の起点 ID を、テスト側のコメント・テスト名に書かない運用を確立した。**
   ...
   **是正**: テスト側から当該 ID を外し（保留の追跡は [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) と
...
1. **契約の不在 6 件**（§2 の B。**要素名基準**＝環流記録の表では 7 行。§2 の「B の数え方」）。
   **planning#197 として起票済み・裁定待ち**であり、実装の再開は裁定の後になる。
2. **SSE 中断時の再試行方針**（[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md) §フォローアップ 2）。
