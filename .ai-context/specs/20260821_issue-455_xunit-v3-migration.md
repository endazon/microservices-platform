...
採用は別 issue（**#882** を起票済み）の段階採用に回す。`TreatWarningsAsErrors` は `false` で
ビルドの成否は変わらない。**抑止は恒久ではなく、#882 の完了時に外す。**
...
> ［2026-08-21 追記 / #455 A-2］🔴 **9 箇所目を落としていた。** AI レビューの指摘で判明。
> `docs/` の 2 文書（`tech-requirements.md` / `TEST_STRATEGY.md`）は本文を `IADR-0231` の決定に
> 沿って書き換えたのに、**trace ブロックの `iadrs:` へ `IADR-0231` を足していなかった**。
> `specs:` と `issues:`（#882）も同様。
>
> **なぜ捕まらなかったか。** 私が引いた語（「残件」「広範」「使用中」「ratchet 管理下」）は
> **本文の記述**を捕まえる語であり、**trace ブロックは本文ではない**（非表示メタデータである）。
> 規則 9・10 は「誤りの側の文字列で引く」と言うが、**足りていない側は文字列として存在しない**。
> 🔴 **欠落は「誤りの語」では引けない。** 新設した ID（`IADR-0231`）の側から
> 「**この ID を引くべき文書はどれか**」を逆に引く必要があった。
>
> **検査器は足さない。** `check-trace-blocks.js` は ID の実在性だけを見て網羅性を見ないが、
> 同型の事故はこれが 1 回目である（同じ文書で `IADR-0216` は追記時に正しく足されていた＝先例あり）。
> 本リポジトリの規約は「**同型の事故が 2 回起きたら**検査器を足す」であり、1 回目は記録に留める。
