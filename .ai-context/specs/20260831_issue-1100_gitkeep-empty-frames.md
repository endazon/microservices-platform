...
- 群 E（`docs/` の種別フォルダ・`.ai-context/specs/`）: **「標準構成要素の枠」ではない。**
  ...
  > 🔴 **［2026-08-31 追記 / #1122］この扱いは粗すぎた。15 件のうち 11 件は #1122 で撤去した。**
  > 本作業は `.gitkeep` の**有無**しか見ておらず、**そのディレクトリに実体があるか**を数えていなかった。
  > `.ai-context/specs` には追跡下のファイルが **549 件**あり、`.gitkeep` は何も keep していなかった。
  > **残るのは実体 0 件の 4 件だけである。** 詳細は
  > [`20260831_issue-1122_unit-level-scaffolding-frames.md`](./20260831_issue-1122_unit-level-scaffolding-frames.md) §1 軸 4。
