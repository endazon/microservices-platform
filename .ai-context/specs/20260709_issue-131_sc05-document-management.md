...
- 関連 ADR: [IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)（本 PR で作成・書き込み BFF とスコープ内限定）、[IADR-0038](../adr/IADR-0038_bff-document-read-abac-gating.md)（読み取り BFF）、[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（管理系ロール）
- Issue: #131（親 #121）

## 目的・背景

SPA 上に SC-05 を実装する。読み取り側（`/bff/documents`）は SC-03（#129）で集約済み。本 PR で**書き込み側**（作成・更新・公開・アーカイブ・削除）を BFF に追加する（Wave B 方針）。管理系のため admin/operator 限定、既存文書操作は ABAC スコープ内限定（[IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)）。
...
- [x] 画面仕様書を作成（[SC-05_document-management.md](../../docs/screens/SC-05_document-management.md)）— 計画・UC-03 と整合。
...
- 書き込みにも読み取りと同じ ABAC 境界を課す（閲覧不可＝変更不可）。[IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md) §決定 2。
