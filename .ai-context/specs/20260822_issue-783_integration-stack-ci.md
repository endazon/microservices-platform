...
| G2 **0 件で緑にしない** | 走査対象（Deployment / pod）が 0 件のときに `wait --all` が自明に成功すること |
| G3 **ツール不在で素通りしない** | k3d / kubectl / helm が入っていないのに検証を飛ばして緑になること。**抜け道の env を置かない** |
| G4 **エッジ・issuer の assert** | `keycloak-edge` Ingress と discovery の `issuer` が失われても気付かないこと |
| G5 **entrypoint の assert** | admin:50000 が立っていないことが静かに素通りすること（§2.6。扱いは §7 の未決事項） |

**G2 は形式的な注意ではない。** `kubectl wait --for=condition=Ready pods --all` は
**対象が 0 件のとき成功する**。数えずに待つと「何も無いから緑」が起きる。
...
| M5 | **M4 を当てたまま readiness ゲートのステップを削除する** | **緑になることを実測する**（＝ゲートが要る証拠） |
| M6 | namespace を空にして走らせる | G2（0 件で緑にしない） |
