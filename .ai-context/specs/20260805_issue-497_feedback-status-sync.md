...
- 非機能（**NFR**）: 計画と実装のトレーサビリティ維持。規約は
  ...
  `feedback/README.md`（環流記録。計画リポ `projects/microservices-platform/10_feedback/README.md` へ移設）（**この `feedback/` は実装リポジトリ側の控えであり、
  原典の反映先は計画リポジトリである**）。
- 画面（**SC-11**）: 構成ビューア（05_screens/01_screens.md（計画リポ））。
...
計画リポジトリ draft/feedback/README.md（計画リポ）（`:19` / `:85`）が正である。
...
計画リポジトリの draft/feedback/README.md（計画リポ） `:85-87` は語彙を
...
1. **書換対象 10 件と同型の未同期（impl=`open` / plan=`accepted`）は、全数で 6 件残る。** 後続 issue の
   ...
   **除外しない**（本書は当初「A 群 3 件」と書いていたが、これは過少である）。
   ...
   「MSP 側は kit 反映のリリース後、暫定デルタを撤去してバイト一致へ戻す（[IADR-0115](../adr/IADR-0115_impl-handoff-kit-as-single-source.md) の運用）」と
