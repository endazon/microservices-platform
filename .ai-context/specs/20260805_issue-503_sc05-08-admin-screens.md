...
related_specs:
  ...
  - ../../docs/tests/SC-05_document-management.md
  - ../../docs/tests/SC-06_datasource-management.md
  - ../../docs/tests/SC-07_conversion-jobs.md
  - ../../docs/tests/SC-08_ai-analysis-dashboard.md
...
- 関連する技術検討（計画）:
  13_frontend-stack（計画リポ）
  （**§shadcn/ui 派生の範囲 の 4 基準**・§実装への移行方針「**旧画面（13 画面）の完全削除**は移行の完了条件の一部」）／
  08_data-egress-policy（計画リポ）／
  INDEX（計画リポ） 決定 21（**色だけで意味を持たせない**）
- モックアップ（**実装の正**）:
  hi-fi/sc-05.html（計画リポ） /
  sc-06.html（計画リポ） /
  sc-07.html（計画リポ） /
  sc-08.html（計画リポ）
- 関連 IADR: **[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md)（本作業の内部設計判断。本書と対で読む）**・[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md)・[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md)・
  [IADR-0124](../adr/IADR-0124_tanstack-router-unit-composition.md)・[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)・[IADR-0126](../adr/IADR-0126_sse-answer-state-and-search-url-state.md)・[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)・[IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)・[IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)
...
内部設計の判断（選択肢の比較・棄却理由）は [IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) を正とする。本節は実装の形を記す。
画面ごとの詳細は画面仕様書（[SC-05](../../docs/screens/SC-05_document-management.md) /
[SC-06](../../docs/screens/SC-06_datasource-management.md) / [SC-07](../../docs/screens/SC-07_conversion-jobs.md) /
[SC-08](../../docs/screens/SC-08_ai-analysis-dashboard.md)）を正とする。
...
| --- | --- | --- |
| SC-07 の**閲覧**（ルート・ナビ） | `platform-admin` **または** `platform-operator`（[IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md) / [IADR-0042](../adr/IADR-0042_conversion-job-read-model.md)） | **変えない**（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1） |
...
（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1）。
...
| 種別 | 対象 | 根拠 |
| --- | --- | --- |
| **A. FR の着手保留** | （本 issue では**該当なし**。SC-05〜08 はいずれも FR-01/02/06/07/09/11/12 に属し、[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) の保留対象 FR-17〜21 を含まない） | [IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1・2 |
...
| 画面 | 要素 | 計画上の根拠 |
| --- | --- | --- |
| SC-05 | 公開 / アーカイブ / 削除の行操作 | 05_screens §SC-05 目的「正規化文書の**CRUD**・版管理」／ FR-06「文書の**CRUD**・バージョン管理・メタデータ管理」／ [IADR-0041](../adr/IADR-0041_document-write-bff-abac-scoped.md)（Accepted） |
| SC-06 | 無効化の行操作 | FR-01「データソースを**登録・同期し、カタログ化**する」のライフサイクル／ [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（Accepted） |
| SC-08 | タスク種別（分析 / 比較 / 抽出）の選択 | FR-07「AI に対し、指定データ範囲での**分析・比較・抽出**を依頼できる」。選択肢が無いと比較・抽出へ到達できない |
| SC-08 | モデル・トークン数の脚注 | 02_requirements トレーサビリティ表 **FR-11 → SC-08（モデル振り分けの利用面）**／ [IADR-0111](../adr/IADR-0111_degraded-answer-model-label.md)（空 model ＝ AI へ未送信の縮退） |
...
**ドメイン語彙**であり、[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 が共有 UI へ入れることを禁じている。
...
**契約が同期健全性を持つまで琥珀を保留する**（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2。色の割当も環流記録の提案 3 に含めた）。
...
| SC-07 | 一覧（`?status=`） / 再変換 | `GET /bff/conversion/jobs`・`POST /bff/conversion/jobs/{id}/retry` | 同上 | 一覧＝ admin または operator ／ **再変換＝ admin のみ**（**［2026-08-05 追記］#501 / [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1 が絞った**。operator は 403・無認証 401） |
...
（[SC-05 対応表](../../docs/screens/SC-05_document-management.md) row 11 は追加・削除の UI を実装したので「**する**」だが、
...
[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 5 が定めた挙動そのものが無検査だったということであり、外れると
...
**「操作を 2 つ続ける」経路が無かった**ためである。是正（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 7）の前後で
...
| **SC-05〜07 の閲覧ロール** | 05_screens §共通シェル に加え、**§SC-05（`01_screens.md:234`。「モックの『管理』バッジ準拠」と根拠つき）・§SC-06（`:242`）・§SC-07（`:250`）の各節が独立して**「管理者ロール限定」と定める | **admin または operator**（据え置き） | [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（Accepted・2026-07-08）が「データソース・変換ジョブ・文書 CRUD はいずれも運用／コンテンツ管理者の職務」として operator を含めた既存決定。**計画 4 箇所と正面から食い違う**ため、どちらが正かは**計画側の裁定**（planning#198 提案 8）を要する。**2026-08-04 の確定は「再変換の実行権限」に限られる**ため、本 issue はそこだけを狭める（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1） |
| **SC-07 の再変換の権限** | 05_screens §SC-07（`01_screens.md:257`）「再変換の実行権限は管理者ロールに限る。**本画面のアクセス制御と API の権限を揃える**」 | **画面は `platform-admin` のみ**。API は #503 時点では admin/operator のまま（**［2026-08-05 追記］#501 / [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1 が admin のみへ絞って解消済み**） | **#503 時点では計画確定事項の未達**（§2）。正当化しない——解消は **#501**（#503 の直後）。**計画側の裁定は不要**（実装の追随だけが要った） |
...
| **SC-06 の琥珀（警告色）の充て先** | `01_screens.md:125`（モック間相違の確定 ②）・`:241`「同期異常は警告表示（警告色＝琥珀）」 | **どの状態にも充てない**（`disabled` は中立） | 琥珀が指すのは**異常**であり、管理者が意図した無効化＝正常な設定状態ではない。契約が同期健全性を持つまで空けておく（[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 2）。**色の割当の裁定も環流記録の提案 3 に含めた** |
...
| 項目 | 引き受け先 |
| --- | --- |
| **SC-07 再変換 API の管理者ロール強制** | **#501**（[IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）。**［2026-08-05 追記］解消済み**——`POST /bff/conversion/jobs/{id}/retry` は `platform-admin` のみとなり（operator は 403・無認証 401）、計画確定事項（`01_screens.md:257`「画面と API の権限を揃える」）は両側で満たされた |
...
| 右レール AI チャットパネル | 移行**第 4 段**（[IADR-0121](../adr/IADR-0121_spa-stack-migration-staging.md) 決定 1・5） |
| `notify`（sonner トースト）の本番の呼び出し元 | 依然 **0 件**（#496 / #502 からの申し送りを引き継ぐ。本 issue は画面内 `Alert` を採った。[IADR-0127](../adr/IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 6） |
| バンドルサイズ（586 kB / gzip 175 kB） | 全画面の再実装が終わってからのコード分割（#490 / #496 / #502 の未決事項を引き継ぐ）。**［2026-08-05 追記］#512 / [IADR-0134](../adr/IADR-0134_spa-route-code-splitting-boundaries.md) で消化**（ルート単位の遅延 ＋ `manualChunks` 3 規則。500 kB 警告は解消し、初期ロードは 632.98 → 577.54 kB / gzip 190.04 → 177.94 kB）。 |
...
3. ~~**再変換の権限（計画確定事項の未達）**（#501）~~ **［2026-08-05 追記］解消済み**（#501 / [IADR-0128](../adr/IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）。計画側の裁定は不要であり、要ったのは実装の追随だけだった。
4. **SC-05/06/07 の閲覧ロール**。計画 §共通シェル（管理者）と [IADR-0039](../adr/IADR-0039_datasource-management-bff-and-role-gating.md)（admin/operator）の差異。
