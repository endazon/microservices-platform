...
- 実装判断: [IADR-0063](../adr/IADR-0063_bff-unit-endpoint-composition.md)（合成方式の設計・段階実装）
- Issue: #229（フォローアップ 3・後続スライス）

## 目的・背景

#229 のイベント契約分離は [IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md)（PR #239）で完了。残る後続スライス「knowledge 固有 DTO の分離」
...
- 対象（本スライス＝設計のみ）:
  ...
  - 本仕様書。
- 対象外（[IADR-0063](../adr/IADR-0063_bff-unit-endpoint-composition.md) 承認後の別スライス）:
...
- [~] 可変機能ユニット追加時に platform 側の契約・BFF を改修せず（または合成点 1 箇所のみで）拡張できる
  → イベント契約は [IADR-0059](../adr/IADR-0059_contract-layering-unit-contracts.md) で達成済み。DTO/BFF は**本 IADR で合成方式を設計**（実装は承認後の別スライス）。
...
- 合成方式（ビルド時合成点）と段階実装計画は [IADR-0063](../adr/IADR-0063_bff-unit-endpoint-composition.md) に記録。方式承認後に段階実装スライスを起票・実施。
