...
| 項目 | 実測 | 判定への効き方 |
| --- | --- | --- |
| `dotnet` | **この対話セッションのコンテナには無い**（`which dotnet` = exit 1。`/usr/share/dotnet` も `~/.dotnet` も存在しない。2026-08-15 実測）。**CI とレビュー用サンドボックスには在る**（前者は `ci.yml` の `build-and-test` が現に両ユニットをビルドしている、後者は AI レビューの報告で `10.0.400`） | **`src/*/backend/**` に 1 行でも掛かる issue は、この作業環境では `/verify` を満たせない → 今回は着手しない。** **「このリポジトリでは不可能」ではない** —— 環境の問題であって実装の問題ではない |
...
## 4. 棚卸し表（OPEN 38 件・全数・**2026-08-15 時点の走査**）
...
| #455 | バックエンド層標準への全面移行 | feat | ~~**blocked**~~ → **3 軸 ○・未着手**［08-16 / #778］ | **隘路。** 両ユニットの全 backend に一律適用。~~dotnet 不在で 1 行も検証できない~~ → **`dotnet 10.0.301` が在り 3 軸すべて ○**（§8 の同日追記）。**ただし本セッションでは着手していない**（利用者の方針決定は #442 側。「着手可になった」と「着手した」を混ぜない）。**子 issue へは分割していない** | ~~2026-08-15 / #455~~ **2026-08-16 / #778** | `src/platform/backend/**`・`src/knowledge/backend/**`・`src/Directory.Packages.props` | なし（**他が従属**） | 5 | `feat/nfr-backend-app-layer-standard` | — |
...
> **［2026-08-16 追記 / #778］上の集計と表の根拠列は 2026-08-15 時点の記録として据え置く。**
> 理由を書く（規則 7 は「数値を 1 つ直したら同じ値を持つ箇所を全走査し直す」と定めるので、
> **据え置くなら据え置く理由が要る**）。
>
> - **判定が動いたのは #442 と #455 の 2 件だけである。** それぞれの行には取り消し線つきで併記した。
>   集計を 27 → 25 へ書き換えると、**§2 の走査（38 件・2026-08-15）に対する集計という位置づけが崩れる** ——
>   母集合そのものが §8 の再測で 35 件へ動いており、時点の違う数が 1 つの表に混ざる。
> - **したがって数え直すのではなく、時点を明示して据え置く。** 生きた状態は GitHub が正である（[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md)）。
> - **表の根拠列に残る「dotnet 不在」は 2026-08-15 時点の記述である。** dotnet 軸は 2026-08-16 に
>   **全 17 行が ○ へ動いた**が、**残り 15 行の blocked を支えているのは先行 × のほう**であり判定は変わらない
>   （§8 の同日追記）。13 行を個別に書き換えると、動いていない判定に時点だけ新しい根拠が付いて誤読を招く。
...
| 軸 | 測り方（本日実行したもの） |
| --- | --- |
| dotnet が入っている | `which dotnet`（**この対話セッションのコンテナ**。§3 の再実行）。**× は「この作業環境では検証できないので着手しない」の意であり、「このリポジトリでビルドできない」ではない** —— CI の `build-and-test` は現にバックエンドをビルドしている（§3） |
...
> **次に #455 / #442 へ着手するセッションは、本表の値を引き継がず自分の環境で `which dotnet` を測り直すこと**
> （[IADR-0180](../adr/IADR-0180_blocked-judgments-expire.md)。**「前回できなかった」は据え置きの根拠にならない**）。
...
- **#442 を子 5 件へ分割した。** 依存は次のとおり（**辺の出典は各子 issue 本文の「依存」欄**。
  本書が依存を持つのはここ 1 箇所だけで、`session-handoff.md` §2 へは複写しない。[IADR-0141](../adr/IADR-0141_audit-rounds-and-population-drawing.md)）。

  | 子 | 範囲 | 先行 | 開ける先 |
  | --- | --- | --- | --- |
  | **#779** | エッジ TLS 終端基盤（cert-manager ＋ 安定した CA ＋ `edge-tls`） | **なし** | #780 / #782 |
  | **#780** | Keycloak をエッジへ出し issuer を https へ | #779 | #388 / #466 / #781 / #783 |
  | **#781** | apiserver の OIDC 検証を再配線 | #780 | #388 → #271 |
  | **#782** | Istio 導入と East-West mTLS の STRICT 化 | **#779**（#780 は不要） | #458 |
  | **#783** | chart / overlay 検証 CI と統合スタックを CI で起こす経路 | **#779 と #780 の両方** | #466 |

  段で言うと 段 0 = #779、段 1 = #780 / #782、段 2 = #781 / #783。
...
**除外する辺は散文ではなくデータとして書き、コードが実際に落とす**（`COMPLETION_ONLY`）。
**落ちなければ例外で止まる。** 前版はこの宣言を散文にだけ置いて計算へ適用し忘れた（下記「なぜ壊れたか」）。

**次のブロックはそのまま貼って実行できる**（`node - <<'EOF'` に渡す JavaScript である）。
**出力は実行結果をそのまま貼っており、待ち行列 0 件の issue も省いていない**（規則 7。切る・潰す・整形するをしない）。

```console
$ node - <<'EOF'
// 辺の出典: 本仕様書 §4「依存（先行 issue）」列。ノードは OPEN 35 件（§8「母集合の再測」）。
// 値は「その issue が着手するために先行して着地していなければならない issue」。
const PRE = {
  455: [],            442: [],
  441: [455],         438: [455, 442],  439: [438],       440: [455],
  443: [455, 442],    444: [455],       445: [455],       446: [439, 452],
  447: [455],         448: [455, 440],  449: [455],       450: [455],
  451: [455, 447],    452: [446, 447, 448, 449, 450, 451], 453: [466],
  466: [442],         388: [442],       271: [388],       458: [442, 438],
  457: [439],         600: [451],       516: [451],       752: [516],
  754: [516],         493: [452],
  546: [], 380: [], 336: [], 743: [], 572: [], 454: [], 767: [], 768: [],
};
// ★ 除外する辺（「完了条件」であって着手を止めないもの）。
//   宣言と適用を同じ場所に置く —— 散文で宣言して適用し忘れたのが前版の事故である。
const COMPLETION_ONLY = [[446, 452], [493, 452]];
for (const [child, parent] of COMPLETION_ONLY) {
  const before = PRE[child].length;
  PRE[child] = PRE[child].filter((p) => p !== parent);
  if (PRE[child].length !== before - 1) throw new Error(`除外辺 #${child}→#${parent} が適用されなかった`);
}
console.log('除外した辺:', COMPLETION_ONLY.map(([c, p]) => `#${c} は #${p} を先行にしない`).join(' / '));
console.log('適用後: PRE[446] =', PRE[446], '/ PRE[493] =', PRE[493]);
console.log();

const XL = [455, 442, 441, 438, 439, 440, 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453];
const ALL = Object.keys(PRE).map(Number).sort((a, b) => a - b);
const waiting = (root) => {                      // root の完了を待つ issue（推移閉包）
  const out = new Set();
  for (let changed = true; changed; ) {
    changed = false;
    for (const n of ALL) {
      if (out.has(n) || n === root) continue;
      if (PRE[n].some((p) => p === root || out.has(p))) { out.add(n); changed = true; }
    }
  }
  return [...out].sort((a, b) => a - b);
};
console.log('--- 待ち行列（0 件の issue は省かず全件出す）---');
for (const n of ALL) {
  const w = waiting(n), x = w.filter((i) => XL.includes(i));
  console.log(`#${n}\t待つ計 ${String(w.length).padStart(2)} 件\tうち XL ${String(x.length).padStart(2)} 件\t[${w.map((i) => '#' + i).join(' ')}]`);
}
const depth = (n, seen = new Set()) =>          // 先行からの最長距離
  seen.has(n) ? 0 : (seen.add(n), PRE[n].length === 0 ? 0 : 1 + Math.max(...PRE[n].map((p) => depth(p, seen))));
console.log('\n--- 段（XL 17 件のみ）---');
const byLvl = {};
for (const n of XL) (byLvl[depth(n)] ||= []).push(n);
for (const d of Object.keys(byLvl).sort())
  console.log(`段 ${d}: ${byLvl[d].sort((a, b) => a - b).map((i) => '#' + i).join(' / ')}  （${byLvl[d].length} 件）`);
console.log('合計', Object.values(byLvl).reduce((s, a) => s + a.length, 0), '件');
EOF
除外した辺: #446 は #452 を先行にしない / #493 は #452 を先行にしない
適用後: PRE[446] = [ 439 ] / PRE[493] = []

--- 待ち行列（0 件の issue は省かず全件出す）---
#271	待つ計  0 件	うち XL  0 件	[]
#336	待つ計  0 件	うち XL  0 件	[]
#380	待つ計  0 件	うち XL  0 件	[]
#388	待つ計  1 件	うち XL  0 件	[#271]
#438	待つ計  5 件	うち XL  3 件	[#439 #446 #452 #457 #458]
#439	待つ計  3 件	うち XL  2 件	[#446 #452 #457]
#440	待つ計  2 件	うち XL  2 件	[#448 #452]
#441	待つ計  0 件	うち XL  0 件	[]
#442	待つ計 11 件	うち XL  6 件	[#271 #388 #438 #439 #443 #446 #452 #453 #457 #458 #466]
#443	待つ計  0 件	うち XL  0 件	[]
#444	待つ計  0 件	うち XL  0 件	[]
#445	待つ計  0 件	うち XL  0 件	[]
#446	待つ計  1 件	うち XL  1 件	[#452]
#447	待つ計  6 件	うち XL  2 件	[#451 #452 #516 #600 #752 #754]
#448	待つ計  1 件	うち XL  1 件	[#452]
#449	待つ計  1 件	うち XL  1 件	[#452]
#450	待つ計  1 件	うち XL  1 件	[#452]
#451	待つ計  5 件	うち XL  1 件	[#452 #516 #600 #752 #754]
#452	待つ計  0 件	うち XL  0 件	[]
#453	待つ計  0 件	うち XL  0 件	[]
#454	待つ計  0 件	うち XL  0 件	[]
#455	待つ計 20 件	うち XL 14 件	[#438 #439 #440 #441 #443 #444 #445 #446 #447 #448 #449 #450 #451 #452 #457 #458 #516 #600 #752 #754]
#457	待つ計  0 件	うち XL  0 件	[]
#458	待つ計  0 件	うち XL  0 件	[]
#466	待つ計  1 件	うち XL  1 件	[#453]
#493	待つ計  0 件	うち XL  0 件	[]
#516	待つ計  2 件	うち XL  0 件	[#752 #754]
#546	待つ計  0 件	うち XL  0 件	[]
#572	待つ計  0 件	うち XL  0 件	[]
#600	待つ計  0 件	うち XL  0 件	[]
#743	待つ計  0 件	うち XL  0 件	[]
#752	待つ計  0 件	うち XL  0 件	[]
#754	待つ計  0 件	うち XL  0 件	[]
#767	待つ計  0 件	うち XL  0 件	[]
#768	待つ計  0 件	うち XL  0 件	[]
...
**2 + 9 + 4 + 1 + 1 = 17。§8 の表の行数と一致する**（スクリプトが自分で合計を出している）。

| 順位 | issue | 待つ issue 数（推移） | うち XL | 読み方 |
| ---: | --- | ---: | ---: | --- |
| 1 | **#455** | **20** | **14** | **最大の隘路。** XL 17 件のうち **#442 と #453 を除く 14 件**が直接または間接に待つ |
| 2 | **#442** | **11** | 6 | 第 2 の隘路。**待つ 11 件のうち 5 件は XL ではない**（#271 / #388 / #457 / #458 / #466）——「実クラスタが要る」債務の集積点である |
| 3 | #447 | 6 | 2 | XL でない従属が多い（#516 / #600 / #752 / #754）。**`decision-needed` 4 件のうち 2 件の器**である |
| 4 | #438 / #451 | 5 | 3 / 1 | — |
| — | #452 | **0** | 0 | **最下流。** 待つ issue は 1 件も無い（#493 は #452 を「完了条件」として引くだけで、着手は止まらない） |

**#455 と #442 はどちらも先行を持たない（段 0）。** 図の形（「土台 #455 → 1 段 #442」）と実測は食い違う ——
**両者は直列ではなく、並べて置ける 2 本の根である。** どちらを先に倒すかは待ち行列の長さで決まり、
**実測では #455（20 件）が #442（11 件）より広い。**

#### なぜ前版が壊れたか（§11 の教訓の実例）

**［2026-08-15 追記 / #454］初版のこの節は 2 つの形で壊れており、AI レビューが 🔴 で検出した。**
**消して書き直さず、壊れ方を残す**（母集合の規則 9・10 / §11 の「再現できない数は検算できない」）。

| # | 壊れ方 | 効き方 |
| --- | --- | --- |
| **1** | **除外を散文で宣言し、計算では適用しなかった。** 本文は「#446 → #452・#493 → #452 は辺に含めない」と書いたのに、コードは **#446 の側しか落としていなかった** | **#493 が `#452 → #446 → #439 → #438 → #455 / #442` の経路で引き込まれ、#452 の下流の全行が一律 +1 ずれた**（#455 = 21→20 / #442 = 12→11 ほか）。**XL の数と段の割付は変わらない**（#493 は XL でないため） |
| **2** | **実行できないブロックを証跡として貼った。** `node - <<'EOF'` の中身が日本語の説明文で、コメント記法も無かった。渡せば `SyntaxError` である | **書いた本人が検算できない。** §11 で「再現できない数は、書いた本人にも検算できない」と教訓化した直後に、同じ文書が同じ型を再演した |

**是正は 2 つとも「同じ場所に置く」ことで行った** —— **除外はデータ（`COMPLETION_ONLY`）にして
コードが落とす**（落ちなければ例外で止まる）、**証跡は実際に実行して出力をそのまま貼る**。
**散文の宣言は、適用したかどうかを機械が見ていない。**
...
- [x] **隘路を語感で決めず、待つ issue 数を計算し直して順位を付けた**（§8「依存の段と隘路の実測」。#455 = 20 件 / #442 = 11 件）
- [x] **§8 の `console` ブロックは実際に `node` へ渡して動く**（貼った出力は実行結果そのままで、0 件の行も省いていない）
- [x] **辺の除外はデータ（`COMPLETION_ONLY`）としてコードが適用し、落ちなければ例外で止まる**（散文の宣言に頼らない）
- [x] **dotnet の判定は「どこで測ったか」を明示した**（§3 / §8。CI とレビュー用サンドボックスには在る）
