...
- **走査範囲**: `git ls-files`（追跡下の全ファイル。**拡張子で絞らない**＝規則 3）から
  `planning/`（submodule・別リポの正本）と `src/ai-stock-trading/`（別プロジェクトの submodule・[IADR-0120](../adr/IADR-0120_excluded-units-from-gitmodules.md)）を除く。
...
2. **必須仕様書** —— 機能仕様書 [`docs/functional/FR-22_user-notifications.md`](../../docs/functional/FR-22_user-notifications.md)、テスト仕様書 [`docs/tests/FR-22_user-notifications.md`](../../docs/tests/FR-22_user-notifications.md)、通信仕様書 [`docs/api/BFF_notifications.md`](../../docs/api/BFF_notifications.md)。
...
| **メール送出の実装** | **SMTP は実装側に 1 件も無い**（利用者裁定 2026-08-15「実環境が要るものは触らない」／[IADR-0197](../adr/IADR-0197_realm-rename-and-auth-policy.md) 決定 5）。**FR-22 が「アプリ内通知を主・メールを補助」と定めているため、メール不在は本 issue のブロッカーではない**（IADR-0215 決定 3） |
| **①②③ の発火の結線** | 発火源（個人資料の論理削除・保存容量・同期トークン）は **FR-19 / FR-20 の機能**であり、**#451 が保留中**（[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) / [IADR-0142](../adr/IADR-0142_fr19-20-scoped-release-by-overturn-range.md)）。**保留中の機能に結線すると、動かないコードが「実装済み」として残る** |
...
`required` は応答スキーマに必ず付ける（[IADR-0132](../adr/IADR-0132_openapi-required-from-csharp-nullability.md)。`required` の無いスキーマは orval が
...
3. **`platform` → 可変ユニットの参照は禁止**（`CLAUDE.md` §サービス境界／[IADR-0056](../adr/IADR-0056_repo-unit-structure-platform-knowledge.md)）。
...
**`@platform/ui` に表示文言は入れない**（[IADR-0125](../adr/IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1）——文言はすべて呼び出し側の Lingui マクロで書く。
...
写像の詳細は [`docs/tests/FR-22_user-notifications.md`](../../docs/tests/FR-22_user-notifications.md) が正本である。要点のみ:
...
| **①②③ の発火の結線** | **#451（FR-19 / FR-20）** | #451 の保留解除（[IADR-0119](../adr/IADR-0119_fr17-21-hold-until-adr-fixed.md) / [IADR-0142](../adr/IADR-0142_fr19-20-scoped-release-by-overturn-range.md)） |
| **メール送出（SMTP リレー）** | **#600 ＋ ADR-0045 のメール基盤の実装** | 実環境の SMTP 資格情報（[IADR-0197](../adr/IADR-0197_realm-rename-and-auth-policy.md) 決定 5 の射程外） |
| **SSE への切り替え** | **#788（移行第 4 段）** | 第 4 段の着手 |
| **通知の一覧画面（SC 番号つきの独立画面）** | 未起票 | 計画に SC 番号が無い。**必要になったら計画へ環流して SC を起こす**（実装側で番号を作らない。[IADR-0179](../adr/IADR-0179_unnumbered-nfr-for-meta-work.md) 決定 2） |
