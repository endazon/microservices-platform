...
3. **二重記載は class 直下の `<lines>` を正とし、`<methods>` 配下は内訳として数えない。**
   ...
   **床 17 の意味が変わる**（決定 4 の追記を参照）。
   ...
   > 🔴 **［2026-08-22 追記 / #900］本決定が扱う「二重記載」は、1 つのレポートの中で `<methods>` 配下と
   > class 直下に同じ行が書かれることであり、**レポート跨ぎの重複とは別軸**である。**
   > レポート間で同じソース行が重複するのは、共有ライブラリを複数のテストプロジェクトが参照するため
   > であって、coverlet の出力構造とは無関係である。本決定の「比 行 2.00」の観測（決定 5 の追記）は
   > **レポート内**の軸しか見ておらず、レポート跨ぎの重複は 1 行も検出しない。
   > 後者は [IADR-0236](IADR-0236_coverage-cross-report-line-dedup.md) が扱う（#900 / #899）。
...
5. **段階的な可視化**（終了コードは変えない）。
   ...
   > 🔴 **［2026-08-22 追記 / #900］ここに並ぶ観測点は、いずれも「1 つのレポートの中の壊れ方」である。**
   > **レポートを跨いだ行の重複はどれにも当たらず、本決定の可視化では 1 件も現れない。**
   > 実際 `#899` では、レポートが 15 → 16 件へ増えて分母が 949 行ふくらんだが、
   > ここに挙げた warn / notice は 1 つも出ないまま床だけが割れた。
   > レポート跨ぎの重複排除量とその 0 件検知は
   > [IADR-0236](IADR-0236_coverage-cross-report-line-dedup.md) 決定 5 が別に持つ。
