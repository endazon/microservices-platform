...
- 状態: **Superseded by [IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md)**（k8s 1.30+ では本 ADR の手順は成立しない。下記「⚠️ 2026-07-25 追記」を必ず読むこと）
- 日付: 2026-07-19
- 決定者: claude（実装）
...
- **apiserver への OIDC フラグ付与は行わない**。**［2026-07-26 更新］この方針は
  [IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md)（#399）でコードに反映済み**で、`scripts/k8s-local-up.sh` の付与経路（下記「決定」§1・§4）は
...
[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md)）と ClusterRoleBinding `headlamp-developer-cluster-admin`（#271 / [IADR-0080](./IADR-0080_headlamp-k8s-management-ui.md)）は恒久化済みで、
...
- 関連 ADR: [IADR-0080](./IADR-0080_headlamp-k8s-management-ui.md)（#271。Headlamp を OIDC token passthrough で導入・RBAC は `oidc:developer`=User に
  cluster-admin を bind＝本 ADR がその live 前提を配線）／[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)（経路B＝k3d dev 環境・issuer 正準名）／
  [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md)（issuer ホスト名の解き方＝手順A・in-cluster `http://keycloak:8080`）
...
[IADR-0080](./IADR-0080_headlamp-k8s-management-ui.md) の Headlamp は **OIDC token passthrough**（利用者 `id_token` を k8s API server の Bearer へ委譲）で認証する。
...
> **［2026-07-26］以下の決定 1・4 の実装は [IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md)（#399）で除去された。** `HEADLAMP=1` 追従により
...
apiserver（cluster 内）から見た Keycloak の正準 issuer は in-cluster サービス名 `keycloak:8080` である（[IADR-0066](./IADR-0066_local-k8s-dev-environment.md) /
[IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) 手順A）。ブラウザ側も手順A（hosts に `127.0.0.1 keycloak` ＋ `port-forward svc/keycloak 8080:8080`）で
...
- **`groups-claim` は付与しない**: #271 は group を一切 bind していないため、groups-claim を足しても有効な認可には寄与
  ...
  という本リポの方針にも沿う。
- ロール/グループ別の権限分離検証は [IADR-0080](./IADR-0080_headlamp-k8s-management-ui.md) 決定2どおり `poc-*` の役割であり、`developer` は dev スーパーユーザー
  （[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)）。将来 group ベース RBAC を導入する場合は、その RBAC 追加とセットで groups-claim を足す（本 ADR の外）。
