...
> [IADR-0321](./IADR-0321_no-empty-scaffolding-frames.md)（枠置き規範の廃止）である。
...
- 関連する計画書 ID: 計画
  ...
  **§`SharedKernel` の粒度・`Worker` の追加（2026-08-17 確定・`status: fixed`）** / 計画
  `ADR-0041`（計画リポ）（Result 型の封じ込め）/ 計画
  `ADR-0030`（計画リポ）（ライブラリ標準）/ 計画
  `ADR-0019`（計画リポ）（決定 4）
- 関連する実装 ADR:
...
| [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 決定 3（ユニット外参照は Shared の 3 つ） | 変更不要 | 変更不要 | **`Platform.Shared.Kernel` を消すため再改定が要る** |
...
[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 決定 3 の「ユニット外参照は `src/platform/backend/Shared/` の **3 プロジェクト**」
...
- Superseded by: **[IADR-0282](./IADR-0282_single-project-vsa-structure.md)**（構成の実体）／
  **[IADR-0321](./IADR-0321_no-empty-scaffolding-frames.md)**（枠置き規範の廃止）。
