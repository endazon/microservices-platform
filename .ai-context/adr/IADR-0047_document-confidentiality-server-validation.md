...
- 関連 ADR: [IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md)（多層防御・サービス最終防衛線）／[IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md)（BFF ABAC スコープ・付与属性の
  厳密検証見送り）／[IADR-0019](./IADR-0019_datasource-default-attributes.md)（データソース既定属性のフェイルセーフ）／ADR-0004（Keycloak）
...
下流は fail-closed（[IADR-0012](./IADR-0012_retrieval-search-fail-closed-scope.md) 検索除外・[IADR-0021](./IADR-0021_wiki-js-sync-graphql-push.md) `isPrivate=true`）で漏えい方向には倒れないが、
計画の受け入れ条件「保存拒否」を満たさず、属性欠落文書は検索にも Wiki にも出ないため「保存できたのに
見えない」運用混乱を招く。[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md) の多層防御方針（サービスが最終防衛線）に整合させ、必須検証を
サーバー側で強制する必要がある。
...
2. **正準値集合は静的定数で持ち、動的な属性辞書照合は行わない（本 PR では）。**
   ...
   独立性が高いため、[IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md) で見送った「付与属性が呼び出し者スコープ内かの厳密検証」と合わせて
...
3. **取り込み（パイプライン）経路は本 PR では変更しない。**
   ...
   データソース既定属性（[IADR-0019](./IADR-0019_datasource-default-attributes.md)）で `confidentiality` が付与される設計であり、ここで 400 を投げると
...
4. **既存の属性欠落文書は「修正要求」方式で移行する。**
   ...
   [IADR-0019](./IADR-0019_datasource-default-attributes.md) のフェイルセーフ既定（`internal`）に揃えた一括バックフィルは任意の ops follow-up とし、
   本 PR には含めない。
   - **注意（欠落だけでなく非正準値も同じ「詰み」になる）**: 取り込み経路（`CreateNormalized`/`ApplyNormalized`）は
     本 PR の検証対象外のため、データソース既定属性（[IADR-0019](./IADR-0019_datasource-default-attributes.md) `DataSource.WithConfidentialityFailsafe`。
     **#516 で `WithRequiredAttributeFailsafe` へ改称し `owner` / `department` を追加した。[IADR-0199](./IADR-0199_ingestion-required-attribute-failsafe.md)**）に
...
- **検証点をサービス側（DocumentService）に置く**: BFF/フロントは利便性の早期検証点であり、最終防衛線は
  サービス（[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md)）。BFF 迂回の直接呼び出しを塞ぐには、正本のサービス側で検証する必要がある。
...
- **静的集合 vs 動的辞書**: 動的辞書照合は正確だが、書き込みごとに AuthorizationService への往復を足し
  （[IADR-0045](./IADR-0045_bff-document-write-roundtrip-deferral.md)／#179 の往復削減方針と逆行）、辞書未整備時の縮退設計も要る。FR-05 の値集合は安定して
...
- 回帰なし: 読み取り（GET）・取り込み経路・版管理・認可（[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md)）は不変。

## フォローアップ

- DocumentService → AuthorizationService の動的属性辞書照合（必須性・許容値・付与属性のスコープ内検証。
  [IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md) 見送り分と統合）。あわせて**比較ポリシーの整合**（DocumentService=Ordinal /
  AuthorizationService=OrdinalIgnoreCase の不一致解消。決定 5 参照）。
- 取り込み経路のフェイルセーフ既定補完＋**値の正準性チェック**（[IADR-0019](./IADR-0019_datasource-default-attributes.md) 既定属性と整合。
