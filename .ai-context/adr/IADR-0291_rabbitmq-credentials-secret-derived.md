...
- 領域宣言の外にあるため触っていない追随: `scripts/README.md` の `check-default-credentials.js` 行
  ...
  **どちらも 0 件になった事実に追随させること。**
  - **［2026-08-28 追記 / #1036］この追随は同一 PR 内で済んでいる**（統合側が引き取り、
    誤りの側の文字列「13 箇所」＋資格情報系の語で走査して 2 箇所を是正した）。**上の行は
    「未対応」ではない。** `ci.yml` はコメントのみの変更で、起動条件（`on: [push, pull_request]`）と
    job 数（10）が変わらないことを yaml パースで確認済み。作業仕様書 2 件（#1012 / #1022）に残る
    「13 件」は**着手時点の実測の記録**なので、凍結記録として書き換えていない。
