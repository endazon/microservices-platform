...
- 関連する実装仕様書: [作業仕様書 #863](../specs/20260818_issue-863_adr-0038-fallback-order-and-429.md)、
  [機能仕様書 FR-11](../../docs/functional/FR-11_llm-egress-routing.md)、
  [テスト仕様書 FR-11](../../docs/tests/FR-11_llm-egress-routing.md)、
  [可観測性仕様書](../../docs/observability/llm-completion-metrics.md)、
  [ピン Runbook](../../docs/operations/llm-model-pin-runbook.md)
...
| # | 案 | 評価 |
| --- | --- | --- |
| **A1** | **`Llm:Routing:PurposeFallbackModels`（用途 → 順序つきモデル配列）を新設する（採用）** | [IADR-0007](./IADR-0007_llm-egress-routing-config-driven.md) の「呼び出し先は設定駆動」と同じ形。`PurposeModels` の兄弟キーとして読める。用途ごとに鎖の有無を分けられる（`trade-decision` に鎖を持たせない、が設定で表現できる） |
...
| A3 | エンドポイントの `Models` の並び順をフォールバック順序とみなす | `Models` は「割当」ではなく**利用を許可する集合**である（[IADR-0106](./IADR-0106_rag-answer-sonnet-5.md)）。順序に意味を持たせると、明示要求モデルのための登録が順序を壊す |
...
| # | 案 | 評価 |
| --- | --- | --- |
| **C1** | **既存カウンタ `llm.completion.total` の `llm.result` に `fallback` を 1 値足す（採用）** | 計器が増えない。**既存の Grafana パネル `sum by (llm_result) (…)` に新系列が自動で現れる**ため、決定 6 の「用途別・モデル別の利用実績へ出す」がダッシュボードの変更なしに成立する。[IADR-0164](./IADR-0164_llm-cost-monthly-review-interim-control.md) の「ゲートウェイのメトリクスは `llm.completion.total` の 1 本だけ」も偽にならない |
| C2 | 新カウンタ `llm.completion.fallback.total` を足す | 計器が 2 本になり、`scripts/scripts.repo.test.js` の「ダッシュボードの式のメトリクス名は `llm_completion_total` ただ 1 つ」という突合、[IADR-0164](./IADR-0164_llm-cost-monthly-review-interim-control.md) の記述、Runbook の説明が同時に追随を要する。得るものは C1 と同じである |
| C3 | ログだけに出す | 「継続的に把握する手段が無い」という [IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) が解いた問題へ戻る。決定 6 は**利用実績へ出す**ことを求めている |
...
2. **`LlmRouter` が鎖を解決し、`RoutingDecision.FallbackModels` に載せる。**
   ...
   **①で落とした要素は warn ログに出す** —— `ADR-0038` 決定 5 は「登録されていなければフォールバックは
   その場で失敗する」と述べており、黙って落とすと [IADR-0102](./IADR-0102_trade-decision-model-pinning.md) / [IADR-0106](./IADR-0106_rag-answer-sonnet-5.md) が実際に踏んだ
   「無音失効」と同型の罠になる。
...
- **計器を増やさないのは、増やすと追随点が 4 つ増えるからである**（ダッシュボード 2 種・突合テスト・
  [IADR-0164](./IADR-0164_llm-cost-monthly-review-interim-control.md) の記述）。属性値 1 つの追加なら、**既存の `llm.result` 別パネルに新系列が出るだけで
  決定 6 が満たされる**。
...
- **カウンタの「補完 1 回」が「上流呼び出し 1 回」の意味に寄った。** フォールバックが起きると
  ...
  従来どおりリクエストあたり最大 1 件**であり、[IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) の拒否率の式は影響を受けない。
...
- Supersedes: なし
- Superseded by: なし
- 前提とする実装 ADR: [IADR-0007](./IADR-0007_llm-egress-routing-config-driven.md)（設定駆動ルーティング。本 ADR は同じ形を踏襲する）／
  [IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)（ZDR 除外＝`EligibleModels`。鎖にも同じ適格性を適用する）／
  [IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md)（`llm.completion.total` と属性の値域）／[IADR-0111](./IADR-0111_degraded-answer-model-label.md)（応答が名乗る使用モデル）／
  [IADR-0164](./IADR-0164_llm-cost-monthly-review-interim-control.md)（ゲートウェイのメトリクスは 1 本。本 ADR はこれを維持する）
- 触れない決定: [IADR-0102](./IADR-0102_trade-decision-model-pinning.md)（取引判断のピン留め）は本 ADR で**強化される**（鎖を持たせないことを
