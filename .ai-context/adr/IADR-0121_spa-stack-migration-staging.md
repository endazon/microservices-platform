...
> 提起の経緯・確定解釈は
> feedback/20260804_frontend-migration-staging-interpretation.md（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260804_frontend-migration-staging-interpretation.md` へ移設）
> に記録した。
...
> **［2026-08-04 追記］決定 1 の「第 2 段」は [IADR-0124](./IADR-0124_tanstack-router-unit-composition.md)（#490）で 2 つへ分割された。**
> 本決定は段の内容・境界を
...
> 未起票の残件として繰り延べられた。** 理由は [IADR-0116](./IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4——ルータ差し替え（合成点＝
...
> 本 ADR も含めて**どの散文もそれを複写しない**（複写は必ず片方が古くなる。[IADR-0205](./IADR-0205_reading-budget-reduction-for-kit-catchup.md) が
...
> 走査対象に含めており（[IADR-0060](./IADR-0060_submodule-unit-operations.md)）、3 本目はそのフロント版である。
> **`../` を跨ぐのは雛形 1 件に限る**（`src/` の中に置けるものを外から拾わない）。
> 整形・lint 側の対応（`src/` の外に専用入口を置いてよい条件）は
> [IADR-0203](./IADR-0203_renovate-husky-hook-scope.md) 決定 3 の 2026-08-16 追記が定める。
>
...
> [IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)（#496）が実値で埋めた（部分改定）。** 本決定は入れる／入れないの**判定規則**と
...
> 要求するのは SC-19 / SC-20 だけで、これは FR-19 / FR-20 に属し [IADR-0119](./IADR-0119_fr17-21-hold-until-adr-fixed.md) 決定 1 が
> 「その受け入れを担う画面」ごと着手を保留しているためである（繰り延べであって放棄ではない。
> 引き受け先は #452）。
> **［2026-08-07 追記 / #599］この根拠は失効した。** **［2026-08-07 / #599］FR-19 / FR-20 の保留は範囲基準へ変わった**（[IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md) 決定 1・2。**範囲の正は計画 `ADR-0037` の「着手可否の注記」**であり、ここには転記しない —— 転記すると計画側が動いたとき一斉に腐る）。
> **計画 `05_screens` の SC-19 / SC-20 が要求する確認ダイアログは、いずれも「本文を編集（Wiki.js）」導線ではない**（完全削除・論理削除・公開範囲の変更・Break-glass・全端末の一括失効）。**件数は数え方が争える**〔「確認ダイアログ」と明記された箇所と、ダイアログ一般を含めるかで変わる〕**ので書かない**——したがって覆り得る範囲の外である。
