...
- 関連 ADR: [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md)（#284。決定3＝ブラウザ OIDC issuer 統一。手順A＝in-cluster 名を browser/cluster で
...
[IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) 決定3 は「ブラウザが受け取る token の `iss` と、サービス側の検証基準（`Auth__Authority`）が
...
CoreDNS 書き換えでしか塞げず、[IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) は README 手順B(iii) に「CoreDNS 追記 or backend の metadata/issuer
...
3. `RequireHttpsMetadata=false`・`ValidateAudience=false`・`RoleClaimType=ClaimTypes.Role`・
   `NameClaimType=preferred_username`（[IADR-0031](./IADR-0031_name-claim-preferred-username.md) 実測由来）・`KeycloakRolesClaimsTransformation`・認可ポリシー
   登録は**現行のまま不変**。
...
- **CoreDNS 書き換えで in-cluster に edge host を解決させる**: [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) 既述のとおり侵襲的で dev 環境ごとに
