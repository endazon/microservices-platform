...
- 前提: [IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md)（応答側の規則）・[IADR-0159](./IADR-0159_openapi-dto-drift-checker.md)（検査器）・[IADR-0122](./IADR-0122_contract-schema-source-and-compat-gate.md)（C# が契約の正本）

## コンテキストと課題

[IADR-0159](./IADR-0159_openapi-dto-drift-checker.md) 決定 3 の検査器は、[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) の論点 A1 ＋ B1 を
...
[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) の**表題そのものが「応答スキーマの `required` は…」**であり、決定 2 の系は明示している:
...
（[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) 決定 5「`/bff/` から到達しない 5 個にも `required` を入れる」と整合）。
...
「C# の非 null 性が唯一の権威」という [IADR-0122](./IADR-0122_contract-schema-source-and-compat-gate.md) 由来の前提は、**ここでは成り立たない** ——
...
- 決定 1 は [IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) の**射程を正しく読み直しただけ**であり、新しい方針ではない。
...
[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) 決定 1（A1）＋決定 2（B1）がそのまま当たる典型例である
...
**★ 「任意である」と書いていた記述 2 件を引き直した**（母集合の規則 8「この変更で新たに誤りになる
記述を引き直す」）。3 プロパティ名を追跡下の全ファイルから引いた結果、**動作は 1 箇所も変わらないが
説明が古くなる箇所が 2 件**あった:

| 箇所 | 何が古くなったか |
| --- | --- |
| `jobStatus.test.ts` のコメント | 「契約上は任意（`diagramsRetained?`）なので」——**required へ移った** |
| `docs/tests/SC-07_conversion-jobs.md` P5 | 同上（**必須仕様書の側にもあった**） |

**コードは直していない。** `hasRetainedFigures` / `isCorrectable` は生成型を import せず
`{ diagramsRetained?: number }` という構造的型で受けており、**省略に耐える形のままでよい** ——
[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) 決定 3 のとおり「**契約上は必須**」と「**実行時に必ず来る**」は別であり、
本文を実行時に検証する層は無い。**変わったのは理由であって挙動ではない**ので、
理由の側だけを書き換えた。
...
- **型の不一致**（[IADR-0159](./IADR-0159_openapi-dto-drift-checker.md) 決定 3 が見ないと決めている）。
...
- Supersedes: なし（[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) は**変更していない**。射程を読み直しただけである）
- Superseded by: なし
