...
- 関連する実装仕様書:
  ...
  [技術要件書](../../docs/tech/tech-requirements.md)「バックエンドアプリケーション層標準」
...
>   **併存**として引き続き有効であり、[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 決定 3 の許容 3 プロジェクトも変わらない。
...
> - **状態は `Accepted` のまま**とする（決定 1〜4 が生きているため）。**先例**: [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) は
...
- Superseded by: なし（**[IADR-0219](./IADR-0219_sharedkernel-granularity-and-worker-standard-component.md) が
  §決定 5 の読み替えと §結果 フォローアップ 2 を部分改定した**。裁定 planning#390 により、計画側の構成図は
  論理レイヤではなく**サービス単位の物理配置**を意味すると確定し、**サービス単位の `SharedKernel` は
