...
- 状態: Accepted
- 日付: 2026-07-31
- 決定者: claude（実装）／利用者（[AST#309](https://github.com/endazon/ai-stock-trading/issues/309) の 3 案から**案 A** を採用）

## 起点・関連

- 起点 issue: [AST#309](https://github.com/endazon/ai-stock-trading/issues/309)
  「月報の割当 `claude-fable-5` が非 ZDR のため既定機密区分では構造的に到達不能」。
- 直前の実装判断: [IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md)（報告書の種別別 purpose 割当・[#422](https://github.com/endazon/microservices-platform/pull/422)）。
  本 IADR は同 IADR の**決定1 の月報行と決定2 の帰結**のみを改定する。IADR-0112 本文と ADR 索引の
  同 IADR 行には、改定済みである旨の日付付き追記を入れた（`docs/adr/README.md` の運用規約と
  [IADR-0084](./IADR-0084_headlamp-oidc-apiserver-flags.md) / [IADR-0087](./IADR-0087_k8s-local-up-optin-smoke-test.md) の前例に倣う。旧 IADR だけを読んだ担当者が古い値を現行と誤認しないため）。
  AST 側の `IADR-0120` の割当表も同様に陳腐化するが、別リポのため
  [AST#285](https://github.com/endazon/ai-stock-trading/issues/285)（実効モデル記述の陳腐化）へ追記して引き渡した。
- 仕様書: `docs/specs/20260731_issue-309_report-monthly-zdr-model.md`。
- 計画への環流: AST/ADR-0014 §決定1 の月報割当を改定する新 ADR を計画リポへ起案する（**別 PR**）。
- ルーティング設計・ティア判定・ZDR 除外ロジック（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)）は変更しない。

## コンテキストと課題

[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 決定1 は利用者の仕様指定に従い `report-monthly` へ `claude-fable-5`（方針階層の最上位＝
...
[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 決定2 はこれを「`confidential` / `restricted` では月報だけが `DefaultModel` へ黙って落ちる」
という**既知事実**として受け入れ、テストで挙動を固定した。しかし方針階層の最上位である月報の割当が
**呼び出し側の機密区分設定に左右されて無音で変わる**状態は、[IADR-0102](./IADR-0102_trade-decision-model-pinning.md) が取引判断について
...
| モデル | ZDR | 根拠 |
| --- | --- | --- |
| `claude-fable-5` | 非対応 | `NonZdrModels` に列挙（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)・30 日保持要件） |
| `claude-opus-5` | 対応 | 未列挙。`DefaultModel`（[IADR-0101](./IADR-0101_default-model-opus-5.md)） |
| `claude-opus-4-8` | 対応 | 未列挙（[IADR-0102](./IADR-0102_trade-decision-model-pinning.md) で確認済） |
| `claude-sonnet-5` | 対応 | 未列挙（[IADR-0106](./IADR-0106_rag-answer-sonnet-5.md) で明記） |
...
ZDR 対応 5 モデルのうち最上位は `claude-opus-5` であり（[IADR-0101](./IADR-0101_default-model-opus-5.md) がグローバル既定＝最上位として
...
種別ごとに用途を分ける仕組み（[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 決定1）は維持する。**同値でも `report-monthly` の明示エントリは
...
#376 / [IADR-0102](./IADR-0102_trade-decision-model-pinning.md) で実際に踏んだ罠）。既存の集合ガード T-19 が全 `PurposeModels` 値 ⊆ `Models` を維持する。

`claude-fable-5` は `Models` に**残す**。`analysis` が ZDR 非要件区分で使う意図的な割当であり
（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)）、削除は明示 `Model` 要求をしている呼び出し側に対する破壊的変更になる。
...
`analysis` は ZDR 非要件区分に限って fable-5 を使う意図的な例外（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)）のため対象に含めない。

## 理由

- 案 A は 3 案のうち唯一、**実装側の判断だけで完結し、かつ情報分類にもポリシーにも触れない**。
  B は情報分類の決定、C は契約事実の確認を要する。
- 月報の割当を ZDR 対応モデルにすることで、[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 決定2 が既知事実として受け入れていた
...
- 決定4 のガードは、今回の矛盾が「1 行の設定ミス」ではなく「2 箇所の設定の突き合わせを人間が
  忘れうる構造」から生じたことへの対処である。[IADR-0102](./IADR-0102_trade-decision-model-pinning.md) / [IADR-0106](./IADR-0106_rag-answer-sonnet-5.md) が `Models` について
...
- フォローアップ:
  ...
  - AST `HttpReportNarrativeDrafter` のログ文言是正（`Sent=false` の 3 分岐を区別せず
    ...
    **[AST#315](https://github.com/endazon/ai-stock-trading/issues/315) で追跡**。
  ...
  - 費用実測（[#380](https://github.com/endazon/microservices-platform/issues/380) /
    [AST#243](https://github.com/endazon/ai-stock-trading/issues/243)）。月報が fable-5 から
    opus-5 へ変わるため、[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) 時点の試算は再ベースラインが要る。

## 関連

- [IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md)（報告書の種別別 purpose 割当。本 IADR が月報行を改定する）
- [IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)（既定 opus 経路・ZDR 除外の導入・`NonZdrModels`）
- [IADR-0101](./IADR-0101_default-model-opus-5.md)（グローバル既定 Opus 5）
- [IADR-0102](./IADR-0102_trade-decision-model-pinning.md) / [IADR-0106](./IADR-0106_rag-answer-sonnet-5.md)（`Models` 未登録による無音失効の罠と集合ガード T-19）
