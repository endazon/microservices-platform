...
- **既存エンドポイントは変更しない**: SC-10 ダッシュボード（FR-10）・フィードバック一覧（FR-08）・
  ...
  > **［2026-08-09 追記 / #544］FR-10（SC-10 ダッシュボード）は運用者へ開放した。**
  > **本決定が定めた手順のとおりに進んだ結果である** —— 「運用者への開放が必要になった場合は
  > **計画側（要求・画面）の更新を経て個別に判断する**」と書いてあり、計画 §SC-10 が
  > 「運用者・管理者ロール限定」と定めて裁定 **Q19 / Q28** で確定したので、それに従った。
  >
  > **FR-08（フィードバック一覧）・FR-09（ABAC 管理）は `AdminOnly` のままである**
  > ——計画側の更新が無く、本決定の条件を満たしていない。
  > 詳細は [IADR-0129](./IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 4 の追記と [IADR-0011](./IADR-0011_dashboard-service-usage-aggregation.md) の追記を参照。
...
- 運用者は構成閲覧（FR-15）のみ可能で、管理系操作（FR-08/09/10 の AdminOnly）は不可のまま。
  **［2026-08-09 追記 / #544］FR-10 の閲覧はこの限りではない**（上の決定への追記を参照）。
  **FR-08 / FR-09 は不可のままである。**
