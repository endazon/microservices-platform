...
- 先行する同型例: [IADR-0209](./IADR-0209_vitest-include-subset-of-frontend-tests-paths.md)（雛形 `templates/*/frontend/**` を `frontend-tests.yml` の
  `paths:` へ入れ、CI の対象にした。#801）

## コンテキストと課題

[IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 3 は「テンプレートは本リポジトリの**ビルド対象ではない**
...
| C. 雛形を `src/` 配下の常設ユニットとして持つ | [IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 3 の括弧内 2 条件を実際に破る。雛形が本番ユニットとして扱われ、依存やパッケージ更新の巻き添えを受ける |
...
登録しない。ビルドするのは一時的な複製であって `templates/` ではない**（[IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 3 の
括弧内 2 条件は不変）。`trap` で必ず片付け、後段ステップ（`if: always()`）が
`git status --short --ignored -- src/` で**残骸ゼロを実測**する。

### 2. `.sample` は複製先へ置かない

`.sample` は「単独リポジトリでビルドするときだけ」置くもの（[IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 4）。配置後を模す
...
- 決定 4 は本リポが繰り返し踏んできた「**検査していないことを違反 0 件と読む**」事故の予防である
  （[IADR-0130](./IADR-0130_test-spec-coverage-ratchet.md) 系の作法と同型）。
- [IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 3 の括弧内 2 条件を保ったまま位置づけだけを変えるため、**改定 ADR ではなく
...
- 悪い影響・トレードオフ:
  ...
  - `templates/` は「ビルド対象ではない」と書かれた文書が複数あり、**読み手には位置づけが
    分かりにくくなる**。[IADR-0060](./IADR-0060_submodule-unit-operations.md) へ追記して導線を張る。
- フォローアップ:
...
- Supersedes: なし
- Superseded by: なし
- [IADR-0060](./IADR-0060_submodule-unit-operations.md) 決定 3 の位置づけを本 ADR が変える（同 ADR に日付つき追記あり。**本文は不変**）
- [IADR-0209](./IADR-0209_vitest-include-subset-of-frontend-tests-paths.md)（雛形 frontend を CI 対象に入れた先行例。#801）
