...
> `remaining: 0` を信じた読み手は**据え置き中のプロジェクトに負債が無いと誤解する** ——
> **緑が古い値を守っている**形である。
>
> **足す場合の費用（実測）**: ユニットごとに **full rebuild を 1 回追加**する
> （knowledge の full rebuild **15 秒** / 増分 6 秒。CI の `backend-build` は 1 ユニット 1〜1.5 分）。
> 🔴 **既存ビルドは再利用できない** —— `NoWarn` が効いているためで、これは `IADR-0231` が
> 警告ノイズを退けた決定そのものである。検証には `-p:NoWarn=` の**別パス**が要る。
>
> **得られるもの**: [[IADR-0238]] の外側にある **#946（アナライザの盲点）により
> 実数より小さいと分かっている数**を、正確に維持すること。つまり**精密に維持された不正確な数**である。
> 加えて未移行プロジェクトへテストが増えるたびに**無関係な PR が baseline 更新を強いられる**
> （陳腐化率がそのまま強制編集の頻度になる）。
>
> **決定: 足さない。** 守るべき不変条件「**移行済みは 0 件を保つ**」は `WarningsAsErrors` が
> `remaining` と無関係に強制しており、件数検証はそれを強くしない。
> 代わりに baseline へ **`remainingMeasuredAt`（測定時点）**を置き、
> **`remaining` が informational であること・盲点で過小であること・手で更新すること**を
> `$comment` に明記した。**古さを隠さず見せる**方向で解いている。

> ［2026-08-23 追記 / #946］🔴 **`remaining: 0` の意味を、さらに 1 段弱く訂正する。**
>
> 上の追記（#968）は「`remaining: 0` は**古くなり得る**」と書いたが、それでも
> 「（測定時点では）まだ移行していない呼び出しが無い」ことは含意していた。**それも誤りだった。**
>
> #946 の実測（形 1〜4。ラムダ／ローカル関数／private ヘルパへ括り出された呼び出し、実測 9 箇所）
> により、`xUnit1051` は **`[Fact]`/`[Theory]` が付与されたメソッドの本体しか検査しない**ことが
> 分かった。ラムダ・ローカル関数・別メソッドへ委譲された呼び出しは、**その委譲先が「別のメソッド
> 本体」であるという一点だけで**、アナライザの走査対象から外れる。`WarningsAsErrors` は同じ
> アナライザに乗っているため、この形の未移行はビルドでも警告でも検出されない
> （#946 の再現実験・#979 の対照実験で実証済み）。
>
> 🔴 **したがって `remaining: 0`（＝許可リストに載り `migrated: true` の状態）は
> 「そのプロジェクトの全呼び出しが `TestContext.Current.CancellationToken` を渡している」ことを
> 意味しない。** 正確には —— **`remaining` は「アナライザが認識した `[Fact]`/`[Theory]` メソッド
> 本体で報告された件数」でしかなく、アナライザが見る範囲（そのメソッド本体そのもの。委譲先の
> ラムダ・ローカル関数・別メソッドは含まない）に限られる。**
>
> **本 ADR の決定 1〜4 が保証する不変条件は変わらない** —— 「移行済みプロジェクトで、アナライザが
> 見る文脈において `xUnit1051` が再発したらビルドが止まる」という範囲では、`WarningsAsErrors` は
> 引き続き有効である。変わるのは**その保証が及ぶ範囲の理解**であり、「プロジェクト全体の全呼び出し」
> ではなく「アナライザが見る文脈の呼び出し」に限定される。
>
> **検査器は追加しない**（#946 裁定。素朴な構文検査器は偽陽性 89 件 対 正検出 9 件で費用が見合わず、
> かつ「正しく検出しても移行してはいけない箇所」が 2 件実在するため、機械的な全数是正は誤った編集を
> 招く）。**形 5**（`FactAttribute` 派生のカスタム属性がメソッド本体ごと検査対象外になる問題）は
> #997 で解消済み（81 箇所を `Assert.SkipUnless` ＋ 静的ヘルパへ移行し、以後 `WarningsAsErrors` の
> 下に入った）。**形 1〜4 はアナライザの設計に起因し構成では直せないため、限界として文書化するに
> 留める。**
...
- **フォローアップ**（**すべて完了した。2026-08-28 追記を参照**）
  - 雛形 `SampleService.Tests` の 1 件（1 行で済む）
  - `Platform.Shared.Infrastructure.Tests`（Wolverine 移行チェーンの着地後）
  - 残り 13 プロジェクト・943 件の段階移行

> ［2026-08-28 追記 / #882］🔴 **移行が完了した。同時に「完了時に器ごと削除する」という
> 申し送りを取り消す。**
>
> 本リポジトリのテストプロジェクト **20 本すべて**が `XUnit1051Migrated` に載り、
> `scripts/xunit1051-baseline.json` の未移行は 0 件になった。最後の 2 本は
> `DocumentService.Api.Tests`（実測 94 → 0。205 テスト緑）と
> `Platform.Bff.Tests`（実測 232 → 0。404 テスト緑・skip 1）である。
>
> `IADR-0231` 決定 4 と本 ADR の状況節は「完了時に `NoWarn` を削除する」と申し送っており、
> `src/Directory.Build.props` のコメントは踏み込んで「**本コメントと 3 つの PropertyGroup ごと
> 削除する**」と書いていた。**これは誤りであり、削除しない。** 削除すると 2 つのことが同時に起きる:
>
> 1. **移行済みプロジェクトが `WarningsAsErrors` を失う。** `TreatWarningsAsErrors` は `false` の
>    ままなので、再発は **warning に落ちて CI は緑**になる —— 決定 1 の実測 3 行目
>    （「`NoWarn` を外すだけ」）が退けた状態そのものへ戻る。**移行を完了させた瞬間に、
>    移行を守っていた仕掛けを外すことになる。**
> 2. **AST が `NoWarn` を失う。** 事実 (2) のとおり本ファイルは import-chain で AST へ届き、
>    AST のテストプロジェクトは 38 本すべてが `Tests` で終わる。`IADR-0231` が退けた警告ノイズが
>    そのまま復活する（AST 側の `.editorconfig` の抑止行は既に削除済みで、当てにできない）。
>
> **「完了」が消すのは移行という作業であって、器の役目ではない。** 器は完了後も 2 つ働き続ける ——
> 移行済みの再発をビルドで止めること（1）と、AST・雛形を巻き込まないこと（2）である。
> 許可リストの意味だけが変わる: **「未移行の受け皿」から「強制の対象一覧」へ**。
> 決定 2 が挙げた許可リストの弱点（新規テストプロジェクトが黙って `NoWarn` を継ぐ）は、
> 完了後は検査器が単独で閉じる —— 新規は baseline に無いので `added` で落ち、
> 登録には `remaining: 0` ＋ `migrated: true` が要る。
>
> **決定 1〜4 は変更しない。** 変えたのは決定の外にあった申し送り（削除の予定）だけである。
> なお `remaining: 0` の意味は 2026-08-23 追記のとおり弱い（アナライザは `[Fact]`/`[Theory]`
> メソッド本体しか見ない）—— **20/20 は「全呼び出しが移行済み」ではなく
> 「アナライザが見る文脈で 0 件」である。** この限界は完了によって解消しない。
...
- **完了したので器ごと削除する** —— 2026-08-28 追記で取り消した（再発が warning へ落ち、
  AST の警告ノイズが復活する）
