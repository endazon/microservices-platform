...
INDEX 決定 21（計画リポ）「色だけで意味を持たせない」が
...
推測させ、[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md) の存在秘匿の趣旨に反する。移行の過程で生まれた退行であり、本決定で戻す。
...
### 決定 9: 認証導線の遷移先は「URL として解決して origin を照合する」（前方一致では不足）

`/login?from=` と OIDC の `state.returnTo` は**どちらも外部由来**（前者は URL に載るため利用者が
書き換えられる。後者は認可サーバを往復する）。この 2 経路の判定を
`foundation/auth/safeRedirect.ts` の `toInternalPath()` **1 箇所へ集約**し、
両方の呼び出し元がそれを使う（同じ条件を 2 か所へ書くと片方だけ直したときに気付けない）。

**文字列の前方一致（`startsWith('/') && !startsWith('//')`）では不足である**（実測。§実測）。
`/\evil.com` は「`/` で始まり `//` で始まらない」ため前方一致を通過するが、
WHATWG URL 仕様（＝ブラウザの解釈）はバックスラッシュをスラッシュと同一視するため
`https://evil.com` へ解決される。**初版の実装はこれを通していた**（オープンリダイレクト）。

判定は「先頭が `/` であること」に加えて、**自 origin を基準に `new URL()` で解決し、
解決後の `origin` が自 origin と一致すること**を要求する。ブラウザと同じパーサで判定するため、
「ブラウザはこう読むがコードはこう読む」という食い違いが原理的に生じない。

- 通した場合は**正規化後のパス（`pathname + search + hash`）**を返す（入力そのままを返さない）。
- 判定は**テストで固定する**（決定 5 と同じ思想——型で捕まえられない領域は実行時テストで塞ぐ）。
  検証条件を外すとテストが落ちることを実測で確認する（§実測）。
...
自 origin を `https://platform.example.co.jp` として、初版の前方一致
（`startsWith('/') && !startsWith('//')`）と、`new URL()` による解決先 origin を突き合わせた。

| 入力 | 前方一致の判定 | `new URL(入力, 自 origin).origin` | 危険 |
| --- | --- | --- | --- |
| `/search` | 通す | 自 origin | — |
| `//evil.com` | 弾く | `https://evil.com` | — |
| `https://evil.com` | 弾く | `https://evil.com` | — |
| `\\evil.com` | 弾く | `https://evil.com` | — |
| **`/\evil.com`** | **通す** | **`https://evil.com`** | **○ オープンリダイレクト** |
| **`/\\evil.com`** | **通す** | **`https://evil.com`** | **○ 同上** |
| `/%5Cevil.com` | 通す | 自 origin（ただのパス文字） | — |

前方一致は `/\evil.com` を通すため、**初版の実装には実際にオープンリダイレクトの穴があった**。
`new URL()` の解決先 origin を照合する方式へ改め、上表の全ケースをテストで固定した。

**検証の実効性（変異試験）**: 判定ロジックを 2 通りに壊して実測した。
...
いずれも復元で全件 green に戻ることを確認した。**3 層（純関数・ルート定義・画面）すべてで落ちる**ため、
どこか 1 層だけを緩めても検知できる。
...
- Supersedes: なし（既存の決定を全面的に置き換えるものは無い）。ただし**部分改定が 4 件ある**。
  ...
  > **［2026-08-05 追記 / #512］[IADR-0134](./IADR-0134_spa-route-code-splitting-boundaries.md)（SPA バンドルの分割境界）は本 IADR の部分改定ではない。**
