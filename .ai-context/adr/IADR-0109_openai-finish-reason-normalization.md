...
- 起点 issue: [#394](https://github.com/endazon/microservices-platform/issues/394)（`enhancement`）。
  [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 2 の消化（起点は #379 / PR #391）。
- 仕様書: `docs/specs/20260728_issue-394_openai-finish-reason.md`。
- 本 IADR は**プロバイダ境界での語彙の写像**のみを扱う。越境ルーティング（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)）・
  エンドポイントの有効化・共有契約の形（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)）は変更しない。

## コンテキストと課題

[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) は共有契約とポートへ `StopReason` を追加したが、**写像を実装したのは `ClaudeProvider`
...
[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) は「拒否直前の断片を下流の判断材料にしない」ことを fail-safe として決めており、
Claude 経路だけその保証があり、セルフホスト／Copilot 経路には無い、という非対称が生まれる。

両エンドポイントは既定 `Enabled=false`（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)）だが、設定 1 つで有効化できる。
...
2. `finish_reason` を素通しし、呼び出し側で両方の語彙を吸収する — 契約は「プロバイダの生の語彙」になり、
   **呼び出し側の数だけ写像が複製**される（`RagOrchestrator` / `LlmGatewayDiagramCoder` / AST 取引判断・
   ...
   [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) の fail-safe が破れる。issue #394 が指摘する「プロバイダによって契約の意味が変わる」
   状態を、呼び出し側へ移し替えるだけである。採らない。
3. `StopReason` を enum 化して両語彙を型で表す — 未知値が既定値へ黙って落ちる。[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) が
...
4. `finish_reason` の**欠落・`null` は `StopReason=null`**（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) の「未対応プロバイダは null」と
   同じ状態）。これは未知語彙ではないため warn ログを出さない（正常系のログを汚さない）。
5. **`content_filter` → `refusal` のときは本文を破棄**する（`Text` を空にする）。[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §決定 3 の
   「refusal のときだけ本文を破棄する」をプロバイダ横断で一貫させる。`length` → `max_tokens` の
   途中結果は**破棄しない**（同じく [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) と一貫）。
6. **ストリーミングは個別実装を追加しない**。両プロバイダは `ILlmProvider` の既定 `StreamAsync`
   （`CompleteAsync` を単一チャンクへ縮退。[IADR-0037](./IADR-0037_llm-sse-streaming.md)）を使うため、正規化後の `StopReason` は
   最終チャンクへ自動的に載る。
...
- **fail-safe の非対称を消す**。`content_filter` を `refusal` へ写像し本文も破棄することで、
  「AST 取引判断が拒否された断片を根拠に売買判断へ進む」（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) が塞いだ穴）が
  セルフホスト／Copilot 経路でも塞がる。
- **未知値の透過は [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) の設計判断の踏襲**である。`stop_reason` を文字列にしたのと同じ理由で、
...
- フォローアップ:
  1. **拒否率の可観測化**（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 3 /
  ...
  3. **`stop` → `end_turn` の写像の再検討条件**を明示しておく。現在の呼び出し側は `refusal` /
     `max_tokens` しか分岐しないため実害はないが、**`stop_sequence`（停止シーケンス到達）を
     `end_turn` と区別する呼び出し側が現れた時点で本写像は情報を失う**（OpenAI の `stop` は両方を含む）。
     その時は選択肢 4（生の `finish_reason` を併載する）か、停止シーケンス指定の有無から
     写像を分岐する案を再評価する。それまでは現状維持でよい。

## 関連

- Supersedes: なし（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 2 を消化する。決定内容は不変）
- Superseded by: なし
- 関連要求 / UC: FR-11（LLM 送信可否の統制）、FR-04 / FR-12（呼び出し側）、UC-01 / UC-02
- 関連 IADR: [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)（`stopReason` の契約と refusal の本文破棄）、[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)（ティアA/C 経路）、
  [IADR-0037](./IADR-0037_llm-sse-streaming.md)（既定 `StreamAsync`）、[IADR-0101](./IADR-0101_default-model-opus-5.md)（`max_tokens` 到達の背景）
