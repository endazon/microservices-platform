...
- 関連 ADR: ADR-0018（契約・イベント疎結合）／[IADR-0027](./IADR-0027_composability-folder-structure.md)（固定/可変の名前空間規約）／[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット第一構成）
...
**本スライスの範囲外（後続スライス）**: knowledge 固有 DTO の `Knowledge.Contracts` 移設と、BFF のユニット別エンドポイント合成方式。これらは platform→可変ユニットの依存禁止（[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) / `src/README.md`）に触れるため、BFF が knowledge DTO へ依存しない**合成点**の設計とセットで行う必要があり、独立スライスとする（#229 に follow-up として残す）。
...
- **依存方向を壊さない**: 6 イベントは knowledge サービスのみが購読/発行し、BFF はイベント**名**（文字列・pipeline.json）でのみ扱い型参照しない。よってイベント移設は platform→knowledge 依存を生まず、[IADR-0057](./IADR-0057_unit-dependency-machine-check.md) の依存方向検査を通る。
...
- BFF のユニット別エンドポイント合成方式（[IADR-0027](./IADR-0027_composability-folder-structure.md) の合成ルート概念の BFF 版）。
- #227（名前空間・アセンブリ改名）で `[MessageUrn]` を削除し、URN を `Knowledge.Contracts.Events` から導出する正準値へ統一済み（後方互換なし・[IADR-0062](./IADR-0062_namespace-assembly-unit-rename.md)）。
