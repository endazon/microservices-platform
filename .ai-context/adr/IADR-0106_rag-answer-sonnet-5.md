...
- 起点 issue: [#381](https://github.com/endazon/microservices-platform/issues/381)（`enhancement` / `priority:should`）。
  [IADR-0101](./IADR-0101_default-model-opus-5.md) §フォローアップ 4「`rag-answer` の Sonnet 5 追随が未消化」の消化。
- 計画根拠: ADR-0022（計画リポ）
...
- ルーティング設計そのものは [IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)。本 IADR は**その用途別割当のモデル版数のみ**を更新し、
...
検知できない。これは [IADR-0102](./IADR-0102_trade-decision-model-pinning.md) が取引判断のピン留めで実際に踏んだ罠であり、本作業は同じ構造を持つ。
...
1. **`PurposeModels` と `Models` を同時に更新し、`max_tokens` は据え置く（採用）** — 計画の確定値へ最短で
   追随し、罠（論点 1）をテストで恒久的に塞ぐ。`max_tokens` は [IADR-0101](./IADR-0101_default-model-opus-5.md) の配分見積もりが Sonnet 5 でも
   成立するため据え置き、実測は #380 に委ねる。
2. `PurposeModels` のみ更新する — 論点 1 により `DefaultModel`（Opus 5）へ黙って落ち、**追随したつもりで
   追随していない**状態になる。#376 / [IADR-0102](./IADR-0102_trade-decision-model-pinning.md) で実際に踏んだ失敗の再演であり、採らない。
3. あわせて `max_tokens` を引き上げる — 新トークナイザの +30% を根拠に増やす案。ただし本文枠 1024 相当が
   約 1331 相当へ増えても残る思考枠は約 2765 で、[IADR-0101](./IADR-0101_default-model-opus-5.md) の配分想定の内側に収まる。実測なしに
...
- `thinking` パラメータは引き続き**送信しない**（[IADR-0101](./IADR-0101_default-model-opus-5.md) の決定を踏襲）。
...
- 良い影響: 定型・高頻度 RAG 回答の品質が向上する。単価は据え置き。計画と実装の乖離（[IADR-0101](./IADR-0101_default-model-opus-5.md)
...
- 悪い影響 / トレードオフ:
  ...
  - `max_tokens` 4096 は**実測前の出発値**であり、Sonnet 5 の実トークン消費で不足する可能性は残る。
    不足時は本文が途中で切れ、`stop_reason: "max_tokens"` として観測される（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) で判別可能）。
- フォローアップ:
  ...
  2. **新トークナイザ前提でのコスト試算・レート制限しきい値の再測定**（ADR-0022 §結果のフォローアップ）。
     ...
     出ないことを確認する。
  3. **縮退経路の「使用モデル」ラベルの見直し**（issue #381「併せて見直す（任意）」・[IADR-0101](./IADR-0101_default-model-opus-5.md)
...
- Supersedes: なし（[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md) の用途別割当のモデル版数のみ更新。ルーティング設計は不変更）
- Superseded by: なし
