...
## 決定 4: 判定は「件数」ではなく「述語」で聞く

段 14（#982 の中心的判定）は `has_probe=$(grep -c … || printf '0')` で件数を数えていた。
`grep -c` は**無マッチでも stdout へ `0` を書きつつ exit 1 を返す**ため `||` の右辺も走り、
値が **2 行の `0`** になって `[ "$has_probe" = "0" ]` が**恒久的に偽**になっていた
——「一覧は非空だが作成した文書が含まれない」を検出する分岐へ**一度も到達しない**。

**在るか否かだけを見たいときに件数を経由しない**（`grep -q` の述語で聞く）。
ファイルが無い場合も 0 へ倒れ、fail-closed 側になる。

🔴 **「200 ＋ 空リストを PASS にしない」ために置かれた判定が、それ自体 fail-open だった。**
検査を足すこと自体は安全を意味しない —— **その検査が落ちることを変異で確かめるまでは、
何も確かめていないのと同じである。** 本件は AI レビューの指摘を受けて実測で裏を取り、
M6（一覧が探針を含まない decoy を返す）を足して修正前 EXIT=0 / 修正後 EXIT=1 を確認した。
