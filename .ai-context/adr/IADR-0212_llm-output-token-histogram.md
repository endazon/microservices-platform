...
- 実装 ADR: [IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md)（`llm.completion.total` を置いた決定・**本 ADR が同じ Meter へ計器を 1 本足す**）、
  [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)（終了理由の語彙）、[IADR-0101](./IADR-0101_default-model-opus-5.md)（既定 `max_tokens` = 4096）。
- **[IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) を Supersede しない。** 決定はすべて生きており、本 ADR は計器を足すだけである。
- 計画 ADR: **ADR-0044**（LLM 利用実績の計測と単価表）。同 ADR は「**未実装: トークン消費量・
  金額換算・フォールバック発火回数・単価表**」と明記している（pin `8cae89d` で実読）。

  | ADR-0044 の決定 | 本 ADR での状態 |
  | --- | --- |
  | 決定 1（用途別・モデル別の粒度で出す。総額のみは採らない） | **充足**（[IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) の Counter ＋ 本 ADR の Histogram。属性に `llm.purpose` / `llm.model` を持つ） |
  | 決定 2（**フォールバック発火回数**も用途別・モデル別に計測する） | **未着手**。本 ADR は補完の結果と出力トークンだけで、フォールバックの発火は数えていない |
  | 決定 3（有効期間つき単価表・**金額換算はゲートウェイ側で行う**） | **未着手**。本 ADR はトークン数までで、単価も金額換算も持たない |
  | 決定 4（可視化面は Grafana・BFF には載せない） / 決定 5（基盤と利用側の費用を合算しない） | 本 ADR の射程外（計器の話ではない） |

  **次にこの領域へ着手する人は ADR-0044 の決定 2（フォールバック計測）と決定 3（単価表）から読むこと。**
  本 ADR が埋めたのは決定 1 の一部（トークン消費量）だけである。
...
[IADR-0101](./IADR-0101_default-model-opus-5.md) は既定 `max_tokens` を 4096 と決め、**「実運用値で再調整する」を #380 へ預けている**。
...
[IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) が**値域を閉じた**もの（未知値は `other`、null は `none` へ集約）をそのまま流用する。
...
**[IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) の Counter は全 8 経路で計上する**（未送信も計上しないと拒否率の分母が欠ける）。
...
**上限付近を細かく刻む**（2048 / 3072 / 4096）—— [IADR-0101](./IADR-0101_default-model-opus-5.md) の 4096 が妥当かは
