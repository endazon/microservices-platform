...
### 決定 2: 統合スタックへ `ISTIO=1` を渡すのは**本 ADR では行わない**（測って分離した。#1316）

当初は本 ADR で入れる予定だった。**撃って測った結果、入らないことが分かった。**
...
#### 🔴 実測（run 34037589847。`workflow_dispatch` を作業ブランチへ撃った）

**failure / 所要 35 分**（`ISTIO` 無しの健全時は 11〜15 分。ジョブ上限 45 分）。

| ステップ | 結果 |
| --- | --- |
| Bring up the integration stack | **success**（Istio の導入とエッジ移設そのものは通る） |
...
原因は 2 つで、どちらも「サイドカーを入れただけ」では済まない。

1. 🔴 **ExternalName の別名がサイドカー注入より後に当たる。** `k8s-local-up.sh` は
   `[6/7] helm → rollout restart → 10 分待ち → [7/7] 別名` の順である。作り直された Pod は
   `rabbitmq` / `redis` の別名が無い状態で起動し、health が 500 を返して**8 回まで再起動**する
   （注入 14:05:33 → probe 失敗 14:06:07 → 別名作成 **14:18:33**）。
   `ISTIO` 無しでは作り直しが起きないので表面化しない。
2. 🔴 **旧 ReplicaSet の Pod が待ちの対象に残る。** タイムアウトした 28 件はサービスあたり 2 つ＝新旧両方で、
   旧側は Ready にならない。**#1055（完了 Job の Pod を待たない）と同型**が別の入口から入っている。

**`ADR-0005` / `ADR-0021` の構成は、これまで CI で一度も起動されたことがなかった。**
実際に起こすと立ち上がらないので、**これは本 ADR の射程を超えた作業である** —— **#1316 へ分離した。**

🔴 **したがって #1304 は本 ADR では閉じない。** 門はまだ 1 度も評価されていない。
決定 1 が用意したのは、**Istio を入れる作業が成立したときに門が黙って飛ばさない**という前提だけである。

### 決定 3: `ISTIO=1` が入口も動かすことを、変更の重さとして記録する

`scripts/k8s-local-up.sh:885`（`:884` は直前のコメント行）は **`LOCALEDGE=1` の枝の内側**で `istio-edge-up.sh` を呼び、
...
- **良い影響（本 ADR で得たもの）**: **Istio を入れると宣言した実行で、門が黙って飛ばさなくなる。**
  #1316 が Istio の投入を成立させた瞬間に、G12 は**設定を 1 バイトも変えずに**評価される側へ回る。
  🔴 **本 ADR だけでは G12 はまだ 1 度も評価されない**（`.github/` に `ISTIO` は 0 件のままである）。
- **得られなかったもの**: G12 が実際に評価される。`IADR-0377` が名前を付けた
  「値が偶然一致していても field manager を奪われていれば壊れている」形が、日次で見張られる。
  あわせて `ADR-0005` / `ADR-0021` の構成が CI で毎日起動されるようになる（従来は誰も起こしていなかった）。
- **悪い影響 / トレードオフ**: 本 ADR の変更だけなら **CI の挙動は 1 バイトも変わらない**（`ISTIO` を
  宣言している実行が 1 つも無いため）。**「直った」と読める記録を残さないこと**が、ここでの最大の注意である。
  #1316 が投入するときには、所要時間の増分（実測 35 分。健全時 11〜15 分）と
  **入口が Istio Ingress Gateway へ移ること**の両方を引き受ける必要がある。
- **測っていて、直していないこと**:
...
- **フォローアップ**:
  (1) **#1316**: 別名の適用順とサイドカー注入の再起動を噛み合わせ、`ISTIO=1` を CI へ入れる。
      緑になった時点で **#1304 を閉じる**（G12 がそこで初めて評価される）
