...
3. 静的ポリシー対（利用者条件 `projects` × 文書条件 `project`）は `ADR-0080`（2026-09-05）が
   ...
   ★［2026-09-07 追記 / #1323］**フォローアップ 2 は着地した**（[[IADR-0411]] 決定 3）。
   集合値キーは `UserAttributeEncoding.Split` の集合と許容値集合の**交差**で判定する。
   🔴 **ただし本項 2（束縛変数 `${current_projects}`）と項 1（必須化）は依然として未達である。**
   `ADR-0085` 決定 1・2・3 が保留しており、`projects` は **dev seed の属性辞書へ入れていない**
   （辞書に無ければ `ValidatePolicy` がそのポリシーを作らせない＝保留を機械で守る）。
