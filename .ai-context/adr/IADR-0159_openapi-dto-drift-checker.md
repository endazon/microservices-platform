...
[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) 論点 B の **B1**。C# の `bool Granted = false` の既定値は**呼び出し側の引数の既定**であって
...
**パス（1 回目の型）も見ない。** それは [IADR-0156](./IADR-0156_bff-authz-contract-checker.md)（#647 の `check-bff-authz-docs`）が
端点の側から見ている領域である。
...
[IADR-0140](./IADR-0140_cross-repo-issue-ref-checker.md) 決定 2 の相乗り。

## 決定 4: `required` の不一致は**ラチェット**にする（既存 10 件は据え置く）

`required` の検査を全数へ当てたところ、**着手時点で 10 件が一致していなかった**（実測）。
**#525 は `AccessScopeResponse` の資源に閉じており**（[IADR-0139](./IADR-0139_domain-bundled-contract-prs.md) 決定 1）、10 件の是正は別の作業である
（**#658** が引き受ける）。リポジトリ既定のラチェット方式
（`backend-library-baseline` / `landed-subject-baseline` と同じ）で**既存を据え置き、新規混入だけを落とす**。

> **［2026-08-10 追記 / #658・[IADR-0162](./IADR-0162_openapi-required-request-vs-response.md)］ラチェットは撤去した。10 件のうち是正すべき乖離は 3 件だけだった。**
>
> | 内訳 | 件数 | 実体 |
> | --- | --- | --- |
> | 応答側の `required` 漏れ | **3** | `ConversionJobDto`。openapi.yaml へ足して解消 |
> | 要求スキーマの既定値つきメンバー | **5** | **本検査器の偽陽性**（下記） |
> | 要求スキーマの「嘘の必須」 | **2** | **意図的な差**（#627 の是正そのもの）。`requiredExceptions` へ宣言 |
>
> **★ 偽陽性 5 件は、本検査器が [IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) の論点 A1＋B1 を要求スキーマへも当てていたためである。**
> B1 は**応答スキーマの規則**であり（[IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) の表題・決定 2 の系が明示）、
> 要求側では C# の既定値が「送らなければこの値になる」を意味する。
> [IADR-0162](./IADR-0162_openapi-required-request-vs-response.md) が到達性（`requestBody` / `responses`）で規則を分けた。
>
> **同じ失敗を 2 度している。** 下記「当初この数は 20 だった」はパーサ欠陥による偽陽性で、
> 今回は**規則の適用範囲**による偽陽性である。**手口は違うが、
> 「検査器の出力をそのまま債務として信じた」点は同じ**である。
> **baseline へ入れる前に 1 件ずつ実データを開くこと** —— この教訓は 1 回では足りなかった。
...
`1 issue = 1 PR`（[IADR-0116](./IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 1）と [IADR-0139](./IADR-0139_domain-bundled-contract-prs.md) 決定 1 の「判定の単位は資源」に反するからである。
...
- ~~**`required` 不一致 10 件の是正**（決定 4）。応答側 3 件と要求側 7 件で扱いが違う。**#658 が引き受ける。**~~
  → **回収済み（2026-08-10 / #658・[IADR-0162](./IADR-0162_openapi-required-request-vs-response.md)）。** 是正 3・偽陽性 5・意図的な差 2 で、ラチェットは撤去した。
...
- **#520 の作業仕様書は書き換えていない。** 確定した過去 PR の記録であり、後から注記を足すのは
  記録の改竄にあたる（`.claude/rules/traceability.md`）。回収の事実は本 ADR と [IADR-0132](./IADR-0132_openapi-required-from-csharp-nullability.md) に残す。
