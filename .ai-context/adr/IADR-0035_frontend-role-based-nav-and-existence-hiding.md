...
- 関連 ADR: ADR-0004（ABAC 認可モデル）／ [IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）／ [IADR-0030](./IADR-0030_operator-role-and-config-viewer-policy.md)（運用者ロール・ConfigViewer）／ [IADR-0031](./IADR-0031_name-claim-preferred-username.md)（NameClaimType=preferred_username）／ [IADR-0033](./IADR-0033_frontend-spa-foundation.md)（フロントエンド SPA 基盤）
...
2. **メニューの存在秘匿**: `FeatureModule.nav` に `requiresAnyRole?` を持たせ、`Layout` は権限のある項目のみを描画する。権限外にはメニューを **表示しない**（[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md) の存在秘匿の UI 表現）。
...
> [IADR-0124](./IADR-0124_tanstack-router-unit-composition.md)（#490）で部分改定された。**
...
> 旧契約（`FeatureModule.nav`）は本リポジトリから変更できないユニット（`@ai-stock-trading`。[IADR-0120](./IADR-0120_excluded-units-from-gitmodules.md)）の
