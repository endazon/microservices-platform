...
- 計画根拠: Vault（秘匿管理）の採用は `planning/.../06_technical/03_tech-stack-selection.md`（L42「秘匿管理｜HashiCorp Vault」・
  ...
  policy `eso-read`・seed/skip・専用 SA・VAULT 併用ガード）は [IADR-0096](./IADR-0096_vault-eso-secret-supply-k8s-auth.md)（PR-1）。同パターンの継続は [IADR-0097](./IADR-0097_vault-eso-secret-supply-pr2.md)（PR-2）／
  [IADR-0098](./IADR-0098_vault-eso-secret-supply-pr3.md)（PR-3）。opt-in オーバーレイは [IADR-0077](./IADR-0077_local-observability-vault-gitops-overlays.md)。（注: PR-1〜3 は誤って ADR-0006（可観測性）を参照していたが、
...
[IADR-0096](./IADR-0096_vault-eso-secret-supply-k8s-auth.md)〜[IADR-0098](./IADR-0098_vault-eso-secret-supply-pr3.md) で LLM／app／OIDC secret を ESO 供給へ移行した。PR-4 は最後の区分＝**基盤 secret**
