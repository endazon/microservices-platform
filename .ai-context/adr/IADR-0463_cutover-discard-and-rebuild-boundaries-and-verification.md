---
title: IADR-0463 再実装版への切替は 6 資産の破棄と realm.json からの作り直しで行う —— 共有 PVC は消さず DB 単位・realm 単位で消し、方式はメンテナンスウィンドウ、検証は「作り直されたか」を時刻で見る
type: impl-adr
status: Accepted
related_ids:
  - NFR-05
...
# IADR-0463: 切替は破棄と作り直し。消すのは DB と realm の単位、確かめるのは作り直しの時刻（#457）
