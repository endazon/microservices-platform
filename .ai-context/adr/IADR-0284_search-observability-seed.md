...
- **フォローアップ**:
  1. **案 2（埋め込みの供給）の裁定**。候補は「ティアB の宛先だけをクラスタ内 stub へ向ける」
     ...
     🔴 **［2026-08-30 追記 / #992］消化した。ただし採った形は上の候補ではない。**
     [[IADR-0313]] が **ティアA へ「決定的なローカル埋め込み」を足す**形（#992 が案 2 の
     第一候補として挙げていたもの）を採り、`SEARCH_HITS=1` を CI へ載せた。
     **「唯一の形」という上の記述は誤りである** —— ティアA は「社外送信なし」という**性質**であって
     特定の 1 実装ではないため、**越境マトリクスを触らずにティアA を増やせる**。
     宛先を偽装する必要は無かった。
     あわせて決定 4 の「ティアA の stub は `..._ruri_v3` へ入るので検索から見えない」という
     理由づけも、**エンドポイントのコレクションが設定値である以上は成り立たない**
     （新しいコレクションを宣言し、検索側の参照先も同時に切り替えれば揃う）。
     **決定 4 の主文（越境判定を 1 バイトも触らない）はそのまま生きており、[[IADR-0313]] も守っている。**
