...
> あわせて `ai-stock-trading/app-secrets` に Discord の環境固有 ID 4 件（秘密ではないが Git に置かない値）を足した。
> `items[]` は **6 KV・書けるプロパティ 20**、`deferred[]` は Vault パスで **18 件**になった（§結果の「4 KV・13 プロパティ」「20 件」は当時の数）。
