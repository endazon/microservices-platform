...
- 関連 ADR: [IADR-0027](./IADR-0027_composability-folder-structure.md)（合成ルート概念）／[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット第一構成・依存方向）／[IADR-0059](./IADR-0059_contract-layering-unit-contracts.md)（契約階層化・イベント分離済み）
- 関連仕様書: `docs/specs/20260711_issue-229-slice2_bff-composition-design.md`
- Issue: #229（フォローアップ 3・後続スライス。イベント契約分離は [IADR-0059](./IADR-0059_contract-layering-unit-contracts.md) で完了）

## コンテキストと課題

[IADR-0059](./IADR-0059_contract-layering-unit-contracts.md) で 6 イベント契約は `Knowledge.Contracts` へ分離済み。しかし **knowledge 固有の DTO と BFF 集約
...
（[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)）に抵触する（鶏卵）。BFF の合成手段（[IADR-0027](./IADR-0027_composability-folder-structure.md) の合成ルート概念の BFF 版）が未整備。
...
> 繋ぎかえ」ではない（＝ADR-0018 違反ではない）。フロントの合成点（[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 例外2）が同種の解釈で
...
- 依存規則（[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) / [IADR-0057](./IADR-0057_unit-dependency-machine-check.md)）に **BFF 合成点の例外**を追加する（フロントの合成点
...
**選択肢 A（ビルド時合成点）を推奨**する。フロントエンドの合成点パターン（[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 例外2）と一貫し、
...
- 依存規則の例外3 追加は [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) / [IADR-0057](./IADR-0057_unit-dependency-machine-check.md) の追補となる（例外3 準備スライスで `check-unit-dependencies.js` に反映）。
