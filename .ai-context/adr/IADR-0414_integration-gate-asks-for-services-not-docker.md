...
［2026-09-09 追記 / #1337］**原因は切り分けられ、是正が入った。** 共有ブローカ上に残る固定名の
購読キューへ他クラスの本物のイベントが滞留すること（配送側）と、共有 DB で固定 Title 由来の
slug が一意索引に衝突すること（受信後の処理側）の 2 つである。是正はどちらも「実行ごとに
一意にする」であり、**主張は緩めていない**（作業仕様書 `20260909_issue-1337_fanout-tests-on-shared-broker`）。
