...
- 🔴 **AST 側 DB は「残す」と読む。これは意図した読みである。** 2026-08-16 の判断表の AST 側 DB の行は、判断欄に「**破棄。**」と書きながら、
  同じ欄で「ai-stock-trading は別リポジトリの submodule で、**本 issue の射程外**」とも書いている。2 つは両立しない。
  本 IADR は**残す側**を採る。理由は 3 つある —— (1) 破棄は戻せず、残すことは後から破棄へ変えられる（非対称なので安全側は残す側）、
  (2) その DB は 2026-09-25 時点でオーナーの売買 PoC が使っている、(3) データの所有者は別リポジトリであり、破棄の判断はそちらが持つ。
  **AST 側 DB を破棄したいなら、それは AST 側の作業として別に決める**（本 PR の手順と検証スクリプトは、AST の DB が消えたら fail を出す向きに作ってある）。
...
- `ARGOCD=1` で立てたクラスタでは Application `microservices-platform` が自動同期（`selfHeal` / `prune`）であり、窓の間は止める
  （削除した PVC を途中で作り直させないため）。起動器の再実行が Application を当て直すので、戻し忘れは起きにくい。
...
| 側 | 見るもの | 合格の条件 |
| --- | --- | --- |
| 破棄した側 | MSP の DB 13 本の作成時刻（`PG_VERSION` の更新時刻）・realm の人間の利用者の `createdTimestamp`・作り直した PVC の `creationTimestamp`（可観測性もこれで見る） | `since` 以降 |
| **触らない側（陰性対照 1: 作り直していない）** | AST の DB 7 本の作成時刻・`postgres-data` / `keycloak-data` / `vault-data` の `creationTimestamp` | **`since` より前のまま** |
| **触らない側（陰性対照 2: 消えていない）** | `--baseline`（切替前の実測）に在った AST の DB と、作り直しの対象でない realm（master・AST realm ほか） | **切替後にも在る**。master は `--baseline` が無くても見る |
...
- 🔴 **陰性対照 1 だけでは「消えた」は捕まらない。** 消えたものには作成時刻が無く、`--baseline` が無いと「AST 未配備」と区別できない
  （監査の実測: 切替前に在った AST の DB が切替後に無くても skip・終了コード 0 だった。realm の消失は見てもいなかった）。
  **陰性対照 2 は `--baseline` を渡したときだけ働く**ので、移行仕様書は `--baseline` を必須とし、無いときは「基準」の行を skip で出す。
- **Prometheus の head の最古サンプル（`headStats.minTime`）は判定に使わない。** 古いブロックが残っていると head の最古は
  TSDB 全体の最古ではなく、作り直していなくても「作り直し後」に見える。参考表示に留め、判定は `prometheus-data` の PVC の作成時刻で行う。
...
- 悪い影響 / トレードオフ: realm のクライアントの secret は realm.json の宣言値に戻る。宣言値と違う値を配っている（ローテーション済み）なら、
  作り直しの後に配り直す必要がある。
- 悪い影響 / トレードオフ: 🔴 **AST の稼働中の身元は作り直す realm `platform` の中にある。** ローカルの連携配備では AST は platform realm で
  認証する（`ai-stock-trading-kb-writer` / `ai-stock-trading-llm-caller` / `ai-stock-trading-svc` / `ai-stock-trading-owner` の 4 クライアント、
  realm ロール `trading-owner` / `trading-service`、`developer` への `trading-owner` の付与）。作り直すと**これらは realm.json の宣言どおりにしか
  戻らない** —— ローテーションした secret は開発用の既定値へ戻り、宣言に無い実行時のロール付与は失われる。**売買 PoC の認証に直接効く**ので、
  移行仕様書の手順 0 で書き出し、手順 6 で戻す（オーナー作業）。「AST realm を触らない」（決定 1）は、AST の身元を守ることを意味しない。
...
1. **オーナー**: 切替の実行（稼働クラスタ）・窓の時刻の選定・任意の保全の要否・TOTP の再登録・実行時に作った利用者の作り直し・
   **AST の身元（`ai-stock-trading-*` 4 クライアントの secret・実行時に付けた `trading-owner` などのロール）の書き出しと復元**・
   `ARGOCD=1` のクラスタでの自動同期の停止と復帰の確認。
