...
### 決定 3 — **LLM コストの予算アラートは置かない。** 別 issue へ分ける
...
`http_response_status_code` である（単位も ms → 秒）。**別 issue へ分けた**（資源も原因も別であり、
...
- **フォローアップ**:
  1. **月次予算の上限アラート** —— しきい値の確定（計画側）と、費用の計器を含むイメージの稼働が前提。
  2. **SLO ルールのメトリクス名・ラベル・単位の是正**（別 issue）。
