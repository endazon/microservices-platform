...
- 起点 issue: [#395](https://github.com/endazon/microservices-platform/issues/395)（`enhancement`）。
  [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 3「拒否率の可観測性」の消化（起点は #379 / PR #391）。
- 仕様書: `docs/specs/20260728_issue-395_refusal-metrics.md`。
- **本 IADR は [IADR-0109](./IADR-0109_openai-finish-reason-normalization.md)（#394 / PR #415）を参照する**が、両者に実装上の依存は無い（正規化は
...
[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) で終了理由（`refusal` / `max_tokens` / 正常終了）は区別できるようになったが、
...
2. **`stop_reason`** — [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)（および #394 / [IADR-0109](./IADR-0109_openai-finish-reason-normalization.md)）の決定により、**未知の終了理由は
...
1. **カウンタ 1 本＋属性で軸を分ける／属性値は有限集合へ丸める（採用）** — `llm.completion.total` に
   ...
   [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) が定めたとおり**独立した軸**であり、属性を分けるとその意味構造がそのまま集計に出る。
   拒否率は `stop_reason="refusal"` ÷ `result="sent"` で得られる。未知値は `other` バケットへ集約し、
   原文はログ側（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) / [IADR-0109](./IADR-0109_openai-finish-reason-normalization.md)）が保持する。
...
> 計画 `ADR-0038`（`Accepted`）決定 6「フォールバックの発火を可観測にする」の実装（[IADR-0225](./IADR-0225_llm-purpose-fallback-chain-and-429-boundary.md)）による。
> **本 ADR の決定 1〜7 はいずれも覆っていない** —— 計器は `llm.completion.total` の 1 本のままであり
> （新しい計器を足さない選択を [IADR-0225](./IADR-0225_llm-purpose-fallback-chain-and-429-boundary.md) が明示的に採った）、属性を有限集合へ丸める原則も、
...
4. **`llm.result` と `llm.stop_reason` は独立した軸として扱う**（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §決定の踏襲）。
...
- **`result` と `stop_reason` を別属性にする**ことで、[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) が守った「越境が成立したか」と
...
- **`purpose` を設定で閉じる**のは、非有界な入力を有界化する唯一の確実な方法である。加えて
  ...
  検知できる（[IADR-0102](./IADR-0102_trade-decision-model-pinning.md) / [IADR-0106](./IADR-0106_rag-answer-sonnet-5.md) が繰り返し踏んだ「割当の無音の失効」に対する遅い警報になる）。
...
- Supersedes: なし（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) §フォローアップ 3 を消化する）
- Superseded by: なし
- 関連要求 / UC: FR-11（LLM 送信可否の統制）、NFR（可観測性）、UC-01 / UC-02
- 関連 IADR: [IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)（`Sent` と `StopReason` の軸の分離）、[IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md)（ルーティング）、
  [IADR-0101](./IADR-0101_default-model-opus-5.md)（既定 Opus 5・`max_tokens`）
