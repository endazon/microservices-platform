...
**[IADR-0124](./IADR-0124_tanstack-router-unit-composition.md) 決定 1 の性質は無傷である**——変えるのは `component` の値だけで、
...
- **`experimentalMinChunkSize` による自動併合**: 20000 で試すと**画面 7 本が初期チャンクへ併合された**
  （JS チャンクが 23 → 5 本）。しきい値の微調整に依存し、しかも分割の意図と逆向きに働く。
...
**穴の形を正確に書く。** E2E（`bundle-splitting.smoke.spec.ts`）は初期ロードで取得する
`/assets/*.js` が **2 本以上**であることを見ている。実測した検出範囲は次のとおりで、
「まったく検出手段が無い」わけでも「規則を守らせている」わけでもない。

| 壊し方 | E2E | 理由（実測） |
| --- | --- | --- |
| **分割を丸ごと失う**（ルート遅延も `manualChunks` も無い＝V0 相当） | **落ちる**（`Received: 1`） | 初期ロードが `index-*.js` の 1 本になる |
| `manualChunks` の **3 規則を全部外す**（V1 相当。ルート遅延は残る） | **落ちない** | `/` は SC-01 へ解決され、ガードの無いルートは `preloadRouteComponents` が `.preload()` を呼ぶ。実測で `/assets/*.js` を **8 本**取得する（画面チャンクと共有チャンク） |
| **1 規則だけ欠ける**（M6 / M7） | **落ちない** | 同上。本数は増えこそすれ 1 本にはならない |

つまり E2E が守るのは「**ルート分割が消えていないこと**」であって、
`manualChunks` の規則構成ではない。規則の構成を守るには §申し送り 3 の機械検査が要る。
...
- **決定 4 の 2 本は [IADR-0124](./IADR-0124_tanstack-router-unit-composition.md) 決定 5 と同じ思想である**——型で捕まえられない配線の誤りは、
...
- 悪い影響・トレードオフ:
  ...
  - **`@platform/ui` を初期側に置く判断は gzip 14.31 kB のコストを持つ**（V2 との差）。
    実体は `Tabs` が引き込む Radix（`@radix-ui/*` 計 **61.01 kB rendered**）であり、`Tabs` を使う画面は SC-09 だけである。
    **この Radix を初期側へ引き込んでいるのは [IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1（公開面は `src/index.ts` の 1 ファイル）ではなく、
    本決定 3 の `ui` 規則が barrel ごと 1 チャンクへ寄せていることである。** 実測 2 件が根拠:
    - (a) `manualChunks` を入れない **V1 では barrel があるのに Radix は初期チャンクに来ない**。
      Rollup の per-export tree-shaking が `Tabs` を SC-09 の遅延チャンクへ落とすためで、
      V1 の `AdminAbacSettingsPage-*.js` は 38.19 kB（V5 では 11.78 kB）になり、
      V1 の初期チャンクに Radix の痕跡（`data-orientation`）は**現れない**（実測）。
    - (b) **`ui` 規則から `Tabs.tsx` と barrel だけを除く 2 行**（`@platform/ui` の公開面・ESLint・
      [IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) はいずれも**無改変**）で、初期ロード JS が 577.54 → **551.23 kB**、
      同 gzip が 177.94 → **168.98 kB（−8.96 kB）**になる（変種 V5b）。最大チャンク（274.33 kB）・
      JS チャンク数（17）・1 kB 未満の遅延チャンク数（3）・500 kB 警告の不在はいずれも V5 と同じで、
      増えるのは **SC-09 の遅延チャンクだけ**（11.78 → 38.03 kB＝使う画面が払う）。
    - **それでも V5b は採らない。** issue #512 が「`@platform/ui` のプリミティブは初期チャンクに残す」と
      明示しており、V5b は `Tabs` を遅延側へ出すためこの文言に反する。**決定は変えない**が、
      この gzip 8.96 kB は「規約のせいで直せない費用」ではなく **issue の指定を守るために払っている費用**である。
  - **チャンク構成の退行を止める機械が無い**（決定 4 の但し書き）。
  - **決定 5（`build:analyze` を残す）は開発依存 11 件を常時抱える決定でもある。**
    ...
    `src/pnpm-lock.yaml` に **113 行**の追加）。うち **`open` / `powershell-utils` / `wsl-utils` /
    `is-in-ssh` は OS 連携**（既定テンプレートが HTML をブラウザで開くための実装）であり、
    本リポは `template: 'raw-data'` しか使わないので**機能としては一度も通らない**。
    成果物には入らない（devDependency）が、**サプライチェーンの面数は増える**。
    「一度測って捨てる」を採らなかった代償はここにあり、外す／置き換える判断の受け皿を
    §フォローアップ 6 に置いた。
  - ルートの `component` に 2 つの形（遅延そのもの／ガード＋遅延）が混在する（決定 2）。
  - **ガード配下の画面（SC-05 / SC-09 で実測）は `findBy*` の待ち時間に動的 import が乗る。**
    カバレッジ計測を有効にすると Testing Library の既定 1000 ms に収まらないことがあり（実測: 9 回中 1 回）、
    `asyncUtilTimeout` を 5000 ms へ引き上げた。**決定 2 で B（ガードごと遅延）を採れば
    事前読み込みが効いてこの揺れは出なかった**——権限外へコードを配らないことの代償である。
    **引き上げは横断 setup（`platform/frontend/src/test/setup.ts`）ではなく、
    ガード付き画面を描画する唯一の入口 `@foundation/testing/renderUnitRoute` に閉じる**——
    横断 setup は `src/vitest.config.ts` により全ユニットへ効くため、そこへ置くと
    遅延ルートと無関係なテストまで「1 秒で落ちるべき退行が 5 秒待って落ちる」経路になる。
    **入口へ移すだけでは閉じない**点に注意する——`configure()` は `@testing-library/dom` の
    グローバル設定を書き換えるので、効くのは「その 1 回の描画」ではなく**そのテストファイル全体**
    である。したがって同ファイルへ「すぐ失敗すべき」検証を後から足せば同じ弊害が再現する。
    **既定値を捕まえて `afterEach` で戻す**ことで、延長を受けるのを**この入口を実際に呼んだテスト
    だけ**に限る（Vitest はファイル単位で環境を分離するため、他ファイルへは元々漏れない）。
- フォローアップ（詳細は[作業仕様書 §申し送り](../specs/20260805_issue-512_spa-route-code-splitting.md#申し送り)）:
  ...
  2. AST 3 画面（62.09 kB rendered）の遅延化。**AST が新契約（[IADR-0124](./IADR-0124_tanstack-router-unit-composition.md) 決定 2）へ移るのが前提**。
  ...
  6. **`build:analyze` の依存 11 件を減らす**。`emitFile` でモジュールグラフを自前で書き出す
     （外部プロセス起動＝`open` 系が不要な構成）へ寄せるか、常設をやめて計測時だけ
     `pnpm dlx` で入れる。**判断の期限は「次に分割を見直すとき」**であり、
     そのときに計測が再現できることと引き換えなので、**依存を外すなら代替の計測手段を先に用意する**。
...
**［2026-08-05 追記］M1 / M4c / M5a は最終状態で測り直した。**
とくに **M5a は最終状態の全体（557 件）で fail するのが `initialChunk.test.ts` の 1 件だけ**であり、
残り 556 件は green のままだった——**この 1 本が唯一の防波堤である**ことが件数で示されている。
詳細は[作業仕様書 §変異試験](../specs/20260805_issue-512_spa-route-code-splitting.md#変異試験壊すと落ちることの実測)。
...
- **採番について**: 本 ADR は当初 `IADR-0133` を採ったが、**`IADR-0134` へ改番した**（本 PR は改番時点で
  ...
  経緯は次のとおりで、**`0133` は #526 が使っている**。
  - `develop`（`9dc6c13`）に一時 **`IADR-0132` が 2 件あった** —— `IADR-0132_openapi-required-from-csharp-nullability.md`（#520 / PR #528。先着）と
    `IADR-0132_abac-dev-seed.md`（#526。後着）が相次いでマージされ、同番のまま残った。
  - 先着尊重により後着の #526 が `IADR-0133` へ動く。本 PR がその番号を空けるため `0134` を採った。
  - **#530（`6a0336e`）で #526 側の改番が入り、この採番衝突は解消した。**
    `IADR-0130`〜`IADR-0134` は欠番なしで揃っている（索引 `docs/adr/README.md` にも注記した）。
