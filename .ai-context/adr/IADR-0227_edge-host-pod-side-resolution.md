...
- 先行: [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md)（#779。`edge-tls` と安定した CA）／[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md)（#841。admin:50000 の TLS 終端と
...
| — | [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) 決定 5 の Supersede |
...
[IADR-0116](./IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4（着手時の分割）に沿う。
...
| `ExternalName` Service | **不可**。`keycloak.localhost` は**ドットを含むため Service 名にできない**（DNS-1035 label 制約）。[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) の既存エイリアスが素の `keycloak` にしか使えないのはこのためである |
| `coredns` ConfigMap を直接編集 | 却下。k3s の管理物である |

**なぜ pod 側の解決が要るのか**: [IADR-0086](./IADR-0086_oidc-issuer-metadata-split.md) の metadata / issuer 分離は **.NET 側しか救わない**。
...
[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) が記録した事故の再来を防ぐ。CoreDNS が答えを持たないと、クラスタ外の名前として
...
ためにも要る。** 既存の `keycloak` ExternalName エイリアス 3 件は**残す**（[IADR-0086](./IADR-0086_oidc-issuer-metadata-split.md) の
...
**admin:50000 に置くと redirect URI が全クライアントで `:50000` 付きになり**、[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) の改定と
...
### 6. 葉証明書は作らない —— [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が既に `platform-infra` に置いている

`Ingress` の `spec.tls.secretName` は**同じ namespace の Secret しか参照できない**ため
`platform-infra` にも `edge-tls` が要るが、**[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md)（#841）が `edge-certificate.yaml` の中で
...
- **admin:50000 の TLS 化**は [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が済ませており、本 ADR は触らない
