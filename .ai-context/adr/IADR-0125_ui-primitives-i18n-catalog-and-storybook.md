...
- 関連する計画書 ID:
  ...
  01_screens（計画リポ）（§画面詳細・`mockups/hi-fi/`）／
...
> [SC-01 画面仕様書 §UI 部品](../../docs/screens/SC-01_search-chat.md)）。
...
> **［2026-08-07 追記・根拠事実の更新 / #599］本決定の前提は失効した。** **［2026-08-07 / #599］FR-19 / FR-20 の保留は範囲基準へ変わった**（[IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md) 決定 1・2。**範囲の正は計画 `ADR-0037` の「着手可否の注記」**であり、ここには転記しない —— 転記すると計画側が動いたとき一斉に腐る）。
...
- フォローアップ:
  - **Dialog の移植**（決定 2）: FR-19 / FR-20 の保留解除後（**［2026-08-07 / #599］保留は範囲基準へ変わった。[IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md)**）、#452 が当該画面と同時に行う。
