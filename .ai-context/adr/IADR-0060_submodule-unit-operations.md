...
- 関連 ADR: [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット第一構成）／[IADR-0057](./IADR-0057_unit-dependency-machine-check.md)（依存方向検査）／[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)（private submodule の CI 取得）
...
- 追加ユニットは**別リポジトリ（多くは private）**の submodule であり、CI での取得にはトークンが要る（[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) と同じ論点）。
...
2. **submodule 取得**: 追加ユニット（private）を CI で取得するには `actions/checkout` の `submodules: recursive` + read 権限を持つトークンを有効化する（[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) の `doc-links-planning.yml` と同型。未取得ユニットは glob に現れず対象外になるため、取得の有効化が組み込みの前提）。
...
［2026-08-18 追記 / #830］**決定 3 の位置づけは [IADR-0224](./IADR-0224_template-backend-ci-build-by-staged-copy.md) により変わった。上の条文は書き換えていない。**
...
- 複製ビルド方式・`.sample` の除去・`--artifacts-path`・**実行件数の下限**での判定・
  ...
  **件数下限の抽出がテスト名の ASCII 始まりを前提にする**という既知の限界は [IADR-0224](./IADR-0224_template-backend-ci-build-by-staged-copy.md) が持つ。
- 同型の先行例は [IADR-0209](./IADR-0209_vitest-include-subset-of-frontend-tests-paths.md)（雛形 frontend を `frontend-tests.yml` の `paths:` へ入れた。#801）。
...
- **既存様式との一貫**: private submodule の CI 取得はトークン付き（[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)）で統一。
...
- Supersedes: なし
- Superseded by: なし
- フォローアップ改定: 決定②（追加ユニットの CI submodule 取得方式）は [IADR-0065](./IADR-0065_public-unit-submodule-ci-fetch-no-token.md) で改定した。
  private な `planning`（[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)）を巻き込む `submodules: recursive` を避け、`src/*` のユニット
