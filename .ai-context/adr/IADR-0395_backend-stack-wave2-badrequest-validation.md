...
**［2026-09-05 追記 / #1278］群 3 の落着先は `IADR-0398` である。** 同 ADR が基点 `4eff9bb4` で
母集合を引き直し、**呼び出し 37（ガード単位 42）のうち 26 を移し 16 を残す**と決めた。
🔴 **その過程で本 ADR 決定 9 の「群 3 ＝ 全違反を返す」という括りが不正確だったことが判った** ——
振る舞いで数え直すと**全違反を返すのは 11 箇所だけで、残る 26 箇所は「最初の違反 1 件を 1 つの鍵で返す」**
（DocumentService は 1 つも前者に当たらない）。したがって群 3 でも `IADR-0371` 決定 2 の
「`Errors[0]` を採る」「宣言順が応答の契約」は**大半でそのまま持ち越せる**。
群 4（#1279）は依然として別 issue が持つ。
