...
- 関連する計画書 ID: **FR-12** ／ **UC-06** ／ **SC-07**
  （05_screens/01_screens.md（計画リポ） §SC-07 §データソース・**2026-08-04 確定**）
- 関連 ADR: [IADR-0042](./IADR-0042_conversion-job-read-model.md)（決定 3 を本 IADR が部分改定＝ retry を例外化）／
...
- 関連する実装仕様書: [作業仕様書 #501](../specs/20260805_issue-501_retry-admin-only.md) ／
  [画面仕様書 SC-07](../../docs/screens/SC-07_conversion-jobs.md) ／ [テスト仕様書 SC-07](../../docs/tests/SC-07_conversion-jobs.md)
- 本リポジトリの起点: #501（画面側 #503 / PR #508・親 #454）

## コンテキストと課題

> **★［2026-08-10 追記 / #553］planning#198 提案 8 の裁定が出た。本 ADR が「据え置き」とした向きが確定した。**
>
> **利用者裁定 2026-08-05（質問票 第12回 Q19）で、計画側が実装へ合わせて改訂された。**
> 計画 `05_screens/01_screens.md` の現在の文言は次のとおりである（実測）:
>
> | 節 | 文言 |
> | --- | --- |
> | `:124` | アクセス制御の割当（2026-08-05 に是正）: **SC-05/06/07 = 閲覧は管理者・運用者／破壊的操作は管理者限定** |
> | `:273`（SC-05） | **閲覧は管理者・運用者。作成・編集・削除は管理者限定** |
...
>
> **つまり [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1（admin または operator）が正しかった。** 本文中の
> 「計画は管理者ロール限定と定める」「planning#198 の裁定に従う」「裁定を待たずに実装が先に答えを出さない」は
> **当時の状況として正しい記録**なので**書き換えない**（本追記が現在を示す）。
>
> **#501 の成果は有効なままである。** 裁定は「**破壊的操作は管理者限定を維持**」としており、
> `POST /bff/conversion/jobs/{id}/retry` を `AdminOnly` にした是正はそのまま正しい。
> **変わったのは閲覧側の向きだけ**である。
>
> **`status` は `Accepted` のまま据え置く。** 本 ADR の決定 1〜3 は**どれも覆っていない** ——
> 覆ったのは「計画と実装のどちらが正か」という外部の争点であり、本 ADR の決定そのものではない。
> `Superseded` にすると「再変換の管理者限定」まで無効になったと読める。
...
（[IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3 が定めた姿）。画面のボタンを隠しても、API を直接叩ける運用者は retry できる。

同時に、次の 2 点が「単純にグループを絞る」ことを許さない。

1. **照会（閲覧）は、計画と実装が既に食い違っている。** 計画は §共通シェル
   （`01_screens.md:115`（計画リポ）
   ...
   [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 に由来する**既知の逸脱**（実装が計画より緩い）である。
...
| D | 認可を BFF に置かず、下流（ConversionService）で判定する | 下流には認証基盤が無い（§(3)）。エッジ認証の一元化（[IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3・[IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md)）を崩す |

### (2) 照会の扱い

| 案 | 評価 |
| --- | --- |
| **据え置き（採用）** | 2026-08-04 の確定が命じたのは「**再変換の実行権限**」の是正であり、本 PR はそこだけを直す。照会が計画（`:115`・`:250` の管理者ロール限定）と食い違っていること自体は既知だが、それは [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 由来の**別の差異**であり、**その是正の向き（計画改訂か実装是正か）は planning#198 提案 8 の裁定に従う**。ここで併せて絞ると、**裁定を待たずに実装が先に答えを出す**ことになる |
| 一緒に admin へ絞る | 実装は計画の字面へ寄るが、**planning#198 の裁定を実装が先取りする**。裁定が「計画側を admin ＋ operator へ改訂する」と出れば戻す作業が要る。さらに同じ [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 が適用される SC-05・SC-06 だけが取り残され、**同一の差異の扱いが画面ごとにばらつく** |
...
| 下流に `RequireAuthorization` を足す | **そのままでは起動後に `/jobs/*` が全滅する**（認可ミドルウェア不在で `Endpoint contains authorization metadata, but a middleware was not found`）。認証配線と全環境への `Auth:Authority` 注入を伴い、[IADR-0029](./IADR-0029_config-info-api-placement-and-drift-granularity.md) / [IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3 の構造変更になる。**本 issue が求める「揃える」の範囲を超える独立の決定** |
...
2. **照会（`GET /bff/conversion/jobs`・`GET /bff/conversion/jobs/{id}`）は据え置く**（admin または operator）。
   これは計画（`01_screens.md:115`・`:250` の**管理者ロール限定**）に対する [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 由来の
...
3. **下流 ConversionService の `/jobs/*` の姿勢そのものは変えない。新しいのは代償統制の機械化である。**
   - **（既存決定の維持を確認した部分。本 IADR の新規決定ではない）** アプリ層の認可を課さないことは、
     [IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3（「ワーカー自身は最小 HTTP サーフェスに留め認可は課さない」）と [IADR-0029](./IADR-0029_config-info-api-placement-and-drift-granularity.md) が
...
4. **権限テストは「拒否される」側を主とする。** `Retry_AsOperator_IsForbidden`（403）を置く。
   ...
   **応答を 403（無認証は 401）とする根拠は [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 3** である —— 「権限外は 403（無認証は 401）
   とする。データソースは文書のような『存在自体の秘匿』対象ではなく（[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md) とは性質が異なる）、
...
- 決定 1 の形（案 A）は、**変更の影響範囲がコードの差分と一致する**。グループ定義に手を入れないため、
  ...
  > 計画 §SC-10 を正として **admin ＋ operator** へ広げたためである（[IADR-0129](./IADR-0129_sc09-11-admin-ops-screen-composition.md) 決定 4 の追記）。
...
- 決定 2 は、**本 PR が是正するのは 2026-08-04 に確定した retry だけである**ことの帰結である。
  照会の差異（**計画は管理者限定・実装は admin ＋ operator**）は [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 由来の既知の逸脱であり、
...
- 決定 3 は、下流の姿勢が**ロールの非対称ではない**という実測に基づく。BFF を絞ることで
  ...
  Keycloak ロールではなく**クラスタ権限**の問題で、[IADR-0026](./IADR-0026_mesh-mtls-supersedes-network-isolation.md) の防御対象である。
...
- 良い影響:
  ...
  - 照会の既知の逸脱（[IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 由来）が据え置かれ、planning#198 の裁定を実装が先取りしない。
...
- 悪い影響・トレードオフ:
  - **retry の認可がグループ定義とエンドポイント定義の 2 箇所に分かれる**。読み手が
    ...
    [IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3 の［追記］の両方で例外であることを明示する。
  - **画面と API の一時的な不整合**。本変更が PR #508（画面側）より先にマージされると、その間だけ
    ...
    **［2026-08-05 追記］PR #508（#503 / [IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md)）が先に develop へ入ったため、この過渡状態は生じない** —— 本 PR のマージで画面と API の権限が同時に揃う（[`docs/adr/README.md`](README.md) の索引行・[作業仕様書 #501](../specs/20260805_issue-501_retry-admin-only.md) §目的・背景 の［追記］と同旨）。
...
- フォローアップ:
  ...
  3. ~~**閲覧ロールの差異の裁定**（planning#198 提案 8。**計画は SC-07 全体を管理者限定と定め、実装は
     ...
     決定 2 を追随させる。SC-05・SC-06 も同じ [IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 の適用先であり、まとめて扱う。~~
     → **［2026-08-10 追記 / #553］条件は満たされた。追随すべきものは無かった。**
     裁定 **Q19**（2026-08-05）で向きは**「計画改訂」**と決まり、計画 `01_screens.md:124` / `:273` / `:298` / `:314` が
     「**閲覧は管理者・運用者／破壊的操作は管理者限定**」へ改訂された。
     **照会側の権限（admin ＋ operator）はそのままで正しく、決定 2 の据え置きは追認された。**
     SC-05・SC-06 も同じ結論である（同じ裁定が 3 画面に共通で効く）。**本項はこれで閉じる。**
...
- Supersedes: なし（[IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3 を**部分改定**する。同決定の本文と
  [`docs/adr/README.md`](README.md) の索引行の両方に日付付き［追記］を入れた
  —— 書式は `planning/.claude/rules/adr.md` §部分改定 2・4 と先例 [IADR-0084](./IADR-0084_headlamp-oidc-apiserver-flags.md) / [IADR-0087](./IADR-0087_k8s-local-up-optin-smoke-test.md) に倣う）
- Superseded by: なし
- **[IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) は本 IADR の改定対象外である。** 同 IADR §影響 は
  「**SC-05・SC-07 も本方針（管理系＝ admin/operator）に従う（各 IADR で個別記録）**」と明記して
  画面ごとの記録を委任しており、SC-07 の個別記録は [IADR-0042](./IADR-0042_conversion-job-read-model.md)（→ 本 IADR）の系列が担っている。
  よって retry の権限改定は [IADR-0042](./IADR-0042_conversion-job-read-model.md) 決定 3 の部分改定として閉じ、[IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 の本文には
  手を入れない（一般方針としての決定 1 は `Accepted` のまま有効。SC-05・SC-06 への適用も不変）。
  なお **PR #508（#503 / [IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md)）は develop へマージ済みであり**（`5ce3ec9`）、[IADR-0039](./IADR-0039_datasource-management-bff-and-role-gating.md) 決定 1 へ
