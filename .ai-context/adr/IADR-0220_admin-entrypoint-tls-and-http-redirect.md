...
- 実装 ADR: [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md)（経路B のエッジ TLS 終端＝ cert-manager の selfsigned→CA・`edge-tls`）、
  [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md)（経路B のエッジは Traefik・admin:50000 へホスト名ベース集約）、
  [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md)（経路B の SSO の恒久化。**`admin` ユーザーの ADR であり entrypoint の ADR ではない**。
...
[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) は 443（websecure）に載る `platform-frontend-edge` 1 件だけへ `spec.tls` を付け、
...
`secretName` は **[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) が安定させた `edge-tls` をそのまま使う**（`ADR-0047` 決定 2 の設計要件
...
（[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) 決定 5 が親 kustomization について述べたのと同じ形の fail-safe）。
...
**これは [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) 決定 4 の 3 命題のうち 2 つ（P2「admin:50000 の 7 件へは `spec.tls` を足さない」・
...
**条文の側（[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) の「`NFR-11` 適用外」という枠付けの撤回）は #834 が持ち、`848111cd` で
develop へマージされた。** それを受けて、**同 ADR 決定 4 へ本 ADR による部分 Supersede の注記**
（`［2026-08-17 追記 / #841］`）を入れてある —— **Supersede される側にも後継への導線を置く**ためである
（`traceability.repo.md`「旧 ID を残し、後継を併記する」。先例は [IADR-0117](./IADR-0117_platform-shared-kernel-placement.md)）。
...
[IADR-0092](./IADR-0092_argocd-keycloak-oidc.md) が置いた**設定は変えない。変えたのは前提の説明だけ**である ——
「edge が平文だから」から「エッジで終端し in-cluster は平文だから」へ改めた。
**改めた先は 2 箇所**: `deploy/local/argocd/oidc/argocd-cmdparams-patch.yaml` のコメントと、
[IADR-0092](./IADR-0092_argocd-keycloak-oidc.md) 本体の `［2026-08-17 追記 / #841］`（**本文は書き換えず追記で読み替えを示す**形）。

> **★ 同型の前提崩れを持つ live な ADR は [IADR-0092](./IADR-0092_argocd-keycloak-oidc.md) だけではなかった。** 誤りの側の語で
> 追跡下を全走査した結果（規則 9・7 軸）、**`Accepted` の live な ADR 4 件**が
> 「エッジ（`admin:50000`）は平文 http」を前提にしたままだった ——
> [IADR-0092](./IADR-0092_argocd-keycloak-oidc.md)（ArgoCD）・[IADR-0093](./IADR-0093_minio-keycloak-oidc.md)（MinIO）・[IADR-0094](./IADR-0094_vault-keycloak-oidc.md)（Vault）・[IADR-0095](./IADR-0095_wikijs-keycloak-oidc.md)（Wiki.js）。
> **4 件すべてへ同じ形の追記を入れた**（本文は書き換えない・`status` は `Accepted` のまま）。
> **この「4 件」は同じ形の追記を新規に入れたものの数であり、同型の前提崩れを持つ live な ADR の全数ではない**
> —— [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md)（決定 3 の追記）と [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md)（決定 4 の追記）は**別扱いで是正済みであり、計 6 件**である。
...
- **良い影響**: 経路B の**エッジ**（`web`:80 / `websecure`:443 / `admin`:50000 の 3 entrypoint）が
  ...
  **認証基盤（Keycloak）**を名指ししているが、**OIDC issuer は [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) 決定 5
...
- **悪い影響 / トレードオフ**:
  ...
  - **平文 `http://<tool>.localhost:50000` は TLS ハンドシェイクに失敗する。** 「404」ではなく
    ...
    **その記述は前提を [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) に帰していたが、同 ADR にその決定は無い**（上記 §関連）。
    書き換えにあたって**誤帰属も併せて解消した**。
  - **証明書が 3 本になる**（namespace ごと）。更新は cert-manager が担うため運用手数は増えない。
- **フォローアップ**:
  - **条文の追随（[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) の「`NFR-11` 適用外」という枠付けの撤回）は #834 が持ち、`848111cd` で
...
- **ブラウザが証明書を信頼するか**は検査しない（信頼ストアは環境の側にある。[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) と同じ）。
- **issuer（`http://keycloak:8080`）の https 化は、`NFR-11` の残件として開いている。** 本 ADR は検出も是正もしない。
  ...
  [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) **決定 5**（最小案 = `keycloak:8080` 維持。`Accepted`・live・**本 ADR は改めない**）のままであり、
...
- Supersedes: **2 本の ADR の、それぞれ一部**（**「`IADR-0206` だけ」ではない**）。
  - **[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) 決定 4 の 2 命題** —— **P2**（admin:50000 の 7 件へは `spec.tls` を足さない）と
    ...
    **P1**（443 の `platform-frontend-edge` へ `edge-tls` を追加する）と**決定 1・2・3・5・6 は改めない**ため、
    同 ADR は `Accepted` のまま残置する（先例: [IADR-0117](./IADR-0117_platform-shared-kernel-placement.md) が [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) 決定 3 を部分改定した形）。
    **対応する注記は同 ADR 決定 4 の `［2026-08-17 追記 / #841］` にある**（#843 マージ後に入れた）。
  - **[IADR-0094](./IADR-0094_vault-keycloak-oidc.md) 決定 2 の「http/https 両登録」** —— 同決定は「edge admin:50000 は現状 http だが、
    ...
    **対応する注記は同 ADR §代替案 の直後の `［2026-08-17 追記 / #841］` にある。**
    ...
    Supersede の射程外である。）
- Superseded by: なし
- **[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) は Supersede しない。** 本 ADR の初稿は「同 ADR が前提にしていた『admin entrypoint は平文 http』」と
  ...
  [`../../docs/operations/local-sso-recovery-runbook.md`](../../docs/operations/local-sso-recovery-runbook.md) の括弧書きの側で、
...
> | 箇所 | 何が過大 / 過小だったか | 是正 |
> | --- | --- | --- |
> | §結果 §良い影響 | 「**経路B の全エンドポイントが https**」が**過大**。`NFR-11` は対象に**認証基盤（Keycloak）**を名指しするが、issuer は [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) 決定 5 のまま `http://keycloak:8080` である | 射程を**エッジ 3 entrypoint**（80 / 443 / 50000）へ限定し、**残件を §検出しないこと（明示）へ挙げた**（解消は #780） |
> | §関連 `Supersedes` | 「[IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) 決定 4 の 2 命題**のみ**」が**過小**。本 ADR は [IADR-0094](./IADR-0094_vault-keycloak-oidc.md) **決定 2**（http/https 両登録）も覆しており、**同 ADR の却下代替案を採用形へ反転させている** | **2 本立て**に書き直した |
> | §決定 4 の ★ ブロック | 「live な ADR **4 件**」が母集合を狭く見せる（**同型の前提崩れを持つ live な ADR は計 6 件**） | 「4 件 = 新規に追記を入れた数」であることと、**[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) / [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) を含めて計 6 件**であることを明記 |
>
> **実作業に漏れは無い**（追記は 6 件すべてに入っており、`Supersede` した実体も正しい）。
> **誤っていたのは、それを説明する記述の射程だけ**である。`related_ids` へ
> [IADR-0092](./IADR-0092_argocd-keycloak-oidc.md) / [IADR-0093](./IADR-0093_minio-keycloak-oidc.md) / [IADR-0094](./IADR-0094_vault-keycloak-oidc.md) / [IADR-0095](./IADR-0095_wikijs-keycloak-oidc.md) を追加して**相互リンクにした**
