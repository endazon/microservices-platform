...
2. **分類 B で許容する固有デルタは次の 4 種のみ**とする。これ以外の独自記述は同期時に削除する。
   1. リポジトリ構成（ユニット第一構成 `src/*/{backend,frontend}`・submodule 取得ステップ。[IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md) / [IADR-0060](./IADR-0060_submodule-unit-operations.md)）
...
> **［2026-08-15 追記 / #516 の前段］本決定は [IADR-0198](./IADR-0198_kit-delta-fifth-kind-and-review-verdict.md) 決定 1 で部分改定した。**
...
- フォローアップ:
  ...
  - 第 12 ラウンド（pin `cd6c4f4`）で planning#136 が反映された一方、同じ変更で
    **`scripts.test.js` が GitHub Actions 上で失敗する**不具合を検出した
    ...
    是正後に撤去してバイト一致へ戻す。
  - 第 13 ラウンド（pin `9cd3499`）で planning#140 が反映され、**暫定デルタを撤去して
    `scripts.test.js` をバイト一致へ戻した**。分類 A のファイルへ一時デルタを置く運用
    （コメントで環流先 issue を参照し、是正後に撤去）が 1 往復で回ることを確認した。
  - 13 ラウンドの実績として、**環流した 17 件がすべて決着し、固有デルタはリポジトリ構成・
    技術スタックに起因するものへ収束した**。
