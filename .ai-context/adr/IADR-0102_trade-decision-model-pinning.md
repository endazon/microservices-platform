...
- `AST/ADR-0011`（Accepted）の §フォローアップ「基盤 LLM ゲートウェイの取引用途区分に固定モデル ID を
  設定する実装（IADR 起票）」の消化。
- [IADR-0101](./IADR-0101_default-model-opus-5.md)（ADR-0025 追従・グローバル既定を Opus 5 へ）と対になる決定。本 IADR は
...
[IADR-0101](./IADR-0101_default-model-opus-5.md) がその `DefaultModel` を `claude-opus-4-8` → `claude-opus-5` へ改定したことで、
...
- **`Models` への差し戻しは必須である。** `LlmRouter.ResolveModel` の用途別解決は
  ...
  ピン留めが無効化される。[IADR-0101](./IADR-0101_default-model-opus-5.md) は `Models` から `claude-opus-4-8` を除去していたため差し戻す。
  これは [IADR-0101](./IADR-0101_default-model-opus-5.md) の受け入れ基準 3 を意図的に改めるものだが、`Models` は「そのエンドポイントで
...
- Supersedes: なし（[IADR-0101](./IADR-0101_default-model-opus-5.md) の受け入れ基準 3〔`Models` から opus-4-8 を除外〕のみを改める）
- Superseded by: なし
