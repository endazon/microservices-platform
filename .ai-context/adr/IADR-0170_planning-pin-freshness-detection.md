...
| FR-19 / FR-20 | `ADR-0036`・`ADR-0037` が `Accepted` **かつ着手する範囲が「覆り得る範囲」の外**であること（[IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md) 決定 1）。**覆り得るのは SC-19 の「本文を編集（Wiki.js）」導線ただ 1 つ**で、範囲の正は計画 `ADR-0037` の着手可否の注記 |
...
これは [IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md) 決定 1 が既に置き換えた条件である**（「**前提検証の完了」は着手条件から外れる**）。
...
（[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md)。#626 の「`IADR-0134` だけを直すと 251 件のうち 2 件だけが揃った不統一になる」と同じ）。
