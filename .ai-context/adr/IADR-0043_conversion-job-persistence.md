...
- 関連 ADR: [IADR-0042](./IADR-0042_conversion-job-read-model.md)（変換ジョブ読み取りモデル・MVP インメモリ）／ [[ADR-0002]]（サービスごとの DB）／ [[ADR-0003]]（MassTransit。Superseded by ADR-0027・注記は #580）／ [IADR-0029](./IADR-0029_config-info-api-placement-and-drift-granularity.md)（ワーカーの最小 HTTP サーフェス）
- 関連仕様書: `docs/data/conversion-job.md` / `docs/screens/SC-07_conversion-jobs.md`
- Issue: #173（[IADR-0042](./IADR-0042_conversion-job-read-model.md) フォローアップ）

## コンテキストと課題

[IADR-0042](./IADR-0042_conversion-job-read-model.md) で、SC-07（変換状況・失敗一覧・人手補正）を実現するため ConversionService に
...
5. **API・BFF・画面・DTO（`ConversionJobDto`）は不変**。[IADR-0042](./IADR-0042_conversion-job-read-model.md) で抽象化した `IConversionJobStore`
