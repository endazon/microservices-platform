...
- 契約は **#543**（[IADR-0154](./IADR-0154_manual-figure-correction-phase1.md)）で揃っていた。本 IADR は**画面側の決定**に閉じる。
...
**直す場所を `bffFetch` にした理由**: [IADR-0121](./IADR-0121_spa-stack-migration-staging.md) 決定 3 が「SPA から出る HTTP はこの関数
1 箇所に収束させる」と定めた**その 1 箇所**である（[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md) 参照点を 1 つに畳む）。
...
（`usageCount` つき 409）と同じ形**である（[IADR-0154](./IADR-0154_manual-figure-correction-phase1.md) 決定 4）。

`not_retryable` の 409 とは**明確に区別する**——取り違えると「実行中のジョブに補正破棄を
勧める」ことになる。本文の `error` で出し分ける。

### `role="alertdialog"` は名乗らない

WAI-ARIA の `alertdialog` は「**モーダル**であり、応答するまで**背後を操作できない**」ことを
意味する。本確認はページ内に差し込む**非モーダル**の注意であり、**背後は操作でき、`Esc` も
奪わない**（共有 UI に Dialog プリミティブが無い。[IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 の公開面に無い）。

> **［2026-08-10 追記］この段落は当初「焦点も移さない」と書いていたが、下の追補で焦点は移す
> ことにしたので訂正した。** `alertdialog` を名乗らない理由は**モーダルでないこと**であって、
> 焦点を移さないことではない。**焦点移動とモーダル性は別物**である。

**実装しない振る舞いを role で宣言しない**——支援技術の利用者に嘘をつくことになる。
`role="alert"` で即時に読み上げ、確認は本文とボタンで表す。
**試験も role ではなく「破棄ボタンが在ること」で確認を同定する**（振る舞いで見る）。

#### ただし**焦点は移す**（AI レビュー 🟡 を受けて追加）

`role="alert"` はライブリージョンであってランドマークではないため、**読み上げは届いても
操作手段が保証されない**という指摘を受けた。**実測したところ、これは実害のある欠陥だった**:

- 確認は**表より前**に描かれる（`ConversionJobsPage.tsx` の確認ブロックは表の上）。
- それを出す「再変換」ボタンは**表の行の中**にある。
- すなわち確認のボタンは焦点位置より **DOM 上で手前**にあり、**前方 Tab では決して届かない**
  （Shift+Tab を知っている人しか操作できない）。

したがって**確認が出たら、その最初のボタンへ焦点を移す**。

**これは焦点の横取りではない**——利用者自身の押下に対する応答であり、かつ**破壊的操作を
差し止めている**確認だからである。**モーダルにしない判断とも両立する**: 背後は操作でき、
`Esc` も奪わない。**移すのは焦点だけ**である。

`@platform/ui` の `Button` は ref を転送しないので、**公開面を広げず**に包む側の ref から
引いている（[IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md) 決定 1 の公開面を変えない）。

#### 追補 2: 焦点は**非破壊側**へ移す（AI レビュー 2 巡目の 🟡）

最初の実装は `querySelector('button')` で**先頭のボタン**を焦点にしていた。ところが先頭は
`variant="primary"` の**「補正を破棄して再変換」**——すなわち**破壊的操作**だった。

**既定の焦点は `Enter` の当たり先でもある。** 「再変換」を押した直後に反射的にもう一度
`Enter` / `Space` を押すと、**確認を読まずに補正が消える**。ARIA APG の Alert/Confirm も
ネイティブのダイアログも、既定を**非破壊側**に置く。

**指摘の核心は「どちらを焦点にするか」を自分が選んでいなかったことである。**
`querySelector('button')` が機械的に先頭を拾った結果であり、**判断の跡が無い**。

是正は 2 つ:

1. **並びを変えた** —— 「取り消す」を先、「補正を破棄して再変換」を後にした（非破壊が先）。
2. **焦点を名指しにした** —— `[data-confirm-default]` で**明示的に**非破壊側を指す。
   **「先頭のボタン」に頼らない**のは、並びを変えたときに既定の焦点が**黙って破壊側へ移る**からである。

##### 変異試験が示したこと（正直に書く）

| 変異 | 結果 |
| --- | --- |
| 名指し `[data-confirm-default]` を `button`（先頭）へ戻す | **緑のまま** |
| `data-confirm-default` を破壊側のボタンへ付け替える | **落ちる** |

**前者が緑なのは、並べ替え後は「先頭のボタン」＝「取り消す」だからである。**
すなわち**試験が固定しているのは結果（非破壊側に焦点がある）であって、選択の方法ではない。**
名指しは**将来の並べ替えに対する備え**であり、試験では代替できない——この限界は自覚しておく。

## 決定 3: 縮退・補正の表示は導出であり、`status` の 5 値目にしない

`05_screens:320`「ジョブ状態モデルは 4 値である」＋ [IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md)「状態表示は契約から
...
区別できず、[IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1 が求める理由の文言を選べなくなる。

## 決定 4: ミューテーションが 2 本になったので `beginOperation()` 形へ移す

[IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 7。列挙は `Object.values(useConversionJobActions())` で辿る
...
404（コード化済み・未知の図・ストレージ未解決）は区別せず中立に倒す（[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md)）が、
...
- **デッドレターの内訳表示は束ねなかった**——同じ画面だが別の資源である（[IADR-0139](./IADR-0139_domain-bundled-contract-prs.md)）。
...
| **確認への焦点移動を外す** | **到達性の試験**（レビュー 1 巡目の 🟡 を受けて追加） |
| **既定の焦点を破壊側のボタンへ付け替える** | **同試験**（レビュー 2 巡目の 🟡 を受けて追加） |
...
## バンドル初期ロードの床を +1.92 kB 更新した（[IADR-0134](./IADR-0134_spa-route-code-splitting-boundaries.md) / `check-chunk-budget`）
...
起動時に読む（[IADR-0125](./IADR-0125_ui-primitives-i18n-catalog-and-storybook.md)）。遅延化はアーキテクチャの変更であり本 issue の射程外である。
