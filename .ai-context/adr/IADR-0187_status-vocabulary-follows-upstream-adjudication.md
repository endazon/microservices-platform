...
> ［2026-08-15 追記 / #757］**段 2 の `isBotAuthorName` は既に存在しない。** #757 で本リポ側を
> キットの正準名 `isBotLogin` へ改名し、`scripts.test.js` はキット版とバイト一致へ戻した
> （分類 A）。上の表は**当時そう壊れたという記録**であって、現在の API 名ではない。
