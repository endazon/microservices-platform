...
- 悪い影響・トレードオフ:
  ...
  - 🔴 **既存負債: `NotFound` が自前の `<main>` を持ち、`Layout` の `<main id="main-content">` と
    ...
    - ［2026-09-12 追記 / #1438］**解消済み**（[IADR-0442](IADR-0442_notfound-landmark-and-axe-scan-surfaces.md)）。
      「片方に合わせるともう片方が `<main>` を失う」は、**シェル外の経路にだけ器（`NotFoundPage`）を
      与える**ことで解いた。検出できない件も、axe のタグへ `best-practice` を足し走査面へ
      存在秘匿の 404 を加えて塞いだ。
- フォローアップ:
  - `NotFound` の `<main>` 入れ子の解消（上記）。
    - ［2026-09-12 追記 / #1438］**完了**（IADR-0442）。
