...
- 計画: FR-13 / UC-07。Wiki.js 配備・ABAC ゲートウェイは [IADR-0020](./IADR-0020_wiki-js-deployment-abac-gateway.md)。エッジ集約は [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md)、集約 route 追加の
  先例は [IADR-0093](./IADR-0093_minio-keycloak-oidc.md)（MinIO）。
...
[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) の Traefik `admin:50000` に **wiki 用 Ingress を新規1ファイル**（`deploy/local/edge/admin-ingress-wiki.yaml`・
host `wiki.localhost`→`wiki-js:3000`・microservices-platform ns）追加し kustomization に 1 行足す。**既存 grafana/argocd/
minio 等の Ingress・ファイルは無改変**（[IADR-0093](./IADR-0093_minio-keycloak-oidc.md) の MinIO と同型）。
...
ローカルログインは既定無効で OIDC 単一経路（[IADR-0020](./IADR-0020_wiki-js-deployment-abac-gateway.md)）。変更は edge route 追加＋realm redirect 追加＋docs に限定し、
helm chart・`values.yaml`・Wiki.js Deployment は無改変（Ingress 既定 disabled の非公開運用は不変）。edge は経路B opt-in。

**ABAC ゲートウェイ方針との関係**: [IADR-0020](./IADR-0020_wiki-js-deployment-abac-gateway.md) は Wiki.js への直接到達を塞ぎ WikiService の ABAC ゲートウェイを前段に
置く方針だが、その**非公開制約は本番系（Helm）限定**であり、[IADR-0032](./IADR-0032_wikijs-dev-exposure-opt-in.md)（IADR-0020 追補）が **dev ホストでの直接公開を
...
（履歴不変の原則により本文は書き換えない。[IADR-0032](./IADR-0032_wikijs-dev-exposure-opt-in.md) が [IADR-0026](./IADR-0026_mesh-mtls-supersedes-network-isolation.md) §2 を Amends した前例と同型）。

§2 は既存 URL を「**port-forward 用**（`http://localhost:3001/*`・`http://wiki-js:3000/*`）」と記したが、
`3001` は **compose(dev) の host 公開ポート**（[IADR-0032](./IADR-0032_wikijs-dev-exposure-opt-in.md) の `ports: 3001:3000`）であり、**k8s（k3d）の
...
| 経路 | URL | 根拠 |
| --- | --- | --- |
| edge 集約（`LOCALEDGE=1`） | `http://wiki.localhost:50000/*` | 本 IADR §1・[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) |
| k8s の port-forward（非 edge） | `http://localhost:3300/*` | #385。`port-forward svc/wiki-js 3300:3000` と同値 |
| compose(dev) の host 公開 | `http://localhost:3001/*` | [IADR-0032](./IADR-0032_wikijs-dev-exposure-opt-in.md)（`ports: 3001:3000`） |
