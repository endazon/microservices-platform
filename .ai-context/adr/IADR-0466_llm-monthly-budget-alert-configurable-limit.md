...
- 暦月との差: 31 日の月では、暦月の合計が予算を超えても、どの 30 日窓も超えないことが理論上あり得る（1 日分の差）。
  逆に、月をまたいだ 30 日で超えれば暦月ではどちらの月も予算内でも発火する。**早く・保守的に鳴る側**であり、
  予算の統制としてはこちらを採る（月次確認の Runbook は暦月で記録しており、数字がずれることを Runbook に書いた）。
