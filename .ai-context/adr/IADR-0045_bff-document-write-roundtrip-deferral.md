...
- 関連 ADR: [IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md)（文書書き込みの BFF ABAC スコープゲート）／[IADR-0038](./IADR-0038_bff-document-read-abac-gating.md)（読み取りの BFF ABAC ゲート）／[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）／[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md)（後段サービスの認可・多層防御）
- 関連仕様書: `docs/security/security.md`
- Issue: #179（PR #171 レビュー 🟢 / NFR / priority:could）

## コンテキストと課題

`DocumentBffEndpoints.ForwardIfInScope`（[IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md)）は、書き込み（更新／メタ更新／公開／
...
「[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md) で後段 `DocumentService` に認可を課したのだから、BFF のプリフライト GET は冗長では
...
[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md)（#174）が後段に課したのは **ロール認可（`platform-admin` / `platform-operator`）のみ**で、
**文書単位の ABAC スコープ照合ではない**。`DocumentService` の書き込みハンドラは対象文書の属性
（`confidentiality` 等）と利用者スコープの突合を行わない。すなわち：

- **文書単位の ABAC スコープ強制は BFF のプリフライト GET が唯一の実施点**（[IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md)）。
- プリフライト GET を削除すると、admin/operator ロールを持つが当該文書の ABAC スコープ**外**の
  利用者が、閲覧できない文書を更新・削除できてしまう（[IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md) の存在秘匿＝スコープ外は 404 も破れる）。

したがって [IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md) 後も 2 往復は**冗長ではなく、ABAC 書き込みゲートの実体**である。
...
1. **後段 `DocumentService` に ABAC スコープ判定を内包する条件付き書き込み**: 利用者の解決済み
   ...
   [IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md) が「ABAC スコープは BFF に集約」とした方針の再検討を要し、スコープ解決の重複
...
- コード変更なし（本 ADR は決定の記録）。既存の 2 往復・存在秘匿・[IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md) のゲートを維持する。
...
- [IADR-0041](./IADR-0041_document-write-bff-abac-scoped.md) / [IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md) / [IADR-0038](./IADR-0038_bff-document-read-abac-gating.md) / [IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md)
