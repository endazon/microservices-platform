...
- 関連する計画書 ID（MSP・機械追跡）: **NFR**（運用・保守。「再現可能なビルド環境」は同区分からの類推であり、計画書 NFR 表に逐語の項目はない）／**ADR-0007**（CI/CD・GitOps。コンテナイメージが配布単位＝[IADR-0067](./IADR-0067_service-image-build-ci-gate.md) と同じ起点付け）。本件は純粋なビルドインフラ改善のため、機能要求（FR）は起点にしない。
- 関連 ADR: [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)（frontend の unit 構成・platform=アプリホスト）／[IADR-0067](./IADR-0067_service-image-build-ci-gate.md)（サービスイメージのビルド CI ゲート `images.yml`。NFR/ADR-0007 起点の先例）／[IADR-0068](./IADR-0068_image-mapping-drift-check.md)（#275 image-mapping ドリフト検査）／[IADR-0078](./IADR-0078_frontend-k8s-serving.md)（frontend の k8s chart 配信・ローカルビルド常用化）
- Issue: MSP #325（本 issue・bug/infrastructure・priority:should）／派生元 #313（[IADR-0078](./IADR-0078_frontend-k8s-serving.md)）・親トラッカ #284
...
frontend は元々 compose 専用だったが、[IADR-0078](./IADR-0078_frontend-k8s-serving.md)（#313）で k8s chart 配信へ移行し、#284 のライブ
...
`scripts/check-image-mapping.js`（[IADR-0068](./IADR-0068_image-mapping-drift-check.md)）は `deploy/docker-compose.yml` の `build.args` と
