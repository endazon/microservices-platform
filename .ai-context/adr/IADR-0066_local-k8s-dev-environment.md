...
- **経路B infra の永続化（#324 / [IADR-0082](./IADR-0082_local-k8s-infra-persistence.md)）**: 本 ADR は経路B の infra を `emptyDir`（Pod 再起動で再 init）と
  割り切った。[IADR-0082](./IADR-0082_local-k8s-infra-persistence.md) はこの割り切りを **opt-in（`PERSIST=1`）で部分的に見直し**、Keycloak（realm+runtime state）/
  ...
  > ［2026-08-16 追記 / #787］ **同じ割り切りは 2 度目に広がった。** 後継は [IADR-0210](./IADR-0210_local-k8s-observability-persistence.md)
  > （[IADR-0082](./IADR-0082_local-k8s-infra-persistence.md) と同じく **opt-in の追加**であり、本 ADR も IADR-0082 も Supersede しない）。
  ...
  > **rabbitmq / redis / otel-collector は非永続のまま**（[IADR-0082](./IADR-0082_local-k8s-infra-persistence.md) の却下理由が成り立つ）。
  > **本 ADR 決定 2 の既定は依然として `emptyDir` であり、この記述は変わらない** ——
  > 追記の目的は、**本 ADR だけを読む人が「現在 opt-in で永続化できる範囲」を取り違えない**ようにすることである
  > （[IADR-0082](./IADR-0082_local-k8s-infra-persistence.md) の 2 種 ＋ [IADR-0210](./IADR-0210_local-k8s-observability-persistence.md) の 5 種 = 計 7 種）。
