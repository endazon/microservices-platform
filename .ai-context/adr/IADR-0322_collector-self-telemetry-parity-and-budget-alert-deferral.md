...
> ［2026-09-26 追記 / #1111］**本決定のうち「配線を置かない」部分は `IADR-0466` が改めた。「金額を置かない」
> 「`absent()` を置かない」「併存させない」は存続する。** 根拠 (2)（計器）と (3)（保持。PR #1355 で 35 日）は
> 解消し、(1) は所有者の裁定（金額を所有者が設定する値にできるなら配線してよい）で**配線と金額を切り離す**形に
> なった。(4) の併存は、金額が未設定のあいだルールが評価対象を持たないこと（不活性）と、金額を設定する変更が
> 同じ変更で Runbook を `superseded` にすること（`scripts/scripts.repo.test.js` が食い違いを落とす）で避ける。
