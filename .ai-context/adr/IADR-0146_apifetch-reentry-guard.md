...
- 関連する実装 ADR: [IADR-0121](./IADR-0121_spa-stack-migration-staging.md) 決定 3（BFF 境界・手書き HTTP クライアント禁止）／[IADR-0131](./IADR-0131_openapi-as-bff-contract-source.md) 決定 4（SSE は orval の生成対象外）／[IADR-0135](./IADR-0135_generated-client-adoption-and-cache-keys.md)（生成物の採用）／[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md)（母集合）
...
**`apiFetch` は `foundation/api` の正規 API なので止まらない**（[IADR-0121](./IADR-0121_spa-stack-migration-staging.md) 決定 3）。
...
SSE は orval が扱えず生成物が存在しないため（[IADR-0131](./IADR-0131_openapi-as-bff-contract-source.md) 決定 4）、
...
- **`platform/frontend/src/features/` 配下**。ここは合成点（`features/index.ts`）1 ファイルだけであり、
  画面は置かれない（[IADR-0121](./IADR-0121_spa-stack-migration-staging.md)）。platform ブロックは `foundation/api` 自身を含むため
...
- **`ai-stock-trading/frontend`**。別プロジェクトの submodule であり、本リポの規約を及ぼさない
  （[IADR-0120](./IADR-0120_excluded-units-from-gitmodules.md) / 既存の除外と同じ扱い）。
...
> 絞ったことも明示したことも無かった** ——[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md) が名指しした「**条文が片側しか書いていない**」型である。
>
