...
- 関連 ADR: [IADR-0051](./IADR-0051_datasource-connector-port-and-filesystem.md)（コネクタ抽象 Discover/Fetch・同期基盤）
...
- **失敗時**: HTTP 失敗（ネットワーク/4xx/5xx）は Discover/Fetch が**例外を送出**する。オーケストレータ
  （[IADR-0051](./IADR-0051_datasource-connector-port-and-filesystem.md) 決定3a）が discover 失敗として watermark を進めず、継続失敗アラート経路に載せる（UC-04 再試行）。
...
- **プラグイン方針との一貫性**: 「新規ソースは `IDataSourceConnector` 追加のみ」の方針（[IADR-0051](./IADR-0051_datasource-connector-port-and-filesystem.md)）に沿い、
...
- **失敗時挙動の再利用**: 例外送出により、既存の watermark 非前進・継続失敗アラート（[IADR-0051](./IADR-0051_datasource-connector-port-and-filesystem.md)）をそのまま活かす。
