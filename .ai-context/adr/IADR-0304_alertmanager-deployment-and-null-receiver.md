...
> ［2026-09-26 追記 / #1111］**「金額（しきい値）を置かない」は存続する。配線だけを `IADR-0466` が置いた。**
> 所有者の裁定（金額を所有者が設定する値にできるなら配線してよい）により、上限は用途別の設定値
> `Llm:Budget:MonthlyLimits`（**既定なし**）とし、ゲートウェイのゲージと直近 30 日の費用を比べるルールを置いた。
> 金額が未設定のあいだルールは評価対象を持たず発火しないので、本決定の「絶対額を置かない」とは衝突しない。
