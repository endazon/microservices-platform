...
6. **各 PR は `/verify` 通過と [`docs/DEFINITION_OF_DONE.md`](../../docs/DEFINITION_OF_DONE.md) の充足を条件**とする。
...
7. **ADR-0035（GraphRAG 検索戦略）は未起案**であり、RAG へのグラフ組み込み部分には着手しない
   ...
   - > **［2026-08-07 追記・事実の更新。本規約 7 の保留は解けた / #586］`ADR-0035` は 2026-08-07 に
     ...
     > **［2026-08-07 追記 / #599］この記述は同日中に古くなった。** **［2026-08-07 / #599］FR-19 / FR-20 の保留は範囲基準へ変わった**（[IADR-0142](./IADR-0142_fr19-20-scoped-release-by-overturn-range.md) 決定 1・2。**範囲の正は計画 `ADR-0037` の「着手可否の注記」**であり、ここには転記しない —— 転記すると計画側が動いたとき一斉に腐る）。
...
> | ゲート | コマンド | しきい値 / 判定 |
> | --- | --- | --- |
> | 受け入れ基準 → テストの写像 | `node scripts/check-test-traceability.js` | `docs/tests/` に仕様書がある FR/SC にテストが 1 件も無ければ **fail**。`scripts/test-traceability-allowlist.json` にある未写像は warn、写像済みなのに allowlist へ残置は **fail**。加えて逆方向（#472）: 計画レンジにあって仕様書が無い ID は warn、うちテストが先行している ID は同 JSON の `specMissing` による ratchet で **fail**（判定の正は[テスト戦略](../../docs/tests/TEST_STRATEGY.md)のゲート一覧） |
> | バックエンド カバレッジ床 | `node scripts/check-coverage-floor.js`（`ci.yml` の `build-and-test`） | [`src/coverage-floor.json`](../../src/coverage-floor.json) の床 **`line 90` / `branch 75`** 未満は **fail**（[IADR-0118](IADR-0118_backend-coverage-floor.md)。ratchet のため引き上げ後は本表も追随させる。値の正は同 JSON。**測定基準の変更で 2 度置き直した——#571 / [IADR-0138](IADR-0138_coverage-exclude-generated-code.md) で `line 34 → 33`（EF の生成コードを落とした）、#574 / [IADR-0195](IADR-0195_coverage-exclude-source-generator-output.md) で `line 33 → 39` / `branch 17 → 27`（source generator の出力も落とした）。いずれも退行でも強化でもなく、分子・分母から同じものを抜いた結果である。**3 度目は #899 で `line 39 → 38`** —— テストプロジェクトを 1 つ増やしたら割れた。集計がレポート跨ぎで重複排除しないため、共有ライブラリの行が参照数だけ分母に載る（正しい直し方は #900）。**4 度目は #900 / [IADR-0236](IADR-0236_coverage-cross-report-line-dedup.md) で `line 38 → 88` / `branch 27 → 68`** —— 集計がレポートを跨いで行を重複排除するようになり、分母の水増し（18604 行）が取れた。**被覆の実態は 1 行も変わっていない**（旧定義での実測は `line 38.5%` で #899 の記録を再現している）。**5 度目は #901 で `line 88 → 90` / `branch 68 → 75`** —— 🔴 **これだけは置き直しではなく本物の ratchet である。**測定基準は 1 つも変えておらず、共有ライブラリへテストを足した結果として実測が上がった（develop `3939e72` の Integration run で `line 91.07%` / `branch 76.12%`）。**切り下げ値（91 / 76）は採っていない** —— 余裕が 10 行 / 4 本しか残らず、テスト 1 件の skip で「成果物は正しいのに赤」になるためである（算定は同 JSON の `$comment`）。**） |
...
> [テスト戦略](../../docs/tests/TEST_STRATEGY.md)を参照する。`/verify` 通過と
> [`docs/DEFINITION_OF_DONE.md`](../../docs/DEFINITION_OF_DONE.md) の充足という規約 6 本文の条件は変わらない。
...
>   - **★［2026-08-16 追記 / #791］上限は 4 件へ改まった**（利用者裁定 2026-08-15 / planning#370。
>     反映 PR は planning#375〔裁定依頼 planning#370〕）。**本行と、下の「上記の実効上限 2 件は
>     この目安の帰結である」は 2026-08-07 時点の記述であり、消していない。**
>     **ファイル数・行数の目安は、もう件数の上限を決めない。** 正本は
>     [IADR-0139](IADR-0139_domain-bundled-contract-prs.md) 決定 1 §束の上限の
>     ［2026-08-16 追記］であり、**ここへ内容を写さない**。
>     **対象（裁定済みの同型な契約追加）と判定単位（資源）は変わらず、規約 1 の原則も変わらない。**
>
...
> **［2026-08-27 追記・規約 1 の第 3 の限定例外（#1018。改定は
> [IADR-0279](IADR-0279_wave-stacked-prs.md)）］**
>
> **規約 1（子 issue 1 件 = ブランチ 1 本 = PR 1 本）に、条件つきの例外をもう 1 つ加えた。**
> リポジトリオーナーの明示指示（2026-08-27。**全 open issue の一括対応**と**複数 issue を 1 PR へ
> 束ねること**の許可）により、**セッション横断の「波単位の積み上げ PR」を認める**。例外が働くのは
> [IADR-0279](IADR-0279_wave-stacked-prs.md) 決定 1 の **4 条件（W-A〜W-D）をすべて満たすとき**に限る。
> 条件を満たさないものは本規約 1 のままである。
>
> - **判定の芯は「オーナーの明示許可がセッション単位で存在するか」である**（同 決定 1 条件 W-A）。
>   第 1・第 2 の例外が**対象の同型性**（資源単位・ファイル領域）で歯止めをかけるのに対し、
>   本例外は**同型性を要求しない**——波は製品コードも検査器も文書も含む。
>   **過去のセッションの許可は引き継がない。許可が無ければ 1 件も適用できない。**
> - **件数の上限は置かない**（同 決定 2）。第 1・第 2 の例外の 4 件は**同型な作業が並ぶ前提の数**であり、
>   本例外へ持ち込んでも意味を成さないためである。**代わりに波が検証と巻き戻しの単位になる**
>   （W-D。波ごとに push し CI の緑を確認してから次の波を積む）。**ここへ理由を写さない。**
> - **既存 2 例外を置き換えない。** 対象が交わったときは**厳しい側（既存例外の条件）を満たす**こと
>   （同 決定 2 後段）。W-A の許可は [IADR-0139](IADR-0139_domain-bundled-contract-prs.md) の 6 条件も
>   [IADR-0230](IADR-0230_meta-work-bundled-prs.md) の M-A〜M-E も**免除しない**。
> - **本 IADR は `Superseded` にしない。** 原則（1 issue = 1 branch = 1 PR）は残り、限定例外が
>   もう 1 つ増えるだけである（第 1・第 2 の例外と同じ扱い）。
> - **規約 4（PR ではなく issue を分割する）は変わらない。** 波が大きくなりすぎるなら**波を分ける**。
> - **束ねた PR は `Closes #NNN` を issue ごとに 1 行**書く（[IADR-0139](IADR-0139_domain-bundled-contract-prs.md)
>   決定 3 を援用）。加えて **PR 本文に issue ↔ 作業の対応表と「閉じない issue の理由」**を書く（W-C）。
> - **機械検査は W-B（コミット件名）以外に置いていない**（同 決定 4。記録の無い積み上げ＝**PR #1009** が
>   1 度目であり、2 度目が起きたら検査器を足す）。
...
>   - **［2026-08-14 追記 / #735］この行の「1 回まで」は [IADR-0194](./IADR-0194_audit-rounds-follow-upstream-no-numeric-cap.md) が撤廃した**（上流裁定 planning#307。**測っていない数値を規約にすると、そちらを疑えなくなる**）。
>     **2 巡目以降を是正差分のみとする点・単位が「対象」である点は残る。** **際限のない巡回は、下の「打ち切り条件を明言させる」で抑える。**
...
- **Amended by: [IADR-0279](IADR-0279_wave-stacked-prs.md)**（#1018・2026-08-27。
  規約 1 に第 3 の限定例外「オーナーの明示指示があるセッションでは、波単位の積み上げ PR に
  複数 issue を束ねてよい」を足す。条件（W-A〜W-D）・件数の上限を置かない理由・機械検査を
  置かない理由は同 IADR にある。**本 IADR は `Accepted` のまま**）
