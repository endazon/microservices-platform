...
related_specs:
  ...
  - ../specs/20260926_issue-1583_purge-reread-before-delete.md
  - ../specs/20260926_issue-1598_maintenance-loop-foreign-cancellation.md
---
...
> **［2026-09-26 追記 / #1583］削除の直前に所有者の状態を読み直す。** 上の「削除の前に件数を 1 行ログへ残す」ために
> 判定を全員分先に済ませる形にした結果、ある所有者の判定から削除までの隙間が**判定の 1 巡分**（最悪で所有者数 × 5 秒）に
> 広がり、その間に再有効化された利用者も判定時の答えのまま消えるようになっていた（#1579 の差分監査で発見）。
>
> - `PurgeDepartedOwnersAsync` は、各所有者の資料を消す**直前**に `IOwnerRetentionDirectory.GetAsync` をもう一度読み、
>   `IsPurgeable` が引き続き真のときだけ消す。**読み直しが `null`（輸送の失敗・5 秒の時間切れ）・例外なら消さない**
>   （例外はその所有者だけを見送り、同じ周期の他の所有者と後段の定期処理は止めない。定期処理そのものの取り消しだけは伝える）。
>   見送った人数を 1 行ログへ残す（所有者 ID は出さない）。
> - 1 巡目の判定は**削除の根拠ではなく候補の選別**になった。件数のログ（決定 6）はそのまま残る。
> - 往復: 対象と判定した所有者だけが 2 往復になる（判定で落ちた所有者は 1 往復のまま）。日次粒度であり受容する。
> - 🔴 **1 巡目（判定）の例外は従前どおり捕まえない**（本追記で変えていない）。口の契約は失敗を `null` で返すことであり
>   （`GrpcOwnerRetentionDirectory` は輸送の失敗・時間切れを `null` へ畳む）、例外は契約の外である。契約の外の例外が 1 巡目で出ると
>   その周期の定期処理は止まるが、**止まる向きは「消さない」側**である。読み直しだけを隔離したのは、そこが不可逆な削除の直前の最後の門であり、
>   1 人の失敗で同じ周期の他の対象者の削除（計画の 30 日の窓）を遅らせないためである。
> - 🔴 **残る隙間は「読み直し → 1 人分の削除（本文の実体 → 行）」だけ**である。これを 0 にするには名簿と DB を跨ぐ排他が要り、
>   ADR-0096 決定 2（述語を 1 つ足す形）の射程を越えるので取らない。所有者（利用者）は本削除が不可逆であり配備で有効であることを受け入れている（決定 5）。
> - 同じ変更で、監査で生き残った変異と試験の穴を塞いだ：M8（退職の窓の照会で、`null` を受けたとき呼び出し元の取り消しを
>   投げ直す `ct.ThrowIfCancellationRequested()` を外しても通った）、同期の経路の s2s 失敗の枝のログ文言、
>   `GrpcOwnerAccountDirectory` が同期の読み口（`GetAccountStatusAsync`）を使うこと（2 つの読み口は応答の写し方が同じで、
>   取り違えは失敗時の文言でしか見えないため、記録ロガーで測る）。退職の窓の実装が退職の窓の読み口を使うことも対で固定した。
> - 変異の実測は下の「結果」の表の［#1583］行。仕様書は `../specs/20260926_issue-1583_purge-reread-before-delete.md`。
>
> **［2026-09-26 追記 / #1598］読み直しの試験の抜けを塞いだ（コードは変えていない）。** #1591 の監査で、読み直しで資格の判定を
> 見ない変異と、読み直しの取り消しのフィルタを外す変異が生き残っていた。読み直しで `WithinWindow`・`NotEvaluable`（いずれも無効のまま）に
> なった所有者を消さない試験 2 件と、読み直しの最中の定期処理の取り消しをサービスの水準で測る試験 1 件を足した。あわせて、試験の
> スタブの約束「列の最後の答えに例外を置かない」を、宣言の時点で最後の答えを評価して強制する形にした（`StubOwnerRetentionDirectory`）。
> 変異は「結果」の表の［#1598］行。仕様書は `../specs/20260926_issue-1598_maintenance-loop-foreign-cancellation.md`。
...
- **変異による裏取り**（宣言ではなく実測。1 か所ずつ書き換えて実行。当初の 11 種は新しい試験 20 件、監査後の 5 種は同期・写像・退職者削除の試験 49 件、M6 は DocumentService の全 613 件、#1583 の 9 種は退職者削除・2 つの gRPC 写像・同期の所有者の試験 54 件と共有クライアントの状態の試験 5 件）:
  ...
  | ［#1583］削除の直前の読み直しを外す | **4**（再有効化・読み直しで `null`・読み直しの失敗・窓が閉じたままの陽性対照の照会回数） |
  | ［同］読み直しの `null` を「消してよい」へ倒す | **1**（読み直しで名簿を引けなければ消さない） |
  | ［同］読み直しで述語を見ない（`null` でなければ消す） | **1**（判定の後に再有効化された所有者の資料は消えない） |
  | ［同］読み直しの例外を捕まえない | **1**（読み直しが失敗したら消さず他の所有者は続ける） |
  | ［同］M8: 退職の窓の照会で `null` の枝の `ct.ThrowIfCancellationRequested()` を外す | **1**（定期処理そのものの取り消し・本番の `RpcException(Cancelled)` の形）。従前は 0 件 |
  | ［同］`GrpcOwnerAccountDirectory` が退職の窓の読み口を使う | **1**（同期の読み口を使い失敗を同期要求の拒否として記録する）。従前は 0 件 |
  | ［同］`GrpcOwnerRetentionDirectory` が同期の読み口を使う | **1**（退職の窓の読み口を使い失敗を削除しない旨で記録する） |
  | ［同］s2s 失敗の枝の文言を入れ替える／同期の枝だけを退職の窓の文言にする | **2**／**1**（s2s 失敗の文言の 2 件）。従前は 0 件 |
  | ［#1598］読み直しで資格の判定を見ない（`status is { Found: true, Enabled: false }` で消す） | **2**（読み直しで窓が再び開いた・判定不能になった。いずれも無効のまま）。従前は 0 件（#1591 の監査で生き残った） |
  | ［同］読み直しの `catch` から取り消しのフィルタを外す | **1**（読み直しでの定期処理の取り消しは見送りに畳まず伝える。照会の合計が 3 → 4）。従前は 0 件。🔴 `RunAsync` は変異の下でも後段で取り消しを投げるので、例外の有無ではなく照会の回数で測る |
  | ［同］スタブ: 列の最後の答えを宣言時に評価しない | **1**（スタブの自己試験。列の最後に例外を投げる答えを置くと宣言の時点で失敗する） |
