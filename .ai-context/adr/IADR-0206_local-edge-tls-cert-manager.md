...
- 実装 ADR: [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md)（経路B のエッジは Traefik）、[IADR-0086](./IADR-0086_oidc-issuer-metadata-split.md)（issuer と metadata の分離）、
  [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md)（エッジ BFF ルーティングと OIDC hostname）、[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md)（admin entrypoint は平文 http）、
  [IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md)（apiserver OIDC フラグ配線の除去）。
  > **［2026-08-17 追記 / 波 11 末クロス監査］上の `[[IADR-0103]]（admin entrypoint は平文 http）` という帰属は
  > 誤りである**（同 ADR は `admin` という**ユーザー**の ADR であり、`50000` も `entrypoint` も `平文` も 0 件）。
  > **後継は [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md)**（admin:50000 を TLS 終端にした）。**詳細は決定 4 の追記**を参照。
  > **本文は当時の記録として書き換えない** —— この目印は、§起点・関連 から決定 4 の追記へ導線を届かせるために置く。
...
`issuer.url` に https を強制する**（[IADR-0084](./IADR-0084_headlamp-oidc-apiserver-flags.md)（Superseded by [IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md)）の 2026-07-25 追記が単一情報源。
...
既定（env 未設定）では何も適用せず**バイト等価**を保つ（[IADR-0077](./IADR-0077_local-observability-vault-gitops-overlays.md) の opt-in オーバーレイの流儀）。
...
- **消費側が違う。** `ADR-0023` の決定は「Istio Ingress Gateway（Envoy）が Secret を参照して TLS 終端する」形で
  ...
  （[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md)）である。したがって本 ADR は `ADR-0023` の**配布層（cert-manager）と設計要件は踏襲し、
  消費側だけが違う**という関係にある。
...
> **範囲の明確化は計画へ環流済み** —— 記録は
> `feedback/20260816_adr-0023-scope-local-route.md`（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260816_adr-0023-scope-local-route.md` へ移設）、
...
[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) の改定と 7 OIDC クライアントの redirect 追記に波及するので #780 と同時に扱う。
...
> **loopback（`127.0.0.1`）へ bind する閉域のローカル開発環境**であり（[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) 決定 4 の公開範囲）、
...
> [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が Supersede した。決定 4 まるごとが死んだわけではない。**
>
> 本決定は 3 つの命題からなる。**生きているもの／覆ったものを命題ごとに書き分ける。**
>
> | # | 決定 4 の命題 | 現状 |
> | --- | --- | --- |
> | P1 | **443 に載る `platform-frontend-edge` へ `spec.tls`（`secretName: edge-tls`）を追加する** | **引き続き有効。** TLS 終端の形（cert-manager の `edge-tls` で終端し、`secretName` を安定させる）は [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) もそのまま踏襲している |
> | P2 | **admin:50000 に載る 7 件へは `spec.tls` を足さない** | **[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が Supersede した。** 7 件すべてへ `spec.tls`（`secretName` は本 ADR が安定させた `edge-tls` のまま）を足し、`--entryPoints.admin.http.tls=true` で entrypoint 側も TLS 終端にした |
> | P3 | **`http` 経路を残す・`--entryPoints.web.http.redirections.*` を足さない** | **[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が Supersede した。** `web`(80) は `websecure`(443) へ恒久リダイレクトする（`NFR-11`「平文 HTTP を残さない」） |
>
> **P2・P3 が覆ったのは、本決定が挙げた「足しても効かない」「7 クライアントの再設定を巻き込む」という
> 理由が解消したからである** —— [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) は entrypoint 側の TLS を同時に入れ、7 クライアントの
...
> 切り替えの局所化）は [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) も守っており、**`secretName` は `edge-tls` のままである**
...
> ① 「admin:50000 の TLS 化は [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) の改定に波及する」 —— **同 ADR にその決定は無い**
> （`50000` も `entrypoint` も `平文` も 0 件。同 ADR は `admin` という**ユーザー**の ADR である）。
> **誤帰属であり、[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) は同 ADR を Supersede していない。**
> ② 「この境界は静的検査で固定する（`admin:50000 の Ingress には spec.tls を足さない`）」 ——
> **その試験は #841 で期待値を反転した**（`#841: admin:50000 は TLS 終端で、そこに載る Ingress は
> spec.tls(edge-tls) を持つ`）。**本文は当時の記録として残す。**

> **［2026-08-17 追記 / 波 11 末クロス監査］上の #834 追記の「経路B に残る平文」の列挙が 1 件足りない。**
>
> 同追記は「**admin:50000 の管理ツール群と、80 の併存**」の 2 つだけを挙げ、
> 「**その解消は #841 が担う**」と書いた。**#841 はその 2 つを解消した**（[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md)）。
> **しかし経路B の平文はまだ尽きていない。**
>
> | 残存平文 | 状態 | 担い手 |
> | --- | --- | --- |
> | admin:50000 の管理ツール 7 件 | **解消済み**（`--entryPoints.admin.http.tls=true` ＋ 7 ルータの `spec.tls`） | #841 / [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) |
> | 80 の平文併存 | **解消済み**（`web`→`websecure` の恒久リダイレクト） | #841 / [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) |
> | **OIDC issuer `http://keycloak:8080`** | **未解消。`NFR-11` の残件として開いている** | **#780**（Keycloak をエッジへ出し issuer を https のエッジ host へ移す） |
>
> **`NFR-11` は対象に「基盤 SPA・BFF・認証基盤（Keycloak）・Wiki.js」を名指ししている。**
> issuer を最小案（`keycloak:8080` 維持）に留めたのは [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) **決定 5** であり、
> **同決定は `Accepted`・live のまま本波でも改定されていない**（本 ADR §[IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) との関係 も
> 「決定 5 は動かさない。それは #780 の射程」と述べている）。
> **エッジを TLS 終端にしても issuer は平文のままである** —— issuer はエッジを経由しない in-cluster の正準名だからである。
>
> **したがって `NFR-11` は経路B について「解消済み」ではない。**
> **後続の監査がこの未達を閉じないこと。** 閉じてよいのは #780 が着地したときである。
> **決定 1〜6 は本追記でも改めない**（`status` は `Accepted` のまま）。
...
[IADR-0105](./IADR-0105_remove-apiserver-oidc-flag-wiring.md) が除去した apiserver OIDC フラグ配線を**復活させない**。
...
## [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) との関係
...
> **［2026-08-17 追記 / 波 11 末クロス監査］上の 2 文は、いま 2 つの意味で実体と合わない。本文は書き換えない。**
>
> ① **引用元がもう存在しない。** `git grep -c '実 TLS 証明書・admin entrypoint の TLS 化' -- deploy/` は **0 件**である
> —— #841 が `deploy/local/edge/README.md` の当該 Tier 記述を書き換えた。
> ② **「実 TLS の側だけ」は偽になった。** 現行の README は「**エッジ TLS は Tier 3 から外れた**（IADR-0206・#779）」
> に続けて「**admin(50000) の TLS 化も Tier 3 から外れた**（[IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md)・#841）」と書いており、
> **admin の側も動いた**。**Tier 3 に残っているのは公的 CA（Let's Encrypt）による証明書だけ**である
> （`*.localhost` にはドメイン所有を検証できないため原理的に発行できない）。
>
> **本 ADR の決定は 1 つも変わらない** —— 変わったのは、本 ADR が引用した**他ファイルの側**である。
> 走査と除外理由は作業仕様書 [`../specs/20260817_wave11-audit-followup.md`](../specs/20260817_wave11-audit-followup.md) が持つ。
...
決定 4（管理ツールは admin:50000 にホスト名ベース）と [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md)（admin entrypoint は平文 http）も**そのまま**である
—— admin:50000 の TLS 化は 7 クライアントの redirect 追記に波及するため、子 2 と同時に扱うのが安全である。

> **［2026-08-17 追記 / 波 11 末クロス監査］直上の `[[IADR-0103]]（admin entrypoint は平文 http）` という帰属は
> 誤りである**（同 ADR は `admin` という**ユーザー**の ADR である）。**詳細は決定 4 の追記**を参照。
> **`admin:50000` はもう平文でもない** —— [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が TLS 終端にした。
> [IADR-0091](./IADR-0091_local-edge-aggregation-traefik.md) 決定 4（ホスト名ベース集約）が「そのまま」であることは**引き続き真**である（変わったのは scheme だけ）。
...
- cert-manager の導入は CRD を伴い、**大 CRD の annotation 262144B 上限**に当たるため
  `kubectl apply --server-side --force-conflicts` を使う（[IADR-0088](./IADR-0088_image-reference-redeploy-safety.md) が ArgoCD で同じ問題を是正した先例）。
