...
- **`integration.yml`（日次）は `--filter "Category=Integration"` に依存する。**
  ...
  そのため **fail-closed の門を `integration.yml` へ置いた** —— 全ユニット合わせて
  1 件も実行されなければ落とす。`dotnet test` はフィルタに 1 件も一致しなくても exit 0 で
  終わるため、門が無ければ「0 件実行のまま緑」で通り続ける。
  **門の正の側（テストが走ったときに検出できること）は実測で確かめた** ——
  run `32496937488` の統合ジョブのログに対し、判定に使う正規表現が
  **34 行に一致**する（`Passed <テスト名> [23 s]` 形式の per-test 行）。
  *注: GitHub の保存ログは行頭に時刻を付けるため、判定と同じ条件で測るには
  時刻を落としてから照合すること（付いたままだと `^` が効かず 0 件に見える）。*
  **この門は 1 度失った** —— 決定 3 の改定 2 で `backend-integration` を畳んだとき、
  ジョブごと消してしまった（AI レビューの指摘で気付いた）。**フィルタに依存する実行を
  書くときは門を併設する**という規則として残す。門の判定はテストランナーの出力書式に
  依存するので、ランナーを替えるときは門も引き直すこと。
