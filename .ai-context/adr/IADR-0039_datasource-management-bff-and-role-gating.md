...
- 関連 ADR: ADR-0004（ABAC 認可モデル）／ [IADR-0009](./IADR-0009_wiki-browsing-404-hides-existence.md)（存在秘匿）／ [IADR-0035](./IADR-0035_frontend-role-based-nav-and-existence-hiding.md)（フロントのロール別ナビ・存在秘匿）／ [IADR-0030](./IADR-0030_operator-role-and-config-viewer-policy.md)（ConfigViewer ロール）
...
1. **管理系 Wave B 画面（SC-06 データソース管理・SC-07 変換ジョブ・SC-05 文書管理）は `platform-admin` もしくは `platform-operator` に限定する。** データソース・変換ジョブ・文書 CRUD はいずれも運用／コンテンツ管理者の職務であり、一般社員（閲覧者）には露出しない。SC-09（管理者設定・ABAC）は計画（Issue #135）の明示により **`platform-admin` のみ**とする（本 IADR の対象外。SC-09 側 IADR で記録）。
   ...
   > 画面のボタンは #503（PR #508。[IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md) 決定 1）、
   > API（`POST /bff/conversion/jobs/{id}/retry`）は #501（[IADR-0128](./IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1）が担った。
   > **照会（`GET /bff/conversion/jobs` 系）は本決定 1 のまま admin/operator で据え置いてある**
   > ——ここで併せて絞ると裁定を待たずに実装が先に答えを出すためである（[IADR-0128](./IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 2）。
   > 据え置きは `GetList_AsOperator_IsAllowed` / `GetById_AsOperator_IsAllowed` で回帰ガードしている。
   > **本 IADR は裁定まで `Accepted` のまま有効**であり、[IADR-0127](./IADR-0127_sc07-retry-admin-only-and-derived-states.md) / [IADR-0128](./IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) は本 IADR を置換しない。
   ...
   >   （[IADR-0128](./IADR-0128_conversion-retry-admin-only-and-downstream-posture.md) 決定 1 と同じ形・[IADR-0044](./IADR-0044_backend-service-authorization-defense-in-depth.md) の多層防御）。`PUT` / `PATCH` は #534 が既に同形で作っている。
   ...
   > **#629 として起票した**（[IADR-0116](./IADR-0116_reimplementation-branching-and-pr-policy.md) 規約 4）。
   >
   ...
   > > `platform-operator` しか持たない（[IADR-0075](./IADR-0075_ast-kb-writer-service-client.md) が最小権限を理由に `platform-admin` を却下している）。
