...
- 関連 ADR: [IADR-0067](./IADR-0067_service-image-build-ci-gate.md)（compose 単一情報源のイメージビルド検証。本件をフォローアップとして切り出した元）／[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)（`k8s-local-images.sh` の追加元。`MAPPING` の出自）
...
`scripts/k8s-local-images.sh`（[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)）は、ローカル k8s へイメージを供給するために
「chart-image（`values.yaml` の `services.<name>.image`）: Dockerfile パス」の対応表を **bash 配列 `MAPPING`** として持つ。
一方 `deploy/docker-compose.yml` の `build` 定義は [IADR-0067](./IADR-0067_service-image-build-ci-gate.md) で **イメージビルド検証の単一情報源**になった。
...
[IADR-0067](./IADR-0067_service-image-build-ci-gate.md) は「独自 YAML パーサで build 定義を抽出」を却下した。理由は**実ビルド**において補間・アンカー・
...
compose を「ビルドの単一情報源」とする [IADR-0067](./IADR-0067_service-image-build-ci-gate.md) の役割分担は変えない（本検査は**対応表の整合のみ**を見る）。

### 4. `ci.yml` に足さず、独立ワークフロー `image-mapping.yml` にする

[IADR-0067](./IADR-0067_service-image-build-ci-gate.md) と同じ理由: `ci.yml` は複数作業が同時に触る共有ファイルで競合しやすい。独立ワークフローで
