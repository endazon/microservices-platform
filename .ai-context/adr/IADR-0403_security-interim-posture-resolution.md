...
（planning#651）が「BFF が中継した利用者の資格情報を後段が自ら検証する」と定め、[IADR-0462](./IADR-0462_conversion-service-validates-relayed-user-credential.md) が `AddPlatformAuth` と 5 口すべての
...
［2026-09-26 追記 / #1520］**本決定は `ConversionService` について [IADR-0462](./IADR-0462_conversion-service-validates-relayed-user-credential.md) 決定 4 が置き換えた。** 本決定の理由
...
- フォローアップ（**いずれも本 PR の外**）:
  ...
  3. east-west gRPC の残り 31 本の展開に伴い、決定 3 の残差 3 件が S へ動くことを確認する。
     ...
     「後段が中継された利用者の資格情報を自ら検証する」に置き換え、[IADR-0462](./IADR-0462_conversion-service-validates-relayed-user-credential.md) がそれを実装した（形は N → R）。
