...
- 直前の実装判断: [IADR-0113](./IADR-0113_report-monthly-zdr-model.md)。同 IADR は残作業として **「live 実原因の確定（稼働環境不触のため未確認）」**
...
- 割当モデル・ルーティング（[IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) / [IADR-0113](./IADR-0113_report-monthly-zdr-model.md)）、リクエスト側パラメータ方針（[IADR-0101](./IADR-0101_default-model-opus-5.md)）、
  終了理由の契約と本文破棄（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md)）は変更しない。
...
一方、現在の割当モデルは [IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) / [IADR-0113](./IADR-0113_report-monthly-zdr-model.md) により
...
無効化不可）である。[IADR-0101](./IADR-0101_default-model-opus-5.md) は `MaxTokens` を思考込みで見積もる対処を入れていたが、
...
> 追随の実装と経緯は [IADR-0022](./IADR-0022_default-opus-and-fable5-copilot-routes.md) / [IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) / [IADR-0113](./IADR-0113_report-monthly-zdr-model.md) の同日追記を参照。
>
> **★ 本 IADR の決定（未知 content ブロックを許可リストで除去する）は影響を受けない。**
> 上の記述が根拠として挙げているのは「**割当モデルはいずれも thinking が既定で有効**」という性質であり、
> **`claude-opus-5` も thinking が既定で有効である** —— 計画 ADR-0025（計画リポ） §結果
...
> および [IADR-0101](./IADR-0101_default-model-opus-5.md)（`ClaudeProvider` の `MaxTokens` 見積り）。
...
**ストリーミング（`/complete/stream`）は壊れていない**ことも実測で確認した。SSE 経路（[IADR-0037](./IADR-0037_llm-sse-streaming.md)）に
...
- SDK に手を入れない・`ClaudeProvider` を変えないことで、ルーティング・機密区分・終了理由・
  メトリクスといった既存の決定（[IADR-0104](./IADR-0104_llm-stop-reason-refusal.md) / [IADR-0109](./IADR-0109_openai-finish-reason-normalization.md) / [IADR-0110](./IADR-0110_llm-completion-stop-reason-metrics.md) / [IADR-0112](./IADR-0112_report-kind-purposes-and-trade-decision-sonnet-5.md) /
  [IADR-0113](./IADR-0113_report-monthly-zdr-model.md)）に一切干渉しない。
