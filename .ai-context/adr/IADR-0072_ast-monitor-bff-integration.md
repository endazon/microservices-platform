...
# IADR-0072: AST 監視銘柄（AST/SC-02 watchlist）の MSP 組み込み
...
- 関連 ADR: [IADR-0056](./IADR-0056_repo-unit-structure-platform-knowledge.md)（ユニット構成）／[IADR-0057](./IADR-0057_unit-dependency-machine-check.md)（一方向依存）／[IADR-0063](./IADR-0063_bff-unit-endpoint-composition.md)（BFF 合成点・例外3）／
  [IADR-0068](./IADR-0068_image-mapping-drift-check.md)（image-mapping ドリフト検査）／[IADR-0070](./IADR-0070_ast-frontend-integration.md)（AST フロント第1スライス AST/SC-01）／
  [IADR-0071](./IADR-0071_ast-risk-controls-bff-integration.md)（先行・AST/SC-02/03 risk-controls）／
...
#289（IADR-0071）で AST の AST/SC-02（リスク設定）/ AST/SC-03（統制状態参照）を `/bff/risk-controls/*` 経由で MSP SPA へ
載せた。その後 AST develop で AST/SC-02（`settings/risk`）に **監視銘柄（watchlist）変更 UI**（AST#196/PR #197・
...
1. **どの `/monitor/*` 経路を BFF に登録するか**。MarketMonitorService は `/monitor` 配下に `settings`(GET/PUT)・
   `watchlist`(GET/POST/DELETE)・`watchlist/history`(GET) を持つが、AST/SC-02 の watchlist UI が実際に叩くのは一部である。
...
> **更新（2026-07-19・#286 / [IADR-0073](./IADR-0073_ast-unit-owned-bff-migration.md)）**: 本 interim は #286 で解消済み。`MonitorBffEndpoints` は
> AST 側 unit-owned Bff プロジェクト `AiStockTrading.Bff.Endpoints`（AST PR）へ挙動不変で移設され、合成点は
> 例外3 で参照する。

### 2. BFF は AST/SC-02 watchlist UI が実消費する 4 経路のみを登録する（未使用経路は登録しない）
...
| --- | --- | --- | --- | --- |
| `/bff/monitor/watchlist` | GET | `/monitor/watchlist` | なし | AST/SC-02 |
| `/bff/monitor/watchlist` | POST | `/monitor/watchlist` | あり（symbol/market/reason） | AST/SC-02 |
| `/bff/monitor/watchlist` | DELETE | `/monitor/watchlist` | **あり**（symbol/market/reason） | AST/SC-02 |
| `/bff/monitor/watchlist/history` | GET | `/monitor/watchlist/history` | なし | AST/SC-02 |
