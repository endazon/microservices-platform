...
- 関連 ADR: [IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)（private submodule の CI 取得はトークン付き）／[IADR-0060](./IADR-0060_submodule-unit-operations.md)（submodule 運用・
...
[IADR-0060](./IADR-0060_submodule-unit-operations.md) は、CI が `src/*/backend/backend.slnx` を自動発見してユニットをビルド／テストする一方、
**submodule は既定の `actions/checkout` では取得されず、未取得ユニットは glob に現れずビルド対象外になる**
ため「取得の有効化が組み込みの前提」と定めた。[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)（planning submodule）は private を前提に、
...
検査は夜間トークン付き `doc-links-planning.yml` に分離していた = [IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md)）。したがって「public ユニット
だから recursive で足りる」とはならず、**planning を巻き込まない取得方法**が要る。

## 検討した選択肢

1. **private 前提を踏襲し、public ユニットにも PAT（`UNIT_REPO_TOKEN`）を要求する**: [IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) と一様に
...
2. **取得しないジョブ**: `doc-links` は従来どおり submodule 未取得（planning のリンク検査は夜間トークン付き
   ...
   ユニット実体を要しないため付与しない。
3. **private ユニットへの拡張点**: 将来 private な追加ユニットを組み込む場合は、[IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) 型の
...
- **最小権限・最小運用**: public ユニットに不要なトークンを課さず、secret 露出面を増やさない。ユニット追加は
  checkout 1 行の変更で済み、[IADR-0060](./IADR-0060_submodule-unit-operations.md) の「ゼロ編集に近い組み込み」方針と整合する。
- **段階的拡張**: private が必要になった時点で [IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) の確立様式へ拡張でき、既存の private 経路
  （planning）とも矛盾しない。
...
- `docs/how-to/adding-a-unit-submodule.md`: public ユニットはトークン不要（`src/*` を非再帰 init）、
  private ユニットは [IADR-0058](./IADR-0058_doc-links-planning-submodule-ci.md) 型トークンを与える、の分岐と planning を巻き込まない理由を追記。
...
- Supersedes: [IADR-0060](./IADR-0060_submodule-unit-operations.md) の**決定②（追加ユニットの CI submodule 取得方式）のみを改定**する
