...
- 関連 ADR: ADR-0006（運用基盤）。経路B k8s dev 統括 [IADR-0066](./IADR-0066_local-k8s-dev-environment.md)、prod エッジ（Istio）ルーティング設計
  [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md)、opt-in オーバーレイの流儀 [IADR-0077](./IADR-0077_local-observability-vault-gitops-overlays.md)、ゲート横断 smoke test [IADR-0087](./IADR-0087_k8s-local-up-optin-smoke-test.md)。
...
> **［2026-08-16 追記 / #779］本決定 3 の証明書に関する部分は [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) が Supersede した。**
...
> **決定 4・決定 5 と [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md)（admin:50000 は平文 http）は動いていない。**
>
> **［2026-08-17 追記 / #841］直上の 1 文を 2 点で是正する（元の記述は消さずに残す）。**
> ① **`admin:50000` が平文であることの根拠を [IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) に帰していたのは誤りである** ——
> 同 ADR に `50000` も `entrypoint` も `平文` も 1 件も無く、扱っているのは **`admin` という「ユーザー」**
> （realm への恒久定義・ツール別 claim 設計）であって **entrypoint ではない**。
> ② **`admin:50000` はもう平文ではない** —— [IADR-0220](./IADR-0220_admin-entrypoint-tls-and-http-redirect.md) が TLS 終端にした
...
- 80/443 はホスト権限・既存サービスとの衝突・443 自己署名の制約あり（README 明記。**［2026-08-16 追記 / #779］
  自己署名の制約は [IADR-0206](./IADR-0206_local-edge-tls-cert-manager.md) が解いた** —— cert-manager の CA 発行証明書へ移行済み）。占有時は LOCALEDGE を使わず
