...
- 先例: [IADR-0061](./IADR-0061_deploy-rename-migration.md)（`knowledge-platform` → `microservices-platform` の改名。#228）
...
着手時は [IADR-0061](./IADR-0061_deploy-rename-migration.md) の先例に倣って `platform-realm.json` へ改めたが、**`check-doc-links` が破損リンク 10 件を検出して差し戻した** ——
...
（[IADR-0061](./IADR-0061_deploy-rename-migration.md) のときは確定済み仕様書が realm ファイルへリンクを張っていなかったため、この衝突は起きなかった。
...
**先例を実測して決めた** —— [IADR-0061](./IADR-0061_deploy-rename-migration.md) の改名（`knowledge-platform` → `microservices-platform`）の後、旧名は
...
ADR 本体を書き換えない理由はもう 1 つある。[IADR-0084](./IADR-0084_headlamp-oidc-apiserver-flags.md) / [IADR-0086](./IADR-0086_oidc-issuer-metadata-split.md) / [IADR-0090](./IADR-0090_grafana-keycloak-oidc-generic-oauth.md) / [IADR-0092](./IADR-0092_argocd-keycloak-oidc.md) / [IADR-0093](./IADR-0093_minio-keycloak-oidc.md) の
realm URL は**その ADR が決定した内容そのもの**であり（「issuer は in-cluster 正準名 … を用いる」）、書き換えると
**「この ADR は改名後の値を決定した」という偽の主張**になる。[IADR-0061](./IADR-0061_deploy-rename-migration.md) に至っては**前回の改名の記録そのもの**である。
...
12 箇所へ日付つき追記ブロックを複写しない（[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md)。#733 で撤去した型を作り直さない）。
...
| 対象 | 理由 |
| --- | --- |
| **`smtpServer`** | 実環境の接続値が要る（利用者裁定 2026-08-15「実環境が要るものは触らない」）。**足りないもの 3 点は [SC-15 画面仕様書](../../docs/screens/SC-15_password-reset.md) に明記した** |
...
- **リスク（こちらの方が確度が高い）**: **dev ユーザーのパスワードを以後変更できなくなる。**
  ...
  **これらのユーザーのパスワード変更が弾かれる**。[IADR-0103](./IADR-0103_local-sso-persistence-and-claim-design.md) 決定 1（`admin` ユーザーの恒久化）と
  [`docs/operations/local-sso-recovery-runbook.md`](../../docs/operations/local-sso-recovery-runbook.md) の復旧手順に直接効く。
...
0. **AST（`src/ai-stock-trading`）側の消費者設定を追随させる。** 本リポジトリの母集合は
   ...
   submodule のため本リポジトリからは是正できない**（[IADR-0120](./IADR-0120_excluded-units-from-gitmodules.md)）。しかし `AST/IADR-0093`（KB writer の
   クロスレルム s2s）により、**AST は MSP のレルムを Authority として消費する**。改名の影響が AST 側へ及ぶ。

   > **`IADR-0093` は名前空間が衝突している。** 本リポジトリの [IADR-0093](./IADR-0093_minio-keycloak-oidc.md) は MinIO の OIDC 連携であり、
...
- Supersedes: なし
- Superseded by: なし
- 先例: [IADR-0061](./IADR-0061_deploy-rename-migration.md)（前回の改名。母集合の取り方をここから実測した）
- 作業仕様書: [`20260815_issue-578_realm-rename-and-auth-policy.md`](../specs/20260815_issue-578_realm-rename-and-auth-policy.md)
- 画面仕様書: [SC-14](../../docs/screens/SC-14_otp-mfa.md) ／ [SC-15](../../docs/screens/SC-15_password-reset.md)
- テスト仕様書: [SC-14](../../docs/tests/SC-14_otp-mfa.md) ／ [SC-15](../../docs/tests/SC-15_password-reset.md)
