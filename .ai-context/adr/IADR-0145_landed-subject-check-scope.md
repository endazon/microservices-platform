...
- 関連する実装 ADR: [IADR-0140](./IADR-0140_cross-repo-issue-ref-checker.md)（CI 結線方式）／[IADR-0141](./IADR-0141_audit-rounds-and-population-drawing.md)（母集合）／[IADR-0144](./IADR-0144_adr-numbering-check-scope.md)（姉妹検査器の作法・ラチェット）／[IADR-0115](./IADR-0115_impl-handoff-kit-as-single-source.md)（`scripts.test.js` は分類 A で変更不可）
...
`validateIdExistence` へ通す。CI は [IADR-0140](./IADR-0140_cross-repo-issue-ref-checker.md) 決定 2 と同じ結線
...
> 着地件名 319 件（**走査基準: develop `2cd8508` 時点**）へ「**変更パス由来の FR/UC/SC ID がスコープに無い**」を当てた実測:
>
> ```
...
**件数は条文にも本 ADR にも書かない**（[IADR-0144](./IADR-0144_adr-numbering-check-scope.md) 決定 2 と同じ理由）。正は baseline ファイルである。
...
- `.github/workflows/` は編集していない（GitHub App 権限で編集不可。[IADR-0140](./IADR-0140_cross-repo-issue-ref-checker.md) 決定 2）。
...
| 着地件名の総数を baseline に焼く | PR がマージされるたびに動き、無関係な PR を赤にする手作業の更新点になる（[IADR-0144](./IADR-0144_adr-numbering-check-scope.md) 決定 2） |
| **変更パス由来の FR/UC/SC ID がスコープに在るかを見る** | **実測すると偽陽性 25/26**（着地件名 319 件で 26 ヒット・実際の事故は 1 件。**走査基準: develop `2cd8508` 時点**）。pin 更新や横断的な是正 PR が触ったファイル名の ID を大量に拾う。**方向が合わない**。ただし**「検出できない」のではない** —— 決定 3 の追記のとおり |
| ID 脱落を検出するため PR タイトルを API で引く | 検査がネットワークと権限に依存し、`scripts/` の「外部依存ゼロ」を崩す。**できないことを明記する方を選んだ**（決定 3。[IADR-0144](./IADR-0144_adr-numbering-check-scope.md) 決定 3 と同じ判断） |
