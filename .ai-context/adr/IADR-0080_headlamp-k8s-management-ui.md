...
- 関連 ADR: ADR-0008（計画リポ）（実行基盤 = k3s）／[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)（ローカル k8s dev 環境・`deploy/local/` の dev 専用資産・`developer` ユーザー・opt-in オーバーレイの器）／[IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md)（ブラウザ OIDC issuer 統一の手順A＝hosts＋port-forward で browser/cluster が同一 issuer を共有）
...
- 計画フィードバック: `feedback/20260719_headlamp-k8s-management-ui.md`（環流記録。計画リポ `projects/microservices-platform/10_feedback/20260719_headlamp-k8s-management-ui.md` へ移設）

## 背景・課題

ローカル k8s dev 環境（[IADR-0066](./IADR-0066_local-k8s-dev-environment.md)）はクラスタ状態把握の GUI を持たず、操作は `kubectl`/`port-forward` に
...
ブラウザと cluster が同一の issuer `http://keycloak:8080` を共有するよう、[IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) の**手順A**（hosts に
...
- これは [IADR-0076](./IADR-0076_edge-bff-routing-and-oidc-hostname.md) が SPA/`/bff` 向けに確立した機構を **Headlamp UI にも適用**するもので、既存 dev の issuer 設計を
