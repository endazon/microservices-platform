...
fail-open は緑のまま通る。**本物の解決器に `HttpMessageHandler` をスタブする単体試験を新設した**
（陰性対照・陽性対照・縮退・経路の固定。**本数は書かない —— 増えるたびに腐る導出値である**）。
...
- 良い影響:
  - **潜在 fail-open が塞がった。** `confidentiality` フィルタを持たない登録者は `restricted` を
    配れない（陰性対照 3 本）。
  - **変異試験で守られていることを実測した。** 旧実装へ戻すと**陰性対照 3 本を含む 5 本が落ち、
    陽性対照はすべて緑のまま通る**（＝「常に空集合を返す」実装で通る試験ではない）。
