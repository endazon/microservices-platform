...
- 関連する実装仕様書:
  ...
  [テスト戦略](../../docs/tests/TEST_STRATEGY.md)（ゲート一覧・検査対象ユニットの切り分け・床の置き方）
...
2. **床の値は実測からの整数切り下げとする。初期値は `line 34` / `branch 17`。**
   ...
   > **［2026-08-15 追記 / #574］床の値を再び置き直した（`line 33` → `39`。`branch 17` → `27`）。**
   > [#574](https://github.com/endazon/microservices-platform/issues/574) /
   > [IADR-0195](IADR-0195_coverage-exclude-source-generator-output.md) が
   > **source generator の出力**（`obj/` 配下。`OpenApiXmlCommentSupport.generated.cs` /
   > `RegexGenerator.g.cs`）も集計から落とし、新しい定義での実測に合わせて置き直した。
   > **これも ratchet の引き上げではなく、測定基準の変更に伴う置き直しである。**
   > **旧定義の 33 / 17 と新定義の 39 / 27 は分母・分子が違うため直接比較できない。**
   > 値が**上がった**のは、source generator の出力が平均より**薄く**被覆されているためである
   > （行 11.2% / 分岐 6.3% に対し旧定義の全体は行 35.14% / 分岐 19.30%）。EF のときは逆に下がった——
   > **どちらも分子・分母から同じものを抜いた結果であり、品質の変化を意味しない。**
   > **上の［2026-08-07 追記］の「branch を据え置くのは生成コードの分岐が 0 だから」は EF の生成コード
   > についての記述であり、source generator の出力には当たらない**（そちらは分岐 3970 を持つ）。
   > **`branch` に切り下げの `28` を採らなかった**——実測 `28.01%` に対し床 `28` は余裕が `0.01pt` しかなく、
   > **被覆分岐 1 本の喪失で割れる**ため床として機能しない。本決定の「実測からの整数切り下げ」は、
   > **切り下げが機能する床を与えないときに限り 1 つ下の整数を採る**（IADR-0195 決定 3）。
   > **今回の床は導出値ではなく実測値である**——#571 のとき読めなかった CI 相当の実測を、
   > 本作業ではローカルで完走できた（SDK を導入し `dockerd` を起動して統合テストを **43/43** 通した）。
   ...
   > **［2026-08-22 追記 / #900］床の値を 4 度目の置き直しで `line 38` → `88` / `branch 27` → `68` にした。**
   > 集計がレポートを跨いで行を重複排除するようになった（[IADR-0235](IADR-0235_coverage-cross-report-line-dedup.md)）。
   > **これも ratchet の引き上げではなく測定定義の変更に伴う置き直しである**——直前の追記が扱った
   > 「テストプロジェクトを増やすと分母が水増しされる」機構そのものを塞いだ結果であり、
   > **被覆の実態は 1 行も変わっていない**（旧定義で測れば `line 38.5%` で #899 の記録を再現する）。
   > 分母は 25896 → 7292（水増し 18604 行が取れた）。
   > CI 実測（run 32540071304 / `workflow_dispatch` / commit `8aecd25` / Release / レポート 16 件）:
   > `line 88.99%（6489/7292）` / `branch 68.66%（1433/2087）`。切り下げて 88 / 68。
   > 🔴 **分母が約 3.5 分の 1 になったので、被覆 1 行あたりの率の動きが約 3.5 倍になった。**
   > 床 88 の耐性は被覆行 72 行、床 68 は被覆分岐 13 本である（#899 の 127 行、#574 の 220 行 / 60 本より狭い）。
...
3. **運用は ratchet とする**（床は上げるが下げない）。テストを増やしたら床を引き上げ、床を割る変更を
   ...
   [テスト戦略](../../docs/tests/TEST_STRATEGY.md)「各ドメイン issue が守ること」に置く。
...
- 悪い影響・トレードオフ:
  - **床の絶対水準は低い**（起票時 line 34 / branch 17。**現在値は
    [`src/coverage-floor.json`](../../src/coverage-floor.json) を正とする**——#571 と #574 で
    測定基準が 2 度変わっており、ここへ現在値を書くと腐る。注記は #574）。品質の水準を保証するもの
    ではなく、あくまで**回帰防止の床**である。各ドメイン issue でテストと床を段階的に引き上げる前提に立つ。
  ...
  - **分岐（branch）の分母は `condition-coverage` の合算**であり、coverlet の `branches-valid` とは
    定義が異なる（CI 実測: 8948 対 9356）。分岐の床（起票時 17。**現在値は
    [`src/coverage-floor.json`](../../src/coverage-floor.json) を正とする**。注記は #574）は
    この方式での実測に基づくため、**分岐の定義を変える
