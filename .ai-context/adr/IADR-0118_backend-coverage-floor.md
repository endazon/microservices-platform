...
2. **床の値は実測からの整数切り下げとする。初期値は `line 34` / `branch 17`。**
   ...
   > 🔴 **［2026-08-29 追記 / #901］例外の発動条件を「被覆単位に対する比」で定義する。**
   > 従来の文言は「**切り下げが機能する床を与えないときに限り 1 つ下の整数を採る**」であり、
   > 例示は `#574` の**耐性 0 本**（被覆分岐 1 本の喪失で割れる）だけだった。
   > **「薄いが 0 ではない」場面をどちらへ倒すかが書かれていない。** 実際 `#900` は耐性 13 本で
   > 「例外を発動する水準ではない」と判断し、`#901` は耐性 4 本で発動させた —— **同じ条文から
   > 逆の結論が出せる状態**であり、次に判断する者によって割れる。
   >
   > **条件を数で閉じる: 切り下げ値の耐性が「被覆単位の 0.5% 未満」なら 1 つ下を採る。**
   > 耐性＝床を割るまでに失える被覆行数・被覆分岐数。分母は**その時点の被覆数**である
   > （🔴 **pt で比べない** —— 分母が変わると pt の意味が変わる。同 JSON の `#900` 追記が
   > 既にそう戒めている）。
   >
   > **この閾値は過去 4 例をすべて再現する**（後付けの独自基準ではないことの担保）:
   >
   > | 判断 | 切り下げ値の耐性 | 被覆単位比 | 実際の結論 | 本条文の判定 |
   > | --- | --- | --- | --- | --- |
   > | #574 `branch 28` | 0 本 / 1663 | 0% | 27 を採用 | 発動 ✅ |
   > | #900 `line 88` | 72 行 / 6489 | 1.11% | 88 を採用 | 非発動 ✅ |
   > | #900 `branch 68` | 13 本 / 1433 | 0.91% | 68 を採用 | 非発動 ✅ |
   > | #901 `line 91` | 10 行 / 12683 | 0.079% | **90 を採用** | 発動 ✅ |
   > | #901 `branch 76` | 4 本 / 2942 | 0.136% | **75 を採用** | 発動 ✅ |
   >
   > **`#901` の判断はこの条文の追加によって初めて規約に根拠を持つ。** それまでは
   > 「保守側だから実害が無い」だけで、**条文の文言とは整合していなかった**（AI レビューが指摘）。
   > 方向が安全なら逸脱してよい、とはしない —— **判断が割れる余地を残すこと自体が欠陥**である。
