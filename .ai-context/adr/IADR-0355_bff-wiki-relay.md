...
［2026-09-03 追記 / #1200］**§結果「残るのは画面である」「画面は依然として委譲先への外部リンク 1 本」は真でなくなった。**
SC-04 の画面が [[IADR-0365]] で入り、ページツリー・本文・検索を本 IADR の 4 経路経由で SPA が描く。
ADR-0073 §残るもの「SC-04 の画面実装が入るまで、本番で Wiki 閲覧ができない」は解けた。
決定 1〜7 は変えていない（画面は透過された 404 / 502 / 200 ＋ 空をそのままの意味で描く）。
