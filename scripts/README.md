...
| `static-checks` | `check-doc-links.js`（相対リンクの実在）／`check-trace-blocks.js`（trace ブロック規約）／`check-trace-followthrough.js`（記録と文書の追随。warn）／`gen-knowledge-graph.js --check`（参照の in-repo 実在）／`check-reading-budget.js`／`check-nul-bytes.js`（#956 / IADR-0247。追跡下ファイルへの生の NUL バイト混入検査。専用ステップは `scripts-tests` の companion 経由の実行とは別で、失敗をジョブ名で見せる） |
