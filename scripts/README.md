...
| `scripts-tests`（再掲 4） | `check-xunit1051-ratchet.js` の `--self-test`・**実データの本走**・実バイナリでの検出力確認（baseline と許可リストをずらすと `props-desync`）。**新ジョブは作っていない**（`.github/workflows/ci.yml` は 1 行も変更していない。IADR-0140 決定 2 の相乗り）。#882 / IADR-0237 |
