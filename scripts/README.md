...
| `static-checks` | `check-doc-links.js`（相対リンクの実在）／`check-trace-blocks.js`（trace ブロック規約）／`check-trace-followthrough.js`（記録と文書の追随。warn）／`gen-knowledge-graph.js --check`（参照の in-repo 実在）／`check-reading-budget.js` |
