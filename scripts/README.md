...
| `check-cross-repo-refs.js` | 他リポジトリ issue / PR 番号の修飾を検査（#507 / IADR-0140）。**型 1**: リポジトリ名の裸書き（`project-planning#NNN` / `ai-stock-trading#NNN`。短縮形でもフルパス形式でもない第 3 の表記）。**型 2**: 修飾付き参照の**直後**に続く裸の `#NNN`（`planning#NNN / #MMM`。後続が**本リポジトリの実在 issue へ誤リンクする**）。対象は追跡下の `*.md` で、**インラインコード／コードフェンスの中は見ない**（GitHub はそこで自動リンクせず害が無い。規約自身の反例を書けなくしないため）。`check-commit-messages.js` からも呼ばれ、コミット件名・本文・PR タイトルを同じ規則で検査する。`--self-test` を持つ | 標準出力（レポート） |
