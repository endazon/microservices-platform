...
| `check-test-name-references.js` | **コード注記が指す試験クラス名が実在すること**を検査する（#1312）。「この帰結は `<X>Tests` が固定する」と書いてあるのに、その名前のクラスが 1 つも無い形は、**「試験がある」と読ませたまま何も固定していない** —— 読み手は名前で検索して見つからず「名前が変わったのだろう」と推測して先へ進むため、**主張は検証されないまま残る**。同型の事故は 7 回（#1311 で 1 件 → #1312 起票時に 5 件 → 着手時の引き直しで 7 件）。走査は `git ls-files` の `src/**` の C# で、**コメント部分だけ**から `*Tests` を集めて型宣言（`class` / `record` / `struct` / `interface`）と突き合わせる。**行コメントとブロックコメントの両方を見る**（片方だけだと、同じ主張をもう一方で書くだけで検査を逃れられる）。**文字列リテラル内は拾わない** —— 拾うと試験名を配列で持つコードが軒並み誤検出になる。リテラル内のスラッシュ 2 つ（URL 等）をコメント開始と誤らないよう、**先にリテラルを伏せてから**探す。0 件走査は fail（`MIN_SCANNED`）。除外は `ALLOWED`（**全項目に理由が要る**）で、**アセンブリ名 `IntegrationTests` / グロブ表記 `EndpointTests` / 「その名前の試験は存在しない」と述べている当の記述 2 件**を持つ。🔴 **機械が言えるのは「名前が実在するか」までである** —— 「指し先は実在するが、その帰結を固定していない」側は人が読むしかない（射程を広げると誤検出だらけになり、検査器ごと無視されるようになる）。`--self-test` を持つ | 標準出力（レポート） |
