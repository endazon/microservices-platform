...
| `check-default-credentials.js` | **既定資格情報の再混入**を止める前方一方向のラチェット（#1012 / IADR-0286）。イメージへ焼かれる構成（`appsettings.json`。`.Development.json` は除く）と合成ルート（`Program.cs`）を走査し、**`Username=` / `Password=` を伴う接続文字列**と**資格情報つき `amqp://user:pass@`** を落とす。ホストと DB だけの値（`Host=postgres;Database=x`）は資格情報ではないので落とさない —— **「秘密を書かせない」検査であって「設定を書かせない」検査ではない**。既知の残件（RabbitMQ 13 箇所。撤去にはブローカ側の資格情報変更を伴う配備作業が要るため別 issue）は `default-credentials-baseline.json` に凍結し、**増やせないが減らすのは自由**（baseline に在るのに実在しない行も落として `--update` を促す）。fail-closed: 走査対象 0 件は緑にしない。`--self-test`（6 件）／`--update` を持つ | 標準出力（レポート） |
