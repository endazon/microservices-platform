...
| `static-checks`（再掲） | `check-realm-constraints.js --self-test` と本検査（#18 / #307 / #385 / #438 / #1115 / #1144。#1115 は**サーバ間の口**（`backchannel.logout.url`）に pod から到達し得ない host（裸の `localhost` / ループバック / `*.localhost`）を書いていないかを見る。**ブラウザ向けの URL 群は対象外**——あちらは裸の `localhost` が正しい **#1144 で「dev 既定の送出先が捕捉用 MTA であること」の門が加わった** —— realm の静的既定と Vault seed の既定の**両方**を見る。片方だけ直しても、もう片方から外へ出る） |
