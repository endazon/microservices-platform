...
module.exports = ({ ok, assert }) => {
  ...
  // --- measure-cutover-inventory: 切替（破棄と再構築）の実在量と作り直しの検証（NFR-05・issue #457・IADR-0459） ---
  //
...
${r.stderr}`);
  ...
  //   必須にしないことで正しく運用されている。機械的に禁じると正当な設定を壊す。
  {
    ...
    ok('#705 / #936: blocked 判定が能力と規則を書き分け、最後に測った時点を持つ', () => {
      const t = fs.readFileSync(path.join(REPO, DOC), 'utf8');
      assert.match(t, /能力の不在/, '「能力の不在」の区分が消えた');
      assert.match(t, /規則による禁止/, '「規則による禁止」の区分が消えた');
      // ★ #936: **日付を固定しない。** 旧版は `最後に測った時点: 2026-08-11` を直に照合していたが、
      //   同節自身が「棚卸しのたびに測り直す」と定めている以上、測り直すたびに本テストの編集が要る形は
      //   規範と衝突する（実際 #936 の再測定で落ちた）。**日付が在ることだけを固定する。**
      assert.match(t, /最後の?測定?[:：]?\s*\d{4}-\d{2}-\d{2}|最後に測った時点[:：]\s*\d{4}-\d{2}-\d{2}/,
        '最後に測った時点（日付）が消えた');
      assert.match(t, /棚卸しのたびに測り直す/, '再測定の規範が消えた');
      assert.match(t, /ToolSearch/, '再測定の手順（MCP ツールの確認）が消えた');
    });

    ok('#936: 手順書が「推奨」ではなく「現在の設定」を述べ、外した推奨に理由を添えている', () => {
      const t = fs.readFileSync(path.join(REPO, DOC), 'utf8');
      // ★ #936 で配備した。旧版は「推奨設定であって現在そうなっているではない」「配備されるまでの
      //   暫定手段」を固定していたが、**どちらも配備によって偽になった**。同じ読み分け
      //   （定めたのか／働いているのか）は保ったまま、固定する側を現状へ移す。
      assert.match(t, /現在の設定/, '「現在の設定」である旨が消えた');
      assert.match(t, /enforce_admins/, 'enforce_admins の設定値が消えた');
      // 表の推奨（Code Owners レビュー必須）から意図的に外した点は、理由つきで残す
      // —— 理由が消えると「設定し忘れ」と読めてしまう。
      assert.match(t, /意図的に外れる|意図的に外した/, '推奨から外した旨が消えた');
      assert.match(t, /required_pull_request_reviews/, '外した設定キーの明示が消えた');
      // 必須にしたことで受け入れたリスクを書いてある（claude-review の実行基盤依存）。
      assert.match(t, /マージ不能/, '必須化で受け入れたリスクの記述が消えた');
      // API 経路（UI だけにしない）。
    ...
    ok('#683: クラス分類が実挙動の git 使用と一致する（両方向）', () => {
      ...
      try {
        ...
        const NOT_CHECKERS = [
          ...
          // #1359: タグ辞書の初期投入器。上の 2 つと同じ「投入」であって「検査」ではない。
  ...
  //   経緯を別紙へ出し、機構の説明を圧縮した分で相殺している。
  {
    ...
    ok('#688: 採番の経緯が別紙へ移り、入口から消えている', () => {
      ...
      assert.match(a, /稼働する製品の要件/, '無採番の根拠（28 件が製品の要件）が別紙に無い');
    });
  }
  ...
  //   本件は CI の起動条件という**工程の統制**であり、計画側の非機能要件表（`NFR-01`〜`NFR-28`）に
  ...
  //   可否には一切触れない。
  {
    ...
    //   統制であり、計画側の非機能要件表〔`NFR-01`〜`NFR-28`〕に当たる番号が無い。IADR-0179 決定 1）。
    //
  ...
  //    JS 側の純粋関数へ出し、**変異させて落ちることを確かめる**。
  {
    ...
    ok('#1163: 母集合はブラウザ OIDC を持つ 6 クライアントである', () => {
      ...
      // #1499 / [[IADR-0461]] 決定 5 で minio（MinIO Console）を realm ごと撤去し 6 件になった。
