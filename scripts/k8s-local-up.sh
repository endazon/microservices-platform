...
#   OBJECT_STORAGE_ACCESS_KEY / OBJECT_STORAGE_SECRET_KEY（IADR-0464。旧 MINIO_*）/ WIKIJS_DB_PASSWORD / WIKIJS_SYNC_APIKEY / ANTHROPIC_API_KEY /
...
# ［IADR-0464 決定 5 / #1499］MinIO Console の OIDC client secret（minio-oidc。IADR-0093）の作成はここにあったが
...
# IADR-0464 決定 3 (#1499): オブジェクトストレージ（SeaweedFS）の S3 資格情報。旧名 minio-credentials から製品名を外した
...
if [ "${ESO:-}" = "1" ]; then
  ...
  # IADR-0098 (#310) PR-3: OIDC client secret 群。grafana/vault/headlamp-oidc は platform-infra ns（MSP ns の minio-oidc は IADR-0464 で撤去）。
  ...
  # 誤解させない）。MSP ns は常時 17 本（#1022 で rabbitmq-app、#1107 で bff-oidc、#1101 で identity-admin-oidc、#1290 で retrieval-service-token / ingestion-service-token、#1255 の第 2 スライスで aianalysis / graph / conversion の 3 本、第 3 スライスで wiki / datasource / mcp-server の 3 本、通知の面（IADR-0419）で document-service-token の 1 本を追加し 6 → 7 → 8 → 9 → 11 → 14 → 17 → 18 へ、IADR-0464（#1499）で minio-oidc を撤去し 17 へ数え直した。**値は上の msp_es を数え直して出す** —— 継ぎ足すと必ずずれる）＋有効ゲートの wikijs-oidc（#1127）と synthetic-monitor-oidc（#1287）。infra ns は基盤 3 本＋vault-oidc/keycloak-smtp 常時（#1102 で keycloak-smtp を追加し 4 → 5、#1245 で reset-gate-oidc を追加し 5 → 6 へ数え直した）＋有効ゲートの grafana/headlamp-oidc。
  ...
  # 「空」または「旧値」の env を保持し続ける。実害として MinIO（当時。IADR-0464 で撤去）=`unauthorized_client / Invalid client credentials`
  ...
  #      seaweedfs         : object-storage-credentials（S3 の管理者資格情報。IADR-0464）
...
if [ "${LOCALEDGE:-}" = "1" ]; then
  ...
  #   mesh 内の 3 Service（frontend / bff / wiki-js。MinIO Console は IADR-0464 で撤去）へ平文で入っている。名前空間全体へ
