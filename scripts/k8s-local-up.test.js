...
// IADR-0461 決定 5 (#1499): MinIO Console の SSO（IADR-0093）は撤去した。`minio-oidc` を作り直す回帰を止める
...
// IADR-0097 (#310) PR-2: ESO=1 で object-storage-credentials（IADR-0461。旧 minio-credentials）/wikijs-db/wikijs-sync も
...
// IADR-0098 (#310) PR-3: ESO=1 で OIDC client secret 群（grafana/vault/headlamp-oidc。minio-oidc は IADR-0461 で撤去）も
...
// grafana/vault/headlamp-oidc は各ゲート有効時（minio-oidc は IADR-0461 で撤去）。ゲートを全て有効化して手動 apply の存置を確認する。
...
ok('ESO=1: 供給後に ESO 管理 secret を参照する Deployment を網羅的に rollout restart する', () => {
  const res = runUp({ VAULT: '1', ESO: '1' });
  // seaweedfs=object-storage-credentials（IADR-0461）, llmgateway-service=llm-provider-credentials,
...
ok('realm.json: admin ユーザーとツール別 claim 設計が恒久化されている', () => {
  ...
  // IADR-0461 決定 5 (#1499): MinIO Console の SSO（IADR-0093）を撤去したので、minio client・client ロール
...
// IADR-0461 決定 5 (#1499): admin-ingress-minio.yaml（MinIO Console）は撤去した。
...
ok('#841: admin:50000 は TLS 終端で、そこに載る Ingress は spec.tls(edge-tls) を持つ', () => {
  ...
  // 管理ツール 6 件（grafana / headlamp / vault / qdrant / wiki / argocd）。MinIO Console は IADR-0461 で撤去し 7 → 6。
...
// #1435: 上の検査の母集合は kustomize の overlay だけで、**helm チャートの Deployment が入っていなかった**。
// wiki-js（PVC wiki-js-data を掴む）が RollingUpdate のまま残り、新旧 2 つの Pod が空の DB へ同時に
// migration を走らせて knex のロック表に行が 2 つでき、以後どの Pod も起動しなくなった
// （integration-stack の run 36188276324 / 35277541043 / 34748189616）。
// チャートの全テンプレートを読み、PVC をマウントする Deployment の文書すべてに Recreate を求める。
// 対象を名前で列挙しない —— 新しく PVC を掴むテンプレートが増えても、この検査が自動で拾う。
ok('#1435: helm チャートでも PVC を掴む Deployment は Recreate（IADR-0210 決定 7 の母集合を広げる）', () => {
  ...
  for (const name of fs.readdirSync(tplDir).filter((f) => /\.ya?ml$/.test(f)).sort()) {
    for (const doc of readAt(tplDir, name).split(/^---\s*$/m)) {
      ...
      if (!/^\s*persistentVolumeClaim:\s*$/m.test(doc)) continue;
      const deploy = (/^metadata:\s*\n\s+name:\s*(\S+)/m.exec(doc) || [])[1] || name;
      holders.push(deploy);
      assert.ok(
        /^\s{2}strategy:\s*\n\s{4}type:\s*Recreate\s*$/m.test(doc),
        `${name}: Deployment ${deploy} は PVC を掴むのに spec.strategy が Recreate でない` +
          '（新旧 2 つの Pod が同じボリューム・同じ DB の初期化を奪い合う）',
      );
    }
  }
  // 母集合が空なら何も検査していない（「検査していない」と「問題が無い」を同じ出力にしない）。
  assert.ok(
    holders.includes('seaweedfs') && holders.includes('wiki-js'),
    `PVC を掴む Deployment の走査が既知の 2 件（seaweedfs / wiki-js）を拾えていない: [${holders.join(', ')}]`,
  );
});
...
// IADR-0461 決定 5 (#1499): MinIO Console を撤去し 4 → 3（オブジェクトストレージの S3 は Gateway に出さない）。
