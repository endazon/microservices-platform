 ...
 *   | クラス | 読むもの | 見えないもの | 例 |
 *   | --- | --- | --- | --- |
 *   | HEAD    | `git show HEAD:` / `git diff …HEAD` | **未コミットの変更** | check-doc-updated（PR #682 の事故） |
 *   | TRACKED | `git ls-files`                      | **untracked の新規ファイル** | check-cross-repo-refs（PR #708 の事故） |
 *
 *   作業ツリーを直接読む検査器（26 本）は順序に依存しないので、**何も足さない**。
 *
 * 方針（IADR-0183）:
