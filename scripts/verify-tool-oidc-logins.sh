...
#   **ツール側 6 クライアントの OIDC ログイン開始**を機械で測る（当初 7。MinIO Console を #1499 / IADR-0461 で撤去）。
#
# なぜ要るか:
...
#      当時の dev Vault はインメモリで、Pod 再起動で `auth/oidc` ごと消えた（IADR-0457 で永続化が既定になった。
#      PERSIST=0 の Vault では今も消える）。UI は 200 を返し続ける。
