 ...
 * 門としては見逃す（合成試験では揺れ 300 µs のとき差 0.1 ms は 7 割強を見逃し、差 1 ms は検出する。IADR-0468）。
 ...
 * 計画 ADR-0113 決定 1 / #1541。計算法（正確法・同順位の扱い）は IADR-0468。
 *
...
function selfTest() {
  ...
  ok('🔴 順位和検定: 片側 24・同順位なしの有意水準 1% の境界は W = 464 / 465（正規近似では 464 も棄却されない）', () => {
    ...
    // 🔴 近似へ替える変異はここで落ちる（正確法を採った理由の固定。IADR-0468）。
  });
