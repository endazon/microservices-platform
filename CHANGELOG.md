...
- **IADR-0227**: Keycloak をエッジへ出し、エッジ host を pod からも解決できるようにする (#871) (dc581e7)
- **FR-11,ADR-0038,IADR-0225**: 用途別フォールバック順序と 429 の境界・発火の可観測化を実装する (#867) (a5e452a)
- **ADR-0030**: 標準構成 8 要素の .gitkeep を 55 件（＋雛形 1 件）適用する (#838) (d723241)
- **FR-22,IADR-0215**: 通知の実装設計を確定し、BFF 契約とアプリ内通知の受け皿を載せる (#825) (c01bc09)
- **NFR-19,FR-11,IADR-0212**: LLM 出力トークンの Histogram を足す (#820) (6c542d0)
- **ADR-0023**: 経路B のエッジ TLS を cert-manager で終端し、CA を k8s Secret として安定させる (#792) (5f96e00)
- **FR-05,UC-04,SC-06**: データソース登録フォームへ既定属性 lifecycle の入力欄を足す (#798) (ce6302d)
- **FR-05,UC-04,SC-06**: SC-06 の登録フォームへ既定の部門の入力欄を足す (#771) (a5bbad6)
- **NFR**: 担当 issue が無い計画 ID を warn の並びから区別して出す (#764) (e6dc873)
- **SC-14,SC-15,IADR-0197**: レルムを platform へ改名し ADR-0026 の認証ポリシーを realm へ投入する (#746) (1565021)
- **NFR,IADR-0170**: 計画 pin の鮮度を検知し、通知を「赤」ではなく issue で出す (#680) (4acc9bc)
- **NFR-21**: SLO の暫定通知先を Grafana 統合アラートへ配線する（宛先は意図的に書かない） (#673) (9e1971a)
- **NFR,SC-10,IADR-0164**: LLM 費用の月次手動確認を運用仕様へ置き、行き先のダッシュボードを作る (#666) (0296f1f)
- **FR-12,UC-06,SC-07**: SC-07 に人手補正の 2 ペイン編集・導出標識・再変換の確認を実装する（Phase 1） (#654) (e365b96)
- **FR-12,UC-06,SC-07**: 人手補正の本文取得 API と補正投稿 API を新設する（Phase 1 = 図のコード化） (#650) (dc52156)
- **FR-10,SC-10**: 運用ダッシュボードの閲覧を運用者へ広げる (#648) (3047012)
- **FR-09,SC-09**: タグ辞書の追加・改名・削除を BFF へ通し、SC-09 の画面から操作できるようにする (#646) (6447062)
- **FR-05,FR-09,SC-09,UC-05**: ポリシーの dry-run 検証 API を追加する (#643) (c614c34)
- **FR-04,FR-05,SC-01,SC-08,UC-01,UC-02**: 分析の対象範囲フィルタを追加しタグで絞れるようにする (#641) (8cc0280)
- **FR-06,FR-09,SC-05,SC-09,UC-03,IADR-0153**: タグを識別子参照へ移し改名の追随と削除を実装する (#639) (7d9b0e4)
- **FR-06,FR-09,SC-05,SC-09,IADR-0152**: タグ辞書の値集合の照会・追加と使用件数を実装する (#636) (47006a1)
- **FR-04,FR-05,SC-01,SC-08,IADR-0151**: 権限内属性値の照会 API を追加する (#633) (cb2d611)
- **FR-03,UC-01,SC-02,IADR-0150**: 検索の並び順を 2 値（関連度／更新日時）にする (#632) (9f583aa)
- **FR-02,FR-03,UC-01,SC-02,IADR-0149**: 検索結果へ更新日時を追加し索引へ取り込む (#631) (86004bb)
- **FR-01,FR-02,UC-04,SC-06,IADR-0148**: データソースの同期健全性と更新 API を契約へ追加する (#627) (a7b89e4)
- **NFR,IADR-0134,IADR-0147**: manualChunks の規則構成を守る機械検査を新設し CI へ結線する (#618) (44a3141)
- **NFR,IADR-0145**: スカッシュ着地件名を検査し、FR/UC/SC の実在性を機械化する (#612) (2cd8508)
- **NFR,IADR-0144**: IADR 採番の一意性・連続性・索引一致を機械検査する (#606) (b71630f)
...
- **NFR,IADR-0224**: 波 12 末クロス監査の指摘 10 件を是正する (#860) (6bf215d)
- **ADR-0038,IADR-0022,IADR-0112**: analysis から claude-fable-5 を外し claude-opus-5 へ寄せる (#859) (be57211)
- **NFR**: devcontainer を net10.0 / Node 22 へ追随させ setup.sh を SDK を入れる側にする (#855) (b3b8d70)
- **NFR-11,IADR-0220**: 波 11 末クロス監査の是正 7 件 (#845) (e2fd06a)
- **ADR-0047**: 経路B の admin(50000) を TLS 終端にし平文 URL 65 件を追随させる (#844) (7aa0976)
- **ADR-0030**: 波 10 末クロス監査の是正 6 件 (#839) (330be22)
- **IADR-0060**: 雛形のテストを 1 プロジェクトへ畳み、一度も通っていなかったビルドを通す (#829) (d72a376)
- **NFR,IADR-0210**: 経路B の永続化へ Recreate と実機実測を足し、root 実行を撤回する (#819) (a45dea1)
- **NFR,IADR-0210**: 経路B の Qdrant と可観測性 4 種を PVC 化し、compose とのパリティ差を解消する (#816) (1470444)
- **FR-02,FR-03**: 埋め込みでクエリと文書を区別してプロバイダへ伝える (#810) (7e72dd3)
- **FR-02**: Qdrant コレクションが作られない Options のセクション名を是正する (#807) (717679a)
- **FR-14**: unit-template のフロントを現行スタックへ追随させ、雛形を CI の射程へ入れる (#777) (dca76ce)
- **IADR-0202**: pin 鮮度検知の既定を fetch しないへ戻し、fetch を --fetch の opt-in にする (#776) (d7d6cd8)
- **NFR**: AST submodule の bump でフロント CI が起動するよう paths: へ gitlink を足す (#759) (859b9fe)
- **NFR,IADR-0202**: pin-freshness が比較元と pin の前後関係を判定し、逆方向で緑を返さないようにする (#762) (5780d32)
- **FR-05,UC-04**: 取り込み経路に ABAC 必須属性 owner / department / lifecycle の既定を投入する (#753) (38f1b14)
- **NFR,IADR-0195**: source generator の出力をカバレッジ集計から落とし床を置き直す (#741) (d645f0b)
- **NFR,IADR-0193**: 環流記録の status を計画側の裁定へ追随させ、突合を機械検査する (#738) (d5c33e7)
- **NFR,IADR-0186**: SSH.NET の推移依存を修正版 2026.0.0 へピンする (#723) (7a9e5e9)
- **NFR,IADR-0169**: 他リポジトリ参照の検査を .md 外へ広げ、除外はディレクトリ 1 本にする (#679) (1c5ad74)
- **NFR-19,IADR-0168**: Grafana provisioning の経路間乖離を埋め、突合を全体へ広げる (#678) (39d6973)
- **NFR,IADR-0167**: 仕様書 type の値域をテンプレートから決め、種別の衝突を塞ぐ (#677) (e9dface)
- **NFR,IADR-0166**: 仕様書 status の値域を既存規約へ揃え、機械検査で閉じる (#676) (3a79911)
- **NFR**: 0 件走査で静かに緑を返す検査器 5 本へ fail-closed の門を足す (#672) (17c194a)
- **NFR,IADR-0027,IADR-0062,IADR-0163**: 必須仕様書が指す存在しないコードパスを是正し、`.cs` の実在を検査する (#663) (94a9ce3)
- **NFR,IADR-0132,IADR-0162**: required の規則を要求側と応答側で分け、据え置き 10 件を 0 にする (#662) (dec2619)
- **NFR,IADR-0161**: テストの InMemory DB をクラスごとの一意名で分離し、並列競合を止める (#661) (ed3b4f3)
- **NFR-09,SC-05,FR-06,IADR-0160**: 無認証で到達できた BFF 9 端点を塞ぎ、検査器が無認証を見分けられるようにする (#659) (d8a0c49)
- **FR-05,ADR-0004,IADR-0159**: AccessScopeResponse へ granted を載せ、契約と C# DTO の乖離を機械検査する (#657) (256e87e)
- **FR-08,UC-01,SC-01,SC-10,IADR-0158**: フィードバック端点の認可を両層で塞ぐ (#655) (9e3bb98)
- **FR-06,UC-03,SC-05**: 文書の書き込みを管理者限定へ狭める (#645) (a6d93fa)
- **FR-03,SC-02,IADR-0014**: Qdrant の検索結果にタグを復元し、書き込み表現を取り込み側へ揃える (#644) (4ea6950)
- **FR-01,FR-06,FR-09,NFR,SC-05,SC-09,SC-10,IADR-0153**: 取り込み経路がタグを生成・上書きしないようにする (#638) (040edd6)
- **FR-01,UC-04,SC-06,IADR-0039**: データソースの登録・無効化を管理者限定へ狭め、手動同期の分類を裁定へ揃える (#630) (547a314)
- **NFR,IADR-0145,IADR-0146,IADR-0147**: フェーズ B 末クロス監査の指摘を是正する (#625) (1c3f1b5)
- **NFR,IADR-0134**: 分割スモークの接続先を設定から得て、空振りが緑にならないようにする (#616) (0121f78)
- **NFR,IADR-0144,IADR-0145**: フェーズ末クロス監査の指摘を是正する (#615) (96c2dbe)
- **NFR,IADR-0146**: 画面からの apiFetch 再混入を ESLint で止める (#614) (4e06353)
- **NFR**: 同一ディレクトリのベア相対リンクを検査対象に加える (#611) (8fbd730)
...
- **IADR-0216**: ログの出口を Serilog から OTel Logging SDK へ寄せ、ratchet を 42 → 29 へ減らす (#828) (1110b5c)
- **ADR-0041,IADR-0196**: Result 型の方針を計画 ADR-0041 へ追随させる (#742) (a87f50b)
...
- **IADR-0224**: 決定 5 の前提が誤りであることを日付つき追記で訂正する (#868) (16be659)
- **NFR**: 他リポジトリを指す裸の issue 番号 31 件を修飾する (#866) (38211b4)
- **IADR-0166,IADR-0191**: 記録凍結の射程を記録種ごとに分ける (#848) (70d4562)
- **IADR-0139**: Closes の射程を明記し、機械化しない理由を残す (#849) (d136a7e)
- **ADR-0047,IADR-0206**: 経路B は NFR-11 の適用外という整理を撤回する (#843) (848111c)
- **IADR-0219,ADR-0030**: SharedKernel の粒度と Worker の標準構成入りを裁定 planning#390 へ追随させる (#837) (f6ff7b6)
- **IADR-0218**: .gitkeep の適用範囲を決める —— Worker は Api の別形、SharedKernel は対象外 (#832) (26e4529)
- **IADR-0217**: Wolverine のコード生成を実行時コンパイルへ確定し、CPM へ 2 件を宣言する (#831) (d2901da)
- **NFR**: 大玉 17 件の着手可否を測り直し、「不在」を「入手不能」と取り違えた初版を撤回する (#823) (8b9c1c7)
- **NFR,IADR-0121,IADR-0203**: 3 本目の workspace グロブと整形範囲の例外を決定へ追随させる (#811) (6cfb208)
- **NFR**: 見送り条件が解消した 3 箇所を追随させる (#804) (d160f55)
- **NFR,IADR-0191**: 記録凍結の射程を frontmatter の二重記述へ絞り、baseline を allowlist 化する (#800) (1cdbf1e)
- **NFR**: blocked 判定を別環境で測り直し、覆った #442 を層 a と波の表へ書き戻す (#789) (bd90c79)
- **IADR-0139**: 束ねの上限を 2 件から 4 件へ改定する (#794) (0e906f7)
- **NFR**: 追随先の引き直しの 12 軸に引き算と時点を足す (#775) (702903d)
- **NFR**: XL 群の着手ゲートに測定時点と隘路の実測を足し、子 issue 非起票の判断を残す (#770) (ceda9d7)
- **NFR**: 引継資料 §2 の未着手フェーズ表を波計画へ差し替える (#769) (eb1f4a7)
- **NFR**: OPEN issue 38 件の棚卸しと波計画を作業仕様書へ残す (#761) (4b215b6)
- **NFR**: #546 の暫定措置は着地済みであることを実測し、作業仕様書に残す (#760) (d066240)
- **NFR**: CHANGELOG を自動更新 (#605) (818923e)
- **FR-19,FR-20,IADR-0119,IADR-0142**: Wiki.js の個人スコープ可視性制御を検証する (#745) (86bbbd2)
- **NFR,IADR-0187**: feedback/ 11 件の本文から #721 の追記ブロックを撤去する (#744) (f154989)
- **NFR**: キットへの環流 11 件を計画リポへ伝達する (#740) (1d7edce)
- **NFR,IADR-0192**: キット追随の分類表を置き、分類 A のバイト一致を機械検査する (#734) (bed8a6e)
- **NFR,IADR-0191**: 記録の書き換え境界を本文と frontmatter 状態欄で切る (#732) (ad782e5)
- **NFR,IADR-0190**: 母集合節の実例を別紙と正本へ寄せ、必読に恒久的な余白を作る (#731) (de4a5d6)
- **NFR,IADR-0189**: planning#311 の裁定へ入口を追随させ、無採番 NFR の 2 場合を取り込む (#729) (6d2d782)
- **NFR,IADR-0188**: 無採番 NFR の射程を一般化し、必読の予算超過を解消する (#727) (39dd95b)
- **NFR,IADR-0187**: status 語彙を上流裁定へ差し替え、キット追随漏れを解消する (#726) (22ae1db)
- **NFR,IADR-0182**: レビュー指摘に対応する（CodeQL を必須チェック推奨から除外・仕様書へフロントマター追加） (3a26407)
- **NFR,IADR-0185**: feedback/ の欠落節を取り込み、status 語彙を定義する (#715) (62d8896)
- **NFR,IADR-0180**: 棚卸しの観点へ blocked 判定の再検証を足す (#702) (1f2cc2a)
- **NFR**: NFR 採番の環流を計画リポへ起票し、feedback/ の語彙を揃える (#700) (1063d0e)
- **NFR,IADR-0179**: メタ作業の NFR は無採番のままとする規約を明記する (#699) (6bb684f)
- **NFR,IADR-0178**: CLAUDE.md の重複を正本へ畳み、50KB 目安へ到達する (#698) (ef60781)
- **NFR,IADR-0177**: 起点 ID の追随記録を別紙へ出し、入口を尽くす (#696) (c4093a8)
- **NFR,IADR-0176**: Superseded 引用の経緯を別紙へ出し、入口を総括する (#694) (d11e8de)
- **NFR,IADR-0175**: ID 修飾の経緯を別紙へ出し、CLAUDE.md の減量を確定事項とする (#692) (f905d3b)
- **NFR,IADR-0174**: CHANGELOG 補正の仕組みを別紙へ出し、計画の見込みを引き直す (#690) (8cd47a3)
- **NFR,IADR-0173**: コミットメッセージ規約の説明を別紙へ出す (#687) (1dcc0f7)
- **NFR,IADR-0172**: 必読規約の減量計画を確定する (#685) (750c877)
- **FR-11,IADR-0112**: ピン留めモデルの版数移行手順と利用不能時の振る舞いを運用仕様へ明記する (#682) (36bc4ea)
- **NFR,IADR-0171**: 相互リンクの義務を仕様書側の一方向と確定する (#681) (b993aca)
- **SC-01,SC-03,SC-07,SC-09,SC-10,SC-11**: 対応表を 3 値へ畳み、隠れた部分未実装 4 件を掘り出す (#670) (8889f81)
- **SC-05,SC-06,SC-07,IADR-0128**: 裁定で確定した閲覧ロールと機密区分の表示名へ実装側の記述を追随させる (#668) (ce1064f)
- **NFR**: 計画リポの実装作業運用ガイドを CLAUDE.md / AGENTS.md へ組み込み、計画 pin を前進させる (#624) (ae66549)
- **NFR**: 引継資料の環境前提を「実測の手順」へ改める (#621) (74d9d7f)
- **NFR**: issue 消化フェーズの引継資料を追加する (#613) (e4c1157)
...
- **NFR**: git シムを child_process フックへ替え、Windows で完走させる (#852) (54d034b)
- **IADR-0213**: OPTIN_TOKENS を末尾境界で照合し、各トークンの単独検出力を検査で固定する (#821) (90ba652)
...
- **deps**: サブプロジェクトのコミットを更新 (3841089)
- **deps**: planning サブプロジェクトのコミットを 07db93f に更新 (20a2b4b)
- **deps**: ai-stock-trading サブプロジェクトのコミットを f25edda に更新 (f78edb9)
- **deps**: bump src/ai-stock-trading from `91d52c2` to `b2d0ab2` (#725) (d112e90)
...
- **NFR**: 雛形 backend を CI でビルド・テストして配布前に壊れを止める (#857) (1078e55)
- **NFR**: AI ワークフローの許可リストへ、.gitmodules の各 submodule パスに対する git -C grep を 3 パス分足す (#856) (e3cb107)
- **NFR,IADR-0214**: frontend.yml が走らせるゲートの入力を paths: へ入れ、機械で突合する (#822) (98182bc)
- **NFR,IADR-0209**: 雛形のテストを走らせる CI が起動しない穴を塞ぎ、include ⊆ paths を機械で固定する (#814) (49ec8e3)
- **NFR,IADR-0208**: companion の単体実行を沈黙の exit 0 から exit 1 へ変える (#813) (da05768)
- **NFR,IADR-0207**: PR タイトル末尾の (#NNN) が PR 自身の番号と一致することを検査する (#808) (d63c3a6)
...
- Update settings.json (4a58178)
- **NFR**: 計画 pin を 179a69a へ進め、キット大幅更新（役割スロット制・必読別紙化・検査強化）へ追随する (#870) (2ae0dc8)
- **ADR-0030,NFR**: 計画 pin を 282c2d0 へ進め、スキル・MCP を配備して環流 3 件を追随させる (#847) (3ad5ad1)
- **NFR,IADR-0204**: キット分類 X 4 件を IADR-0204 決定 2 の 3 点突合で再判定する (#840) (5ed54b0)
- **NFR,ADR-0030**: 計画 pin を 767a9d48 へ進め、ADR レンジ 0047 とキット追随を引き直す (#833) (29de104)
- **ADR-0031,IADR-0211**: Knip を導入し未使用コード・依存を床 41 件のラチェットで固定する (#818) (f423ca4)
- **NFR,IADR-0205**: 必読規約を減量してキット traceability.md を追随し、分類を A へ戻す (#805) (d121ee8)
- **NFR**: 計画 pin を 8cae89d へ進め、キット追随の分類 X を再判定する (#795) (3818c08)
- **NFR**: Renovate と Husky を導入し、手元フックを CI ゲートの部分集合に限る (#772) (b856726)
- **NFR**: scripts.test.js をキット版へ追随させ、バイト一致を回復する (#766) (fde7252)
- **NFR**: 先行検査器 3 本の優劣を HOWTO の手順で判定し、plan-id をキット版へ差し替える (#765) (6f9edef)
- **NFR**: 計画 pin を 4d6a7d6 へ進め、母集合定義・分類 C 再判定・§11 パリティ・Windows パス不整合を追随する（#751 を束ねる） (#758) (8bf53d8)
- **NFR**: 計画 pin を ce9abd2 へ進め、キット同期 11 件を追随する (#750) (369bad2)
- **NFR,IADR-0194**: キット追随漏れを取り込み、監査の巡数の上限を撤廃する (#739) (8268832)
- **NFR**: CodeQL の PR 解析をコード変更のある PR に限定する (#720) (a63068a)
- **NFR**: CodeQL の PR 解析をコード変更のある PR に限定する (7b8b42a)
- **NFR**: planning pin を cff0e7b へ進める (#718) (51467cc)
- **NFR**: 環流の未送付検査器をキットから取り込み、CI へ配線する (#711) (a671a80)
- **NFR**: 検査器が「偽の緑」を返す条件を警告し、検証順序を DoD へ定める (#709) (623606b)
- **NFR**: 未到達の環流 3 件を計画リポへ伝達し、記録を実態へ揃える (#708) (846d101)
- **NFR,IADR-0182**: 必須チェックの設定手順を正し、blocked:human として記録する (#706) (28e921b)
- **NFR,IADR-0181**: キット e0bc81c への追随漏れ 2 件を回収する (#704) (5933894)
- **NFR**: 2 回持ち越された負債を回収する（orval の知見・frontend-tests.yml の paths） (#671) (4dbd501)
- **NFR**: openapi.yaml が宣言するロールと BFF の実効ロールを突き合わせる検査器を置く (#653) (7d390e6)
- **NFR**: 本文を変えたのに frontmatter の updated: が古いままの文書を CI で止める (#652) (cc451b0)
- **NFR,ADR-0006,ADR-0044**: planning pin を d9c2014 へ進める (#620) (dec2736)
- **NFR**: フロントエンドに prettier の format ゲートを新設し CI へ結線する (#619) (ce96eb8)
