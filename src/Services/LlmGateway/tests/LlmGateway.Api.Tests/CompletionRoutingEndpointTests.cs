    ...
    : IClassFixture<TestWebApplicationFactory>
{
    ...
    // FR-11: Model 未指定なら用途（purpose）に応じてモデルを切り替える（analysis→opus / rag-answer→sonnet / diagram→haiku）。
    // 実運用経路（RagOrchestrator）は Model=null で /complete を呼ぶため、この経路で用途別モデルが発火することを検証する。
    [Theory]
    [InlineData("analysis", "claude-opus-4-8")]
    [InlineData("rag-answer", "claude-sonnet-4-6")]
    [InlineData("diagram", "claude-haiku-4-5")]
