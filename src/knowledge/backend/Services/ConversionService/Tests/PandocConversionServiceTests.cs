...
//
// 🔴 IADR-0317 (#1097): 従前このクラスは「pandoc が無ければプレースホルダへ縮退する」ことを
...
public class PandocConversionServiceTests
{
    ...
    // IADR-0317 決定 3: オブジェクトストレージ上の原本を取り寄せて変換する。
    ...
    // IADR-0317 決定 4: PDF は pandoc の入力形式にならない。**既定の markdown へ落とさない。**
    [Theory]
    ...
    // 🔴 IADR-0317 決定 1 (#1097): **実行時イメージが pandoc を導入していることを機械的に確かめる。**
