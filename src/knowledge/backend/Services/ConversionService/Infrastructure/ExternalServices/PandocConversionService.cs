...
//
// 🔴 IADR-0318 (#1097): **縮退の既定は fail-closed である。**
    ...
    ILogger<PandocConversionService> logger) : IBodyConverter
{
    ...
    // IADR-0318 決定 2: 変換器が動かせないときの分岐点。**既定は例外**である。
    ...
    //
    // 🔴 IADR-0318 決定 4 (#1097): **pandoc が入力に取れない形式は既定へ落とさず拒否する。**
    ...
    //
    // 🔴 IADR-0318 決定 3 (#1097): **オブジェクトストレージの原本を取り寄せる。**
    ...
    // IADR-0318 決定 5 (#1097): pandoc の版（`pandoc --version` の 1 行目）。取得できなければ null
