...
public class DataSource
{
    ...
    // ［2026-09-05 更新 / #752］**`wiki` / `saas` / `db` が更新者を載せるようになった**ので、
    // 2 段目が実際に効く（従前ここには「本段ではどのコネクタも値を載せない」と書いていた）。
    // **`filesystem` は今後も載せない**（構造上の縮退。ADR-0074 決定 3）。
    ...
    private static Dictionary<string, string> WithRequiredAttributeFailsafe(IReadOnlyDictionary<string, string>? attributes)
    {
        ...
        // 当時**どのコネクタも値を載せていなかった**からである。追跡は引き続き #752。
        //
        // ［2026-08-28 追記 / #752］**「残る 1 本は識別子の名前空間が未裁定」は誤りになった。**
        // 解決順は **① Keycloak ユーザー検索 → ② データソース単位の写像表 → 予約値 `system`** と
        // **確定済み**である（裁定 2026-08-16。planning#371）。
        //
        // ［2026-09-05 追記 / #752］**「`db` コネクタに更新者列を足してはならない」も誤りになった。**
        // 解決段（`ResolveOwner`）が #1194 で入り、ADR-0074 決定 5 の先後条件が満たされたので、
        // **`wiki` / `saas` / `db` は更新者を載せる**（`db` は opt-in の `updatedByColumn`）。
        // **生の列値が解決を経ずに `owner` になる経路は無い** —— 写像表に当たらなければ
        // `ResolveOwner` が null を返し、ここが予約値を入れる。計画は「**別名前空間の識別子を
        // そのまま `owner` へ入れてはならない**」「誤った写像は**偽の所有者**を作り、**裁量制御が
        // 意図しない相手に開く**」「安全側は『解決しない』」と定める（ADR-0036）。
        //
        // 🔴 **`filesystem` だけは今後も運べない**（構造上の縮退。ADR-0074 決定 3）。
        // **予約値の件数を達成度として読まない。**
        //
