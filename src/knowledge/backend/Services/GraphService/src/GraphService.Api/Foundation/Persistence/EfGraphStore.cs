...
// FR-17, UC-10, IADR-0239 決定 11: EF Core（PostgreSQL 隣接リスト）によるグラフ読み取り。
//
...
public class EfGraphStore(GraphDbContext db) : IGraphStore
{
    ...
    // （ADR-0034 決定 1 / IADR-0239 決定 2）。署名を Guid へ緩めると型ゲートが無効になる。
    //
        ...
        IReadOnlyList<AuthorizedNode> frontier, CancellationToken ct = default)
    {
        ...
        // ADR-0034 決定 4 / IADR-0239 決定 4: **決定的順序**で返す。
