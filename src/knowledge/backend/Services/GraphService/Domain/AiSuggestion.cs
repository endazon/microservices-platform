...
public class AiSuggestion
{
    ...
    //
    // 発火させるのは `DocumentUpdated` の購読であり、`GraphDocumentSyncConsumer` が
    // 指紋の変化を検出したときに本メソッドを呼ぶ（#911 で配線済み）。
    //
    // 🔴 **［2026-08-28 追記 / #438］ここには 2 つの誤りが並んでいた。**
    // 「本メソッドには本番コードからの呼び出し元が存在しない」——**呼び出し元は在る**
    // （`GraphDocumentSyncConsumer.cs`。#911 の着地で生まれ、この注記が追随しなかった）。
    // 「未配線は `AiSuggestionWiringTests` が機械で固定している」——**そのテストは
    // 一度も存在しない**（クラス定義がリポジトリ全域で 0 件。走査で確認した）。
    // **後者のほうが重い。** 前者は古くなった記述だが、後者は「機械が守っている」と
    // 書いてあるのに機械が居ない状態であり、読んだ人は確かめずに信じてしまう。
    // 「統制を定めた」と「統制が働いている」は、書き手が意識しないと同じ文になる。
    //
