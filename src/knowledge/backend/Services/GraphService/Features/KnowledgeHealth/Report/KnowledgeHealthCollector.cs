...
//      **これは 0 件ではなく実測された件数である。**
//
// ★［2026-09-11 追記 / #1395］**要約の書き手も付いた**（`ClusterSummaryJob`。[[IADR-0430]]）。
// 従前ここには「要約そのものは未生成であり、現況では全クラスタが『要約が 1 つも無い』に当たる」と
// 書いてあったが、生成バッチが来た。**ただし既定はオフ**（`ClusterSummary:Enabled=false`）であり、
// 有効化するまでは従前どおり全クラスタが条件 1 に当たる。**指標の読み方はどちらでも同じ** ——
// 数えているのは「要約が追いついていないクラスタ」であり、生成器の有無で定義は変わらない。
//
    ...
    ILogger<KnowledgeHealthCollector> logger)
{
    ...
    // 🔴 **要約生成バッチ（`ClusterSummaryJob`）が同じ関数を呼ぶ**（#1395 / [[IADR-0430]]）。
    // ここに条件を書き写すと、再生成側と判定側が割れる（`ADR-0035` 決定 6 の再生成条件と
    // `ADR-0083` 決定 3 の未要約条件は同じものの裏表である）。
    //
