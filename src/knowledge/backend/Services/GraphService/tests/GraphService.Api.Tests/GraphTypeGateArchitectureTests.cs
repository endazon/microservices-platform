...
// FR-17, UC-10, ADR-0034 決定 1, IADR-0241 決定 2: **型ゲートが回避不能であることを固定する。**
//
// ADR-0034 決定 1 は「ホップごとに ABAC 判定を行う。終端でまとめてフィルタしない」を定める。
// 本リポジトリの認可 API はフィルタ集合しか返さないため、素朴に書くと「探索してから濾す」形に
// なり、それが情報漏れの経路になる。IADR-0241 決定 2 は 2 段の型ゲートでこれを表現不能にした。
//
...
public class GraphTypeGateArchitectureTests
{
    ...
    // IADR-0241 決定 2: 未フィルタの部分グラフは公開型であってはならない
