...
// 計画 `06_technical/05_observability-ops.md` §ナレッジ健全性の指標（集計範囲・2026-08-02 確定）の
// うち、**観測値（状態のスナップショット）として運ぶ 7 指標**を持つ。**値域は閉じる**
// （未知の指標名は 400）—— 語彙が開いていると、生産者側の綴り違いが「0 件の指標」として
// 静かに現れ、「指標が改善した」と読めてしまうためである。
//
// 🔴 **［2026-09-09 / #1233 / [[IADR-0420]] 決定 4］計画は 2026-09-06 に 8 つ目を加えたが、
// ここへは足さない。** 8 つ目（ユニットの主体が保存した文書のうち `project` を持たない件数。
// `ADR-0085` 決定 4）は**事象の積算**であり、可観測性仕様書が「混ぜない」と定めた 2 経路のうち
// **OTel カウンタ → Grafana** の側に属する（生産者は `DocumentService.UnitProjectMetrics`）。
// **足すと、誰も送らない 8 件目が恒久的に 0 件で並ぶ** —— 計画自身が
// 「生産者の無い指標を 0 件として並べてはならない」（その 0 は「問題なし」と読める）と
// 定めている。**「計画が 8 指標だから語彙も 8」ではない。**
