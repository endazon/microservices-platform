...
public static class DocumentEndpoints
{
    ...
    //
    // 🔴 **`DocumentUpdated` を出す経路はすべてこれを通す**（`PublishUpdatedAsync` を直接呼ぶ
    // 本番経路は残っていない）。一部の経路だけ門を通す形にすると、
    // **「発行の門がある」という説明とコードの実態が食い違い**、後から経路を足した人が
    // どちらの作法に倣えばよいか判らなくなる（PR #1281 のレビュー指摘）。
    // 組織文書は `IsIndexable` が常に true なので、これらの経路の挙動は 1 ビットも変わらない。
    //
