...
// FR-03, FR-05, NFR-09, ADR-0004, [[IADR-0379]] 決定 5 (#1378):
...
[Trait("TestKind", "Unit")]
public class SearchAccessResolverWarnTests
{
        ...
        Func<HttpRequestMessage, HttpResponseMessage> respond)
    {
        ...
        var scope = await new SearchAccessResolver(new StubHttpClientFactory(respond), logger: log)
            .ResolveAsync(Ctx(), Ct);
        return (scope, log);
    }
    ...
    // 🔴 本番の登録（`AddScoped<ISearchAccessResolver, SearchAccessResolver>`）で**ロガーが実際に注入される**ことを見る。
    ...
    public async Task The_production_registration_injects_the_logger()
    {
        ...
        services.AddSingleton<ILogger<SearchAccessResolver>>(log);
        services.AddScoped<ISearchAccessResolver, SearchAccessResolver>();
        await using var sp = services.BuildServiceProvider();

        await using var requestScope = sp.CreateAsyncScope();
        var scope = await requestScope.ServiceProvider.GetRequiredService<ISearchAccessResolver>()
            .ResolveAsync(Ctx(), Ct);
    ...
    internal sealed class CapturingLogger : ILogger<SearchAccessResolver>
    {
