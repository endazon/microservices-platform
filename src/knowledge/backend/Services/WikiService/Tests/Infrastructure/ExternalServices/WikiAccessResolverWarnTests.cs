...
// FR-05, FR-13, NFR-09, ADR-0004, [[IADR-0379]] 決定 5 (#1378):
...
[Trait("TestKind", "Unit")]
public class WikiAccessResolverWarnTests
{
        ...
        Func<HttpRequestMessage, HttpResponseMessage> respond)
    {
        ...
        var scope = await new WikiAccessResolver(new StubHttpClientFactory(respond), logger: log)
            .ResolveAsync(Ctx(), Ct);
        return (scope, log);
    }
    ...
    // 🔴 本番の登録（`AddScoped<IWikiAccessResolver, WikiAccessResolver>`）で**ロガーが実際に注入される**ことを見る。
    ...
    public async Task The_production_registration_injects_the_logger()
    {
        ...
        services.AddSingleton<ILogger<WikiAccessResolver>>(log);
        services.AddScoped<IWikiAccessResolver, WikiAccessResolver>();
        await using var sp = services.BuildServiceProvider();

        await using var requestScope = sp.CreateAsyncScope();
        var scope = await requestScope.ServiceProvider.GetRequiredService<IWikiAccessResolver>()
            .ResolveAsync(Ctx(), Ct);
    ...
    internal sealed class CapturingLogger : ILogger<WikiAccessResolver>
    {
