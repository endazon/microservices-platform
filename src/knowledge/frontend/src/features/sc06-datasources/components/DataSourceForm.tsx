...
export function DataSourceForm({
  ...
  return (
    <Panel heading={<Trans>データソースを登録</Trans>}>
      <form
      ...
      >
        ...
        <div>
          {/* FR-05, UC-04, SC-06（#767）: 既定の所管部門。計画 07_abac-attribute-model は
                ...
                ［2026-09-26 / #1557 / IADR-0472］**値域の検証が入った**（計画 ADR-0115 決定 5）。サーバが書き込み時に
                部門グループのコードかを確かめ、無ければ理由つきで拒否する。**入力欄は自由入力のまま**である ——
                候補の一覧を画面へ出す口は作っていない（サーバは「この値は在るか」だけを答え、一覧を返さない）。
                補助文で「入れるなら部門グループのコード」と伝える。 */}
        ...
        <div>
          ...
          <p id="ds-lifecycle-hint" className="text-xs text-fg-muted">
