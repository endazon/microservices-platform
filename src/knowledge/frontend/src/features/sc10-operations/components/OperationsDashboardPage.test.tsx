...
describe('OperationsDashboardPage (SC-10)', () => {
  ...
  //
  // 🔴 ★［2026-09-11 追記 / #1363］**7 指標すべてに生産者ができた。この否定テストは緑のまま残す。**
  // 上の「3 件」は**書かれた 2 日後には古かった** —— #1246 が 2 件を配線し、残る 1 件
  // （未要約クラスタ）を #1363 がクラスタ検出（Leiden 法・日次バッチ）で塞いだ（[[IADR-0425]]）。
  // **それでも 8 行をまとめて外してはならない。** 集計 API へ**観測値として**届くのは 5 指標で、
  // 残る 2 件（未定義型のフォールバック警告／取り込み経路の未知タグ）は
  // **OTel カウンタ → Grafana** の別経路であり、**集計 API では 0 件で並ぶ** ——
  // planning#494 の「生産者の無い指標を 0 件として並べてはならない」はその 2 件に効き続ける。
  // 加えて **BFF に健全性の口が無い**（あるのは /bff/dashboard/summary の 1 本だけ）。
  // **節を開くのは別の作業であり、その作業が本注記ごと書き換える。**
