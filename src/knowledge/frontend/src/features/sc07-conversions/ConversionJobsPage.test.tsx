...
describe('ConversionJobsPage (SC-07)', () => {
  ...
  it('lists jobs with the four-value status model', async () => {
    mocks.apiFetch.mockResolvedValue([FAILED_JOB, SUCCEEDED_JOB, PROCESSING_JOB]);
    await renderPage();
    ...
    expect(mocks.apiFetch).toHaveBeenCalledWith('/conversion/jobs');
  });

  // 計画確定: 照会 API は状態でのフィルタを備える（「失敗のみ」フィルタの実体）。
  it('sends the status filter to the query API', async () => {
    mocks.apiFetch.mockResolvedValue([FAILED_JOB]);
    ...
    await waitFor(() =>
      expect(mocks.apiFetch).toHaveBeenCalledWith('/conversion/jobs?status=failed'),
    );
  });
  ...
  it('lets an administrator retry a failed job', async () => {
    ...
    await waitFor(() =>
      expect(mocks.apiFetch).toHaveBeenCalledWith(`/conversion/jobs/${FAILED_JOB.id}/retry`, {
        method: 'POST',
      }),
    );
  ...
  it('refetches the list after a successful retry', async () => {
    mocks.apiFetch.mockImplementation((path: string) =>
      path.endsWith('/retry') ? Promise.resolve(undefined) : Promise.resolve([FAILED_JOB]),
    );
    const user = userEvent.setup();
    await renderPage(['platform-admin']);
    ...
    await waitFor(() =>
      expect(
        mocks.apiFetch.mock.calls.filter(([path]) => path === '/conversion/jobs'),
      ).toHaveLength(2),
    );
  });
  ...
  it('offers no retry for jobs that are not failed', async () => {
    mocks.apiFetch.mockResolvedValue([PROCESSING_JOB, SUCCEEDED_JOB]);
    await renderPage(['platform-admin']);
  ...
  it('links a succeeded job to its document (SC-03)', async () => {
    mocks.apiFetch.mockResolvedValue([SUCCEEDED_JOB]);
    await renderPage();
