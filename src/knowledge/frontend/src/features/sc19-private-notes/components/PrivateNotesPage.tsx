...
//
// ■ 🔴 **指定先は個人とグループの両方を扱う**（ADR-0098 決定 1。#1447 で決定 2 の暫定手段が
//   解除された —— `${current_groups}` の束縛と共有先ベースの分岐が認可へ入り、**グループ共有が
//   実際に閲覧を許可するようになった**）。導線と文言は `ShareTargetsDialog` が持ち、
//   **一覧の行は件数までである**（上の注記）。
//
    ...
    <section className="flex flex-col gap-4">
      ...
      */}
      ...
      {/*
        ...
        🔴 **タブと絞り込みの器は取得前に描かない** —— 中身のない「利用中（0）／削除済み（0）」は、
        本当に 0 件なのか、まだ取れていないのかを利用者が読み分けられない。
        失敗には**再試行の導線**が付く（以前は「時間をおいて再度お試しください」と書くだけだった）。
      */}
      <QueryState
        query={notes}
        isEmpty={(data: PrivateNoteListResponse) => data.notes.length === 0}
        loadingLabel={t`個人資料の一覧を読み込み中…`}
        errorTitle={t`個人資料の一覧を取得できませんでした。`}
        empty={
          <EmptyState
            title={t`個人資料がまだありません。`}
            description={t`上の入力欄からタイトルを付けて作成し、本文は Obsidian 連携で書きます。`}
          />
        }
      >
        {() => (
          <>
            ...
            {/*
            ここへ来るのは「資料はあるが、いまのタブ・絞り込みでは 0 件」の場合だけである
            （資料が 1 件も無い場合は QueryState の `empty` が受ける）。**次の行動を必ず示す。**
          */}
            {rows.length === 0 ? (
              <EmptyState
                title={
                ...
                description={
                  search.tab === 'trash'
                    ? t`削除した資料は 90 日間ここに残ります。`
                    : t`絞り込みの語を短くするか、空にしてください。`
                }
              />
            ) : (
              <Table>
