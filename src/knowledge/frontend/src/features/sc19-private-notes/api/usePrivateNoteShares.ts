...
// SC-19 主要素 3, UC-11, FR-19, ADR-0036 D-06 / ADR-0098 決定 1 / IADR-0253 段 4 / IADR-0445 /
// IADR-0447 (#1447): 公開範囲の指定先（共有台帳）の照会・付与・取り消し。
//
// 🔴 **画面は `user` と `group` の両方を送る**（ADR-0098 決定 1 の 2 種別）。決定 2 が
// グループの導線を止めていたのは「認可がグループ共有を一度も評価しない」ためだったが、
// #1447 で `${current_groups}` の束縛と共有先ベースの分岐が入り、**グループ共有は実際に
// 閲覧を許可するようになった**。種別の選択は `ShareTargetsDialog` が持つ（ここは口だけを提供する）。
//
