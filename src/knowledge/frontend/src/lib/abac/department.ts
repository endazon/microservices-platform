...
// Keycloak realm の `department` グループ（`/department/<コード>`）」とされ、② の登録時に
// **登録した管理者の部門グループ（ちょうど 1 つのとき）**でサーバが補うようになった。
// ただし**値域の検証（候補外の拒否）はまだ無く**（realm のグループ一覧を DataSourceService が引く口が無い）、
// 画面は引き続き自由入力である。上の「値集合は持たない」はこの意味で今も正しい。
// ① は**器（写像表の置き場所）が計画で未確定**なので、引き続き入れない。
//
// ［2026-09-26 追記 / #1557 / IADR-0472］**値域の検証が入った**（計画 ADR-0115 決定 5）。SC-06 で明示した部門は
// サーバが書き込み時に realm の部門グループのコードかを確かめ、無ければ理由つきで拒否する（引けなければ 502）。
// **画面は値集合を持たないまま**である —— サーバの読み口は「この値は在るか」だけを答え、一覧を返さない。
// 上の「値集合は持たない」は引き続き正しい。
