...
public class McpClient
{
    ...
    // （同 §基本原則「既定は安全側」）。値の型は Domain/EgressPolicy.cs の EgressTier。
    public int EgressTier { get; private set; } = (int)McpServer.Domain.EgressTier.StandardExternal;
    ...
    public static McpClient Register(
        ...
        IReadOnlyDictionary<string, string>? attributes, McpServer.Domain.EgressTier egressTier,
        DateTimeOffset now)
        => new()
        {
            ClientId = clientId,
