...
//   読み方は `RegistrarScopeReading.ReadAssignableConfidentiality` に 1 つだけ置く
//   （同ファイルの注記が規則の正本。#1255 で輸送が 2 つになったので両実装が呼べる位置へ括り出した）。
//
...
//   `docs/api/east-west-grpc.md` §4）。gRPC 実装（`GrpcRegistrarAttributes`）は転送せず、
//   **呼び出し先の読み口を「この 1 人の属性は何か」に狭めること**で同じ答えを得る ——
//   上の「新設すると SC-12 を触れない主体が名簿を引ける経路ができる」という懸念は、
//   **列挙を s2s の面へ出さない**ことで満たされる（名簿は引けない）。
//   人の側の門（`/mcp-clients` の `AdminOnly`）は本サービスの端点に残る。
//
// ■ 縮退
