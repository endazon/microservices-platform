...
public class BffEndpointCompositionTests
{
    ...
    {
        // 全 17 モジュール。ナレッジ 10 ドメイン（Search/Document/Analysis/Feedback/Dashboard/Conversion/DataSource/TagDictionary/Graph/PrivateNote）は
        // knowledge の Knowledge.Bff.Endpoints へ移設済み・例外3 で合成点参照。platform 固有 2（Config/Authz）は
        // platform 同居（#452 の McpClient を含めて 3 つ）。AST の Assumptions（#283・AST/SC-01）／RiskControls（#287・AST/SC-02/AST/SC-03）／Monitor（#288・AST/SC-02 watchlist）は
        ...
        BffEndpointComposition.Modules.Should().HaveCount(17);
    }
    ...
    {
        // 期待する 17 ルートグループのプレフィックス（各 BFF エンドポイントモジュールの MapGroup）。
