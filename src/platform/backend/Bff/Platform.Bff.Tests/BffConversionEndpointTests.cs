...
public class BffConversionEndpointTests : IClassFixture<BffTestFactory>
{
    ...
    public BffConversionEndpointTests(BffTestFactory factory)
    {
        ...
        _factory.LastConversionForwardedAuthorization = null;
    }
    ...
    [InlineData("GET", "")]
    [InlineData("GET", "/{id}")]
    [InlineData("POST", "/{id}/retry")]
    [InlineData("GET", "/{id}/figures")]
    [InlineData("GET", "/{id}/figures/fig-1/image")]
    [InlineData("POST", "/{id}/figures/fig-1/correction")]
    public async Task EveryRoute_RelaysTheUsersCredential_ToConversionService(string method, string suffix)
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization =
            new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", "relayed-user-token");
        var path = "/bff/conversion/jobs" + suffix.Replace("{id}", BffTestFactory.StubJobId.ToString());
        using var req = new HttpRequestMessage(new HttpMethod(method), path);
        if (suffix.EndsWith("/correction", StringComparison.Ordinal))
        ...
        _factory.LastConversionForwardedAuthorization.Should().Be("Bearer relayed-user-token");
    }
}
