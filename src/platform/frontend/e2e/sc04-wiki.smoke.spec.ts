...
test('SC-04: draws the page tree, opens a page and searches — all through the BFF, inside the SPA', async ({
  ...
  // 🔴 **［2026-09-05 / #1240］バックリンク欄・ローカルグラフについて、本 spec は何も固定しない。**
  //
  // 05_screens は「バックリンク欄・ローカルグラフは **SC-04 のみ**に置き SC-03 には併置しない」と
  // 定めており、**SC-03 側の不在は `sc03-document.smoke.spec.ts` が固定した**。
  // **対になる『SC-04 に在ること』はここへ書かない** —— `docs/screens/SC-04_wiki-access.md`
  // §未決事項 2 のとおり**実現方式が計画側で未確定**であり、まだ実装していない。
  // **未確定のものを E2E で固定すると、計画が決めたときに実装ではなくテストが先に決めたことになる。**
  //
  // **不在も固定しない。** 「SC-04 に無い」を固定すると、計画が決めて実装した瞬間に
  // **正しい実装がテストを落とす**（SC-03 側の不在は「置かない」という確定事項なので固定してよいが、
  // こちらは「未確定」であって「置かない」ではない —— 同じ形に見えて根拠が違う）。
  // 実現方式が確定したら、そのときの issue が本 spec へ陽性の固定を足す。
