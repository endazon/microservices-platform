...
// 共通シェルの**トースト**。「色だけで意味を持たせない」を **API の形**で強制する。
//
// ★ **FR-22 のアプリ内通知（`foundation/notifications/`）とは別物である**（IADR-0215 決定 2）。
// こちらは**一過性**——操作の結果をその場で知らせて消える。あちらは**永続**——既読になるまで残り、
// 保持期間は 90 日で、サーバから取得する。**同じ「通知」という語が 2 つの別物を指すので注意する。**
