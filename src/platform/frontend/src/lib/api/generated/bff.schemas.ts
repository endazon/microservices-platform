...
/**
 * 未指定時は後段が必須属性のフェイルセーフを通る（confidentiality=internal / owner=system / department=unassigned / lifecycle=active）。明示指定は上書きしない。明示した department は後段が値域（realm の /department 直下の部門グループのコード。大小文字を区別）で検証し（更新では保存済みの値から変わったときだけ）、無ければ 400、値域を引けなければ 502 で、いずれも保存しない。予約値 unassigned・空白は検証しない（ADR-0115 決定 5 / IADR-0472 / #1557）
 */
...
/**
 * 指定したときのみ差し替える。差し替え時も必須属性のフェイルセーフを通す（confidentiality / owner / department / lifecycle）。明示した department は後段が値域（realm の /department 直下の部門グループのコード。大小文字を区別）で検証し（更新では保存済みの値から変わったときだけ）、無ければ 400、値域を引けなければ 502 で、いずれも保存しない。予約値 unassigned・空白は検証しない（ADR-0115 決定 5 / IADR-0472 / #1557）
 */
...
export const SecretItemStatusDtoSupplySource = {
  screen: 'screen',
  git: 'git',
  unknown: 'unknown',
} as const;
