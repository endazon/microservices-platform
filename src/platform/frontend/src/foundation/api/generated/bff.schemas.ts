...
export interface TagDto {
  /** 識別子。**改名で変わらない**（SC-09 の「改名は既存文書へ追随する」の土台）。 **［#635］文書はこの識別子を参照して保持する**（表示名を複写しない。IADR-0153 決定 1）。 **改名・削除の口は DocumentService 側にだけ在る**（`PUT` / `DELETE /tags/{id}`。 BFF の書き込み口は #542 の射程外で、別途起票する）。 */
  id: string;
