...
| `Tag` | 分類の名前（SC-02 結果テーブルのタグ・SC-03 属性/タグパネル・SC-01 出典の種別ラベル。モックの `tag`）。**状態ではない**ためアイコンを持たない（`StatusBadge` との違い） |
...
後から足した `Tag` は 4 基準すべてに非該当（非対話・無状態の `<span>`）のため
ネイティブ HTML ＋ `cva` ＋ `cn()` で実装した（#502）。
計画は「本家が Radix を使っていても追随しないこと自体は**逸脱として記録しなくてよい**」とも明記している。
...
- **`Dialog` の移植 → #452**（FR-19 / FR-20 の着手保留が解けたあと。**［2026-08-07 / #599］SC-20 全体は解除済み。覆り得るのは SC-19 の本文編集導線ただ 1 つ**）。計画が確認ダイアログを要求するのは
