...
- 系譜: #460 → PR #461（`grep` / `sort` / `git -C <submodule>` を追加）→ **1 週間以内に #469 で別ルート再発**。
  planning#145 / #146 / #155 / #157 / #158 / #160 / #161 / #162 / #163 と続く同系列の 10 度目である。
...
- 関連: planning#145 / #146（読み取り系 git の欠落）・#147（拒否報告をコマンド名まで出す）・
  ...
  #161 / #162（段階ポリシーと「成果物は正しいのに赤」の常態化）・#163（`grep` / `sort` /
  `git -C <submodule>` / MCP ツール名の版依存）。本件はその系列の続きである。

## 計画側への起票

未（PR 作成と同じタイミングで行う）。planning#163 が同じキットの同じファイル群を対象としているため、
**#163 への追記**（追加提案 7 以降）か新規 issue かはトリアージ側の判断に委ねる。
