...
- 正の一覧はテンプレートに**プレースホルダではなく実物**として置き、`--allowedTools` を
  ...
  あわせて、**禁止だけでなく「よくある作業の代替コマンド」を添える**こと。特に
  **件数の集計**は AI が必ず `… | wc -l` / `… | awk` を書く場面であり、
  `grep -c` / `rg -c` / `rg --count-matches` を代替として明示すれば 1 コマンドに閉じられる
  （下記「追記 4」の実測で、最後に残った 1 件がまさにこれだった）。
...
## 追記 4（2026-08-03）: 拒否 1 件へ収束 — 残ったのは「一覧の網羅漏れ」と「代替の不足」

「追記 3」の対応（正の一覧）を push した後の再々実走（run `30833943957`）で
**権限拒否 1 件**となり、ジョブは success・レビューも全クリアした。

| 拒否 | 件数 | 型 |
| --- | --- | --- |
| `Bash(grep \| wc \| awk)` | 1 | 一覧に無いコマンド（件数集計のパイプ連鎖） |

拒否件数の推移は **7 件（run 30830151995）→ 3 件（run 30832367628）→ 1 件（run 30833943957）**
である。正の一覧（追記 3）で 3 → 1 まで落ちており、**「正の一覧を置く」提案の効果は実走で
確認できた**。ただし issue #469 の受け入れ基準「**拒否 0 件**」は**なお未達**である。

### 残った 1 件が示したこと

1. **正の一覧に添える「未許可の名指しリスト」は網羅漏れを起こす。**
   `awk` は名指し済みだったが **`wc` が漏れていた**。型（構文・粒度）の問題ではなく、
   単なるリストの穴である。名指しは「AI が手を伸ばす順」に厚くする必要があり、
   `wc` は `grep` と並ぶ最頻出の集計手段なので**必ず載せる**べきである。
2. **禁止だけを書いて代替を書かないと、同じ場所で詰まる。**「追記 3」は
   `python3` に対しては代替（Read / `node`）を、`gh auth status` に対しては
   「そもそも不要」を書いたが、**集計については禁止すら書いていなかった**。
   `grep` / `rg` は許可済みであり、`grep -c` / `rg -c` / `rg --count-matches` を使えば
   1 コマンドに閉じられる。**禁止と代替は必ず対で書く**という原則が、ここでも再確認された。

### 本リポジトリの対応（暫定デルタ）

両プロンプトの未許可名指しリストへ `wc` を追加し、
「**件数を数えるときは `grep -c` / `rg -c`（総マッチ数なら `rg --count-matches`）を単体で使う。
`… | wc -l` / `… | awk` のパイプ集計は必ず拒否され、鎖全体が実行されない**」を
出典 run `30833943957` 込みで追記した。`--allowedTools` は変更していない
（`wc` を許可へ足す選択は採らない。許可を広げずプロンプトで手順を狭める方針を維持する）。

### planning#168 へ追記する内容（追記 4 分）

- 正の一覧に添える**未許可コマンドの名指しリストへ `wc` を必ず含める**
  （`python3` / `jq` / `curl` と並ぶ最頻出。実測で最後に残った 1 件がこれだった）。
- **「禁止と代替は対で書く」を原則として明文化する。** 少なくとも次の 3 つは既定文面に含める:
  - 件数の集計 → `grep -c` / `rg -c` / `rg --count-matches`（`wc` / `awk` は使わない）
  - YAML / JSON の構文確認 → Read で読む（機械的確認は許可済みの `node`）
  - 認証状態の確認 → **不要**（環境が構成済み。GitHub の情報は GitHub MCP ツールで取る）
- 効果測定の実績として **7 → 3 → 1 件**の推移を根拠に添えられる。
  負の列挙のみ（7 件）→ 正の一覧を追加（3 件）→ 名指し漏れと代替を補完（1 件）という順で、
  **一覧の網羅性と代替手順の具体性が、そのまま拒否件数に効く**ことが実走で示された。
