...
## 追記 3（2026-08-03）: 拒否 3 件へ改善 — ただし残りは「一覧に無いコマンドの試行」だった

「追記 2」の対応（プロンプトへ型ごとの制約を追記）を載せた PR #480 の再実走
（run `30832367628`）で **権限拒否 3 件**となり、7 件から改善してジョブは success した
（許容値 4 以下）。ただし issue #469 の受け入れ基準「**拒否 0 件**」は**未達**である。
...
**「追記 2」で塞いだ型（3 段パイプ・単独の変数代入・許可外の Actions 系 MCP・`gh run view`）
由来の拒否は 0 件**であり、型ごとの制約を書く方針そのものは効いている。
残った 3 件は**すべて質が違う**。構文の問題でも粒度の問題でもなく、
**そもそも `--allowedTools` に存在しないコマンドを試した**ものである。

### 知見: 負の列挙では新型を塞げない

これまでキット（および本リポジトリの暫定デルタ）が積み上げてきたのは、
**「拒否される形」の列挙＝負の列挙**である（`for` / リダイレクト / `$(…)` / `VAR=1 cmd` /
単独の変数代入 / 3 段パイプ）。しかし負の列挙は原理的に**書いた型しか塞げない**。
AI が知らないのは「何が拒否されるか」ではなく「**何なら使えるか**」であり、
未知の作業に出会うたびに手持ちの一般常識（`python3` / `gh auth` / `curl` / `jq`）へ手が伸びる。
これは #460 以降ずっと続く「個別ツール名を後追いで足す運用では追いつかない」と同じ構造の、
プロンプト側の版である。

したがってキットは、**許可済みコマンドの「正の一覧」をプロンプトへ含めるべきである**。
本リポジトリでは両ワークフローのプロンプト冒頭へ次を追記した（暫定デルタ）:

- `--allowedTools` に実在する Bash エントリを、**そのファイル自身から書き起こした**正の一覧
  （`git` の読み取り系 / 閲覧・検索系 / `gh` の許可形 / 実行系 / Bash 以外のツール）。
  **要約表現は実際の許可より広く読めてはならない**（「`git` 系」とだけ書くと `git push` まで
  許可に見えるため、レビュー用では「読み取りのみ」と明示し、書き込み系 git が無いことを併記した）。
- 一覧に無いことを明示すべき代表例（`python3` / `gh auth` / `curl` / `wget` / `pip` / `jq` /
  `sed` / `awk` / `xargs` / `env` / `chmod` / `rm` / `mv` / `cp` / `touch` / `tee`）と、
  **代替手順**: YAML / JSON の構文確認は Read で読む（機械的な確認が要るなら許可済みの `node`）。
  認証状態の確認（`gh auth status`）は**そもそも不要**である（認証は環境が構成済みであり、
  GitHub の情報は GitHub MCP ツールで取る）。
- **一覧はレビュー用と実装用で異なる**ため、各ファイル自身の `--allowedTools` から書くこと
  （「追記 2」の末尾で述べた 🔴 の再発防止。例: `gh run list` はレビュー用のみ、
  `git add` / `git commit` / `git push` / `find` / `mkdir` は実装用のみ）。

### planning#168 へ追記する内容（追記 3 分）

- **提案 3「実走の作法」に「使える Bash コマンドの正の一覧」を必須項目として加える。**
  負の列挙（型ごとの制約）は残したうえで、**正の一覧を先に置く**。
  拒否の推移が根拠である: 7 件（run 30830151995）→ 3 件（run 30832367628）へ減ったが、
  残った 3 件はすべて一覧外コマンドの試行であり、負の列挙をいくら足しても届かない。
- 正の一覧はテンプレートに**プレースホルダではなく実物**として置き、`--allowedTools` を
  書き換えたら一覧も直す旨を注記する（両者の一致は提案 4 の機械検査へ寄せるのが本筋である）。
- 一覧に無い代表例と代替手順（上記）をキットの既定文面に含める。特に
  `python3` / `jq` / `curl` は「AI が最も自然に手を伸ばす未許可コマンド」として名指しする。
- **注意点として明記すべき落とし穴**: `--append-system-prompt` の値（＝`claude_args` の中）に
  **`--allowedTools` という文字列を書いてはならない**。`check-ai-workflow-config.js` は
  `claude_args` ブロックから `--allowedTools` トークンを探して以降を値とみなすため、
  プロンプト本文のフラグ名を本物のフラグとして拾い、「値が引用符で囲まれていない」と
  **誤検出して CI を落とす**（本リポジトリで実測。「この `claude_args` が指定している許可ツール
  一覧」と言い換えて回避した）。レビュー用の `prompt:` は同スクリプトの走査対象外なので
  この制約が無く、**両者は非対称である**。正の一覧をキットへ入れると必ず踏むため、
  テンプレートの既定文面自体をフラグ名を含まない言い回しにしておくのが安全である。
