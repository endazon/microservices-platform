...
- 提案内容:
  ...
  3. **ADR-0002 の更新**: 「8前後を上限の目安」を実態（**11＋BFF**）に合わせて改訂するか、新規追加 3 サービス
     （LlmGateway / Feedback / Dashboard）を明記して目安を更新する。追加は FR-08 / FR-10 / FR-11 由来である旨を追記。
  4. **ADR-0010 の既定モデル整合**: 以下のいずれかを計画側で決定する。
     - (a) 計画更新: 「既定 = `claude-sonnet-4-6`、最難関用途 = `claude-opus-4-8`、fable-5 / Copilot は将来拡張」
       と実態に合わせて ADR-0010 を改訂する（実装側の意図が正であれば推奨）。
     - (b) 実装追従: 計画（既定 opus / 最難関 fable-5 / Copilot）が正であれば、実装側で既定モデル変更と
       fable-5 / Copilot 経路の追加を別 Issue 化し、実装側 IADR を起票する。
...
>      解消。実態追認（計画更新）か実装追従（実装側 IADR 起票）かを確定。
