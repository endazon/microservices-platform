...
status: open
category: 前提検証の結果
related_ids: [FR-19, FR-20, ADR-0011, ADR-0036, ADR-0037]
...
| 経路 | 成否 |
| --- | --- |
| **閲覧（一覧・本文）** | **構造としては成立する。** WikiService（ABAC ゲートウェイ）が判定してから本文を取りに行くため、Wiki.js の権限モデルに依存しない |
...
「閲覧は構造として成立する」と書いたが、**いま動かしても個人スコープは効かない**。2 つの部品が未実装である。

| 部品 | 実測 |
| --- | --- |
| 文書の **`owner` 属性** | **0 件**（実装側 #456 / PR #515 が実データ 2,368 件で実測） |
| **`${current_user}` の動的束縛**（`ADR-0036` の判定規則 `doc.owner ∈ { ${current_user} }`） | **バックエンド全体で 0 件** |

**これは本検証の範囲外**であり、実装側 #516 と **planning#344**（裁定依頼）が扱っている。
**ただし案 A を採る場合も閲覧側の個人スコープは必要になる**ため、両者は連動する。
