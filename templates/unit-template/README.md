...
<unit>/                                     ← 新ユニットのリポジトリルート（= 配置時の src/<unit>/）
  ...
  frontend/
    ...
    src/                                     ← **ここに在るのは実体だけ**（空枠は置かない。下の注記）
                                            ←   ツリー全体の正本は計画 13_frontend-stack
                                            ←   §ディレクトリ構成（Bulletproof React）である
      features/                              ← Feature 単位
...
> 🔴 **空枠（`.gitkeep` のみのディレクトリ）を置かない。射程は `src/` 直下・feature 内部の両方である。**
> 計画 `ADR-0069` 決定 1（Accepted 2026-09-02。環流 planning#510 が求めた裁定）が、
> `ADR-0065` 決定 4 の **「枠だけの状態は機械にも目視にも『区分が揃っている』と見え、
> **適合の見え方**を作る」** という理由は**フロントエンドにも及ぶ**と定めた。
> **雛形と実装ユニット（`src/platform/frontend` / `src/knowledge/frontend`）は同時に動かした。**
>
> - 🔴 **ツリーの全体像は雛形からは読めない。正本は計画 `13_frontend-stack` §ディレクトリ構成 である**
>   （`ADR-0069` §結果 悪い影響 1・決定 1 のフォローアップ 2）。**上の構成図は「実体があるもの」の
>   一覧であって、区分の必須一覧ではない。** 複製したら、まず正本のツリーを読むこと。
> - **`sample/stores/` は存在しない。** クライアント状態ストア（Zustand）を持つのは例外で、
...
>   **(a) 関心が無い**＝適合している（不在それ自体が情報である）。
>   **(b) 関心はあるが置き場所が違う**＝**非適合である**（枠の有無にかかわらず）。
>   **枠を置いても (b) は直らない。枠は (b) を「揃っている」ように見せるだけである。**
>   ユニットで通常空になる `app/` と `locales/` は型 (a)（アプリホストである `platform/frontend` が
>   持つ、という意図的な不在）である。**区分ごとの理由は `src/platform/frontend/README.md` の注記が持つ。**
> - **機械が守る**: `node scripts/check-scaffolding-frames.js`（CI の `static-checks` ジョブ）が
>   「追跡下に `.gitkeep` のみのディレクトリが無いこと」を検査する（`ADR-0069` 決定 5。述語はこの 1 つだけ）。
>
> ⚠️ **［2026-09-03 追記 / #1195］従前ここには「`src/` 直下（最上位）の 11 区分は**残す**…
> planning#445 はどちらの側も支えない…したがって裁定を待つ」と書いていた。** その裁定が下り、
> 答えは**「消す」**である。`ADR-0069` 決定 2 は、planning#445 の列挙は**非適合の実測**であって
> 必須項目の一覧ではなく、同 issue の「名前だけを揃える対応は採らない」が**空枠を明示的に排除している**
> と読んだ。**支えないのは枠を置く側だけであった。**
>
> ⚠️ さらにその前、ここには「計画 `12_backend-application-stack` §規範性・粒度・置き場 がバックエンドに
> ついて同じ作法を定めており、フロントにも同じ理由が当てはまる」と書いていた。**この根拠は `ADR-0065`
...
> 区分も枠を置いた（IADR-0262）。
> **［2026-09-03 追記 / #1195］この段落は 2026-08-23 時点の記録である。** 置いた枠は
> **feature 内部が #1100 で、ユニット直下が本 issue で撤去済み**であり、**いまの形を述べていない**
> （いまの形は上の注記が持つ）。`stores/` は feature の既定の区分でもない。
>
