...
>
> **`src/` 直下（最上位）の `.gitkeep` は当面そのまま残している。** `app/` と `locales/` は
> ユニットでは通常空になる（アプリホストである `platform/frontend` が持つ）という**別の事情**があり、
> feature 内部と同じ理屈でそのまま消してよいとは限らない。**この 24 件の扱いは #1122 で判断する。**
>
