...
> **［2026-08-06 追記 / #560］`ADR-0035` に ABAC 属性組み合わせ数の実測が反映されたが、状態は
> `Proposed` のままである。** planning#206 が環流 planning#203（#456 / #515 の実測）を反映し、
> 同 ADR の保留注記は「**一部解消・保留は継続**——実測は 2026-08-05 に実施され決定 3 が安全側で
> あることは確認されたが、**その測定は本番相当ではない**（データ源が単一で属性の多様性が 1 通り）
> ため稼働後の再実測まで保留を続ける」となった。
> **「実測が反映された」ことと「着手条件を満たした」ことは別である**——[[IADR-0119]] の着手条件は
> 前提 ADR が `Accepted` であることなので、**#450（FR-17/18）・#451（FR-19/20）の保留は解除されない**。
...
**この書式を適用する母集合は「live な権威文書とコード」に限る**——`docs/specs/`（作業 / PR 単位の
一時点記録）・`feedback/`（計画リポへ送った内容の写し）・`docs/superpowers/`（保管された旧計画）は
