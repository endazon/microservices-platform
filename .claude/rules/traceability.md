...
上記の裸（無修飾）の ID は **MSP（microservices-platform）計画書の名前空間**を指す。MSP の ID レンジは
`FR-01..15` / `UC-01..07` / `SC-01..11`（および計画/実装 ADR）。他プロジェクトの ID は次節の規約で**修飾する**。

## ユニット横断・クロスリポジトリの ID 修飾

本リポジトリは複数ユニット（`src/platform` / `src/knowledge`）と、submodule で取り込む別プロジェクト
（`src/ai-stock-trading` = **AST**）を含む。別プロジェクトの計画書 ID を参照するとき、裸の ID
（例 `FR-17`・`SC-01`）で書くと **MSP の同番号 ID と衝突**し、機械的な突合（監査・`trace-check`）が
「計画書に存在しない ID の参照」「誤帰属」を誤検出する。実際、AST の `FR-17`（MSP は FR-15 まで）や
AST の `SC-01`（MSP の SC-01＝検索/チャットと衝突）で発生した。

**規約**: 他プロジェクト計画書の ID は必ず**プロジェクト修飾**を付ける。

- **書式**: `<PROJ>/<ID>`。例 `AST/FR-17`・`AST/SC-01`・`AST/UC-06`。`<PROJ>` は短縮プロジェクト名
  （ai-stock-trading = `AST`）。裸の `FR-xx`/`UC-xx`/`SC-xx` は常に MSP 名前空間を意味する。
- **適用箇所**: コード内コメント・IADR / 仕様書の本文・テスト名、および**コミット/PR 件名の説明文**。
- **コミット/PR 件名のスコープ `()`**: `scripts/check-commit-messages.js` の `ID_PATTERN` は MSP 名前空間の
  ID のみを検証する。よって**スコープには MSP が所有する ID を置く**（AST 連携作業は担当の `IADR-xxxx` 等が起点）。
  他プロジェクトの ID は**スコープに入れず、説明文で修飾して参照**する。
  例: `feat(IADR-0072): AST/SC-02 監視銘柄の /bff/monitor プロキシを登録`（スコープ=MSP の IADR、説明で `AST/SC-02`）。
- **監査 / trace-check の扱い**: `<PROJ>/<ID>` は外部プロジェクトへの参照であり、MSP 計画書に対する
  「参照切れ」ではない。監査は修飾付き ID を MSP 名前空間の突合対象から**除外**する（`.claude/agents/traceability-auditor.md` 参照）。
...
- 計画書に存在しない ID を参照しない（誤記・廃止に注意）。他プロジェクトの ID は裸で書かず
  `<PROJ>/<ID>`（例 `AST/FR-17`）で修飾する（上記「ユニット横断・クロスリポジトリの ID 修飾」）。
