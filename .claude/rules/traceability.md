...
`FR-01..21` / `UC-01..11` / `SC-01..21` / `ADR-0001..0039`（`ADR-0035` は番号予約のみで未起案・欠番）
である（planning `df8bce5` = project-planning PR #144 時点）。計画側で ID が増減したら本節を追随させる
...
- **計画 ID の `<PROJ>`**: ai-stock-trading = `AST`。例 `AST/FR-17`・`AST/SC-01`・`AST/UC-06`。
  ...
  **AST 側が自前で採番しているレンジは `FR-01..20` / `UC-01..07` / `SC-01..03`**（submodule pin `655e2ed`
  時点で実測。`05_screens` に現れる `SC-11` / `SC-13` / `SC-16` は MSP 画面への参照であって AST の採番ではない）。
