...
- **計画 ID の `<PROJ>`**: ai-stock-trading = `AST`。例 `AST/FR-17`・`AST/SC-01`・`AST/UC-06`。
  ...
  誤検出が発生した。**計画大改定（planning PR #144）で MSP のレンジが `FR-01..21` / `SC-01..21` へ広がり、
  `FR-17`・`SC-18`〜`SC-21` は「一方にしか無い ID」から「双方に存在し意味の異なる ID」へ変わった**ため、
  修飾の欠落は参照切れではなく**誤帰属**（別プロジェクトの要求へ紐づく）として現れる。裸の ID は必ず MSP を指す。
