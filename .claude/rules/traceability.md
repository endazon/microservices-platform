...
（planning `3e58b97` = PR planning#244 時点。**レンジは planning#200 時点から変わっていない**——
planning#206 / planning#207 と PR planning#244 はいずれも既存 ADR の更新・状態遷移と AST 側の反映で、
新規 ADR を起こしていない）。
**`ADR-0035`（GraphRAG 検索戦略）は 2026-08-04 に起案され、欠番ではなくなった**。
**`Proposed` な計画 ADR は planning `3e58b97` 時点で 6 件**である（全 ADR の `status:` を実測。
`Accepted` でも `Proposed` でもないのは `ADR-0003` の `Superseded` 1 件だけ）——
